...
begin
  with g_pConfig, (Sender as TCheckBox) do begin
    case Tag of
      //新建角色过滤
      20: InterlockedExchange(Integer(m_fAllowGetBackChr), Integer(Checked)); //允许找回人物角色
      21: InterlockedExchange(Integer(m_fAllowDeleteChr), Integer(Checked)); //允许找回人物角色
      22: InterlockedExchange(Integer(m_fCheckNewIDOfIP), Integer(Checked));
      23: InterlockedExchange(Integer(m_fNewChrNameFilter), Integer(Checked)); //新注册人物角色字符过滤
      24: InterlockedExchange(Integer(m_fDenyNullChar), Integer(Checked)); //禁止使用空格字符
      25: InterlockedExchange(Integer(m_fDenyAnsiChar), Integer(Checked)); //禁止使用英文和数字
      26: InterlockedExchange(Integer(m_fDenySpecChar), Integer(Checked));  //禁止使用特殊字符
      27: InterlockedExchange(Integer(m_fDenyHellenicChars), Integer(Checked));  //禁止使用希腊字符
      28: InterlockedExchange(Integer(m_fDenyRussiaChar), Integer(Checked));  //禁止使用俄罗斯字符
      29: InterlockedExchange(Integer(m_fDenySpecNO1), Integer(Checked));  //禁止使用以下数字⒈⒉⒊⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛
      30: InterlockedExchange(Integer(m_fDenySpecNO2), Integer(Checked));  //禁止使用以下数字⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇
      31: InterlockedExchange(Integer(m_fDenySpecNO3), Integer(Checked));  //禁止使用以下数字①②③④⑤⑥⑦⑧⑨⑩
      32: InterlockedExchange(Integer(m_fDenySpecNO4), Integer(Checked));  //禁止使用以下数字㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩
      33: InterlockedExchange(Integer(m_fDenySBCChar), Integer(Checked)); //禁止使用全角字符
      34: InterlockedExchange(Integer(m_fDenykanjiChar), Integer(Checked)); //禁止使用假日文
      35: InterlockedExchange(Integer(m_fDenyTabsChar), Integer(Checked)); //禁止使用制表符
      //连接列表
      120: InterlockedExchange(Integer(m_fCheckNullSession), Integer(Checked)); //防止超连接攻击
      121: InterlockedExchange(Integer(m_fKickOverPacketSize), Integer(Checked)); //异常掉线处理
      122: InterlockedExchange(Integer(m_fDefenceCCPacket), Integer(Checked)); //防止CC攻击
