...
begin
  ...
  try
    for i := 0 to g_NewIDOfIPList.Count - 1 do begin
      NewIDAddr := pTNewIDAddr(g_NewIDOfIPList[i]);
      if NewIDAddr.IPaddr = Addr then begin
        if GetTickCount - NewIDAddr.dwIDCountTick < 4 * 1000 then begin
          Inc(NewIDAddr.Count);
          ...
          if NewIDAddr.Count > 0 then
            Dec(NewIDAddr.Count);
          if NewIDAddr.Count <= 0 then begin
            DisPose(NewIDAddr);
