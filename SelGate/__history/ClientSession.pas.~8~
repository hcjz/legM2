...
begin
  ...
  if (Len >= 5) and g_pConfig.m_fDefenceCCPacket then begin
    if (StrPos(PChar(Addr), 'HTTP/') <> nil) then begin//StrPos返回第二个参数在第一个参数中第一次出现的位置指针
  ...
  if (Len >= 1) then begin
    if (StrPos(PChar(Addr), '$') <> nil) then begin
      if g_pLogMgr.CheckLevel(6) then
  ...
  if m_fHandleLogin = 0 then begin
    //所有角色选项操作数据包
    case CltCmd.Cmd of
      CM_QUERYSELCHARCODE,CM_QUERYCHR,CM_NEWCHR,CM_DELCHR,CM_SELCHR,CM_QUERYDELCHR,CM_GETBACKDELCHR: begin
