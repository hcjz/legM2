...
begin
  Result := nil;
  if nSock <> INVALID_SOCKET then begin
    Lock();
    try
      for i := 0 to m_xUserList.Count - 1 do begin
        UserOBJ := TSessionObj(m_xUserList[i]);
        if UserOBJ.m_pUserOBJ._SendObj.Socket = nSock then begin
          Result := UserOBJ;
          Break;
        end;
      end;
...
begin
  ...
  SetTimer(g_hMainWnd, _IDM_TIMER_KEEP_ALIVE, 4 * 1000, Pointer(@OnTimerProc));//每间隔4秒钟向服务器发送一次心跳数据
...
begin
  ...
  KillTimer(g_hMainWnd, _IDM_TIMER_KEEP_ALIVE); //心跳数据检测不到停止
...
begin
  ...
  try
    for i := 0 to FormMain.m_xGameServerList.GameServerCount - 1 do begin
      if FormMain.m_xGameServerList[i].Active then begin
        FormMain.m_xGameServerList[i].SendBuffer('%--$', 4);
      end;
      if FormMain.m_xGameServerList[i].Active and FormMain.m_xGameServerList[i].m_fKeepAlive then begin
        if GetTickCount() - FormMain.m_xGameServerList[i].m_dwKeepAliveTick > 25 * 1000 then begin
