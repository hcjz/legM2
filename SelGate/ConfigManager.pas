...
type
  ...
  TConfigMgr = class(TSyncObj)          //Speed Lower then var record
    ...
    //新加角色过滤功能 2018-09-06
    m_fAllowGetBackChr: LongBool; //允许找回人物角色
    m_fAllowDeleteChr: LongBool; //允许删除人物角色
    m_fNewChrNameFilter: LongBool; //新注册人物角色字符过滤
    m_fDenyNullChar: LongBool; //禁止使用空格字符
    m_fDenyAnsiChar: LongBool; //禁止使用英文和数字
    m_fDenySpecChar: LongBool; //禁止使用特殊字符
    m_fDenyHellenicChars: LongBool; //禁止使用希腊字符
    m_fDenyRussiaChar: LongBool; //禁止使用俄罗斯字符
    m_fDenySpecNO1: LongBool; //禁止使用以下数字⒈⒉⒊⒋⒌⒍⒎⒏⒐⒑⒒⒓⒔⒕⒖⒗⒘⒙⒚⒛
    m_fDenySpecNO2: LongBool; //禁止使用以下数字⑴⑵⑶⑷⑸⑹⑺⑻⑼⑽⑾⑿⒀⒁⒂⒃⒄⒅⒆⒇
    m_fDenySpecNO3: LongBool; //禁止使用以下数字①②③④⑤⑥⑦⑧⑨⑩
    m_fDenySpecNO4: LongBool; //禁止使用以下数字㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩
    m_fDenySBCChar: LongBool; //禁止使用全角字符
    m_fDenykanjiChar: LongBool; //禁止使用假日文
    m_fDenyTabsChar: LongBool; //禁止使用制表符
