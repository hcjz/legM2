...
begin
  with CoXMLHTTPRequest.Create do begin //新版本是 CoXMLHTTP.Create
...
{$ENDIF}
  ...
  if fHandle <> 0 then begin
    try
      ...
      g_DCP_mars.InitStr('LEGEND LoginGate - edHeader 20190131');
...
{$IFDEF  FREE}
          g_DCP_mars.InitStr('legendm2 gate free');
{$ELSE}
          //g_DCP_mars.InitStr(VMProtectDecryptStringA('blue gate'));
          g_DCP_mars.InitStr('legendm2 gate');
{$ENDIF}
...
begin
  ...
  g_pLogMgr.Add('程序名称: 登陆网关');
  g_pLogMgr.Add('程序版本: 2019/01/31');
  g_pLogMgr.Add('程序网站: http://www.legendm2.com');
  g_DCP_mars.InitStr('');
  try
    szStr := g_DCP_mars.DecryptString(g_pszDecodeKey^);
  except
    szStr := '';
  end;
  if szStr <> '' then
    g_pLogMgr.Add(PChar('配套网关: ' + szStr));
...
begin
  ...
  while DWORD(pTRBuf) < DWORD(pTREnd) do begin
    ...
    if DWORD(pTREnd) - DWORD(pTRBuf) <= 2 then
      Break;
    pTRBuffer := pTRBuf + 1;
    while DWORD(pTRBuffer) < DWORD(pTREnd) do begin
      if pTRBuffer^ <> '$' then begin
        Inc(pTRBuffer);
        Continue;
      end;
      Inc(pTRBuf, 1);
      ...
      if nBufLen >= 2 then begin
        with g_ProcMsgThread do begin
          if pTRBuf^ = '+' then begin
            if (pTRBuf + 1)^ = '-' then begin
            ...
            UserOBJ := GetSession(nSock);
            if (UserOBJ <> nil) and (UserOBJ.m_tLastGameSvr = ClientThread) then begin
...
{$I '..\Common\Macros\VMPB.inc'}
  if msg.wParam = 2 then begin
    Label2.Caption := Format('该版本还%d天到期,请尽快续费', [msg.lParam]);
    Label2.Alignment := taCenter;
    Label2.Font.Color := clRed;
  end

  else if msg.wParam = 1 then begin
    Label2.Caption := ('该版本已经到期!!!');
    Label2.Alignment := taCenter;
    Label2.Font.Color := clRed;
  end
  else if msg.wParam = 3 then
  begin
    Label2.Caption := ('该版本已停用,请更新!!!');
    Label2.Alignment := taCenter;
    Label2.Font.Color := clRed;
  end
