...
uses IdSrvClient, ObjMon, M2Share, EDcode, ObjGuard, ObjAxeMon, StartPointManage,
  ObjMon2, ObjMon3, ObjMon5, Event, InterMsgClient, InterServerMsg,
  ObjRobot, HUtil32, svMain, Castle;
...
{$IFEND}
  ...
  for i := 0 to MonSpAbilList.Count - 1 do begin
  ...
  for i := 0 to m_EscortProcList.Count - 1 do begin
  ...
  for i := 0 to m_MagicListPre.Count - 1 do begin
...
{$IF USEHASHLIST = 1}
  ...
  if MonInfo <> nil then Result := MonInfo.btRace;
{$ELSE}
  for i := 0 to MonsterList.Count - 1 do begin
  ...
  try
    for i := m_MerchantList.Count - 1 downto 0 do begin
      ...
      if Merchant.m_PEnvir <> nil then begin
        ...
        Merchant.Initialize;
        if Merchant.m_boErrorOnInit and (not Merchant.m_boIsHide) then begin
...
begin
  ...
  for i := QuestNPCList.Count - 1 downto 0 do begin
    ...
    if NormNpc.m_PEnvir <> nil then begin
      ...
      NormNpc.Initialize;
      if NormNpc.m_boErrorOnInit and (not NormNpc.m_boIsHide) then begin
...
begin
  ...
  try
    ...
    if SwitchDataInfo = nil then begin
      ...
      nCheckCode := 6;
      if (Envir <> nil) and Envir.m_MapFlag.boFIGHT3Zone then begin
      ...
      if (Envir <> nil) and (Castle <> nil) and ((Castle.m_MapPalace = Envir) or Castle.m_boUnderWar) then begin
        if g_CastleManager.IsCastleMember(PlayObject) = nil then begin
      ...
      if Envir = nil then begin
        ...
        if PlayObject.m_Abil.HP <> 14 then MainOutMessageAPI(Format(sChangeServerFail1, [g_nServerIndex, PlayObject.m_nServerIndex, PlayObject.m_sMapName]));
...
procedure TUserEngine.ProcessHumans;
  ...
  begin
    ...
    Result := False;
    if tOpen in [tHumDat, tHeroDat, tResume, tHumDat2] then begin
      for i := 0 to m_PlayObjectList.Count - 1 do begin

        PlayObject := TPlayObject(m_PlayObjectList.Objects[i]);
        if (PlayObject <> nil) and (CompareText(PlayObject.m_sCharName, sChrName) = 0) then begin
          if tOpen = tResume then begin //挂机后继续登陆
            if not PlayObject.m_boOffLineFlag then
              Result := True;
          end else if tOpen in [tHumDat, tHeroDat] then //正常登陆
            Result := True;
          Exit;
        end;
      end;
      ...
      //m_HeroProcList.Lock;
      //try
      for i := 0 to m_HeroProcList.Count - 1 do begin
        HeroObject := m_HeroProcList[i];
        if (HeroObject <> nil) and not HeroObject.m_boGhost and (CompareText(HeroObject.m_sCharName, sChrName) = 0) then begin
          Result := True;
          Exit;
        end;
      end;
    ...
    try
      ...
      try
        ...
        for i := 0 to m_LoadPlayList.Count - 1 do begin
          ...
          if not FrontEngine.IsFull and not IsLogined(sUser, UserOpenInfo.LoadType) then begin
...
{$IFEND VER_ClientType_45}
              ...
              tHeroDat: begin
                  ...
                  if PlayObject <> nil then begin
                    if PlayObject.m_HeroRcd = nil then New(PlayObject.m_HeroRcd);
              ...
              tLoadFail: begin          //Load hero rcd fail...
                  PlayObject := GetPlayObjectNotGhost(UserOpenInfo.LoadUser.sMasterName);
                  if PlayObject <> nil then begin
            ...
            UserOpenInfo := pTUserOpenInfo(m_LoadPlayList.Objects[i]);
            if UserOpenInfo.LoadUser.nSocket <> -1 then begin
  ...
  if (GetTickCount - m_dwProcessSavePlayTick) > 200 then begin
    m_dwProcessSavePlayTick := GetTickCount();
    nCHECK := 101;
    try
      EnterCriticalSection(m_SavePlaySection);
      try
        for i := 0 to m_SavePlayList.Count - 1 do begin
          UserSaveInfo := pTUserSaveInfo(m_SavePlayList[i]); //? FrontEngine.IsFull
          if UserSaveInfo.bNewHero then begin
  ...
  try
    for i := 0 to m_PlayObjectFreeList.Count - 1 do begin
      ...
      if (GetTickCount - PlayObject.m_dwGhostTick) > g_Config.dwHumanFreeDelayTime {5 * 60 * 1000} then begin
        ...
        if PlayObject.m_boSwitchDataNeedDelay and (PlayObject.m_boSwitchDataOK or ((GetTickCount - PlayObject.m_dwChgDataWritedTick) > 10 * 1000)) then begin
  ...
  try
    ...
    while True do begin
      ...
      if dwCurTick - PlayObject.m_dwRunTick > PlayObject.m_nRunTime then begin
...
{$IFEND}
            ...
            if not PlayObject.m_boReadyRun then begin
              ...
              if not PlayObject.m_boOffLineFlag or PlayObject.m_boOffLinePlay {or PlayObject.m_boSearch} then begin
              ...
              if not PlayObject.m_boOffLineFlag then begin
              ...
              PlayObject.Run();
              nCHECK := 29;
              if not FrontEngine.IsFull2 and ((GetTickCount - PlayObject.m_dwSaveRcdTick) > g_Config.dwSaveHumanRcdTime) then begin
    ...
    try
      for i := nMerchantPosition to m_MerchantList.Count - 1 do begin
        ...
        if not MerchantNPC.m_boGhost then begin
          if dwCurrTick - MerchantNPC.m_dwRunTick > MerchantNPC.m_nRunTime then begin
        ...
        end else if (GetTickCount - MerchantNPC.m_dwGhostTick) > 60 * 1000 then begin
          MerchantNPC.Free;
          m_MerchantList.Delete(i);
          Break;
        end;
...
begin
  ...
  try
    ...
    for i := m_nHeroPosition to m_HeroProcList.Count - 1 do begin
      code := 2;
      Hero := m_HeroProcList.Items[i];
      code := 3;
      if not Hero.m_boGhost then begin
        if dwCurrTick - Hero.m_dwRunTick > Hero.m_nRunTime then begin
          Hero.m_dwRunTick := dwCurrTick;
          code := 4;
          if (GetTickCount - Hero.m_dwSearchTick) > Hero.m_dwSearchTime then begin
            Hero.m_dwSearchTick := GetTickCount();
            if not Hero.m_boDeath then begin
          ...
          Hero.Run();
          code := 6;
          if Hero.HeroCanSaveRcd then begin
            if {not FrontEngine.IsFull2 and}((GetTickCount - Hero.m_dwHeroSaveRcdTick) > _MAX(15 * 60 * 1000, g_Config.dwSaveHumanRcdTime)) then begin
        ...
        if (GetTickCount - Hero.m_dwGhostTick) > 5 * 60 * 1000 then begin
          code := 8;
      ...
      code := 9;
      if (GetTickCount - dwRunTick) > 28 then begin //0824
        boProcessLimit := True;
        m_nHeroPosition := i;
        Break;
      end;
...
begin
  ...
  try
    ...
    boProcessLimit := False;
    dwCurrTick := dwRunTick;
    for i := m_nEscortPosition to m_EscortProcList.Count - 1 do begin
      code := 2;
      Escort := m_EscortProcList.Items[i];
      //Escort := TEscortMon(m_EscortProcList.GetValues(m_EscortProcList.Keys[i]));
      code := 3;
      if not Escort.m_boGhost then begin
        if dwCurrTick - Escort.m_dwRunTick > Escort.m_nRunTime then begin
          Escort.m_dwRunTick := dwCurrTick;
          code := 4;
          if (GetTickCount - Escort.m_dwSearchTick) > Escort.m_dwSearchTime then begin
            Escort.m_dwSearchTick := GetTickCount();
            if not Escort.m_boDeath then begin
        ...
        code := 7;
        if (GetTickCount - Escort.m_dwGhostTick) > 5 * 60 * 1000 then begin
          code := 8;
  ...
  try                                   //0406
    ...
    if (GetTickCount - dwRegenMonstersTick) > g_Config.dwRegenMonstersTime then begin
      ...
      if (MonGen <> nil) and (MonGen.sMonName <> '') and not g_Config.boVentureServer then begin
        nCheckCode := 2;
        if (MonGen.dwStartTick = 0) or (LongWord(GetTickCount - MonGen.dwStartTick) > GetZenTime(MonGen.dwZenTime)) then begin
          nCheckCode := 3;
    ...
    for i := m_nMonGenListPosition to m_MonGenList.Count - 1 do begin
      ...
      while (True) do begin
        ...
        if not Monster.m_boGhost then begin
          ...
          if dwCurrentTick - Monster.m_dwRunTick > Monster.m_nRunTime then begin
            Monster.m_dwRunTick := dwRunTick;
{$IF PROCESSMONSTMODE = OLDMONSTERMODE}
            if not Monster.m_boDeath then begin
            ...
            Monster.Run;
{$ELSE}
            if Monster.m_boDeath and Monster.m_boCanReAlive and Monster.m_boInvisible and (Monster.m_pMonGen <> nil) then begin
              if LongWord(GetTickCount - Monster.m_dwReAliveTick) > GetZenTime(Monster.m_pMonGen.dwZenTime) then begin
                if Monster.ReAliveEx(Monster.m_pMonGen) then begin
  ...
  try
    ...
    for i := nNpcPosition to QuestNPCList.Count - 1 do begin
      ...
      if not NPC.m_boGhost then begin
        if dwCurrTick - NPC.m_dwRunTick > NPC.m_nRunTime then begin
      ...
      end else if (GetTickCount - NPC.m_dwGhostTick) > 60 * 1000 then begin
        NPC.Free;
        QuestNPCList.Delete(i);
        Break;
      end;
...
begin
  ...
  for i := 0 to m_EscortProcList.Count - 1 do begin
    Mon := TEscortMon(m_EscortProcList[i]);
    if not Mon.m_boGhost and (CompareText(sCharName, Mon.m_sMName) = 0) then begin
      Result := Mon;
      Break;
    end;
  end;
...
begin
  ...
  ok := False;
  for nX := PlayObject.m_nCurrX - 1 to PlayObject.m_nCurrX + 1 do begin
    for nY := PlayObject.m_nCurrY - 1 to PlayObject.m_nCurrY + 1 do begin
  ...
  nRace := GetMonRace(sMonName);
  BaseObject := AddBaseObject(sMAP, nX, nY, nRace, sMonName);
  if (BaseObject <> nil) and (BaseObject is TEscortMon) then begin
    with BaseObject as TEscortMon do begin
...
begin
  ...
  try
    ...
    if (GetTickCount() - dwSendOnlineHumTime) > 10 * 1000 then begin
      dwSendOnlineHumTime := GetTickCount();
...
begin
  Result := nil;
  Dec(nItemIdx);
  if (nItemIdx >= 0) and (TitleList.Count > nItemIdx) then begin
...
begin
  Result := False;
  for i := 0 to m_OtherUserNameList.Count - 1 do begin
    if CompareText(m_OtherUserNameList[i], sName) = 0 then begin
...
{$IFEND}
    if UseItemsList <> nil then begin
      ...
      if Mon.m_btHair > 1 then Mon.m_btHair := 1;
      ...
      for i := Low(THumanUseItems) to High(THumanUseItems) do begin
        ...
        if iName <> '' then begin
          if CopyToUserItemFromName(iName, @UserItem) then begin
            ...
            if StdItem.StdMode in [15..24, 26] then begin
...
{$IFEND}
  ...
  if (ItemList <> nil) and (ItemList.Count > 0) then begin
    nRate := 0;
    if (ItemOfCreat <> nil) and ((ItemOfCreat.m_btRaceServer = RC_PLAYOBJECT) or ItemOfCreat.IsHero) and (TPlayObject(ItemOfCreat).m_nTagDropPlus > 0) then begin
    ...
    for i := 0 to ItemList.Count - 1 do begin
      ...
      if (MonItem.MaxPoint = 88888888) then begin
        if (Mon.m_btRaceServer <> RC_HERO) and Mon.m_boExplore and not MonItem.IsGold {(CompareText(iName, sSTRING_GOLDNAME) <> 0)} then begin
          New(UserItem);
          if CopyToUserItemFromName(iName, UserItem) then begin
            UserItem.Dura := Round((UserItem.DuraMax / 100) * (20 + Random(80)));
            StdItem := GetStdItem(UserItem.wIndex);
            ...
            SetGatherExpItem(StdItem, UserItem);
            if StdItem.Overlap >= 1 then UserItem.Dura := 1;
...
begin
  if (DefMsg = nil) or PlayObject.m_boOffLineFlag then Exit;
  try
    if Buff = nil then
      sMsg := ''
    else
    ...
    if ((DefMsg.Ident >= 9000) and (DefMsg.Ident <= 11332)) and
      ...
      (DefMsg.Ident <> CM_HEROSERIESSKILLCONFIG) then begin
    ...
    case DefMsg.Ident of
      ...
      CM_HORSERUN,
        ...
        CM_SMITELONGHIT3,
        CM_SMITEWIDEHIT,
        CM_SMITEWIDEHIT2: if DefMsg.Tag in [0..7] then begin
          if g_Config.boActionSendActionMsg then begin
...
{$IFEND}
  ...
  if Cert <> nil then begin
    ...
    if Cert.m_btRaceServer = RC_HERO then begin
      MonGetAbility(Cert);
    ...
    if Cert.m_boErrorOnInit then begin
      p28 := nil;
      if Cert.m_PEnvir.m_MapHeader.wWidth < 50 then
        n20 := 2
      else
        n20 := 3;
      if (Cert.m_PEnvir.m_MapHeader.wHeight < 250) then begin
  ...
  try
    ...
    case btHeroType of
      ...
      1:  begin
          ...
          if HeroObject <> nil then begin
            nCode := 101;
            if MasterObj.m_HeroRcd <> nil then begin
            ...
            if HeroObject.m_Abil.HP <= 14 then begin
    ...
    if HeroObject <> nil then begin
      ...
      while (True) do begin
        nCode := 8;
        if HeroObject.m_PEnvir.CanWalk(nX, nY, False {True}) then begin
          //if btHeroType = 2 then begin
          ...
          if nCount <= 8 then begin
            nX := Random(3) + (nX - 1);
            nY := Random(3) + (nY - 1);
          end else if nCount <= 30 then begin
            nX := Random(5) + (nX - 2);
            nY := Random(5) + (nY - 2);
          end else if nCount <= 50 then begin
            nX := Random(7) + (nX - 3);
            nY := Random(7) + (nY - 3);
          end else begin
            nX := Random(9) + (nX - 4);
        ...
        Inc(nCount);
        if nCount > 70 then Break;
      end;
      if HeroPointer = nil then begin
        nCode := 10;
        HeroObject.Free;
        HeroObject := nil;
      end;
...
begin
  ...
  for i := 0 to m_HeroProcList.Count - 1 do begin
    ...
    if not HeroObject.m_boGhost and (CompareText(HeroObject.m_sCharName, sName) = 0) then begin
      Result := HeroObject;
      Break;
    end;
  end;
...
begin
  ...
  try
    for i := 0 to m_HeroProcList.Count - 1 do begin
      HeroObject := m_HeroProcList.Items[i];
      if not HeroObject.m_boGhost and (CompareText(HeroObject.m_sCharName, sName) = 0) and (CompareText(HeroObject.m_sUserID, sID) = 0) then begin
        Result := HeroObject;
        Break;
      end;
    end;
...
begin
  ...
  try
    for i := 0 to m_PlayObjectList.Count - 1 do begin
      if CompareText(m_PlayObjectList.Strings[i], sName) = 0 then begin
        PlayObject := TPlayObject(m_PlayObjectList.Objects[i]);
        if CompareText(PlayObject.m_sUserID, sID) = 0 then begin
          Result := PlayObject;
...
{$IFEND}
  ...
  (PlayObject as TPlayObject).m_btActiveTitle := HumData.btActiveTitle;
  (PlayObject as TPlayObject).m_Titles := HumData.HumTitles;
  ...
  if g_Config.boSaveKillMonExpRate then begin
    if (HumData.nKillMonExpRate > 0) and (HumData.dwKillMonExpRateTime > 0) then begin
  ...
  for i := Low(THumMagic) to High(THumMagic) do begin
    if HumMagic[i].wMagIdx <= 0 then Continue;
  ...
  if g_Config.boUseCustomData and (PlayObject.m_btRaceServer = RC_PLAYOBJECT) then begin
...
{$IFEND SERIESSKILL}
  if (PlayObject.m_btRaceServer = RC_PLAYOBJECT) then begin
    for i := 0 to MAXMISSION - 1 do begin
...
begin
  if g_StartPointManager.m_InfoList.Count > 0 then begin
...
{$IFEND VER_ClientType_45}
  ...
  for i := Low(PlayObject.m_wStatusArrValue) to High(PlayObject.m_wStatusArrValue) do begin
    if PlayObject.m_wStatusArrValue[i] > 0 then begin
...
{$IFEND VER_ClientType_45}
  ...
  for i := Low(SwitchData.SlaveArr) to High(SwitchData.SlaveArr) do
    if SwitchData.SlaveArr[i].sSalveName <> '' then begin
      New(SlaveInfo);
  ...
  for i := Low(SwitchData.StatusValue) to High(SwitchData.StatusValue) do begin
...
begin
  ...
  for i := 0 to m_MagicList.Count - 1 do begin
    ...
    if (Magic.wMagicId = nMagIdx) and (Magic.btClass = btClass) then begin
      if btClass = 0 then begin
        if boIshero then begin
          if CompareText(Magic.sDescr, g_sHeroName) = 0 then begin
            Result := Magic;
            Break;
          end;
        end else begin
...
{$IFEND}
        ...
        if BaseObject.m_boExplore then begin
        ...
        with BaseObject do begin
          ...
          if m_spMonAbil.boMonParalysis then m_boParalysis := True;
          if m_spMonAbil.boMonUnParalysis then m_boUnParalysis := True;
          if m_spMonAbil.boMonUnAllParalysis then m_boUnAllParalysis := True;
          if m_spMonAbil.boMonUnRevival then m_boUnRevival := True;
          if m_spMonAbil.boMonUnMagicShield then m_boUnMagicShield := True;
        end;
        ...
        for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
          ...
          if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
            ...
            if (BaseObject <> nil) and
              ...
              not TPlayObject(BaseObject).m_boOffLineFlag then begin
...
var
  ...
  Envir                     : TEnvirnoment;
begin
  for i := 0 to m_MapEffectList.Count - 1 do begin
    ...
    if c > 0 then begin
      ...
      for ii := 1 to d do begin
        ...
        n := 0;
        while (True) do begin
          if Envir.CanWalk(x, y, True) then begin
...
begin
  for i := m_MagicEventList.Count - 1 downto 0 do begin
    ...
    if MagicEvent <> nil then begin
      ...
      if (MagicEvent.BaseObjectList.Count <= 0) or ((GetTickCount - MagicEvent.dwStartTick) > MagicEvent.dwTime) or ((GetTickCount - MagicEvent.dwStartTick) > 180000) then begin
        MagicEvent.BaseObjectList.Free;
        III := 0;
        while (True) do begin
  ...
  for i := m_MagicEventCloseList.Count - 1 downto 0 do begin
    Event := TEvent(m_MagicEventCloseList[i]);
    if Event.m_boClosed and (GetTickCount - Event.m_dwCloseTick > 5 * 60 * 1000) then begin
...
begin
  ...
  try
    for i := 0 to m_MerchantList.Count - 1 do begin
      Merchant := TMerchant(m_MerchantList.Items[i]);
      if not Merchant.m_boGhost and not Merchant.m_boIsHide then begin //By Blue
...
begin
  for i := 0 to QuestNPCList.Count - 1 do begin
    NPC := TNormNpc(QuestNPCList.Items[i]);
    if not NPC.m_boIsHide then begin    //By Blue
      NPC.ClearScript;
      NPC.LoadNpcScript;
    end;
  end;
...
begin
  ...
  for i := 0 to m_MonGenList.Count - 1 do begin
    ...
    for ii := 0 to MonGen.CertList.Count - 1 do begin
      BaseObject := TBaseObject(MonGen.CertList.Items[ii]);
      if not BaseObject.IsHero and not BaseObject.m_boDeath and not BaseObject.m_boGhost and (BaseObject.m_PEnvir = Envir) and (boSlave or (BaseObject.m_Master = nil)) then begin
...
begin
  ...
  if Envir = nil then
    Exit;
  if m_PlayObjectList.Count <= 0 then Exit;

  Guild := nil;
  for i := 0 to m_PlayObjectList.Count - 1 do begin
    PlayObject := TPlayObject(m_PlayObjectList.Objects[i]);
    if not PlayObject.m_boDeath and not PlayObject.m_boGhost and (PlayObject.m_PEnvir = Envir) then begin
...
begin
  for i := 0 to m_PlayObjectList.Count - 1 do begin
    PlayObject := TPlayObject(m_PlayObjectList.Objects[i]);
    if not PlayObject.m_boGhost and not PlayObject.m_boOffLineFlag then begin
...
{$IFEND}
  ...
  if (GetTickCount() - dwProcessEffectsTick) > 2500 then begin //0824
    dwProcessEffectsTick := GetTickCount();
    ProcessEffects();
  end;
...
var
  ...
  SwitchDataInfo            : pTSwitchDataInfo;
begin
  for i := m_ChangeServerList.Count - 1 downto 0 do begin
...
begin
  ...
  for i := 0 to m_ChangeServerList.Count - 1 do begin
    ...
    if SwitchData = nil then Continue;
...
begin
  for i := m_ChangeServerList.Count - 1 downto 0 do begin
    if GetTickCount - pTSwitchDataInfo(m_ChangeServerList[i]).dwWaitTime > 30 * 1000 then begin
...
begin
  for i := 0 to m_PlayObjectFreeList.Count - 1 do begin
    ...
    if hum.m_sSwitchDataTempFile = flName then begin
...
begin
  ...
  for i := m_OtherUserNameList.Count - 1 downto 0 do begin
...
begin
  apmode := GetValidStr3(uname, Name, [':']);
  for i := m_OtherUserNameList.Count - 1 downto 0 do begin
    if (CompareText(m_OtherUserNameList[i], Name) = 0) and (Integer(m_OtherUserNameList.Objects[i]) = sNum) then begin
      m_OtherUserNameList.Delete(i);
      //Add User To UserMgr When Other Server Login...
...
begin
  ...
  if Envir.InSafeZone(x, y) then Exit;
  nRANDOM := 0;
  if Envir.m_MapFlag.nThunder <> 0 then Inc(nRANDOM);
  if Envir.m_MapFlag.nGreatThunder <> 0 then Inc(nRANDOM);
  if Envir.m_MapFlag.nLava <> 0 then Inc(nRANDOM);
  if Envir.m_MapFlag.nSpurt <> 0 then Inc(nRANDOM);
  ...
  try
    while nRANDOM <> 0 do begin
      ...
      if (Envir.m_MapFlag.nThunder <> 0) and (Random(nRANDOM + 1) = 0) then begin
        //
        ...
        for i := xTargetList.Count - 1 downto 0 do begin
          ...
          if (BaseObject <> nil) then begin
            if (Target = BaseObject) or Target.IsProperFriend(BaseObject) then begin
        ...
        Result := True;
        Break;
      end else if (Envir.m_MapFlag.nGreatThunder <> 0) and (Random(nRANDOM + 1) = 0) then begin
        ...
        Result := True;
        Break;
      end else if (Envir.m_MapFlag.nLava <> 0) and (Random(nRANDOM + 1) = 0) then begin
        ...
        for i := xTargetList.Count - 1 downto 0 do begin
          BaseObject := TBaseObject(xTargetList.Items[i]);
          if (BaseObject <> nil) then begin
            if Target.IsProperFriend(BaseObject) or (Target = BaseObject) then begin
              magpwr := Dmg div 2 + Random(Dmg);
              g_BaseObject.m_PEnvir := Envir;
              BaseObject.SendDelayMsg(g_BaseObject, RM_MAGSTRUCK, 0, magpwr, 1, 0, '', 600);
            end;
        ...
        Result := True;
        Break;
      end else if (Envir.m_MapFlag.nSpurt <> 0) and (Random(nRANDOM + 1) = 0) then begin
...
begin
  ...
  for i := 0 to xTargetList.Count - 1 do begin
    ...
    if BaseObject.m_boDeath then
      Continue;                         //0701
    if (BaseObject.m_btRaceServer = RC_PLAYOBJECT) or BaseObject.IsHero then begin
      if abs(abs(BaseObject.m_nCurrX - x) + (abs(BaseObject.m_nCurrY - y))) < dist then begin
        ...
        Result := BaseObject;
        if (BaseObject.m_nCurrX = x) and (BaseObject.m_nCurrY = y) then begin
          //Result := BaseObject;
          Break;
        end;
      end;
