...
begin
  Result := False;
  if not m_boSmiteLongSkill3 then begin
    if (GetTickCount - m_dwLatestSmiteLongTick3) >= g_Config.SuperSkillInvTime * 1000 then begin
      Result := True;
      Exit;
    end;
    HPper := Round(m_WAbil.HP / m_WAbil.MaxHP * 100);
    if (HPper <= 60) and (HPper >= 10) and (GetTickCount - m_dwLatestSmiteDamageTick >= _MAX(12, (g_Config.SuperSkillInvTime div 2)) * 1000) then begin
...
begin
  Result := False;
  if m_MagicArr[0][68] <> nil then begin
    nSpellPoint := GetSpellPoint(m_MagicArr[0][68]);
    if m_nInPowerPoint >= nSpellPoint then begin
      if (GetTickCount - m_dwLatestSkill68Tick) >= g_Config.nSuperSkill68InvTime * 1000 then begin
...
begin
  ...
  if (m_MagicArr[0][68] <> nil) and (m_TargetCret <> nil) then begin
    nSpellPoint := GetSpellPoint(m_MagicArr[0][68]);
    if m_nInPowerPoint >= nSpellPoint then begin
      X := abs(m_nCurrX - m_TargetCret.m_nCurrX);
      Y := abs(m_nCurrY - m_TargetCret.m_nCurrY);
      if (X <= 3) or (Y <= 3) then begin
        if (GetTickCount - m_dwLatestSkill68Tick) >= g_Config.nSuperSkill68InvTime * 1000 then begin
          m_dwLatestSkill68Tick := GetTickCount();
          HeroDoSpell(m_MagicArr[0][68], self, boTrainOk);
          m_dwHitTick := GetTickCount();
...
begin
  ...
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin

    if GetTickCount - m_dwCheckTitleTick >= 60 * 60 * 1000 then begin
      m_dwCheckTitleTick := GetTickCount;
      tObjCount := 0;
      for i := Low(THumTitles) to High(THumTitles) do begin
        if m_Titles[i].Index > 0 then begin
          ...
          if StdItem <> nil then begin
            ...
            if nInteger <= 0 then begin
              if (m_btActiveTitle in [1..6]) and (m_btActiveTitle - 1 = i) then begin
    ...
    if (m_dwUnParalysisTick > 0) then begin
      if GetTickCount > m_dwUnParalysisTick then begin
        m_dwUnParalysisTick := 0;
        SysMsg('防止石化能力消失！', c_Red, t_Hint);
      end else begin
        if GetTickCount - m_dwUnParalysisTick2 > 1000 then begin
          m_dwUnParalysisTick2 := GetTickCount;
          if m_nInPowerPoint > 0 then begin
    ...
    if m_wStatusTimeArr[POISON_STONE] > 0 then begin
      ...
      if (m_btReadySeriesSkill in [0, 2]) and (m_nInPowerLevel > 0) then begin
        if (GetTickCount - m_dwReadySeriesSkillTick > _MAX(12 * 1000, g_Config.nSeriesSkillReleaseInvTime - m_btReduceSpellTime * 1000)) then begin
      ...
      if (m_Master = nil) or (m_Master.m_btHeroRelax = 0) then begin
        if ((m_TargetCret = nil) and (GetTickCount - m_dwSearchEnemyTick > 1000)) then begin //((GetTickCount - m_dwSearchEnemyTick) > 6000) or
...
begin
  ...
  if m_TargetCret <> nil then begin
    ...
    if UseSkill68_2() then begin
      Result := True;
      Exit;
    end;

    UserMagic := m_MagicArr[0][118];
    if m_boDoubleStruck2 and m_boDoubleStruck3 and (UserMagic <> nil) and (GetTickCount - m_dwDragonRageSkillTick > _MAX(5 * 1000, g_Config.nMagicDragonRageInterval * 1000 - m_btDragonRageSkillLevel * 1500)) then begin
    ...
    if ProperTarget() then begin
      if (m_nInPowerLevel > 0) and
        ...
        (m_btSeriesSkillSetpMax in [1..4]) then begin
    ...
    if (m_MagicArr[0][115] <> nil) and (m_MagicArr[0][115].btKey = 0) and AllowSmiteLongSkill3() and GetLongAttackDirX(m_TargetCret, btDir) then begin
    ...
    //warr
    if (m_MagicArr[0][114] <> nil) and (m_MagicArr[0][114].btKey = 0) and GetLongAttackDirX(m_TargetCret, btDir) and (GetTickCount - m_dwLatestSmiteWideTick2 > _MAX(5 * 1000, g_Config.nSmiteWideHitSkillInvTime - m_btReduceSpellTime * 1000)) then begin
      if Integer(GetTickCount - m_dwHitTick) > dwAttackTime then begin
    ...
    if (m_MagicArr[0][66] <> nil) and (m_MagicArr[0][66].btKey = 0) and (GetTickCount - m_dwLatestHeroLongHitTick > 4 * 1000) and
      ...
      if (m_MagicArr[0][SKILL_ERGUM] <> nil) and (m_MagicArr[0][SKILL_ERGUM].btKey = 0) and (Random(nRange + 32) = 0) or (abs(m_nCurrX - m_TargetCret.m_nCurrX) > 2) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 2) then begin
        ...
        if (m_MagicArr[0][77] <> nil) and (m_MagicArr[0][77].btKey = 0) then begin
          if GetTickCount - m_dwLatestSkill77Tick > g_Config.Skill77Inv * 1000 + 3000 then begin
        ...
        if GetAttackDir(m_TargetCret, btDir) then begin
          if Integer(GetTickCount - m_dwHitTick) > dwAttackTime then begin
            m_dwHitTick := GetTickCount();
            //warr
            if (m_MagicArr[0][114] <> nil) and (m_MagicArr[0][114].btKey = 0) and (GetTickCount - m_dwLatestSmiteWideTick2 > _MAX(5 * 1000, g_Config.nSmiteWideHitSkillInvTime - m_btReduceSpellTime * 1000)) then begin
              m_dwLatestSmiteWideTick2 := GetTickCount;
              m_dwWalkTick := GetTickCount();
              nHitMode := 23;
              m_boSmiteWideSkill2 := True;
            ...
            if (m_MagicArr[0][115] <> nil) and (m_MagicArr[0][115].btKey = 0) and AllowSmiteLongSkill3() then begin
              m_dwLatestSmiteDamageTick := GetTickCount();
              m_dwLatestSmiteLongTick3 := GetTickCount();
              if m_boDecHP then begin
                m_boDecHP := False;
              ...
              if (GetTickCount - m_dwDoMotaeboTick) > (6 * 1000 + Random(5 * 1000)) then begin
                ...
                if m_WAbil.MP >= nSpellPoint then begin
                  ...
                  if TPlayObject(self).DoMotaebo(m_btDirection, UserMagic.btLevel) then begin
                    ...
                    if nTick < 0 then m_dwHitTick := dwTick + nTick + 400 + 90;
                    Result := True;
                    Exit;
                  end;
                end;
            ...
            LabelStartAttack:
            m_dwTargetFocusTick := GetTickCount();
            if nHitMode in [0, 3, 4, 5, 7, 9, 12, 13, 20..25] then begin
              m_dwHitTick := GetTickCount();
              if m_boLAT then
...
begin
  ...
  if (m_Abil.Level >= 7) and m_boHaveMag then begin
    if m_TargetCret <> nil then begin
      ...
      //wizard
      if (m_MagicArr[0][114] <> nil) and (m_MagicArr[0][114].btKey = 0) and (nAbsX < 8) and (nAbsY < 8) and (GetTickCount - m_dwLatestSmiteWideTick2 > _MAX(5 * 1000, g_Config.nSmiteWideHitSkillInvTime - m_btReduceSpellTime * 1000)) then begin
        if Integer(GetTickCount - m_dwHitTick) > dwAttackTime then begin
          m_dwLatestSmiteWideTick2 := GetTickCount;
          m_dwHitTick := GetTickCount();
          m_dwWalkTick := GetTickCount();
      ...
      if (m_WAbil.MP > 0) and ((nAbsX > 1) or (nAbsY > 1)) and (not boWarr or not bola) then begin
        ...
        if (nAbsX <= g_Config.nMagicAttackRage) and (nAbsY <= g_Config.nMagicAttackRage) then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            ...
            if (m_MagicArr[0][116] <> nil) and (m_MagicArr[0][116].btKey = 0) and AllowSuperSkill3() then begin
              m_dwTargetFocusTick := GetTickCount();
        ...
        if (m_TargetCret.m_PEnvir = m_PEnvir) then begin
          ...
          //labLoop2:
          if (m_Master = nil) or (((abs(nX - m_Master.m_nCurrX) < 13) and (abs(nY - m_Master.m_nCurrY) < 13))) then begin
            SetTargetXY(nX, nY);
          end else
            DelTargetCreat();
        end else
    ...
    if GetAttackDir(m_TargetCret, btDir) then begin
      ...
      Result := True;
    end else if m_TargetCret.m_PEnvir = m_PEnvir then begin
      SetTargetXY(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
    end else
      DelTargetCreat();
  end;
...
begin
  ...
  if (m_Abil.Level < 18) or
    ...
    if m_TargetCret = nil then begin
      ...
      //taos
      if (m_MagicArr[0][114] <> nil) and (m_MagicArr[0][114].btKey = 0) and (nAbsX <= 6) and (nAbsY <= 6) and (GetTickCount - m_dwLatestSmiteWideTick2 > _MAX(5 * 1000, g_Config.nSmiteWideHitSkillInvTime - m_btReduceSpellTime * 1000)) then begin
        if Integer(GetTickCount - m_dwHitTick) > dwAttackTime then begin
          m_dwHitTick := GetTickCount();
          m_dwWalkTick := GetTickCount();
          m_dwLatestSmiteWideTick2 := GetTickCount;
      ...
      if (m_WAbil.MP > 0) and ((nAbsX > 1) or (nAbsY > 1)) and (not boWarr or not bola) then begin
        if (nAbsX <= g_Config.nMagicAttackRage) and (nAbsY <= g_Config.nMagicAttackRage) then begin
          if Integer(GetTickCount - m_dwHitTick) > (m_nNextHitTime) then begin
                    ...
                    if StdItem <> nil then begin
                      if (StdItem.Shape = 1) then begin
                        if (GetTagXYRangeCount(0, 1) > 2) then begin
                          if (m_MagicArr[0][38] <> nil) and (m_MagicArr[0][38].btKey = 0) then begin
                          ...
                          if (m_MagicArr[0][6] <> nil) and (m_MagicArr[0][6].btKey = 0) then begin
            ...
            if (m_MagicArr[0][117] <> nil) and (m_MagicArr[0][117].btKey = 0) and AllowSuperSkill3() then begin
              m_dwTargetFocusTick := GetTickCount();
