...
var
  i, ii, nRet, nCheckCode, nHeroFlag: Integer;
  TempList                  : TList;
...
begin
  try
    ...
    for i := 0 to m_SaveRcdTempList.Count - 1 do begin
      ...
      if SaveHumRcdToDB(SaveRcd.sAccount, SaveRcd.sChrName, SaveRcd.nSessionID, SaveRcd.HumanRcd, nHeroFlag) or (SaveRcd.nReTryCount > 16) then begin //0625
        if SaveRcd.sChrName <> '' then begin
          New(UserSaveInfo);
          UserSaveInfo.bNewHero := False;
          UserSaveInfo.RcdSaved := True;
          UserSaveInfo.sChrName := SaveRcd.sChrName;
          if (nHeroFlag >= 0) and (SaveRcd.nReTryCount <= 50) then begin
    ...
    try
      for i := 0 to m_LoadRcdTempList.Count - 1 do begin
        ...
        if LoadHumFromDB(LoadDBInfo, boReTryLoadDB) = 0 then begin
...
var
  i                         : Integer;
  boAddTo                   : Boolean;
  LoadRcd, LoadRcdInfo      : pTLoadDBInfo;
begin
  boAddTo := True;
  New(LoadRcdInfo);
  ...
  try
    for i := 0 to m_LoadRcdList.Count - 1 do begin
      ...
      if LoadRcd.btQueryMsg = 1 then begin
        boAddTo := False;
        Break;
      end;
    end;
...
begin
  ...
  case LoadUser.btClinetFlag of
    0: if LoadUser.sMasterName <> '' then begin //Load Hero Rcd
        ...
        if PlayObject <> nil then begin
          ...
          if LoadHumRcdFromDB(LoadUser.sAccount, LoadUser.sCharName, LoadUser.sIPaddr, HumanRcd, {HerosInfo,} LoadUser.nSessionID) then begin
    ...
    1: begin                            //Load Human Rcd
        ...
        if UserEngine.GetHeroObjectCS_IdName(LoadUser.sAccount, LoadUser.sCharName) <> nil then begin
          UserEngine.KickPlayer(LoadUser.sCharName);
          boReTry := True;
          Exit;
        end;
...
{$IFEND V_TEST}
    ...
    2: begin                            //Load "recall player" Rcd
        PlayObject := UserEngine.GetPlayObjectCS_Name(LoadUser.sMasterName);
        if PlayObject <> nil then begin
          if LoadHumRcdFromDB(LoadUser.sAccount, LoadUser.sCharName, LoadUser.sIPaddr, HumanRcd, {HerosInfo,} LoadUser.nSessionID) then begin
            HumanRcd.Data.sHeroMasterName := LoadUser.sMasterName;
    ...
    3: begin
        ...
        if (LoadUser.nLRResult <> -3) then begin
          New(UserOpenInfo);
    ...
    4: begin                            //Load Human Rcd
        ...
        try
          if not LoadHumRcdFromDB(LoadUser.sAccount, LoadUser.sCharName, LoadUser.sIPaddr, HumanRcd, {HerosInfo,} LoadUser.nSessionID) then begin
            //RunSocket.SendOutConnectMsg(LoadUser.nGateIdx, LoadUser.nSocket, LoadUser.nGSocketIdx);
          end else begin
            New(UserOpenInfo);
    ...
    5: begin
        Result := 4;
        if LoadHerosFromDB(LoadUser.sAccount, LoadUser.sCharName, LoadUser.sIPaddr, HerosInfo, LoadUser.nSessionID) then begin
          New(UserOpenInfo);
...
begin
  ...
  try
    for i := 0 to m_SaveRcdList.Count - 1 do begin
      if CompareText(pTSaveRcd(m_SaveRcdList.Items[i]).sChrName, sChrName) = 0 then begin
        Result := True;
        Break;
      end;
    end;
...
begin
  ...
  try
    for i := 0 to m_SaveRcdList.Count - 1 do begin
      p := m_SaveRcdList[i];
      if (CompareText(p.sChrName, sChrName) = 0) and (p.nSessionID = nSessionID) then begin
        p.HumanRcd := HumanRcd;
        Break;
      end;
    end;
