...
begin
  ...
  try
    if CheckBoxHero.Checked then begin
      for i := 0 to UserEngine.m_HeroProcList.Count - 1 do begin
...
begin
  ...
  case nSort of
    ...
    5: for i := 0 to ViewList.Count - 1 do begin
        PlayObject := TPlayObject(ViewList.Objects[i]);
    ...
    6: for i := 0 to ViewList.Count - 1 do begin
        PlayObject := TPlayObject(ViewList.Objects[i]);
        if PlayObject.m_btRaceServer = RC_PLAYOBJECT then
    ...
    7: for i := 0 to ViewList.Count - 1 do begin
...
begin
  if (ViewList.Count > 0) and ((GetTickCount - dwTimeOutTick > 60 * 1000) or (boRefGridSession and (GetTickCount > dwRefGridSession))) then begin
    boRefGridSession := False;
    ViewList.Clear;
    RefGridSession();
  end;
...
begin
  ...
  if not CheckBoxHero.Checked then begin
    ...
    if Integer(PlayObject) <> Integer(ViewList.Objects[nSelIndex]) then begin
    ...
    if Integer(HeroObject) <> Integer(ViewList.Objects[nSelIndex]) then begin
...
begin
  ...
  try
    for i := 0 to UserEngine.m_PlayObjectList.Count - 1 do begin
      ...
      if EditLevelMIN.Value > EditLevelMAX.Value then begin
...
begin
  if EditLevelMAX.Value < EditLevelMIN.Value then begin
