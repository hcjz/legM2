...
begin
  ...
  try
    ...
    if not m_boDeath then begin
      if (m_WAbil.HP > 0) and (m_WAbil.HP < m_WAbil.MaxHP) and (m_nHealthTick >= g_Config.nHealthFillTime) then begin
        nAddPoint := (m_WAbil.MaxHP div 75) + 1;
      ...
      if (m_WAbil.MP < m_WAbil.MaxMP) and (m_nSpellTick >= g_Config.nSpellFillTime) then begin
        nAddPoint := (m_WAbil.MaxMP div 18) + 1;
  ...
  try
    if not m_boDeath then begin         //0901
      ...
      for i := Low(m_dwStatusArrTick) to High(m_dwStatusArrTick) do begin
        if (m_wStatusTimeArr[i] > 0) and (m_wStatusTimeArr[i] < g_MaxStatusTime) then begin
            ...
            if (m_wStatusTimeArr[i] = 0) then begin
              ...
              case i of
                ...
                POISON_STONE: begin
                    ...
                    if m_btMedusaEyeAttack > 0 then begin
      ...
      for i := Low(m_wStatusArrValue2) to High(m_wStatusArrValue2) do begin
        if m_wStatusArrValue2[i] > 0 then begin
          if GetTickCount() > m_dwStatusArrTimeOutTick2[i] then begin
  ...
  try
    if not m_boDeath then begin
      boNeedRecalc := False;
      for i := Low(m_wPowerRateTick) to High(m_wPowerRateTick) do begin
        if (m_wPowerRate[i] > 0) and (GetTickCount() > m_wPowerRateTick[i]) then begin
          m_wPowerRate[i] := 0;
          m_sStatuName := '';
        end;
      end;
...
begin
  ...
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if not m_boWalkWaitLocked and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
      ...
      if (m_Master <> nil) and not m_Master.m_boGhost then begin
        if (m_PEnvir = m_Master.m_PEnvir) and (abs(m_nCurrX - m_Master.m_nCurrX) <= 11) and (abs(m_nCurrY - m_Master.m_nCurrY) <= 11) then begin
      ...
      if (m_nTargetX <> -1) and (m_Master <> nil) then begin //blue
        GotoTargetXY();
      end;
    end;
  end;
...
begin
  if (g_MapEventNPC <> nil) and (m_PEnvir <> nil) then begin
    if m_ExpHitter <> nil then
      CSObject := GetMainPlayer(m_ExpHitter)
    else if m_LastHiter <> nil then
      CSObject := GetMainPlayer(m_LastHiter);

    if (CSObject <> nil) and (CSObject.m_btRaceServer = RC_PLAYOBJECT) and (CSObject <> m_Master) then begin
  ...
  try
    if (m_btRaceServer <> RC_PLAYOBJECT) and (m_LastHiter <> nil) then begin
      ...
      if (m_ExpHitter <> nil) then begin
        ...
        if CSObject.m_btRaceServer = RC_PLAYOBJECT then begin
