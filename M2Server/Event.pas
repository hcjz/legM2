...
begin
  ...
  m_boAddToMap := True;
  if nType in [ET_ITEMMINE1..ET_ITEMMINE3] then begin
    if m_Envir.AddToMapItemEvent(nX, nY, OS_EVENTOBJECT, self) = nil then
      m_boAddToMap := False;
    m_boVisible := False;
...
begin
  ...
  try
    for i := m_EventList.Count - 1 downto 0 do begin
      ...
      if Event.m_boActive and ((GetTickCount - Event.m_dwRunStart) > 500) then begin
...
begin
  if (Event = nil) or not Event.m_boVisible then Exit; //0710
  m_EventList.Lock;
  try
...
begin
  if (m_nDamage > 0) and (m_Envir <> nil) and (m_Owner <> nil) and (GetTickCount - m_dwTickTime > 3000) then begin
    ...
    try
      try
        ...
        for i := 0 to BaseObjectList.Count - 1 do begin
          ...
          if (TargeTBaseObject <> nil) and (m_Owner.IsProperTarget(TargeTBaseObject)) then begin
...
begin
  ...
  if (m_Envir <> nil) and (m_boVisible) then begin
    if m_Envir.AddToMap(m_nX, m_nY, OS_EVENTOBJECT, self) = nil then begin
      //m_boVisible := False;             //0406
      //m_Envir := nil;
    end;
  end else
    m_boVisible := False;
...
begin
  Result := False;
  if (m_nEventType = ET_FIRE) and g_Config.boFireBurnEventOff then begin
...
begin
  if not m_boClosed and (GetTickCount - m_dwOpenStartTick > m_dwContinueTime) or CanClose() then begin
    //m_Owner := nil;                     //0618    Í¬²½
    try
      Close();
    finally
