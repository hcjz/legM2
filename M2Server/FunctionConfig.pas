...
type
  TfrmFunctionConfig = class(TForm)
    ...
    procedure CheckMoveMakeSlaveClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
...
uses M2Share, HUtil32, svMain, LocalDB;
...
begin
  ...
  CheckMoveMakeSlave.Checked := g_Config.boMoveMakeSlave; //道士心灵召唤  Development 2019-01-12 添加
...
begin
  if botiOpenSystem <> g_Config.tiOpenSystem then begin
...
{$IF SoftVersion <> VERDEMO}
  ...
  Config.WriteBool('Setup', 'MoveMakeSlave', g_Config.boMoveMakeSlave); //道士心灵召唤  Development 2019-01-12 添加
...
begin
  if not boOpened then
    Exit;
  if Sender = CheckBoxSpiritMutiny then begin
  ...
  end else if Sender = cbSpiritMutinyDie then
...
begin
  if ButtonHeroSetSave2 = Sender then begin
...
begin
  ...
  else g_Config.nMagicShootingStarPowerRate := seMagicShootingStarPowerRate.Value;
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  if Sender = CheckBoxGetFullExp then begin
...
procedure TfrmFunctionConfig.CheckMoveMakeSlaveClick(Sender: TObject);
begin
//道士心灵召唤  Development 2019-01-12 添加
  if not boOpened then
    Exit;
  g_Config.boMoveMakeSlave := CheckMoveMakeSlave.Checked;
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  if Sender = SpinEditShadowExpriesTime then begin
    g_Config.nShadowExpriesTime := SpinEditShadowExpriesTime.Value;
  end else if Sender = EditHeroGainExpRate2 then begin
