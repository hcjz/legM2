  ...
  if PlayObject.m_UseItems[U_ARMRINGL].wIndex > 0 then begin
    ...
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of
        ...
        2: begin
            if (AmuletStdItem.Shape = PlayObject.m_btUseAmulet) and (Round(PlayObject.m_UseItems[U_ARMRINGL].Dura / 100) >= nCount) then begin
              Idx := U_ARMRINGL;
              Result := True;
              Exit;
            end;
  ...
  if PlayObject.m_UseItems[U_BUJUK].wIndex > 0 then begin
    ...
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of
        ...
        2: begin
            if (AmuletStdItem.Shape = PlayObject.m_btUseAmulet) and (Round(PlayObject.m_UseItems[U_BUJUK].Dura / 100) >= nCount) then begin
              Idx := U_BUJUK;
              Result := True;
              Exit;
            end;
  ...
  if PlayObject.m_UseItems[U_ARMRINGL].wIndex > 0 then begin
    ...
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of
        ...
        2: begin
            if (AmuletStdItem.Shape in [1, 2]) and (Round(PlayObject.m_UseItems[U_ARMRINGL].Dura / 100) >= nCount) then begin
              Idx := U_ARMRINGL;
              Result := True;
              Exit;
            end;
  ...
  if PlayObject.m_UseItems[U_BUJUK].wIndex > 0 then begin
    ...
    if (AmuletStdItem <> nil) and (AmuletStdItem.StdMode = 25) then begin
      case nType of
        ...
        2: begin
            if (AmuletStdItem.Shape in [1, 2]) and (Round(PlayObject.m_UseItems[U_BUJUK].Dura / 100) >= nCount) then begin
              Idx := U_BUJUK;
              Result := True;
              Exit;
            end;
...
begin
  if (PlayObject.m_btRaceServer = RC_PLAYOBJECT) or PlayObject.IsHero then begin
    if PlayObject.m_UseItems[Idx].Dura > nCount * 100 then begin
      ...
      if (PCardinal(@PlayObject.m_UseItems[Idx].btValue[22])^ > 0) and (PCardinal(@PlayObject.m_UseItems[Idx].btValue[22])^ = PlayObject.m_dwIdCRC) and g_Config.boBindNoMelt then begin
      ...
      if (not BaseObject.m_boDeath) and (BaseObject <> PlayObject) then begin
        if (PlayObject.m_Abil.Level > BaseObject.m_Abil.Level) and (not BaseObject.m_boStickMode) then begin
          ...
          if (Random(20) < 6 + nPushLevel * 3 + levelgap) then begin
            if PlayObject.IsProperTarget(BaseObject, True) then begin
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if PlayObject.IsProperFriend(BaseObject) or (PlayObject.m_btRaceServer = RC_HERO) then begin
...
var
  ...
  begin
    Result := False;
    if (PlayObject.m_PEnvir <> nil) and (PlayObject.m_PEnvir.m_MapFlag.boNotAllowUseMag) then begin
      if PlayObject.m_PEnvir.m_MapFlag.sNotAllowUseMag <> nil then begin
        for i := 0 to PlayObject.m_PEnvir.m_MapFlag.sNotAllowUseMag.Count - 1 do begin
          if cMag.wMagicId = Integer(PlayObject.m_PEnvir.m_MapFlag.sNotAllowUseMag[i]) then begin
...
begin
  Result := False;
  if PlayObject.m_boTDBeffect then Exit;
  ...
  if UserMagic.MagicInfo.wMagicId <> 74 then begin
    nPower := 0;
    if UserMagic.MagicInfo.wMagicId = SKILL_AMYOUNSUL then begin
      if g_Config.boHeroNeedAmulet or ((PlayObject.m_btRaceServer = RC_PLAYOBJECT) and not PlayObject.m_boOffLinePlay) then begin
  ...
  if (g_FunctionNPC <> nil) then begin
    if PlayObject.m_btRaceServer = RC_PLAYOBJECT then begin
      ...
      if g_QFLabelList.Exists(sSendFalg) then begin
        g_FunctionNPC.m_OprCount := 0;
        g_FunctionNPC.GotoLable(TPlayObject(PlayObject), sSendFalg, False);
      end;

      if (TargeTBaseObject <> nil) and (TargeTBaseObject.m_btRaceServer = RC_PLAYOBJECT) and PlayObject.IsProperTarget(TargeTBaseObject) then begin
  ...
  case UserMagic.MagicInfo.wMagicId of
    SKILL_FIREBALL, SKILL_FIREBALL2: begin //火球术 大火球
    ...
    SKILL_HEALLING {2}: begin
        ...
        if PlayObject.IsProperFriend(TargeTBaseObject) or (PlayObject.m_btRaceServer = RC_HERO) then begin
          if TargeTBaseObject.m_btRaceServer <> rc_Escort then begin
          ...
          case UserMagic.MagicInfo.wMagicId of
            SKILL_57 {噬血术}: begin
                if PlayObject.IsProperTarget(TargeTBaseObject) then begin
                  ...
                  if bb or (Random(15) >= TargeTBaseObject.m_nantiMagic) then begin
                    if (abs(TargeTBaseObject.m_nCurrX - nTargetX) <= 2) and (abs(TargeTBaseObject.m_nCurrY - nTargetY) <= 2) then begin
            ...
            SKILL_SKELLETON {17}: begin //召唤骷髅
                if UserMagic.btLevel >= 4 then begin
                  if MagMakeSlave(PlayObject, UserMagic, nTargetX, nTargetY) then
                    boTrain := True;
                end else if MagMakeSlave(PlayObject, UserMagic) then begin
                  boTrain := True;
                end;
              end;
    ...
    SKILL_SHOWHP {28}: begin
        if (TargeTBaseObject <> nil) and not TargeTBaseObject.m_boShowHP then begin
          if Random(6) <= (_MIN(4, UserMagic.btLevel) + 3) then begin
        ...
        if b then begin
          if UserMagic.btLevel >= 4 then begin
            if MagMakeSinSuSlave(PlayObject, UserMagic, nTargetX, nTargetY) then
              boTrain := True;
          end else if MagMakeSinSuSlave(PlayObject, UserMagic) then begin
            boTrain := True;
          end;
          boSpellFail := False;
        end;
    ...
    SKILL_UNAMYOUNSUL {34}: begin       //解毒术
        ...
        if PlayObject.IsProperFriend(TargeTBaseObject) or (PlayObject.m_btRaceServer = RC_HERO) then begin
          if Random(7) - (_MIN(4, UserMagic.btLevel) + 1) < 0 then begin
            ...
            if TargeTBaseObject.m_wStatusTimeArr[POISON_SHOCKED] <> 0 then begin
            ...
            if TargeTBaseObject.m_wStatusTimeArr[POISON_DONTMOVE] <> 0 then begin
              TargeTBaseObject.m_wStatusTimeArr[POISON_DONTMOVE] := 1;
              //TargeTBaseObject.m_boTDBeffect := False;
              boTrain := True;
            end;
            if TargeTBaseObject.m_wStatusArrValue[6] <> 0 then begin
    ...
    SKILL_GROUPDEDING {39 地钉}: begin
        if GetTickCount - PlayObject.m_dwMagNailTick > g_Config.dwMagNailTick then begin
          PlayObject.m_dwMagNailTick := GetTickCount();
          if MagGroupDeDing(PlayObject, UserMagic, nTargetX, nTargetY, TargeTBaseObject) then
            boTrain := True;
        end else PlayObject.SysMsg('技能施展时间未到！', c_Red, t_Hint);
      end;
    SKILL_41: begin                     //狮子吼
        ...
        if PlayObject <> nil then begin
          with PlayObject do begin
            ...
            if m_wStatusArrValue[2] = 0 then begin
              if GetTickCount - m_dwDoubleScTick > g_Config.DoubleScInvTime * 1000 then begin
    ...
    69: begin
        if UserMagic.btLevel >= 4 then begin
          if MagMakeSnowMonSlave(PlayObject, UserMagic, nTargetX, nTargetY) then begin
            boTrainOk := True;
            Result := True;
          end;
        end else if MagMakeSnowMonSlave(PlayObject, UserMagic) then begin
          boTrainOk := True;
          Result := True;
        end;
        PlayObject.SendRefMsg(RM_MAGICFIRE, UserMagic.btLevel,
    ...
    74: begin
        if (PlayObject.m_btRaceServer = RC_PLAYOBJECT) then begin
          if (GetTickCount - PlayObject.m_dwCloneSelfTick > g_Config.dwCloneSelfTime) then begin
    ...
    77: begin
        if PlayObject.MagPossessedUp(STATE_13, UserMagic.btLevel, g_Config.Skill77Time) then begin
          boTrainOk := True;
          Result := True;
        end;
        PlayObject.SendRefMsg(RM_MAGICFIRE, UserMagic.btLevel,
    ...
    80: begin
        ...
        if MagicIceRain(PlayObject, nTargetX, nTargetY, nPower, UserMagic) then begin
          boTrain := True;
          //boSpellFail := False;
        end else begin
          boSpellFail := True;
    ...
    81: begin
        ...
        if MagMoveXY(PlayObject, nTargetX, nTargetY, nPower, UserMagic) then begin
          boTrain := True;
          boSpellFail := False;
        end else begin
          boSpellFail := True;
    ...
    82: begin
        ...
        if MagicDeadEye(PlayObject, nTargetX, nTargetY, nPower, UserMagic) then begin
          boTrain := True;
          //boSpellFail := False;
        end else begin
          boSpellFail := True;
    ...
    118: begin
        if MagDragonRage(PlayObject, UserMagic) then begin
          boTrain := True;
          //boSpellFail := False;
          PlayObject.SendRefMsg(RM_MAGICFIRE, UserMagic.btLevel,
            MakeWord(UserMagic.MagicInfo.btEffectType, UserMagic.MagicInfo.btEffect),
...
{$IF SERIESSKILL}
    ...
    111: begin                          //万箭归宗
        ...
        if MagBigExplosion(PlayObject,
          ...
          nTargetX, nTargetY,
          2,
          UserMagic.MagicInfo.wMagicId) then begin
          boTrain := True;
          PlayObject.m_dwReadySeriesSkillTick := GetTickCount();
        end;
      end;
...
{$IFEND SERIESSKILL}
  ...
  if ((PlayObject.m_btRaceServer = RC_PLAYOBJECT) or PlayObject.IsHero) and (UserMagic.btLevel < PlayObject.MagicMaxTrainLevel(UserMagic)) and boTrain then begin
    if UserMagic.wMagIdx in [100..111] then
      n1C := PlayObject.m_nInPowerLevel
    else
      n1C := PlayObject.m_Abil.Level;
    if UserMagic.MagicInfo.TrainLevel[UserMagic.btLevel] <= n1C then begin
  ...
  if (UserMagic <> nil) then begin
    ...
    if boTrain then begin
      if (UserMagic.btLevel < PlayObject.MagicMaxTrainLevel(UserMagic)) then begin
        if UserMagic.MagicInfo.TrainLevel[UserMagic.btLevel] <= PlayObject.m_nInPowerLevel then begin
          PlayObject.TrainSkill(UserMagic, Random(3) + 1);
          if not PlayObject.CheckMagicLevelup(UserMagic) then
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if (TargeTBaseObject.m_btRaceServer >= RC_ANIMAL) and (TargeTBaseObject.m_TargetCret = BaseObject) then begin
      if (abs(TargeTBaseObject.m_nCurrX - BaseObject.m_nCurrX) > 1) or (abs(TargeTBaseObject.m_nCurrY - BaseObject.m_nCurrY) > 1) or (Random(2) = 0) then begin
...
begin
  Result := False;
  if not (TargeTBaseObject.m_btRaceServer in [RC_PLAYOBJECT, RC_HERO]) and ((Random(4 - nMagicLevel) = 0)) then begin
    TargeTBaseObject.m_TargetCret := nil;
          ...
          if Random(3) = 0 then begin
            if Random((BaseObject.m_Abil.Level + 20) + (nMagicLevel * 5)) > (TargeTBaseObject.m_Abil.Level + g_Config.nMagTammingTargetLevel {10}) then begin
              if (TargeTBaseObject.m_btLifeAttrib = 0) and
                ...
                (BaseObject.m_SlaveList.Count < g_Config.nMagTammingCount) then begin
                ...
                if (TargeTBaseObject.m_Master <> BaseObject) and (Random(n14) = 0) then begin
                  ...
                  if TargeTBaseObject.m_boCanReAlive and (TargeTBaseObject.m_Master = nil) then begin
                    TargeTBaseObject.m_boCanReAlive := False;
                    if (TargeTBaseObject.m_pMonGen <> nil) then begin
                      if (TargeTBaseObject.m_pMonGen.nActiveCount > 0) then begin
...
begin
  ...
  if (PoseBaseObject <> nil) and
    ...
    if (abs(PlayObject.m_nCurrX - PoseBaseObject.m_nCurrX) <= 1) and
      ...
      if Random(18) < _MIN(4, UserMagic.btLevel) * 6 + 6 + (PlayObject.m_Abil.Level - PoseBaseObject.m_Abil.Level) then begin
...
begin
  ...
  end else if (PlayObject.m_btRaceServer = RC_PLAYOBJECT) and (PlayObject.m_HeroObject <> nil) then begin
    nPower := Round(PlayObject.GetAttackPower(GetPower(MPow(UserMagic), UserMagic) + LoWord(PlayObject.m_WAbil.DC), SmallInt(HiWord(PlayObject.m_WAbil.DC) - LoWord(PlayObject.m_WAbil.DC)) + 1) * 3.59 * (g_Config.nSkillTWPowerRate + PlayObject.m_wInPowerRateEx) / 100);
  ...
  for nCX := nTargetX - 3 to nTargetX + 3 do begin
    for nCY := nTargetY - 3 to nTargetY + 3 do begin
      if abs(nCX - nTargetX) = abs(nCY - nTargetY) then begin
...
begin
  ...
  for nCX := nTargetX - 2 to nTargetX + 2 do begin
    for nCY := nTargetY - 2 to nTargetY + 2 do begin
      BaseObject := PlayObject.m_PEnvir.GetMovingObject(nCX, nCY, True);
      if PlayObject.IsProperTarget(BaseObject, True) then begin
        nPower := n;
        if BaseObject.m_btLifeAttrib = LA_UNDEAD then
...
begin
  ...
  if not PlayObject.IsProperTarget(TargeTBaseObject, True) then begin
    TargeTBaseObject := nil;
    Exit;
  end;
  if (TargeTBaseObject.m_btRaceServer = RC_PLAYOBJECT) or (TargeTBaseObject.m_btRaceServer >= RC_ANIMAL) then
  ...
  if (PlayObject.m_Abil.Level > TargeTBaseObject.m_Abil.Level) and (not TargeTBaseObject.m_boStickMode) then begin
    if (Random(g_Config.nMagIceBallRange) < 6 + _MIN(4, UserMagic.btLevel) * 3) then begin
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    TargeTBaseObject := TBaseObject(BaseObjectList.Items[i]);
    if BaseObject.IsProperTarget(TargeTBaseObject, True) then begin
      BaseObject.SetTargetCreat(TargeTBaseObject);
...
begin
  ...
  n := nPower;
  Randomize();
  for nCX := nTargetX - nRage to nTargetX + nRage do begin
    for nCY := nTargetY - nRage to nTargetY + nRage do begin
      BaseObject := PlayObject.m_PEnvir.GetMovingObject(nCX, nCY, True);
      if PlayObject.IsProperTarget(BaseObject, True) then begin
        nPower := n;
        if BaseObject.m_btLifeAttrib = LA_UNDEAD then
        ...
        if ((abs(nCX - nTargetX) = 1) and (abs(nCY - nTargetY) in [0, 1])) or ((abs(nCX - nTargetX) in [0, 1]) and (abs(nCY - nTargetY) = 1)) then begin
          nPower := Round(n * 0.82);
        end else if ((abs(nCX - nTargetX) = 2) and (abs(nCY - nTargetY) in [0..2])) or ((abs(nCX - nTargetX) in [0..2]) and (abs(nCY - nTargetY) = 2)) then begin
          nPower := Round(n * 0.55)
        end else if ((abs(nCX - nTargetX) = 3) and (abs(nCY - nTargetY) in [0..3])) or ((abs(nCX - nTargetX) in [0..3]) and (abs(nCY - nTargetY) = 3)) then begin
          nPower := Round(nPower * 0.35);
        end;
        if Random(3) = 0 then begin
          if lv > Random(22) then begin
...
begin
  ...
  if BaseObjectList.Count > 0 then begin
    ...
    for i := 0 to BaseObjectList.Count - 1 do begin
      ...
      if PlayObject.IsProperTarget(BaseObjectA, True) then begin
...
begin
  ...
  if BaseObject.m_PEnvir.CanWalk(nX, nY, True) then begin
    nl := 5;
    if (TPlayObject(BaseObject).m_MagicArr[1][nmid] <> nil) then begin
    ...
    for i := 0 to BaseObjectList.Count - 1 do begin
      ...
      if (TargeTBaseObject.m_btRaceServer >= RC_ANIMAL) and
        ...
        (TargeTBaseObject.m_btRaceServer <> rc_Escort) then begin
        TargeTBaseObject.OpenHolySeizeMode(nPower * 1000);
        if MagicEvent = nil then begin
          New(MagicEvent);
          for ii := 0 to 7 do MagicEvent.Events[ii] := nil;
    ...
    end else if MagicEvent <> nil then begin
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    TargeTBaseObject := TBaseObject(BaseObjectList.Items[i]);
    if BaseObject.IsProperFriend(TargeTBaseObject) or (BaseObject.m_btRaceServer = RC_HERO) then begin
      if TargeTBaseObject.m_wStatusTimeArr[STATE_TRANSPARENT {0x70}] = 0 then begin
    ...
    if BaseObject.IsProperTarget(TargeTBaseObject) then begin
      if (TargeTBaseObject.m_nantiMagic <= Random(10)) and (abs(TargeTBaseObject.m_nCurrX - nTargetX) <= 1) and (abs(TargeTBaseObject.m_nCurrY - nTargetY) <= 1) then begin
        ...
        if (Random(2) + (BaseObject.m_Abil.Level - 1)) > TargeTBaseObject.m_Abil.Level then begin
          ...
          if (Random(g_Config.nMabMabeHitRandRate {100}) < _MAX(g_Config.nMabMabeHitMinLvLimit, (nLevel * 8) - nLevel + 15 + nLv)) then begin
...
begin
  Result := False;
  if not PlayObject.MakeSinSuSlave then begin
    ...
    if nX >= 0 then begin
      if PlayObject.m_PEnvir <> nil then begin
        if PlayObject.m_PEnvir.CanWalk(nY, nY, True) then begin
          if PlayObject.MakeSlaveEx(sMonName, nX, nY, nMakeLevel, nExpLevel, nCount, dwRoyaltySec) <> nil then
            Result := True;
        end else if PlayObject.MakeSlave(sMonName, nMakeLevel, nExpLevel, nCount, dwRoyaltySec) <> nil then
          Result := True
        else
...
//召唤神兽  Development 2019-01-12 分析
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := True;
  //设置心灵召唤，再次按技能键神兽会飞到身边 Development 2019-01-12
    if (PlayObject.m_SlaveList.Count > 0) and g_Config.boMoveMakeSlave then
    begin
      for i := 0 to PlayObject.m_SlaveList.Count - 1 do
      begin
        Slave := TBaseObject(PlayObject.m_SlaveList.Items[i]);

        if (Slave <> nil) and (not Slave.m_boDeath) then
        begin
          //if CompareText(Slave.m_sCharName, sMonName) = 0 then
          //begin
            //if Slave.m_TargetCret <> nil then
              //Slave.m_TargetCret := nil; //召唤到身边后,不再回去打原来的目标  Development 2019-01-12
...
//召唤月灵  Development 2019-01-12 分析
...
begin
  ...
  begin
    ...
    if PlayObject.MakeSlave(sMonName, nMakeLevel, nExpLevel, nCount, dwRoyaltySec) <> nil then
      Result := True;
  //设置心灵召唤，再次按技能键月灵会飞到身边 Development 2019-01-12
    if (PlayObject.m_SlaveList.Count > 0) and g_Config.boMoveMakeSlave then
    begin
      for i := 0 to PlayObject.m_SlaveList.Count - 1 do
      begin
...
//召唤骷髅  Development 2019-01-12 分析
...
begin
  ...
  begin
    ...
    begin
      ...
      Result := True;
  //设置心灵召唤，再次按技能键骷髅会飞到身边 Development 2019-01-12
    if (PlayObject.m_SlaveList.Count > 0) and g_Config.boMoveMakeSlave then
    begin
      for i := 0 to PlayObject.m_SlaveList.Count - 1 do
      begin
...
begin
  ...
  if PlayObject.m_wStatusTimeArr[POISON_STONE] > 1 then begin
    ...
    try
      ...
      for i := 0 to BaseObjectList.Count - 1 do begin
        ...
        if BaseObject.m_boDeath or (BaseObject.m_boGhost) or (PlayObject = BaseObject) then
          Continue;
        if PlayObject.IsProperTarget(BaseObject, True) then begin
          if not BaseObject.m_boUnParalysis and (GetTickCount >= BaseObject.m_dwUnParalysisTick) then begin
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if BaseObject.m_boDeath or (BaseObject.m_boGhost) or (PlayObject = BaseObject) then
      Continue;
    if PlayObject.IsProperTarget(BaseObject, True) then begin
      if (not BaseObject.m_boUnParalysis) and (not BaseObject.m_boFastParalysis) then begin
    ...
    if (BaseObject.m_btRaceServer = RC_PLAYOBJECT) or (BaseObject.m_btRaceServer >= RC_ANIMAL) then
      Result := True;
    if n > (_MIN(4, UserMagic.btLevel) + 1) * 15 then Break;
  end;
  BaseObjectList.Free;
end;
    ...
    if (abs(PlayObject.m_nCurrX - BaseObject.m_nCurrX) <= 1) and (abs(PlayObject.m_nCurrY - BaseObject.m_nCurrY) <= 1) then begin
      if (not BaseObject.m_boDeath) and (BaseObject <> PlayObject) then begin
        if (PlayObject.m_Abil.Level >= BaseObject.m_Abil.Level) and (not
          BaseObject.m_boStickMode) then begin
          levelgap := PlayObject.m_Abil.Level - BaseObject.m_Abil.Level;
          if (Random(20) < 11 + nPushLevel * 3 + levelgap) then begin
            if PlayObject.IsProperTarget(BaseObject, True) then begin
              push := 1 + _MAX(0, nPushLevel - 1) + Random(2);
              nDir := GetNextDirection(PlayObject.m_nCurrX, PlayObject.m_nCurrY, BaseObject.m_nCurrX, BaseObject.m_nCurrY);
              BaseObject.CharPushed(nDir, push);
...
begin
  ...
  if (TargeTBaseObject <> nil) then begin
    if PlayObject.IsProperTarget(TargeTBaseObject) then begin
      if ((TargeTBaseObject.m_btRaceServer >= RC_ANIMAL) or (g_Config.boMagCapturePlayer and (TargeTBaseObject.m_btRaceServer = RC_PLAYOBJECT))) and (not TargeTBaseObject.m_boStickMode) and not (TargeTBaseObject.m_btRaceServer in [RC_NPC, RC_PEACENPC]) then begin
        if PlayObject.m_Abil.Level {+ (nLevel * 1)} > TargeTBaseObject.m_Abil.Level then begin
          if (nLevel + 5) >= _MAX(abs(PlayObject.m_nCurrX - nTargetX), abs(PlayObject.m_nCurrY - nTargetY)) then begin
            if PlayObject.GetFrontPosition(nX, nY) then begin
              if PlayObject.GetRecallXY(nX, nY, 3, nRecallX, nRecallY) then begin
                if (PlayObject.m_PEnvir <> nil) and PlayObject.m_PEnvir.CanWalk(nRecallX, nRecallY, True) then begin
                  if TargeTBaseObject.m_PEnvir.DeleteFromMap(TargeTBaseObject.m_nCurrX, TargeTBaseObject.m_nCurrY, OS_MOVINGOBJECT, TargeTBaseObject) > 0 then begin
                    TargeTBaseObject.SendRefMsg(RM_DISAPPEAR, 0, 0, 0, 0, '');
                    TargeTBaseObject.m_nCurrX := nRecallX;
                    TargeTBaseObject.m_nCurrY := nRecallY;
                    if TargeTBaseObject.m_PEnvir.AddToMap(TargeTBaseObject.m_nCurrX, TargeTBaseObject.m_nCurrY, OS_MOVINGOBJECT, TargeTBaseObject) = TargeTBaseObject then begin
                      ...
                      TargeTBaseObject.SendRefMsg(RM_TURN, TargeTBaseObject.m_btDirection, TargeTBaseObject.m_nCurrX, TargeTBaseObject.m_nCurrY, TargeTBaseObject.GetFeatureToLong, TargeTBaseObject.GetShowName);
                      Result := True;
                    end;
                  end;
                end;
...
begin
  Result := False;
  with PlayObject do begin
    if (GetTickCount - m_dwDoRecallSlaveTick) > _MAX(1, ((8 - nLevel) * 1000)) then begin
      ...
      for i := m_SlaveList.Count - 1 downto 0 do begin
        ...
        if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, m_btDirection, i + 1, nRecallX, nRecallY) then begin
          if not m_PEnvir.CanWalk(nRecallX, nRecallY, False) then begin
            GetFrontPosition(nX, nY);
            GetRecallXY(nX, nY, 3, nRecallX, nRecallY);
          end;
          with BaseObject do begin
            if (TargeTBaseObject <> nil) and PlayObject.IsProperTarget(TargeTBaseObject, True) then begin
              PlayObject.SetTargetCreat(TargeTBaseObject);
            ...
            SendRefMsg(RM_TURN, m_btDirection, m_nCurrX, m_nCurrY, GetFeatureToLong, GetShowName);
            Result := True;
          end;
        end;
      end;
    end else SysMsg('技能施展时间未到!', c_Purple, t_Hint);
  end;
end;
...
begin
  Result := False;
  if PlayObject.m_PEnvir <> nil then begin
    if not PlayObject.m_PEnvir.CanWalk(nX, nY, False) or (abs(nX - PlayObject.m_nCurrX) > 20) or (abs(nY - PlayObject.m_nCurrY) > 20) then begin
  ...
  if GetTickCount - PlayObject.m_dwDoFlyTick > 2500 then begin
...
begin
  Result := False;
  if GetTickCount - PlayObject.m_dwMagicIceRainTick > _MAX(5 * 1000, g_Config.nMagicDeadEyeInterval * 1000 - PlayObject.m_btReduceSpellTime * 1000) then begin
    ...
    try
      ...
      for i := 0 to xObjectList.Count - 1 do begin
        pBaseObject := TBaseObject(xObjectList.Items[i]);
        if PlayObject.IsProperTarget(pBaseObject, True) then begin
          ...
          if ((pBaseObject.m_btRaceServer <> RC_PLAYOBJECT) and not pBaseObject.IsHero) or g_Config.fMagicDeadEyeParalysisPlayer then
...
begin
  Result := False;
  if GetTickCount - PlayObject.m_dwDragonRageSkillTick > _MAX(5 * 1000, g_Config.nMagicDragonRageInterval * 1000 - PlayObject.m_btDragonRageSkillLevel * 1500) then begin
...
begin
  Result := False;
  if GetTickCount - PlayObject.m_dwMagicIceRainTick > _MAX(5 * 1000, g_Config.nMagicIceRainInterval * 1000 - PlayObject.m_btReduceSpellTime * 1000) then begin
    PlayObject.m_dwMagicIceRainTick := GetTickCount();
...
begin
  Result := False;
  if PlayObject.m_PEnvir <> nil then begin
    if {not PlayObject.m_PEnvir.CanWalk(nX, nY, True) or}(abs(nX - PlayObject.m_nCurrX) > 15) or (abs(nY - PlayObject.m_nCurrY) > 15) then begin
      PlayObject.SysMsg(sTagNoFreeMove, c_Red, t_Hint);
      Exit;
    end;
    if not PlayObject.m_PEnvir.CanWalkOfItem(nX, nY, False, g_Config.fPosMoveAttackOnItem) then begin
      PlayObject.SysMsg(sTagNoFreeMove, c_Red, t_Hint);
      Exit;
    end;
  end else begin
  ...
  if GetTickCount - PlayObject.m_dwDoFlyTick > _MAX(5 * 1000, g_Config.nPosMoveAttackInterval * 1000 - PlayObject.m_btReduceSpellTime * 1000) then begin
    PlayObject.m_dwDoFlyTick := GetTickCount();
    if (PlayObject.m_PEnvir <> nil) and
      (PlayObject.m_PEnvir.MoveToMovingObject(PlayObject.m_nCurrX, PlayObject.m_nCurrY, PlayObject, nX, nY, False) > 0) then begin
...
var
  i, n, nn, nRage, nTime, nOldCharStatus: Integer;
  BaseObjectList            : TList;
  BaseObject                : TBaseObject;
begin
  Result := False;
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if n > (30 + UserMagic.btLevel * 10) then Break;
  end;
  BaseObjectList.Free;
end;
