...
begin
  try
    ...
    on E: Exception do MainOutMessageAPI(sExceptionMsg);
  end;
end;
...
procedure TFrmSrvMsg.DecodeSocStr(ps: pTServerMsgInfo);
  ...
  begin
    for i := Low(m_SrvArray) to High(m_SrvArray) do begin
...
begin
  ...
  ufilename := Body;
  psui := nil;
  if g_nServerIndex = sNum then begin
    try
...
begin
  ...
  if (nIndex in [0..9]) and (m_SrvArray[nIndex].Socket = Socket) then begin
...
begin
  ...
  end else if g_nServerIndex <> sNum then begin
    g := g_GuildManager.FindGuild(gname);
    if g <> nil then begin
  ...
  if sNum = 0 then begin
    ...
    if (gname <> '') and (warguildname <> '') then begin
      ...
      if (g <> nil) and (WarGuild <> nil) then begin
        ...
        pgw := nil;
        for i := 0 to g.m_GuildWarList.Count - 1 do begin
          ...
          if pgw <> nil then begin
            if pgw.Guild = WarGuild then begin
        ...
        if pgw = nil then begin
          if not g.m_GuildWarList.Exists(WarGuild.sGuildName) then begin
            New(pgw);
            pgw.Guild := WarGuild;
            pgw.dwWarTick := StrToInt64(StartTime) - Int64(g_nServerTickDifference);
            pgw.dwWarTime := StrToInt64(remaintime);
...
begin
  if sNum = g_nServerIndex then begin
    ...
    hum := UserEngine.GetPlayObject(uname);
    if hum <> nil then begin
      if hum.m_boAllowGuildReCall then begin
