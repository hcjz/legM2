...
begin
  ...
  if IsProperTarget(target) then begin
    if Random(10) >= target.m_nantiMagic then begin
      WAbil := @m_WAbil;
...
begin
  ...
  if (Random(2) = 0) and (Random(m_TargetCret.m_btAntiPoison + 7) <= 6) then begin
...
begin
  ...
  for i := 0 to target.m_VisibleActors.Count - 1 do begin
    ...
    if (abs(target.m_nCurrX - BaseObject.m_nCurrX) <= 3) and (abs(target.m_nCurrY - BaseObject.m_nCurrY) <= 3) then begin
      ...
      if BaseObject <> nil then begin
        if (BaseObject.m_PEnvir = self.m_PEnvir) and IsProperTarget(BaseObject) then begin
...
begin
  if (not m_boDeath) and (not m_boGhost) then begin
    ...
    if Integer(GetTickCount - m_dwWalkTick) > (m_nWalkSpeed) then begin
      m_dwWalkTick := GetTickCount();
      SendRefMsg(RM_TURN, m_btDirection, m_nCurrX, m_nCurrY, GetFeatureToLong, '');
    end;
  end;
  inherited;
end;
...
begin
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if (m_TargetCret <> nil) and (Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime) and
      ...
      (abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 2) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 2) and
      ((abs(m_nCurrY - m_TargetCret.m_nCurrY) + abs(m_nCurrX - m_TargetCret.m_nCurrX)) <= 3) then begin
      m_dwHitTick := GetTickCount();
    ...
    if (m_TargetCret <> nil) then begin
      ...
      if (Integer(GetTickCount - m_dwSpellTick) > (m_nNextHitTime + nDir)) then begin
        m_dwSpellTick := GetTickCount();
        BlueCircle();
      end;
    end;
...
begin
  ...
  for i := 0 to m_TargetCret.m_VisibleActors.Count - 1 do begin
    ...
    if BaseObject <> nil then begin
      if (abs(m_TargetCret.m_nCurrX - BaseObject.m_nCurrX) <= 8) and (abs(m_TargetCret.m_nCurrY - BaseObject.m_nCurrY) <= 8) then begin
        if (BaseObject.m_PEnvir = self.m_PEnvir) and IsProperTarget(BaseObject) then begin
          if Random(10) >= BaseObject.m_nantiMagic then begin
            WAbil := @m_WAbil;
...
begin
  ...
  case m_btDirection of                 //
    ...
    DR_UPRIGHT: begin
        if (nX < (Envir.m_MapHeader.wWidth - 2)) and (nY > 0) then begin
          Inc(nX, 2);
          Dec(nY, 2);
        end;
      end;
    DR_RIGHT: begin
        if nX < (Envir.m_MapHeader.wWidth - 2) then Inc(nX, 2);
      end;
    DR_DOWNRIGHT: begin
        if (nX < (Envir.m_MapHeader.wWidth - 2)) and (nY < (Envir.m_MapHeader.wHeight - 2)) then begin
          Inc(nX, 2);
          Inc(nY, 2);
        end;
      end;
    DR_DOWN: begin
        if nY < (Envir.m_MapHeader.wHeight - 2) then Inc(nY, 2);
      end;
    DR_DOWNLEFT: begin
        if (nX > 0) and (nY < (Envir.m_MapHeader.wHeight - 2)) then begin
          Dec(nX, 2);
          Inc(nY, 2);
        end;
      end;
    ...
    if m_TargetCret <> nil then begin
      ...
      if MagCanHitTarget(m_nCurrX, m_nCurrY, m_TargetCret) then begin //make sure the 'line' in wich magic will go isnt blocked
        if (nX <= 3) and (nY <= 3) then begin
          if m_boUseMagic or ((nX = 2) or (nY = 2)) or ((nX = 3) or (nY = 3)) then begin
            if (Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime) then begin
              m_dwHitTick := GetTickCount();
              nAttackDir := GetNextDirection(m_nCurrX, m_nCurrY, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
...
begin
  ...
  for i := 0 to 4 do
    for k := 0 to 4 do begin
      if g_Config.SpitMap[m_btDirection, i, k] = 1 then begin
        mx := m_nCurrX - 2 + k;
        my := m_nCurrY - 2 + i;
...
begin
  ...
  end else if m_wAppr = 813 then begin
    btEffect := 3;
    SendRefMsg(RM_SPELL, btEffect, MakeLong(TargeTBaseObject.m_nCurrX, TargeTBaseObject.m_nCurrY), pwr, 5, '');
    SendRefMsg(RM_MAGICFIRE, 3, MakeWord(1, btEffect), MakeLong(TargeTBaseObject.m_nCurrX, TargeTBaseObject.m_nCurrY), Integer(TargeTBaseObject), '');
  end else
...
var
  ...
  cret                      : TBaseObject;
begin
  if (targ = nil) or (m_PEnvir = nil) then Exit;
  ...
  if not BoRecallComplete then begin
    if m_WAbil.HP <= m_WAbil.MaxHP div 2 then begin
      ...
      if cret <> nil then begin
        ...
        if m_TargetCret <> nil then cret.SetTargetCreat(m_TargetCret);
      end;
...
begin
  if not m_boDeath then begin
    if m_nPushedCount >= DeathCount then begin
      Die;
    end;
  end;
...
begin
  ...
  if FindTarget then begin
    if GetCurrentTime - m_dwHitTick > m_nNextHitTime then begin
      ...
      if (abs(m_nCurrX - m_TargetCret.m_nCurrX) <= m_nViewRangeX) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= m_nViewRangeY) then begin
...
begin
  ...
  for i := 0 to m_VisibleActors.Count - 1 do begin
    ...
    if (not cret.m_boDeath) and (cret.m_PEnvir = self.m_PEnvir) and IsProperTarget(cret) then begin
      if (abs(m_nCurrX - cret.m_nCurrX) <= m_nViewRangeX) and (abs(m_nCurrY - cret.m_nCurrY) <= m_nViewRangeY) then begin
        if (cret.m_btRaceServer = RC_PLAYOBJECT) or (cret.IsHero) then begin
          if m_TargetCret = nil then begin
            m_TargetCret := cret;
          end else begin
        ...
        if Random(10) = 0 then begin
          ...
          for i := 0 to rlist.Count - 1 do begin
            ...
            if (not cret.m_boDeath) and IsProperTarget(cret) then begin
              if ((cret.m_btRaceServer = RC_PLAYOBJECT) or cret.IsHero) and ((abs(m_nCurrX - cret.m_nCurrX) > 3) or (abs(m_nCurrY - cret.m_nCurrY) > 3)) then begin
...
begin
  ...
  for k := 0 to list.Count - 1 do begin
    if TBaseObject(list[k]).m_btRaceServer in [162..169] then begin
