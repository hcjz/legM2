...
uses
  Windows, Classes, SysUtils, StrUtils, ObjBase, Grobal2, IniFiles,DateUtils, MD5;
...
type
  ...
  TSayingProcedure = record
    ...
    sElseSayMsg: string;
    boLabel: Boolean;  //Flase 使用#IF判断 True 使用#OR
  end;
  ...
  private
    ...
    procedure ActionOfOpenBigDialogBox(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo); //使用NPC大对话框 Development 2019-01-14
  ...
  try
    ...
    if (sData <> '') and (sData[1] = '@') then begin
      ...
      if TUserCastle(m_Castle).IsMasterGuild(TGuild(PlayObject.m_MyGuild)) and
        (PlayObject.IsGuildMaster) then begin
        ...
        if CompareText(sLabel, sSL_SENDMSG) = 0 then begin
        ...
        else if CompareText(sLabel, sCASTLENAME) = 0 then begin
          sMsg := Trim(sMsg);
          if sMsg <> '' then begin
        ...
        else if CompareText(sLabel, sWITHDRAWAL) = 0 then begin
        ...
        else if CompareText(sLabel, sRECEIPTS) = 0 then begin
          ...
          PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0, s18);
        end else if CompareText(sLabel, sOPENMAINDOOR) = 0 then begin
          TUserCastle(m_Castle).MainDoorControl(False);
        end
        else if CompareText(sLabel, sCLOSEMAINDOOR) = 0 then begin
          TUserCastle(m_Castle).MainDoorControl(True);
        end
        else if CompareText(sLabel, sREPAIRDOORNOW) = 0 then begin
          RepairDoor(PlayObject);
          GotoLable(PlayObject, sMAIN, False);
        end
        else if CompareText(sLabel, sREPAIRWALLNOW1) = 0 then begin
          RepairWallNow(1, PlayObject);
          GotoLable(PlayObject, sMAIN, False);
        end
        else if CompareText(sLabel, sREPAIRWALLNOW2) = 0 then begin
          RepairWallNow(2, PlayObject);
          GotoLable(PlayObject, sMAIN, False);
        end
        else if CompareText(sLabel, sREPAIRWALLNOW3) = 0 then begin
          RepairWallNow(3, PlayObject);
          GotoLable(PlayObject, sMAIN, False);
        end
        else if CompareLStr(sLabel, sHIREGUARDNOW, Length(sHIREGUARDNOW)) then begin
        ...
        else if CompareLStr(sLabel, sHIREARCHERNOW, Length(sHIREARCHERNOW)) then begin
          s20 := Copy(sLabel, Length(sHIREARCHERNOW) + 1, Length(sLabel));
          HireArcher(s20, PlayObject);
          PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0, '');
        end
        else if CompareText(sLabel, sEXIT) = 0 then begin
          PlayObject.SendMsg(Self, RM_MERCHANTDLGCLOSE, 0, Integer(Self), 0, 0, '');
        end
        else if CompareText(sLabel, sBACK) = 0 then begin
...
begin
  inherited;
  if ClassNameIs(TMerchant.ClassName) then begin
    try
      if not m_boCastle or not ((m_Castle <> nil) and TUserCastle(m_Castle).m_boUnderWar) then begin
        if not PlayObject.m_boDeath and (sData <> '') and (sData[1] = '@') then begin
          ...
          if (CompareText(sLabel, sSL_SENDMSG) = 0) or (CompareText(sLabel, sOFFLINEMSG) = 0) or
            ...
            CompareLStr(sLabel, sINPUTINTEGER, Length(sINPUTINTEGER)) or CompareLStr(sLabel, sINPUTSTRING, Length(sINPUTSTRING)) then begin
            if sMsg = '' then Exit;
          end;
          ...
          if CompareText(sLabel, '@@SecretProperty') = 0 then begin
            PlayObject.SendDefMessage(SM_SecretProperty, 1, PlayObject.m_btSPLuck, PlayObject.m_btSPEnergy, 1, '');
          end else if CompareText(sLabel, sOFFLINEMSG) = 0 then begin
            if m_boOffLineMsg then
              SetOffLineMsg(PlayObject, sMsg);
          end else if CompareText(sLabel, sSL_SENDMSG) = 0 then begin
            if m_boSendmsg then begin
              if IsInGuildRankNameFilterList(sMsg) then begin
                PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(PlayObject), 0, 0, '输入的信息中包含了非法字符，已被过滤，请重新输入。');
                Exit;
              end;
              SendCustemMsg(PlayObject, sMsg);
            end;
          end else if CompareText(sLabel, sSUPERREPAIR) = 0 then begin
            if m_boS_repair then
              SuperRepairItem(PlayObject);
          end else if CompareText(sLabel, sBUY) = 0 then begin
            if m_boBuy then
              BuyItem(PlayObject, 0);
          end else if CompareText(sLabel, sSELL) = 0 then begin
            if m_boSell then
              SellItem(PlayObject);
          end else if CompareText(sLabel, sREPAIR) = 0 then begin
            if m_boRepair then
              RepairItem(PlayObject);
          end else if CompareText(sLabel, sMAKEDURG) = 0 then begin
            if m_boMakeDrug then
              MakeDurg(PlayObject);
          end else if CompareText(sLabel, sPRICES) = 0 then begin
            if m_boPrices then
              ItemPrices(PlayObject);
          end else if CompareText(sLabel, sSTORAGE) = 0 then begin
            if m_boStorage then
              Storage(PlayObject);
          end else if CompareText(sLabel, sGETBACK) = 0 then begin
            if m_boGetback then
              GetBack(PlayObject);
          end else if CompareText(sLabel, sUPGRADENOW) = 0 then begin
            if m_boUpgradenow then
              UpgradeWapon(PlayObject);
          end else if CompareText(sLabel, sGETBACKUPGNOW) = 0 then begin
            if m_boGetBackupgnow then
              GetBackupgWeapon(PlayObject);
          end else if CompareText(sLabel, sGETMARRY) = 0 then begin
            if m_boGetMarry then
              GetMarry(PlayObject, sMsg);
          end else if CompareText(sLabel, sGETMASTER) = 0 then begin
            if m_boGetMaster then
              GetMaster(PlayObject, sMsg);
          end else if CompareLStr(sLabel, sREMOTEMUSIC, Length(sREMOTEMUSIC)) then
          ...
          else if CompareLStr(sLabel, sDEALGOLD, Length(sDEALGOLD)) then begin
            if m_boDealGold then
              StartDealGold(PlayObject, sLabel, sMsg);
          end else if CompareText(sLabel, sMAKEPEUMA) = 0 then begin
            if m_boMakeHero then begin
              if IsInGuildRankNameFilterList(sMsg) then begin
          ...
          end else if CompareText(sLabel, sCERATEHERO) = 0 then begin
            PlayObject.m_sTempHeroName := '';
            if m_boCreateHero then begin
              if IsInGuildRankNameFilterList(sMsg) then begin
          ...
          end else if CompareText(sLabel, sCERATEHERO2) = 0 then begin
            PlayObject.m_sTempHeroName := '';
            if m_boCreateHero then begin
              if IsInGuildRankNameFilterList(sMsg) then begin
                PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(PlayObject), 0, 0, '输入的英雄名字中包含了非法字符，请重新编辑！');
          ...
          end else if CompareLStr(sLabel, sUSEITEMNAME, Length(sUSEITEMNAME)) then begin
            if m_boUseItemName then begin
              if IsInGuildRankNameFilterList(sMsg) then begin
                PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(PlayObject), 0, 0, '自定义装备的名字中包含了非法字符，请重新编辑！');
                Exit;
              end;
              if Length(sMsg) > 14 then begin
          ...
          end else if CompareLStr(sLabel, sINPUTSTRING, Length(sINPUTSTRING)) then begin
            if m_boInputString then begin
              ...
              if Length(sMsg) >= High(byte) then begin
          ...
          end else if CompareText(sLabel, sEXIT) = 0 then
            PlayObject.SendMsg(Self, RM_MERCHANTDLGCLOSE, 0, Integer(Self), 0, 0, '')
          else if CompareText(sLabel, sBACK) = 0 then begin
            if PlayObject.m_sScriptGoBackLable = '' then
              PlayObject.m_sScriptGoBackLable := sMAIN;
...
begin
  nCheckCode := 0;
  try
    if (GetTickCount - dwRefillGoodsTick) > 30 * 1000 then begin
    ...
    nCheckCode := 2;
    if not (m_wAppr in [60..67, 84, 85]) then begin
      if Random(50) = 0 then
        TurnTo(Random(8))
    ...
    end else if m_boFixedHideMode then begin
      m_boFixedHideMode := False;
      SendRefMsg(RM_HIT, m_btDirection, m_nCurrX, m_nCurrY, 0, '');
    end;

    nCheckCode := 4;
    if m_boAutoChangeColor and (m_dwAutoChangeColorTime > 0) and (GetTickCount - m_dwAutoChangeColorTick > m_dwAutoChangeColorTime) then begin
...
begin
  ...
  for i := 0 to m_SellOffList.Count - 1 do begin //Momory Leaked - Fixed By Blue
...
begin
  for i := m_UpgradeWeaponList.Count - 1 downto 0 do begin
    ...
    if Integer(Round(Now - UpgradeInfo.dtTime)) >=
      g_Config.nClearExpireUpgradeWeaponDays then begin
      Dispose(UpgradeInfo);
      m_UpgradeWeaponList.Delete(i);
    end;
  end;
...
begin
  ...
  if sVariable = '$QUERYYBDEALLOG' then begin
    sText := '';
    if m_PostGoldList <> nil then begin
      boFound := False;
      nCount := 0;
      for i := 0 to m_PostGoldList.Count - 1 do begin
        PostGold := pTPostGold(m_PostGoldList.Items[i]);
        if PostGold.sCharName = PlayObject.m_sCharName then begin
          boFound := True;
          Break;
        end;
      end;
      if boFound then begin
        n18 := 0;
        sText := '你的寄售物品在:\';
        for i := Low(PostGold.EGold) to High(PostGold.EGold) do begin
          if PostGold.EGold[i].nDealGold > 0 then begin
            if PostGold.EGold[i].boDealFlag then begin
        ...
        if n18 > 0 then begin
          ...
          if PlayObject.m_nGameGold + n18 < High(Integer) then begin
      ...
      for i := 0 to m_PostGoldList.Count - 1 do begin
        PostGold := pTPostGold(m_PostGoldList.Items[i]);
        for ii := Low(PostGold.EGold) to High(PostGold.EGold) do begin
          if PostGold.EGold[ii].nDealGold > 0 then begin
            if PostGold.EGold[ii].sTargName = PlayObject.m_sCharName then begin
  ...
  //寄卖系统税率
  if sVariable = '$SELLOFFRATE' then begin
  ...
  if sVariable = '$SELLOFFITEM' then begin
    if m_SellOffList <> nil then begin
      sText := '';
      for i := m_SellOffList.Count - 1 downto 0 do begin
        ...
        for ii := 0 to List.Count - 1 do begin
          ...
          if SellOff^.sCharName = PlayObject.m_sCharName then begin
            s14 := '';
  ...
  if sVariable = '$SELLOUTGOLD' then begin
    if m_SellOffList <> nil then begin
      sText := '';
      for i := 0 to m_SellgoldList.Count - 1 do begin
...
begin
  ...
  StdItem := UserEngine.GetStdItem(UserItem.wIndex);
  if (StdItem <> nil) then begin
    if not (StdItem.StdMode in [5, 6, 10..13, 15..24, 26..30, 51..54, 62..64]) then begin
      Result := -1;
      Exit;
    end;

    if StdItem.Need in [0..3] then begin
      n14 := StdItem.NeedLevel;
    end else if StdItem.Need = 4 then begin
      n14 := LoWord(StdItem.NeedLevel);
    end;

    if n14 < 38 then begin
      Result := -2;
      Exit;
    end;
    if n14 > 160 then begin
      Result := -3;
      Exit;
    end;

    n10 := n14;
    for nC := Low(UserItem.btValue) to High(UserItem.btValue) do begin
      if UserItem.btValue[nC] > 1 then
        Inc(n10);
    end;
    for nC := Low(UserItem.btValueEx) to High(UserItem.btValueEx) do begin
  ...
  if n10 > 0 then begin
    ...
    if (StdItem <> nil) then begin
      if (StdItem.Overlap < 1) and (StdItem.StdMode > 4) and
        (StdItem.DuraMax > 0) and (UserItem.DuraMax > 0) then begin
        ...
        if (StdItem.Overlap < 1) and (StdItem.StdMode > 4) then begin
          n14 := 0;
          nC := 0;
          while (True) do begin
...
begin
  ...
  for i := 0 to m_GoodsList.Count - 1 do begin
    ...
    StdItem := UserEngine.GetStdItem(UserItem.wIndex);
    if StdItem <> nil then begin
      if PlayObject.IsAddWeightAvailable(UserEngine.GetStdItemWeight(UserItem.wIndex, BuyCount)) then begin
        sUserItemName := '';
        ...
        if sUserItemName = sItemName then begin
          for ii := 0 to List20.Count - 1 do begin
            ...
            if (StdItem.StdMode <= 4) or
              ...
              (UserItem.MakeIndex = nInt) or (StdItem.Overlap >= 1) then begin
              ...
              if (PlayObject.m_nGold >= nPrice) and (nPrice > 0) then begin
                if StdItem.Overlap >= 1 then begin
                  ...
                  if PlayObject.UserCounterItemAdd(StdItem.StdMode, StdItem.Looks, BuyCount, StdItem.Name, False) then begin
...
procedure TMerchant.ClientGetDetailGoodsList(PlayObject: TPlayObject; sItemName: string; nInt: Integer);
var
...
{$IFEND VER_ClientType_45}
    ...
    PlayObject.SendMsg(Self, RM_GETSELLITEMSLIST, 0, Integer(Self), nItemCount, nInt, sSENDMSG);
    Exit;
  end;
{$IF VER_ClientType_45}
  if PlayObject.m_nSoftVersionDateEx = 0 then begin
    ...
    for i := 0 to m_GoodsList.Count - 1 do begin
      ...
      if (pStdItem <> nil) and (pStdItem.Name = sItemName) then begin
        ...
        //for ii := List.Count - 1 downto 0 do begin
        for ii := nInt to List.Count - 1 do begin
    ...
    PlayObject.SendMsg(Self, RM_SENDDETAILGOODSLIST, 0, Integer(Self), nItemCount, nInt, sSENDMSG);
  end else begin
{$IFEND VER_ClientType_45}
    nItemCount := 0;
    for i := 0 to m_GoodsList.Count - 1 do begin
  ...
  if m_SellOffList <> nil then begin
    ...
    for i := 0 to m_SellOffList.Count - 1 do begin
      ...
      if (List = nil) or (List.Count = 0) then
        Continue;
      SellOff := pSellOff(List[0]);
      if SellOff <> nil then begin
        sUserItemName := UserEngine.GetStdItemName(SellOff.item.wIndex);
        pStdItem := UserEngine.GetStdItem(SellOff.item.wIndex);
        if pStdItem <> nil then begin
          if PlayObject.IsAddWeightAvailable(pStdItem.Weight) then begin //0625
            if CompareText(sUserItemName, sItemName) = 0 then begin

              for ii := List.Count - 1 downto 0 do begin
                SellOff := pSellOff(List[ii]);
                if (SellOff.item.MakeIndex = nMakeIndex) then begin
                  if CompareText(SellOff.sCharName, PlayObject.m_sCharName) = 0 then
                    nPrice := 0
                  else
                    nPrice := SellOff.Price;
                  if (PlayObject.m_nGameGold >= nPrice) and (nPrice >= 0) then begin
                    New(UserItem);
...
begin
  ...
  if (nPrice > 0) and (not m_boNoSeal) and sub_4A1C84(UserItem) then begin
...
begin
  ...
  if (nPrice > 0) and (not m_boNoSeal) and CanSell(UserItem) then begin
    if PlayObject.IncGold(nPrice) then begin
      if m_boCastle or g_Config.boGetAllNpcTax then begin
        if m_Castle <> nil then
          TUserCastle(m_Castle).IncRateGold(nPrice)
      ...
      PlayObject.SendMsg(Self, RM_USERSELLCOUNTITEM_OK, 0, PlayObject.m_nGold, remain, nCount, '');
      StdItem := UserEngine.GetStdItem(UserItem.wIndex);
      if StdItem.NeedIdentify = 1 then
        AddGameDataLogAPI('10' + #9 + PlayObject.m_sMapName + #9 + IntToStr(PlayObject.m_nCurrX) + #9 + IntToStr(PlayObject.m_nCurrY) + #9 + PlayObject.m_sCharName + #9 + StdItem.Name + #9 + IntToStr(UserItem.MakeIndex) + #9 + '1' + #9 + m_sCharName);
      Result := True;
    ...
    if boCanRepair and (nPrice > 0) and (UserItem.DuraMax > UserItem.Dura) and (StdItem.StdMode <> 43) then begin
      ...
      if PlayObject.DecGold(nRepairPrice) then begin
        if m_boCastle or g_Config.boGetAllNpcTax then begin
          ...
          end else if g_Config.boGetAllNpcTax then begin
            g_CastleManager.IncRateGold(g_Config.nUpgradeWeaponPrice);
          end;
        end;
        if PlayObject.m_sScriptLable = sSUPERREPAIR then begin
...
begin
  ...
  if Player <> nil then begin
    if Player.m_boRemoteMsg then begin
...
begin
  ...
  if nDealGold >= 0 then begin
    if PlayObject.m_nGameGold >= nDealGold then begin
      PoseBaseObject := PlayObject.GetPoseCreate();
      if (PoseBaseObject = nil) or (PoseBaseObject.GetPoseCreate <> PlayObject) or (PoseBaseObject.m_btRaceServer <> RC_PLAYOBJECT) or (PlayObject.m_DealGoldCreat <> PoseBaseObject) then begin
      ...
      if TPlayObject(PoseBaseObject).m_nGameGold + nDealGold < High(Integer) then begin
...
begin
  ...
  if btWhere in [Low(THumanUseItems)..High(THumanUseItems)] then begin
    ...
    if InLimitItemList('', UserItem.wIndex, t_dCustomName) then begin
...
begin
  ...
  if Magic <> nil then begin
    ...
    if not PlayObject.IsTrainingSkill(Magic.wMagicId, Magic.btClass) then begin
      ...
      if (PlayObject.m_MagicArr[0][SKILL_ERGUM] <> nil) and not PlayObject.m_boUseThrusting then begin
...
begin
  ...
  case cMethod of
    ...
    '-': begin
        if Hero.m_btCreditPoint > byte(nCreditPoint) then begin
          Dec(Hero.m_btCreditPoint, byte(nCreditPoint));
        end else
          Hero.m_btCreditPoint := 0;
      end;
    '+': begin
        if Hero.m_btCreditPoint + byte(nCreditPoint) > High(byte) then begin
          Hero.m_btCreditPoint := High(byte);
        end else
...
begin
  ...
  case cMethod of
    '=': if nExp >= 0 then begin
        ...
        //dwInt := LongWord(nExp);
        if PlayObject.IsHero then begin
          PlayObject.m_Master.SendMsg(PlayObject.m_Master, RM_HEROWINIPEXP, 0, 0, 0, 0, '');
        end else if not PlayObject.m_boOffLineFlag then
...
var
  nCode                     : Integer;
begin
  if PlayObject.m_btPostSell = 0 then begin
    if PlayObject.m_nGameGold > 0 then begin
      if (QuestActionInfo.nParam1 > 0) and (PlayObject.m_nGameGold >= QuestActionInfo.nParam1) then begin
...
var
  ...
  sItemName, sSellInfo, sSENDMSG: string;
  boFound                   : Boolean;
  pdc                       : PTDelayCallNPC;
begin
  sSENDMSG := '';
  ...
  nCode := -2;
  boFound := False;
  if (TMerchant(Self).m_PostSellList <> nil) and (TMerchant(Self).m_PostSellList.Count > 0) then begin
    for i := 0 to TMerchant(Self).m_PostSellList.Count - 1 do begin
      PostSell := pTPostSell(TMerchant(Self).m_PostSellList[i]);
      if PostSell.sCharName = PlayObject.m_sCharName then begin
  ...
  if boFound then begin
    ...
    for i := Low(PostSell.aPostItems) to High(PostSell.aPostItems) do begin
      ...
      if UserItem.wIndex > 0 then begin
        ...
        if pStdItem <> nil then begin
          ...
          ////////////
{$IF VER_ClientType_45}
          if PlayObject.m_wClientType = 46 then begin
{$IFEND VER_ClientType_45}
            Move(StdItem, ClientItem.s, SizeOf(TClientStdItem));
            sItemName := '';
            if UserItem.btValue[13] = 1 then
...
{$IFEND VER_ClientType_45}
    ...
    if PostSell.nPostStone > 0 then begin
...
begin
  ...
  if (TMerchant(Self).m_PostSellList <> nil) and (TMerchant(Self).m_PostSellList.Count > 0) then begin
    for i := 0 to TMerchant(Self).m_PostSellList.Count - 1 do begin
      PostSell := pTPostSell(TMerchant(Self).m_PostSellList[i]);
      if PostSell.sTargName = PlayObject.m_sCharName then begin
        DateTime := PostSell.dPostTime;
        boFound := True;
        Break;
      end;
...
begin
  ...
  if HeroObject <> nil then begin
    if HeroObject.m_btJob <> nJob then begin
...
begin
  ...
  if not (mode in [0..6]) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_CHANGEATTACKMODE);
    Exit;
  end;
  if (mode <> PlayObject.m_btAttatckMode) then begin
...
begin
  ...
  case cMethod of
    '=': begin
        if (nLevel > 0) and (nLevel <= High(Word)) then begin
...
begin
  ...
  for i := 0 to PlayObject.m_MagicList.Count - 1 do begin
    ...
    if CompareText(UserMagic.MagicInfo.sMagicName, sn) = 0 then begin
      if UserMagic.btLevel < 15 then begin
        if UserMagic.MagicInfo.btClass = 0 then
          lv := PlayObject.m_Abil.Level
        else
          lv := PlayObject.m_nInPowerLevel;
        if UserMagic.MagicInfo.TrainLevel[UserMagic.btLevel] <= lv then begin
...
begin
  ...
  case cMethod of
    '=': begin
        if (nLevel >= 0) and (nLevel <= MAX_IPLEVEL) then begin
...
begin
  ...
  if (Envirnoment = nil) or (nX < 0) or (nY < 0) or (nRange <= 0) then begin
  ...
  for nX := nStartX to nEndX do begin
    for nY := nStartY to nEndY do begin
      if Envirnoment.GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) and (MapCellInfo.ObjList <> nil) then begin
        i := 0;
        while (True) do begin
...
begin
  ...
  if Envirnoment.GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo <> nil) and (MapCellInfo.chFlag = 0) then begin
    if MapCellInfo.ObjList <> nil then begin
      ...
      while True do begin
        ...
        if (OSObject = nil) or (OSObject.btType <> OS_GATEOBJECT) then begin
          inc(i);
          Continue;
        end;
...
begin
  Hero := PlayObject.GetHeroObjectA;
  if Hero = nil then Exit;
  for i := Hero.m_MagicList.Count - 1 downto 0 do begin
...
begin
  ...
  if b then PlayObject.RecalcAbilitys();
end;
...
begin
  ...
  if (sMonName = '') or (Envir = nil) or (nX1 < 0) or (nY1 < 0) or (nX2 < 0) or (nY2 < 0) then begin
  ...
  for i := MonList.Count - 1 downto 0 do begin
    ...
    if CompareText(BaseObject.m_sCharName, sMonName) = 0 then begin
...
begin
  ...
  if (nStep < 0) or (QuestActionInfo.sParam1 = '') then begin
  ...
  case cMethod of
    '+': begin
        ...
        if nst <= 0 then begin
          ...
          if st <> '' then sDesc := GetLineVariableText(PlayObject, st); //'title=%s desc=%s';
          if sDesc <> '' then PlayObject.SendMsg(PlayObject, RM_SETMISSION, 1, Str_ToInt(sid, 0), nClass, 0, sDesc);
        end else if nst <> nStep then begin
          st := M2Share.GetMissionSendMsg(sid, nStep, nClass);
          if st <> '' then sDesc := GetLineVariableText(PlayObject, st); //'title=%s desc=%s';
          if sDesc <> '' then PlayObject.SendMsg(PlayObject, RM_SETMISSION, 1, Str_ToInt(sid, 0), nClass, 1, sDesc);
        end;
      end;
    '-': begin
        if PlayObject.DeleteMission(sid) >= 0 then begin
          if g_FunctionNPC <> nil then g_FunctionNPC.GotoLable(PlayObject, '@CancelMission', False);
          M2Share.GetMissionSendMsg(sid, 1, nClass);
          if nClass <> 0 then PlayObject.SendMsg(PlayObject, RM_SETMISSION, 2, Str_ToInt(sid, 0), nClass, 0, '');
        end;
      end;
    '^': begin
...
var
  ...
  s                         : string;
begin
  for i := 0 to PlayObject.m_MissionList.Count - 1 do begin
    s := PlayObject.m_MissionList[i];
    M2Share.GetMissionSendMsg(s, 1, c);
    if c <> 0 then PlayObject.SendMsg(PlayObject, RM_SETMISSION, 2, Str_ToInt(s, 0), c, 0, '');
  end;
...
begin
  ...
  for i := Low(THumTitles) to High(THumTitles) do begin
    ...
    if pStdItem <> nil then begin
      if CompareText(pStdItem.Name, QuestConditionInfo.sParam1) = 0 then begin
        Inc(nStep);
      end;
    end;
  end;
...
var
  i, nNo, nR, nGetLineOk    : Integer;
  LoadList                  : TStringList;
  sNo, sLine, sName, sRate, sListFileName: string;
begin
  ...
  if (nNo < 0) or not FileExists(sListFileName) then begin
    ...
    for i := 1 to nRange do begin
      for ii := -i to i do begin
        for III := -i to i do begin
          nDX := nOrgX + III;
          nDY := nOrgY + ii;
          if Envir.GetItemEx(nDX, nDY, nItemCount) = nil then begin
            if Envir.m_boChFlag then begin
              Result := True;
              Break;
            end;
          end else if Envir.m_boChFlag and (n24 > nItemCount) then begin
            n24 := nItemCount;
            n28 := nDX;
            n2C := nDY;
          end;
...
begin
  ...
  if CompareText(sMAP, 'SELF') = 0 then begin
  ...
  if (sMAP = '') or (tX < 0) or (tY < 0) or (sItemName = '') then begin
  ...
  if Envir <> nil then begin
    for i := 0 to nCount - 1 do begin
      ...
      if UserEngine.CopyToUserItemFromName(sItemName, UserItem) then begin
        ...
        if (StdItem.Overlap >= 1) and (UserItem.Dura > 1) then begin
...
begin
  ...
  if (nCount < 0) or (nCount > 1000) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_BONUSABIL);
    Exit;
  end;
  cMethod := QuestActionInfo.sParam2[1];
  if QuestActionInfo.sParam1 = 'DC' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
          Inc(PlayObject.m_BonusAbil.DC, nCount);
          boChg := True;
        end;
      '-': if PlayObject.m_BonusAbil.DC >= nCount then begin
          Dec(PlayObject.m_BonusAbil.DC, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'MC' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.MC >= nCount then begin
          Dec(PlayObject.m_BonusAbil.MC, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'SC' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.SC >= nCount then begin
          Dec(PlayObject.m_BonusAbil.SC, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'HP' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.HP >= nCount then begin
          Dec(PlayObject.m_BonusAbil.HP, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'MP' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.MP >= nCount then begin
          Dec(PlayObject.m_BonusAbil.MP, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'AC' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.AC >= nCount then begin
          Dec(PlayObject.m_BonusAbil.AC, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'MAC' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.MAC >= nCount then begin
          Dec(PlayObject.m_BonusAbil.MAC, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'HIT' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.HIT >= nCount then begin
          Dec(PlayObject.m_BonusAbil.HIT, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'SPD' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.Speed >= nCount then begin
          Dec(PlayObject.m_BonusAbil.Speed, nCount);
          Inc(PlayObject.m_nBonusPoint, nCount);
          boChg := True;
        end;
    end;
  end else if QuestActionInfo.sParam1 = 'X2' then begin
    case cMethod of
      '+': if PlayObject.m_nBonusPoint >= nCount then begin
          Dec(PlayObject.m_nBonusPoint, nCount);
      ...
      '-': if PlayObject.m_BonusAbil.X2 >= nCount then begin
...
begin
  if CompareText(QuestActionInfo.sParam1, 'CLEAR') = 0 then begin
  ...
  if not (Pos in [0..6]) or ((value < 0) or (value > 65535)) or (addtime < 0) then begin
...
begin
  ...
  if (PoseBaseObject <> nil) and (PoseBaseObject.m_btRaceServer = RC_PLAYOBJECT)
    and (PoseBaseObject.m_btGender <> PlayObject.m_btGender) then begin
...
begin
  ...
  if m_sMDlgImgName <> QuestActionInfo.sParam1 then begin
...
begin
  ...
  if (fc in [0..255]) and (bc in [0..255]) and (fc <> bc) then begin
    case QuestActionInfo.nParam1 of
      ...
      8..10: if PlayObject.m_MyGuild <> nil then begin
          if Sec > 0 then
    ...
    if (Sec > 0) and (QuestActionInfo.sParam6 <> '') then begin
      New(pdc);
  ...
  case QuestActionInfo.nParam1 of
    ...
    8: if PlayObject.m_MyGuild <> nil then TGuild(PlayObject.m_MyGuild).SendGuildMsg(PlayObject.m_sCharName + ': ' + sMsg, 1);
    9: if PlayObject.m_MyGuild <> nil then TGuild(PlayObject.m_MyGuild).SendGuildMsg(PlayObject.m_sCharName + ': ' + sMsg, 2);
    10: if PlayObject.m_MyGuild <> nil then TGuild(PlayObject.m_MyGuild).SendGuildMsg(PlayObject.m_sCharName + ': ' + sMsg, 3);
  ...
  if CompareText(QuestActionInfo.sParam1, 'RESPONSEMASTER') = 0 then begin
    if CompareText(QuestActionInfo.sParam2, 'OK') = 0 then begin
      if (PlayObject.m_PoseBaseObject = PoseHuman) and
        (PoseHuman.m_PoseBaseObject = PlayObject) then begin
        if PlayObject.m_boStartMaster and PoseHuman.m_boStartMaster then begin
...
begin
  if (QuestActionInfo.sParam1 <> '') and (QuestActionInfo.nParam2 > 0) and (QuestActionInfo.nParam3 > 0) then begin
    ...
    if CompareText(sMapName, 'SELF') = 0 then begin
      if PlayObject.m_PEnvir <> nil then begin
...
begin
  ...
  if g_Config.boFireBurnEventOff then Exit;
  Envir := g_MapManager.FindMap(sMAP);
  if Envir <> nil then begin
...
begin
  ...
  for i := 0 to nCount - 1 do begin
    if (nRace <> RC_MISSION) and (nRange > 0) then begin
...
{$IF VER_PATHMAP = 1}
      if (Mon.m_btRaceServer = RC_MISSION) and (Mon.m_PEnvir <> nil) then begin
        Mon.m_PEnvir.m_mLatestWayPointList.Lock;
        try
          boFind := False;
          for ii := 0 to Mon.m_PEnvir.m_mLatestWayPointList.Count - 1 do begin
            ...
            if (pWayPointInfo.StartPoint.X = Mon.m_nCurrX) and (pWayPointInfo.StartPoint.Y = Mon.m_nCurrY) and
              (pWayPointInfo.EndPoint.X = Mon.m_nMissionX) and (pWayPointInfo.EndPoint.Y = Mon.m_nMissionY) then begin
...
begin
  ...
  if (BasePoseObject = nil) or (BasePoseObject.GetPoseCreate <> PlayObject) or (BasePoseObject.m_btRaceServer <> RC_PLAYOBJECT) then begin
...
var
  nTime, nPoint, nKickOffLine: Integer;
  //sOffLineStartMsg          : string;
  HeroObject                : TBaseObject;
resourcestring
    ...
    if QuestActionInfo.sParam1 = '' then begin
      ...
      if PoseHuman.GetPoseCreate = PlayObject then begin
        if (PlayObject.m_sDearName = PoseHuman.m_sCharName)
          {and (PosHum.AddInfo.sDearName = Hum.sName)}then begin
...
begin
  ...
  if (nSize <= 0) or (nSize > PlayObject.GetMaxBagItem) then begin
...
begin
  ...
  if not (nMIN in [0..100]) or (nMax < 0) then begin
    ScriptConditionError(PlayObject, QuestConditionInfo, sSC_CHECKHP);
    Exit;
  end;
  case cMethodMin of
    '=': begin
        if Round(PlayObject.m_WAbil.HP / PlayObject.m_WAbil.MaxHP * 100) = nMIN then begin
          Result := CheckHighHP;
        end;
      end;
    '>': begin
...
//检查文件是否包含指定文本  Development 2019-01-21
function CheckTextInList(sTest, sListFileName: string): Boolean;
var
  I: Integer;
  LoadList: TStringList;
begin
  ...
  begin
    ...
    try
      ...
      MainOutMessageAPI('文件读取失败(CHECKLISTTEXT) => ' + sListFileName);
    end;
    if LoadList.Count > 0 then
    begin
      for I := 0 to LoadList.Count - 1 do
      begin
        if AnsiContainsText(Trim(LoadList.Strings[I]), sTest) then
        begin
          Result := True;
          Break;
        end;
  ...
  begin
    MainOutMessageAPI('文件没有找到 => ' + sListFileName);
  end;
end;
...
begin
  ...
  case cMethodMin of
    '=': begin
        if Round(PlayObject.m_WAbil.MP / m_WAbil.MaxMP * 100) = nMIN then begin
          Result := CheckHighMP;
        end;
      end;
    '>': begin
...
begin
  ...
  nGameGold := Str_ToInt(sGameGold, -1);
  if (nGameGold < 0) or (nCount < 0) then begin
...
begin
  ...
  if (wYear < 0) or not (wMonth in [1..12]) or not (wDay in [1..31]) then begin
...
begin
  Result := False;
  nStdMode := -1;
  if (PlayObject.m_nDlgItemIndex > 0) then begin
    for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
      UserItem := PlayObject.m_ItemList.Items[i];
      if UserItem.MakeIndex = PlayObject.m_nDlgItemIndex then begin
        StdItem := UserEngine.GetStdItem(UserItem.wIndex);
        if StdItem <> nil then begin
  ...
  if nStdMode < 0 then Exit;
  if QuestConditionInfo.sParam1 = 'DRESS' then begin
    if nStdMode in [10, 11] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'WEAPON' then begin
    if nStdMode in [5, 6] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'MEDAL' then begin
    if nStdMode in [30] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'NECKLACE' then begin
    if nStdMode in [19, 20, 21] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'HELMET' then begin
    if nStdMode in [15] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'HELMETEX' then begin
    if nStdMode in [16] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'ARMRING' then begin
    if nStdMode in [24, 26] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'RING' then begin
    if nStdMode in [22, 23] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'BOOTS' then begin
    if nStdMode in [28] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'BELT' then begin
    if nStdMode in [27] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'CHARM' then begin
    if nStdMode in [7, 29] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'BUJUK' then begin
    if nStdMode in [25, 51, 61] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'DRUM' then begin
    if nStdMode in [17] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'HORSE' then begin
    if nStdMode in [18] then Result := True;
  end else if QuestConditionInfo.sParam1 = 'FASHION' then begin
    if nStdMode in [12, 13] then Result := True;
  end;
end;
...
begin
  Result := False;
  if (PlayObject.m_nDlgItemIndex = 0) then Exit;
  for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
    UserItem := PlayObject.m_ItemList.Items[i];
    if UserItem.MakeIndex = PlayObject.m_nDlgItemIndex then begin
      StdItem := UserEngine.GetStdItem(UserItem.wIndex);
...
begin
  ...
  if (nAddValue < 0) or not (nAVWhere in [0..30]) then begin
    ...
    if UserItem.MakeIndex = PlayObject.m_nDlgItemIndex then begin
      if nAVWhere > 13 then begin
        if nAVWhere = 14 then
          ...
          nAddAllValue := UserItem.DuraMax div 1000
        else if nAVWhere = 16 then begin
          nAddAllValue := UserItem.btValue[14];
        end else if nAVWhere = 17 then begin
          nAddAllValue := UserItem.btValue[15] div 16;
        end else if nAVWhere = 18 then begin
...
begin
  ...
  if not (nWhere in [Low(THumanUseItems)..High(THumanUseItems)]) or (nValue2 < 0) then begin
  ...
  if (StdItem <> nil) then begin
    ...
    if StdItemA.Eva.EvaTimes > 0 then begin
      for i := Low(StdItemA.Eva.Abil) to High(StdItemA.Eva.Abil) do begin
        if (StdItemA.Eva.Abil[i].btValue > 0) and (StdItemA.Eva.Abil[i].btType in [1..30]) then begin
...
begin
  ...
  if HeroObject <> nil then begin
    cMethod := QuestConditionInfo.sParam1[1];
    case cMethod of
      '=': if HeroObject.m_Abil.Level = nLevel then Result := True;
      '>': if HeroObject.m_Abil.Level > nLevel then Result := True;
      '<': if HeroObject.m_Abil.Level < nLevel then Result := True;
    else if HeroObject.m_Abil.Level >= nLevel then Result := True;
    end;
  end;
end;
...
begin
  ...
  if (PoseHuman <> nil) and (PoseHuman.m_btRaceServer = RC_PLAYOBJECT) and (PoseHuman.GetPoseCreate = PlayObject) then begin
  ...
  if objidx <> nil then begin
    ...
    case VarIdx of
      ...
      v_DLGITEMNAME: begin
          if (PlayObject.m_nDlgItemIndex = 0) then Exit;
          for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
            if pTUserItem(PlayObject.m_ItemList.Items[i]).MakeIndex = PlayObject.m_nDlgItemIndex then begin
      ...
      v_DEALGOLDPLAY: begin
          if PlayObject.m_DealGoldCreat <> nil then begin
      ...
      v_MONKILLER: begin
          if PlayObject2.m_LastHiter <> nil then begin
      ...
      v_COLLECTIPEXP: begin
          sText := IntToStr(PlayObject.m_dwCollectIpExp);
          sMsg := ExVariableText(sMsg, '<$COLLECTIPEXP>', sText);
        end;
      v_GAINCOLLECTEXP: begin
          if (PlayObject.m_btCollectExpLv in [2..4]) then begin
            sText := '0';
            if (PlayObject.m_PEnvir <> nil) {and (PlayObject.m_PEnvir.m_MapFlag.PCollectExp <> nil)} then
...
{$IFEND}
    ...
    if CompareLStr(sVariable, '$TAGMAPNAME', Length('$TAGMAPNAME')) then begin
      for i := Low(TMakerMapInfo) to High(TMakerMapInfo) do begin
        ...
        if sVariable = sMAP then begin
          if (PlayObject.m_MakerMapInfo[i].sMapName <> '') then begin
    ...
    if CompareLStr(sVariable, '$H.HUMAN(', Length('$H.HUMAN(')) then begin
      Hero := PlayObject.GetHeroObjectA;
      if Hero = nil then begin
        sMsg := ExVariableText(sMsg, Str, '???');
        Exit;
      ...
      ArrestStringEx(sVariable, '(', ')', s14);
      boFoundVar := False;
      for i := 0 to PlayObject2.m_DynamicVarList.Count - 1 do begin
    ...
    if CompareLStr(sVariable, '$H.STR(', Length('$H.STR(')) then begin
      Hero := PlayObject.GetHeroObjectA;
      if Hero = nil then begin
        sMsg := ExVariableText(sMsg, Str, '???');
        Exit;
    ...
    if CompareLStr(sVariable, '$PARAM(', Length('$PARAM(')) then begin
      ArrestStringEx(sVariable, '(', ')', s14);
      n18 := Str_ToInt(s14, -1);
      if n18 >= 0 then begin
        case n18 of
...
//
//  function IsPlayObject: Boolean;//主体自身定义  Development 2019-01-21
//  begin
//    Result := (TBaseObject <> nil) and (TBaseObject.m_btRaceServer = RC_PLAYOBJECT) and (TBaseObject = PlayObject);
//  end;
  ...
  var
    ...
    begin
      ...
      if CompareLStr(sItemType, '[HELMETEX]', 4) then begin
        if PlayObject.m_UseItems[U_HELMETEX].wIndex > 0 then begin
    ...
    begin
      ...
      with PlayObject do begin
        for i := 0 to m_MagicList.Count - 1 do begin
          if (pTUserMagic(m_MagicList.Items[i]).MagicInfo.sMagicName = MagicName) then begin
            Result := True;
            Exit;
          end;
        end;
...
//        Name := Trim(Name);
//        if CharName = Name then begin
////          ssDay := Trim(sText);
////          nnday := Str_ToDate(ssDay);
////          UseDay := Round(Date - nnday);
////          LastDay := nDay - UseDay;
//          nDay := High(Integer);
//          if TryStrToDateTime(sDay, dOldDate) then
//          begin
//              nDay := GetDayCount(Now, dOldDate);
//          end;
//
//          MainOutMessageAPI('loading fail.... => ' + IntToStr(UseDay));
//          MainOutMessageAPI('loading fail.... => ' + IntToStr(LastDay));
//
//          case sMethod[1] of
//            '=': if LastDay = 0 then Result := True;
//            '<': if LastDay > 0 then Result := True;
    ...
    try
      for i := 0 to ConditionList.Count - 1 do begin
        ...
        if (QuestConditionInfo.sParam1 <> '') then begin
          if (QuestConditionInfo.sParam1[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam1) > 0 then
        ...
        if (QuestConditionInfo.sParam2 <> '') then begin
          if (QuestConditionInfo.sParam2[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam2) > 0 then
        ...
        if (QuestConditionInfo.sParam3 <> '') then begin
          if (QuestConditionInfo.sParam3[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam3) > 0 then
        ...
        if (QuestConditionInfo.sParam4 <> '') then begin
          if (QuestConditionInfo.sParam4[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam4) > 0 then
        ...
        if (QuestConditionInfo.sParam5 <> '') then begin
          if (QuestConditionInfo.sParam5[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam5) > 0 then
            QuestConditionInfo.sParam5 := GetLineVariableText(PlayObject, QuestConditionInfo.sParam5);
        end;
        if (QuestConditionInfo.sParam6 <> '') then begin
          if (QuestConditionInfo.sParam6[1] = '$') then begin
          ...
          end else if Pos('>', QuestConditionInfo.sParam6) > 0 then
            QuestConditionInfo.sParam6 := GetLineVariableText(PlayObject, QuestConditionInfo.sParam6);
        end;

        //参数变量解释以主执行人物为依据
        if (QuestConditionInfo.sOpName <> '') then begin
          if (Length(QuestConditionInfo.sOpName) > 2) then begin
            if (QuestConditionInfo.sOpName[1] = '$') then begin
            ...
            end else if Pos('>', QuestConditionInfo.sOpName) > 0 then
            ...
            if Human <> nil then begin
              PlayObject := Human;
              QActObject := Human;
              if (QuestConditionInfo.sOpHName <> '') and (QuestConditionInfo.sOpHName = 'H') then begin
          ...
          end else if CompareText(QuestConditionInfo.sOpName, 'H') = 0 then begin
            Human := TPlayObject(PlayObject.GetHeroObjectA);
            if (Human <> nil) then begin
              PlayObject := Human;
            end else begin
        ...
        if IsStringNumber(QuestConditionInfo.sParam1) then QuestConditionInfo.nParam1 := Str_ToInt(QuestConditionInfo.sParam1, 0);
        if IsStringNumber(QuestConditionInfo.sParam2) then QuestConditionInfo.nParam2 := Str_ToInt(QuestConditionInfo.sParam2, 1);
        if IsStringNumber(QuestConditionInfo.sParam3) then QuestConditionInfo.nParam3 := Str_ToInt(QuestConditionInfo.sParam3, 1);
        if IsStringNumber(QuestConditionInfo.sParam4) then QuestConditionInfo.nParam4 := Str_ToInt(QuestConditionInfo.sParam4, 0);
        if IsStringNumber(QuestConditionInfo.sParam5) then QuestConditionInfo.nParam5 := Str_ToInt(QuestConditionInfo.sParam5, 0);
        if IsStringNumber(QuestConditionInfo.sParam6) then QuestConditionInfo.nParam6 := Str_ToInt(QuestConditionInfo.sParam6, 0);
...
//            end;
          ...
          nSC_CHECKSTRINGLIST: if not CheckStringList(QuestConditionInfo.sParam2, m_sPath + QuestConditionInfo.sParam1) then Result := False;
          nSC_CHECKMISSION: if not ConditionOfCheckMission(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKTITLE: if not ConditionOfCheckTitle(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nEQUAL: begin
              ...
              if n10 >= 0 then begin
                case n10 of
                  0..9: if PlayObject.m_nVal[n10] <> n18 then Result := False;
                  100..199: if g_Config.GlobalVal[n10 - 100] <> n18 then Result := False;
                  200..299: if PlayObject.m_DyVal[n10 - 200] <> n18 then Result := False;
                  300..399: if PlayObject.m_nMval[n10 - 300] <> n18 then Result := False;
                  400..499: if g_Config.GlobaDyMval[n10 - 400] <> n18 then Result := False;
                  500..599: if g_Config.GlobaDyTval[n10 - 500] <> s14 then Result := False;
                  600..699: if PlayObject.m_nSval[n10 - 600] <> s14 then Result := False;
                  700..799: if g_Config.HGlobalVal[n10 - 700] <> n18 then Result := False;
                end;
              end else
                Result := False;
            end;
          nLARGE: begin
              ...
              if n10 >= 0 then begin
                case n10 of
                  ...
                  300..399: if PlayObject.m_nMval[n10 - 300] <= n18 then Result := False;
                  400..499: if g_Config.GlobaDyMval[n10 - 400] <= n18 then Result := False;
                  500..599: if Str_ToInt(g_Config.GlobaDyTval[n10 - 500], -1) <= n18 then Result := False;
                  600..699: if Str_ToInt(PlayObject.m_nSval[n10 - 600], -1) <= n18 then Result := False;
                  700..799: if g_Config.HGlobalVal[n10 - 700] <= n18 then Result := False;
                end;
              end else
                Result := False;
          ...
          nSC_CHECKGAMEGOLDDEAL: if PlayObject.m_btPostSell = 0 then Result := False;
          ...
          nSC_CHECKIPLEVEL: if not ConditionOfCheckIPLevel(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nSC_CHECKNIMBUS: if not ConditionOfCheckNimbus(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nSC_CHECKHEROCREDITPOINT: if not ConditionOfCheckHeroCreditPoint(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nSC_CHECKLISTTEXT:
            begin //检查文件是否包含指定文本  Development 2019-01-21
              if (TPlayObject(PlayObject).m_btRaceServer = RC_PLAYOBJECT) then
              begin
                s01 := QuestConditionInfo.sParam2;
                if (s01 <> '') and (s01[1] = '<') and (s01[2] = '$') then//增加支持<$Str()>  Development 2019-01-21
                  s01 := GetLineVariableText(PlayObject, QuestConditionInfo.sParam2)
                else
                  GetVariableText(PlayObject, QuestConditionInfo.sParam2, s01);

                if QuestConditionInfo.sParam1 <> '' then
                begin
                  if QuestConditionInfo.sParam1[1] = '\' then
                    s02 := Copy(QuestConditionInfo.sParam1, 2, Length(QuestConditionInfo.sParam1) - 1);
                  if QuestConditionInfo.sParam1[2] = '\' then
                    s02 := Copy(QuestConditionInfo.sParam1, 3, Length(QuestConditionInfo.sParam1) - 2);
                  if QuestConditionInfo.sParam1[3] = '\' then
                    s02 := Copy(QuestConditionInfo.sParam1, 4, Length(QuestConditionInfo.sParam1) - 3);
                end;
                s02 := GetLineVariableText(PlayObject, s02); //路径支持变量  Development 2019-01-21
                if not CheckTextInList(s01, s02) then
                  Result := False;
              end
              else
                Result := False;
          ...
          nSC_CHECKRANGEMONCOUNTEX: if not ConditionOfCHECKRANGEMONCOUNTEX(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nSC_CHECKNAMEDATELIST, nCHECKUSERDATE: if not ConditionOfCheckNameDateList(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKMAPHUMANCOUNT: if not ConditionOfCheckMapHumanCount(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKMAPRANGEHUMANCOUNT: if not ConditionOfCheckMapRangeHumanCount(PlayObject, QuestConditionInfo) then Result := False;
          ...
          nSC_ISHIGH: if not ConditionOfIsHigh(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKISONMAP: if not ConditionOfCheckIsOnMap(PlayObject, QuestConditionInfo) then Result := False;
          nSC_IsSameGuildOnMap: if not ConditionOfIsSameGuildOnMap(PlayObject, QuestConditionInfo) then Result := False;

          nSC_ISDUPMODE: if PlayObject.m_PEnvir.GetXYObjCount(PlayObject.m_nCurrX, PlayObject.m_nCurrY) <= 1 then Result := False;
          nSC_INSAFEZONE: if not PlayObject.InSafeZone() then Result := False;
          nSC_CHECKDLGITEMADDVALUE: if not ConditionOfCheckDlgItemAddValue(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKDLGITEMTYPE: if not ConditionOfCheckDlgItemType(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKDLGITEMNAME: if not ConditionOfCheckDlgItemName(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKPOSDLGITEMNAME: if not ConditionOfCheckPosDlgItemName(PlayObject, QuestConditionInfo) then Result := False;
          nSC_GIVEOK: if not ConditionOfGiveItem(PlayObject, QuestConditionInfo) then Result := False;
          nSC_UNWRAPNIMBUSITEM: if not ConditionOfUnSealItem(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKNIMBUSITEMCOUNT: if not ConditionOfCheckNimbusItemCount(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKATTACKMODE: if not ConditionOfCheckAttackMode(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKESCORTINNEAR: if not ConditionOfCheckEscortInNear(PlayObject, QuestConditionInfo) then Result := False;
          nSC_IsEscortIng: if not ConditionOfIsEscortIng(PlayObject, QuestConditionInfo) then Result := False;

          nSC_CHECKMAPNIMBUSCOUNT: if not ConditionOfCheckMapNimbusCount(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKVENATIONLEVEL: if not ConditionOfCheckVenationLevel(PlayObject, QuestConditionInfo) then Result := False;

          nSC_KILLBYHUM: if (PlayObject.m_LastHiter <> nil) and ((PlayObject.m_LastHiter.m_btRaceServer <> RC_PLAYOBJECT) and not PlayObject.m_LastHiter.IsHero) then Result := False;
          nSC_KILLBYMON: if (PlayObject.m_LastHiter <> nil) and ((PlayObject.m_LastHiter.m_btRaceServer = RC_PLAYOBJECT) or PlayObject.m_LastHiter.IsHero) then Result := False;
          nSC_CHECKSIGNMAP: if (PlayObject.m_sMarkerMap = '') or (CompareText(PlayObject.m_sMarkerMap, QuestConditionInfo.sParam1) <> 0) then Result := False;
          nSC_CHECKONLINE: if UserEngine.GetPlayObject(QuestConditionInfo.sParam1) = nil then Result := False;
          //nSC_CHECKOFFLINEPLAY: : if not ConditionOfCheckOffLinePlay(PlayObject, QuestConditionInfo) then Result := False;
          nSC_OFFLINEPLAYERCOUNT: if not ConditionOfCheckOffLinePlayerCount(PlayObject, QuestConditionInfo) then Result := False;
          nSC_CHECKHEROONLINE: if not ConditionOfCheckHeroOnLine(PlayObject, QuestConditionInfo) then Result := False;
          nSC_HAVEHERO: if PlayObject.m_sHeroName = '' then Result := False;
      ...
      if QuestConditionInfo <> nil then begin //0822
  ...
  begin
    ...
    if IsOK then begin
      ...
      for i := 0 to PlayObject.m_GroupOwner.m_GroupMembers.Count - 1 do begin
        Actor := TPlayObject(PlayObject.m_GroupOwner.m_GroupMembers.Objects[i]);
        if Actor.m_Abil.Level >= nLv then begin
          Actor.SendRefMsg(RM_SPACEMOVE_FIRE, 0, 0, 0, 0, '');
          if br then
            Actor.MapRandomMove(sMAP, 0)
          else
          ...
          if (g_FunctionNPC <> nil) then g_FunctionNPC.GotoLable(Actor, sLabel, False);
  ...
  begin
    ...
    for i := 0 to nCount - 1 do begin
      if QuestActionInfo.nParam1 = 1 then begin
  ...
  begin
    psd := nil;
    if CompareLStr(sItemName, '[NECKLACE]', Length('[NECKLACE]')) then begin
      if PlayObject.m_UseItems[U_NECKLACE].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[RING]', Length('[RING]')) then begin
      if PlayObject.m_UseItems[U_RINGL].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[ARMRING]', Length('[ARMRING]')) then begin
      if PlayObject.m_UseItems[U_ARMRINGL].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[WEAPON]', Length('[WEAPON]')) then begin
      if PlayObject.m_UseItems[U_WEAPON].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[HELMET]', Length('[HELMET]')) then begin
      if PlayObject.m_UseItems[U_HELMET].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[HELMETEX]', Length('[HELMETEX]')) then begin
      if PlayObject.m_UseItems[U_HELMETEX].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[DRESS]', Length('[DRESS]')) then begin
    ...
    if CompareLStr(sItemName, '[U_BUJUK]', Length('[U_BUJUK]')) then begin
    ...
    if CompareLStr(sItemName, '[U_BELT]', Length('[U_BELT]')) then begin
    ...
    if CompareLStr(sItemName, '[U_BOOTS]', Length('[U_BOOTS]')) then begin
    ...
    if CompareLStr(sItemName, '[U_CHARM]', Length('[U_CHARM]')) then begin
    ...
    if CompareLStr(sItemName, '[U_DRUM]', Length('[U_DRUM]')) then begin
      if PlayObject.m_UseItems[U_DRUM].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[U_HORSE]', Length('[U_HORSE]')) then begin
      if PlayObject.m_UseItems[U_HORSE].wIndex > 0 then begin
    ...
    if CompareLStr(sItemName, '[U_FASHION]', Length('[U_FASHION]')) then begin
      if PlayObject.m_UseItems[U_FASHION].wIndex > 0 then begin
    ...
    for i := Low(THumanUseItems) to High(THumanUseItems) do begin
      ...
      if PlayObject.m_UseItems[i].wIndex > 0 then begin
        ...
        if (psd <> nil) and (CompareText(psd.Name, sItemName) = 0) then begin
  ...
  begin
    ...
    try
      for i := 0 to ActionList.Count - 1 do begin
        ...
        if (QuestActionInfo.sParam1 <> '') then begin
          if (QuestActionInfo.sParam1[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam1) > 0 then
            QuestActionInfo.sParam1 := GetLineVariableText(PlayObject, QuestActionInfo.sParam1);
        end;
        if (QuestActionInfo.sParam2 <> '') then begin
          if (QuestActionInfo.sParam2[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam2) > 0 then
        ...
        if (QuestActionInfo.sParam3 <> '') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam3) > 0 then
            QuestActionInfo.sParam3 := GetLineVariableText(PlayObject, QuestActionInfo.sParam3);
        end;
        if (QuestActionInfo.sParam4 <> '') then begin
          if (QuestActionInfo.sParam4[1] = '$') then begin
        ...
        if (QuestActionInfo.sParam6 <> '') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam6) > 0 then
            QuestActionInfo.sParam6 := GetLineVariableText(PlayObject, QuestActionInfo.sParam6);
        end;
        if (QuestActionInfo.sParam7 <> '') then begin
          if (QuestActionInfo.sParam7[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam7) > 0 then
            QuestActionInfo.sParam7 := GetLineVariableText(PlayObject, QuestActionInfo.sParam7);
        end;
        if (QuestActionInfo.sParam8 <> '') then begin
          if (QuestActionInfo.sParam8[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam8) > 0 then
            QuestActionInfo.sParam8 := GetLineVariableText(PlayObject, QuestActionInfo.sParam8);
        end;
        if (QuestActionInfo.sParam9 <> '') then begin
          if (QuestActionInfo.sParam9[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam9) > 0 then
            QuestActionInfo.sParam9 := GetLineVariableText(PlayObject, QuestActionInfo.sParam9);
        end;
        if (QuestActionInfo.sParam10 <> '') then begin
          if (QuestActionInfo.sParam10[1] = '$') then begin
          ...
          end else if Pos('>', QuestActionInfo.sParam10) > 0 then
            QuestActionInfo.sParam10 := GetLineVariableText(PlayObject, QuestActionInfo.sParam10);
        end;

        //参数变量解释以主执行人物为依据
        if (QuestActionInfo.sOpName <> '') then begin
          if (Length(QuestActionInfo.sOpName) > 2) then begin
            if (QuestActionInfo.sOpName[1] = '$') then begin
            ...
            end else if Pos('>', QuestActionInfo.sOpName) > 0 then
            ...
            if User <> nil then begin
              PlayObject := User;
              if QuestActionInfo.sOpHName = 'H' then begin
          ...
          end else if CompareText(QuestActionInfo.sOpName, 'H') = 0 then begin
            User := TPlayObject(PlayObject.GetHeroObjectA);
            if (User <> nil) then begin
              PlayObject := User;
            end else begin
        ...
        if IsStringNumber(QuestActionInfo.sParam1) then QuestActionInfo.nParam1 := Str_ToInt(QuestActionInfo.sParam1, 0);
        if IsStringNumber(QuestActionInfo.sParam2) then QuestActionInfo.nParam2 := Str_ToInt(QuestActionInfo.sParam2, 1);
        if IsStringNumber(QuestActionInfo.sParam3) then QuestActionInfo.nParam3 := Str_ToInt(QuestActionInfo.sParam3, 1);
        if IsStringNumber(QuestActionInfo.sParam4) then QuestActionInfo.nParam4 := Str_ToInt(QuestActionInfo.sParam4, 0);
        if IsStringNumber(QuestActionInfo.sParam5) then QuestActionInfo.nParam5 := Str_ToInt(QuestActionInfo.sParam5, 0);
        if IsStringNumber(QuestActionInfo.sParam6) then QuestActionInfo.nParam6 := Str_ToInt(QuestActionInfo.sParam6, 0);
        if IsStringNumber(QuestActionInfo.sParam7) then QuestActionInfo.nParam7 := Str_ToInt(QuestActionInfo.sParam7, 0);
        if IsStringNumber(QuestActionInfo.sParam8) then QuestActionInfo.nParam8 := Str_ToInt(QuestActionInfo.sParam8, 0);
        if IsStringNumber(QuestActionInfo.sParam9) then QuestActionInfo.nParam9 := Str_ToInt(QuestActionInfo.sParam9, 0);
        if IsStringNumber(QuestActionInfo.sParam10) then QuestActionInfo.nParam10 := Str_ToInt(QuestActionInfo.sParam10, 0);
        ...
        case QuestActionInfo.nCMDCode of
          ...
          nRESET: for ii := 0 to QuestActionInfo.nParam2 - 1 do PlayObject.SetQuestFlagStatus(QuestActionInfo.nParam1 + ii, 0);
          nSETOPEN: begin
              n28 := Str_ToInt(QuestActionInfo.sParam1, 0);
              n2C := Str_ToInt(QuestActionInfo.sParam2, 0);
          ...
          nRESETUNIT: for ii := 0 to QuestActionInfo.nParam2 - 1 do PlayObject.SetQuestUnitStatus(QuestActionInfo.nParam1 + ii, 0);
          nBREAK: Result := False;
          nTIMERECALL: begin
          ...
          nTAKECHECKITEM: if UserItem <> nil then begin
              StdItem := UserEngine.GetStdItem(UserItem.wIndex);
              if StdItem <> nil then begin
          ...
          nSC_MUL: begin
              ...
              if n14 >= 0 then begin
                case n14 of
                  000..009: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nVal[n14] := PlayObject.m_nVal[n14] * n18;
                  100..199: if QuestActionInfo.nParam2 > 1 then g_Config.GlobalVal[n14 - 100] := g_Config.GlobalVal[n14 - 100] * n18;
                  200..299: if QuestActionInfo.nParam2 > 1 then PlayObject.m_DyVal[n14 - 200] := PlayObject.m_DyVal[n14 - 200] * n18;
                  300..399: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nMval[n14 - 300] := PlayObject.m_nMval[n14 - 300] * n18;
                  400..499: if QuestActionInfo.nParam2 > 1 then g_Config.GlobaDyMval[n14 - 400] := g_Config.GlobaDyMval[n14 - 400] * n18;
                  700..799: if QuestActionInfo.nParam2 > 1 then g_Config.HGlobalVal[n14 - 700] := g_Config.HGlobalVal[n14 - 700] * n18;
                else
          ...
          nSC_DIV: begin
              ...
              if n14 >= 0 then begin
                case n14 of
                  000..009: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nVal[n14] := PlayObject.m_nVal[n14] div n18;
                  100..199: if QuestActionInfo.nParam2 > 1 then g_Config.GlobalVal[n14 - 100] := g_Config.GlobalVal[n14 - 100] div n18;
                  200..299: if QuestActionInfo.nParam2 > 1 then PlayObject.m_DyVal[n14 - 200] := PlayObject.m_DyVal[n14 - 200] div n18;
                  300..399: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nMval[n14 - 300] := PlayObject.m_nMval[n14 - 300] div n18;
                  400..499: if QuestActionInfo.nParam2 > 1 then g_Config.GlobaDyMval[n14 - 400] := g_Config.GlobaDyMval[n14 - 400] div n18;
                  700..799: if QuestActionInfo.nParam2 > 1 then g_Config.HGlobalVal[n14 - 700] := g_Config.HGlobalVal[n14 - 700] div n18;
                else
                  ScriptActionError(PlayObject, '', QuestActionInfo, sSC_MUL);
                end;
              end else
          ...
          nSC_PERCENT: begin
              ...
              if n14 >= 0 then begin
                case n14 of
                  000..009: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nVal[n14] := PlayObject.m_nVal[n14] * 100 div n18;
                  100..199: if QuestActionInfo.nParam2 > 1 then g_Config.GlobalVal[n14 - 100] := g_Config.GlobalVal[n14 - 100] * 100 div n18;
                  200..299: if QuestActionInfo.nParam2 > 1 then PlayObject.m_DyVal[n14 - 200] := PlayObject.m_DyVal[n14 - 200] * 100 div n18;
                  300..399: if QuestActionInfo.nParam2 > 1 then PlayObject.m_nMval[n14 - 300] := PlayObject.m_nMval[n14 - 300] * 100 div n18;
                  400..499: if QuestActionInfo.nParam2 > 1 then g_Config.GlobaDyMval[n14 - 400] := g_Config.GlobaDyMval[n14 - 400] * 100 div n18;
                  700..799: if QuestActionInfo.nParam2 > 1 then g_Config.HGlobalVal[n14 - 700] := g_Config.HGlobalVal[n14 - 700] * 100 div n18;
                else
                  ScriptActionError(PlayObject, '', QuestActionInfo, sSC_MUL);
                end;
              end else
          ...
          nSC_SETSCTIMER: ActionOfSetScriptTimer(PlayObject, QuestActionInfo);
          nSC_KILLSCTIMER: ActionOfKillScriptTimer(PlayObject, QuestActionInfo);
          nCHANGEMODE: begin
              case QuestActionInfo.nParam1 of
          ...
          nEXCHANGEMAP: begin
              ...
              if Envir <> nil then begin
                ...
                if List58.Count > 0 then begin
                  ...
                  if QuestActionInfo.sParam2 = 'APPR' then begin
                    if User.m_sMasterName = m_sCharName then
                      User.MapRandomMove(m_sMapName, 0);
                  end else if QuestActionInfo.sParam2 = 'DEAR' then begin
                    if User.m_sDearName = m_sCharName then
                      User.MapRandomMove(m_sMapName, 0);
                  end else if QuestActionInfo.sParam2 = '' then
                    User.MapRandomMove(m_sMapName, 0);
                end;
                List58.Free;
                PlayObject.MapRandomMove(QuestActionInfo.sParam1, 0);
          ...
          nBATCHMOVE: begin
              for ii := 0 to ListBatch.Count - 1 do begin
          ...
          nSC_SETRANDOMNO: begin
              Randomize;
              while True do begin
                n2C := Random(9999);
                if (n2C >= 100) and (IntToStr(n2C) <> PlayObject.m_sRandomNo) then begin
          ...
          nSC_OPENBIGDIALOGBOX: ActionOfOpenBigDialogBox(PlayObject, QuestActionInfo);  //使用NPC大对话框 Development 2019-01-14
          ...
          nSC_CLEARDELAYGOTO: begin
              for n2C := 0 to PlayObject.m_DelayCallList.Count - 1 do begin
      ...
      if QuestActionInfo <> nil then begin
...
begin
  ...
  if (m_LastOprLabel = sLabel) and (m_LastOprObject = PlayObject) and (GetTickCount() - m_dwGotoLabelTick <= 1) then begin
    Inc(m_OprCount);
    if m_OprCount > {15}1500 then begin  //超过15个GOTO就会出现脚本死循环现象 Development 2019-01-26 修改
      s := Format('[脚本死循环] %s%s-%s.txt [%s]', [m_sPath, m_sCharName, m_sMapName, sLabel]);
      if (PlayObject.m_btPermission >= 6) then PlayObject.SysMsg(s, c_Red, t_Hint);
      MainOutMessageAPI(s);
      m_OprCount := 0;
      Exit;
    end;
  end else if m_OprCount > 0 then
    m_OprCount := 0;
  ...
  if Script = nil then begin
    if (PlayObject.m_Script <> nil) and (m_ScriptList <> nil) then begin
    ...
    if Script = nil then begin
      if (m_ScriptList <> nil) then begin
        for i := m_ScriptList.Count - 1 downto 0 do begin
    ...
    for ii := 0 to Script.RecordList.Count - 1 do begin
      ...
      if CompareText(sLabel, SayingRecord.sLabel) = 0 then begin
        ...
        for III := 0 to SayingRecord.ProcedureList.Count - 1 do begin
          ...
          boCheckOK := False;
          tQuestUser := nil;
          if QuestCheckCondition(SayingProcedure.ConditionList, PlayObject, tQuestUser) then begin
            sSENDMSG := sSENDMSG + SayingProcedure.sSayMsg;
            if not QuestActionProcess(SayingProcedure.ActionList, PlayObject) then Break;
            if boCheckOK then begin
              if (tQuestUser <> nil) then begin
            ...
            if not QuestActionProcess(SayingProcedure.ElseActionList, PlayObject) then Break;
            if boCheckOK then begin
              if (tQuestUser <> nil) then begin
                //if tQuestUser.IsHero then
                //  tqu := tQuestUser.m_Master as TPlayObject
...
{$IF VER_ClientType_45}
  if not (PlayObject.m_wClientType in [45, 46]) then begin
    if CompareText(sLabel, '@selloff') = 0 then PlayObject.SendMsg(Self, RM_SELLOFF, 0, 0, 0, 0, '');
    if CompareText(sLabel, '@buyoff') = 0 then PlayObject.SendMsg(Self, RM_BUYOFF, 0, 0, 0, 0, '');
    //取款
    ii := 0;
    III := 0;
    if CompareText(sLabel, '@getsellgold') = 0 then begin
      List := TMerchant(Self).m_SellgoldList;
      for i := List.Count - 1 downto 0 do begin
        SellOff := pSellOff(List[i]);
        if SellOff^.sCharName = PlayObject.m_sCharName then begin
          s14 := '';
          if SellOff^.item.btValue[13] = 1 then s14 := ItemUnit.GetCustomItemName(SellOff^.item.MakeIndex, SellOff^.item.wIndex);
          if s14 = '' then s14 := UserEngine.GetStdItemName(SellOff^.item.wIndex);
...
begin
  if m_Master <> nil then m_Master := nil;
  inherited;
end;
...
begin
  ...
  if Mon <> nil then begin
    ...
    else if QuestActionInfo.nParam6 > 0 then begin
...
begin
  ...
  if Mon <> nil then begin
    if (QuestActionInfo.sParam7 <> '') and (QuestActionInfo.sParam7[1] = '1') then
      Mon.m_boAutoChangeColor := True
    else if QuestActionInfo.nParam8 > 0 then begin
      Mon.m_boFixColor := True;
...
var
  ...
  Envir                     : TEnvirnoment;
  MonList                   : TList;
  SlaveObject, BaseObject   : TBaseObject;
begin
  if CompareText(QuestActionInfo.sParam1, 'SELF') = 0 then begin
    if PlayObject.m_PEnvir <> nil then
      Envir := PlayObject.m_PEnvir;
  end else
    Envir := g_MapManager.FindMap(QuestActionInfo.sParam1);
  ...
  nCount := Str_ToInt(QuestActionInfo.sParam6, -1);
  if (Envir = nil) or (nX < 0) or (nY < 0) or (nRange < 0) or (nCount < 0) or (sMonName = '') then begin
  ...
  for i := MonList.Count - 1 downto 0 do begin
    BaseObject := TBaseObject(MonList.Items[i]);
    if BaseObject.IsHero or (BaseObject.m_Master <> PlayObject) then Continue;
    if (sMonName = '*') or (CompareText(BaseObject.m_sCharName, sMonName) = 0) then begin
      if QuestActionInfo.nParam7 = 0 then
...
begin
  ...
  if (PlayObject.m_Escort <> nil) and
    ...
    not PlayObject.m_Escort.m_boDeath then begin
    GotoLable(PlayObject, '@isEscorting', False);
    Exit;
  end;
...
begin
  ...
  if (PlayObject.m_Escort = nil) or
    ...
    PlayObject.m_Escort.m_boDeath then begin
    GotoLable(PlayObject, '@MoveToEscort_Fail_1', False);
    Exit;
  end;

  if PlayObject.m_Escort.m_PEnvir = nil then begin
    GotoLable(PlayObject, '@MoveToEscort_Fail_2', False);
    Exit;
  end;

  if PlayObject.m_Escort.GetRecallXY(PlayObject.m_Escort.m_nCurrX, PlayObject.m_Escort.m_nCurrY, 3, nX, nY) then begin
    PlayObject.SendRefMsg(RM_SPACEMOVE_FIRE, 0, 0, 0, 0, '');
...
begin
  ...
  if (PlayObject.m_Escort = nil) or
    ...
    if ((abs(m_nCurrX - PlayObject.m_Escort.m_nCurrX) > 8) or (abs(m_nCurrY - PlayObject.m_Escort.m_nCurrY) > 8)) then begin
...
begin
  ...
  if Mon = PlayObject.m_Escort then begin
    Mon.MakeGhost;
    Mon.m_Master := nil;
...
begin
  ...
  PlayObject.m_DefMsg := MakeDefaultMsg(SM_SHELLEXECUTE, 0, 0, 0, 0);
...
begin
  ...
  if nWhere in [Low(THumanUseItems)..High(THumanUseItems)] then begin
    if PlayObject.AutoTakeOnItem(nWhere, sItemName) then begin
      PlayObject.SendMsg(PlayObject, RM_SENDUSEITEMS, 0, 0, 0, 0, '');
      PlayObject.FeatureChanged();
    end;
  end
...
//设置个人定时器功能  Development 2019-01-14
...
begin
  ...
  //if not (nTimerID in [0..9]) then begin
  if not (nTimerID in [0..255]) then begin //扩展SETSCTIMER ID(0-255) Development 2019-01-14
...
//关闭个人定时器功能  Development 2019-01-14
...
begin
  nTimerID := Str_ToInt(QuestActionInfo.sParam1, -1);
  //if not (nTimerID in [0..9]) then begin
   if not (nTimerID in [0..255]) then begin  //扩展KILLSCTIMER ID(0-255) Development 2019-01-14
...
var
  ...
  begin
    if StdItem.Eva.EvaTimesMax > 0 then begin
      ...
      if nAdvAbil in [1..127] then begin
      ...
      if nSpSkill in [1..127] then begin
      ...
      if nEvaTimes in [1..StdItem.Eva.EvaTimesMax] then begin
...
begin
  ...
  nStep := 0;
  for i := Low(THumTitles) to High(THumTitles) do begin
    if PlayObject.m_Titles[i].Index = nPoint then begin
      Exit;
    end;
  end;
  ...
  nStep := 0;
  for i := Low(THumTitles) to High(THumTitles) do begin
    if PlayObject.m_Titles[i].Index = 0 then begin
...
begin
  ...
  if nLoop > 1 then nLoop := 1;
  if nLoop < 0 then nLoop := 0;
  ...
  if CompareText(szMapName, 'all') = 0 then begin
    for i := 0 to UserEngine.m_PlayObjectList.Count - 1 do begin
      Player := TPlayObject(UserEngine.m_PlayObjectList.Objects[i]);
      if not Player.m_boGhost and not Player.m_boOffLineFlag then begin
  ...
  end else if (CompareText(szMapName, 'guild') = 0) and (PlayObject.m_MyGuild <> nil) then begin
    for i := 0 to UserEngine.m_PlayObjectList.Count - 1 do begin
      Player := TPlayObject(UserEngine.m_PlayObjectList.Objects[i]);
      if not Player.m_boGhost and not Player.m_boOffLineFlag and (Player.m_MyGuild <> nil) then begin
    ...
    if Envir <> nil then begin
      for i := 0 to UserEngine.m_PlayObjectList.Count - 1 do begin
        Player := TPlayObject(UserEngine.m_PlayObjectList.Objects[i]);
        if not Player.m_boGhost and not Player.m_boOffLineFlag and (Player.m_PEnvir = Envir) then begin
          Player.SendMsg(Player, RM_PLAYSOUND, nLoop, 0, 0, 0, szFileName);
        end;
      end;
    end else begin
...
begin
  ...
  if CompareText(QuestActionInfo.sParam1, 'all') = 0 then begin
    nStep := 0;
    for i := Low(THumTitles) to High(THumTitles) do begin
      if PlayObject.m_Titles[i].Index > 0 then begin
        if (PlayObject.m_btActiveTitle in [1..6]) and (PlayObject.m_btActiveTitle - 1 = i) then begin
    ...
    if nStep > 0 then begin
      PlayObject.SendMyTitles(True);
      if PlayObject.m_btActiveTitle > 0 then begin
...
begin
  if PlayObject.IsHero then begin
    if not PlayObject.m_boHeroSaveRcd then begin
...
begin
  ...
  nCount := Str_ToInt(QuestConditionInfo.sParam6, -1);
  Envir := g_MapManager.FindMap(sMapName);
  if (Envir = nil) or (nX < 0) or (nY < 0) or (sMonName = '') or (nCount < 0) then begin
    ScriptConditionError(PlayObject, QuestConditionInfo, sSC_CHECKRANGEMONCOUNT);
    Exit;
  end;
  MonList := TList.Create;
...
begin
  ...
  if (PlayObject.m_MyGuild <> nil) then begin
    if CompareText(TGuild(PlayObject.m_MyGuild).sGuildName,
      QuestConditionInfo.sParam1) = 0 then begin
      Result := True;
    end;
  end;
end;
...
begin
  ...
  end else if nValType = 15 then begin
    nAddPoint := (nPoint * 1000);
...
begin
  ...
  if (nValType < 0) or (nValType > 30) or (nWhere < 0) or (nWhere > High(THumanUseItems))
    or (nRate < 0) or (nPoint < 0) or (nPoint > 255) then begin
  ...
  if not (nRatePoint in [0..10]) then begin
    case nUpgradeItemStatus of          //
      0: if QuestActionInfo.sParam6 = '' then PlayObject.SysMsg('装备升级未成功', c_Red, t_Hint);
      1: begin
          PlayObject.SendDelItems(UserItem);
          if StdItem.NeedIdentify = 1 then
            AddGameDataLogAPI('10' + #9 +
              ...
              m_sCharName);
          UserItem.wIndex := 0;
          if QuestActionInfo.sParam6 = '' then PlayObject.SysMsg('装备破碎', c_Red, t_Hint);
        end;
      2: begin
          if QuestActionInfo.sParam6 = '' then PlayObject.SysMsg('装备升级失败，装备属性恢复默认', c_Red, t_Hint);
          nAddPoint := 0;
          if nValType > 13 then begin
            if nValType = 16 then begin
  ...
  if nValType = 14 then begin
    ...
    if UserItem.DuraMax + nAddPoint > High(Word) then
      nAddPoint := High(Word) - UserItem.DuraMax;
    UserItem.DuraMax := UserItem.DuraMax + nAddPoint;
  end else if nValType >= 16 then begin
    nAddPoint := nPoint;
    if nValType = 16 then begin
      nAddPoint := nPoint;
  ...
  PlayObject.SendUpdateItem(UserItem);
  if QuestActionInfo.sParam6 = '' then PlayObject.SysMsg('装备升级成功', c_Green, t_Hint);
...
var
  ...
  pdc                       : PTDelayCallNPC;
begin
  if (QuestActionInfo.sParam2 = '') or (QuestActionInfo.nParam1 <= 0) then begin
...
begin
  ...
  nCount := Str_ToInt(QuestConditionInfo.sParam6, -1);
  if (nCount < 0) or (nX < 0) or (nY < 0) or (nR < 1) or (Envir = nil) then begin
...
begin
  ...
  if ClassNameIs(TMerchant.ClassName) then begin
    if (TMerchant(Self).m_PostSellList <> nil) and (TMerchant(Self).m_PostSellList.Count > 0) then begin
      for i := 0 to TMerchant(Self).m_PostSellList.Count - 1 do begin
        PostSell := pTPostSell(TMerchant(Self).m_PostSellList[i]);
        if PostSell.sCharName = sCharName then begin
          DateTime := PostSell.dPostTime;
          Result := True;
          Break;
        end;
...
begin
  ...
  nRate := Str_ToInt(QuestActionInfo.sParam2, -1);
  nTime := Str_ToInt(QuestActionInfo.sParam3, -1);
  if (nRate < 1) or (nTime < 0) or (nIndex < 0) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_POWERRATE);
    Exit;
  end;
...
//使用NPC大对话框 Development 2019-01-14
...
begin
  ...
  if nImageIndex < 0 then begin
    ScriptActionError(PlayObject,'', QuestActionInfo, sSC_OPENBIGDIALOGBOX);
    Exit;
  end;
...
begin
  ...
  try
    if (sData <> '') and (sData[1] = '@') then begin
      ...
      else if CompareText(sLabel, sSCL_GUILDWAR) = 0 then begin
        ReQuestGuildWar(PlayObject, sMsg);
      end
      else if CompareText(sLabel, sDONATE) = 0 then begin
        DoNate(PlayObject);
      end
      else if CompareText(sLabel, sREQUESTCASTLEWAR) = 0 then begin
        ReQuestCastleWar(PlayObject, sMsg);
      end
      else if CompareLStr(sLabel, sREQUESTCASTLEWAR, Length(sREQUESTCASTLEWAR)) then begin
...
begin
  ...
  if sGuildName = '' then
    Result := -4;
  if Length(sGuildName) > 14 then begin
...
begin
  ...
  if PlayObject.IsGuildMaster and not Castle.IsMember(PlayObject) then begin
...
begin
  ...
  if PlayObject.m_PEnvir.m_MapFlag.boNOGUILDRECALL then begin
  ...
  if PlayObject.m_wGroupRcallTime > 0 then begin
    PlayObject.SysMsg(Format('%d 秒之后才可以再使用此功能', [PlayObject.m_wGroupRcallTime]), c_Red, t_Hint);
    Exit;
  end;
  for i := 0 to TGuild(PlayObject.m_MyGuild).m_RankList.Count - 1 do begin
    ...
    for ii := 0 to GuildRank.MemberList.Count - 1 do begin
      ...
      if GuildMember <> nil then begin
        if PlayObject = GuildMember then Continue;
        if GuildMember.m_boAllowGuildReCall then begin
...
begin
  ...
  with PlayObject do begin
    for i := Low(m_UseItems) to High(m_UseItems) do begin
      ...
      if InLimitItemList(sUserItemName, -1, t_dRepair) then begin
...
var
  sJob, sSex                : string;
begin
  if PlayObject.GetHeroObjectA = nil then begin
    if PlayObject.m_sHeroName <> '' then begin
...
begin
  ...
  if not (nIdx in [1..6]) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_TAGMAPINFO);
    Exit;
  end;
  if (PlayObject.m_PEnvir <> nil) and PlayObject.m_PEnvir.m_MapFlag.boNoTagMapInfo then begin
    PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(PlayObject), 0, 0, sWarMsg);
    Exit;
  end;
  if GetTickCount - PlayObject.m_dwSaveHumanDataTick > 60 * 1000 then begin
...
begin
  ...
  if (PlayObject.m_MakerMapInfo[nIdx].sMapName <> '') and (PlayObject.m_MakerMapInfo[nIdx].wMapX > 0) and (PlayObject.m_MakerMapInfo[nIdx].wMapY > 0) then begin
...
begin
  ...
  if GetTickCount - PlayObject.m_dwQueryBagItemsTick > nIntervalTime * 1000 then begin
...
begin
  ...
  if not nAttribute in [0..5] then begin
...
begin
  try
    ...
    if UserEngine.GetPlayObjectNotGhost(sCharName) <> nil then begin
      PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0, sCharOnline);
      Exit;
    end;
    if UserEngine.GetHeroObject(sCharName) <> nil then begin
      PlayObject.SendMsg(Self, RM_MENU_OK, 0, Integer(Self), 0, 0, sCharOnline);
      Exit;
    end;
    if (sCharName <> '') then
...
begin
  ...
  if g <> nil then begin
    if PlayObject.m_boAllowGuild then begin
      if not g.IsMember(PlayObject.m_sCharName) then begin
        if (PlayObject.m_MyGuild = nil) then begin
          if g.AddMember(PlayObject) then begin
...
begin
  ...
  if (Envir <> nil) and (TarEnvir <> nil) then begin
    List := TList.Create;
...
begin
  ...
  if not p in [0..5] then begin
    Exit;
  end;

  if p = 0 then begin
    PlayObject.m_VenationInfos[v].Point := 0;
    if bIshero then begin
  ...
  end else if PlayObject.m_VenationInfos[v].Point <> 5 then begin
  ...
  case cMethod of
    ...
    '-': begin
        if PlayObject.m_VenationInfos[v].Level > p then begin
  ...
  if nChg <> PlayObject.m_VenationInfos[v].Level then begin
    if bIshero then begin
    ...
    if PlayObject.m_VenationInfos[v].Level = 2 then begin
...
begin
  ...
  if PlayObject.m_VenationInfos[v].Point < p then begin
    if p - PlayObject.m_VenationInfos[v].Point = 1 then begin

      Inc(PlayObject.m_VenationInfos[v].Point);
      if PlayObject.m_VenationInfos[v].Point = 5 then begin
...
var
  ...
  bExists                   : Boolean;
  Magic                     : pTMagic;
  UserMagic, UserMagic2     : pTUserMagic;
...
begin
  ...
  n := -1;
  for i := 0 to PlayObject.m_MagicList.Count - 1 do begin
    if (pTUserMagic(PlayObject.m_MagicList.Items[i]).MagicInfo.sMagicName = s) then begin
      n := i;
      Break;
    end;
  end;
...
begin
  nValType := Str_ToInt(QuestActionInfo.sParam2, -1);
  if (nValType < 0) or (nValType > 30) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_GETDLGITEMVALUE);
    Exit;
  end;
  nValNo := GetValNameNo(QuestActionInfo.sParam1);
  if (nValNo >= 300) and (nValNo <= 399) then begin
    if (PlayObject.m_nDlgItemIndex = 0) then Exit;
    for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
      UserItem := PlayObject.m_ItemList.Items[i];
      if (UserItem.MakeIndex = PlayObject.m_nDlgItemIndex) then begin
...
begin
  ...
  if FileExists(sListFileName) then
    LoadList.LoadFromFile(sListFileName);
  if LoadList.IndexOf(QuestActionInfo.sParam2) < 0 then begin
...
begin
  ...
  if (nValType < 0) or (nValType > 30) or not (nRate in [0..100]) or (nPoint < 0) or (nPoint > 255) then begin
  ...
  for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
    ...
    if (UserItem.MakeIndex = PlayObject.m_nDlgItemIndex) then begin
      ...
      if not (StdItem.StdMode in [5..7, 10..13, 15..30, 51, 61]) then begin
      ...
      if nRate >= Random(100) then begin
        nPoint := _MAX(1, Random(nPoint));
        if nValType = 14 then begin
          nAddPoint := (nPoint * 1000);
          if UserItem.Dura + nAddPoint > High(Word) then nAddPoint := High(Word) - UserItem.Dura;
          UserItem.Dura := UserItem.Dura + nAddPoint;
        end else if nValType = 15 then begin
          nAddPoint := (nPoint * 1000);
          if UserItem.DuraMax + nAddPoint > High(Word) then nAddPoint := High(Word) - UserItem.DuraMax;
          UserItem.DuraMax := UserItem.DuraMax + nAddPoint;
        end else begin
          nAddPoint := nPoint;
          if nValType >= 16 then begin
            if nValType = 16 then begin
              ...
              if UserItem.btValue[nValType - 5] + nAddPoint > High(byte) then nAddPoint := High(byte) - UserItem.btValue[nValType - 5];
              UserItem.btValue[nValType - 5] := UserItem.btValue[nValType - 5] + nAddPoint;
            end;
          end else begin
            if UserItem.btValue[nValType] + nAddPoint > High(byte) then nAddPoint := High(byte) - UserItem.btValue[nValType];
            UserItem.btValue[nValType] := UserItem.btValue[nValType] + nAddPoint;
          end;
        end;
        PlayObject.SendUpdateItem(UserItem);
        if QuestActionInfo.sParam5 = '' then PlayObject.SysMsg('你的 ' + StdItem.Name + ' 升级成功', c_Purple, t_Hint);
      end else begin
        case nUpgradeItemStatus of
          0: if QuestActionInfo.sParam5 = '' then PlayObject.SysMsg('升级未成功', c_Red, t_Hint);
          1: begin
              if StdItem.NeedIdentify = 1 then
              ...
              if QuestActionInfo.sParam5 = '' then PlayObject.SysMsg('升级失败,装备已破碎', c_Red, t_Hint);
            end;
          2: begin
              if QuestActionInfo.sParam5 = '' then PlayObject.SysMsg('升级失败,装备属性恢复默认', c_Red, t_Hint);
              if nValType >= 16 then begin
                if nValType = 16 then
                  UserItem.btValue[14] := 0
                else if nValType = 17 then
...
begin
  ...
  nCount := Str_ToInt(QuestConditionInfo.sParam3, -1);
  if sMAP <> '' then Envir := g_MapManager.FindMap(sMAP);
  if (nCount <= 0) or (Envir = nil) then begin
...
begin
  ...
  for i := 0 to PlayObject.m_SlaveList.Count - 1 do begin
    BaseObject := TBaseObject(PlayObject.m_SlaveList.Items[i]);
    if not BaseObject.m_boDeath and (CompareText(sSlaveName, BaseObject.m_sCharName) = 0) then begin
      BaseObject.m_WAbil.HP := 0;
      //Break;
    end;
  end;
...
begin
  ...
  nTime := Str_ToInt(QuestActionInfo.sParam3, -1);
  if sMAP <> '' then Envir := g_MapManager.FindMap(sMAP);
  if not (nRate in [1..255]) or (Envir = nil) or (nTime <= 0) then begin
  ...
  //nRate := 256 - nRate;
  Randomize();
  if Envir.NimbusCount <= 0 then begin
    for nW := 0 to Envir.m_MapHeader.wWidth - 1 do begin
      for nH := 0 to Envir.m_MapHeader.wHeight - 1 do begin
        if Random(nRate) = 0 then begin
          if Envir.CanWalk(nW, nH, True) then begin
...
begin
  ...
  for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
    ...
    pStdItem := UserEngine.GetStdItem(UserItem.wIndex);
    if (pStdItem <> nil) and (pStdItem.StdMode = 2) and (pStdItem.Shape = 12) and (UserItem.btValue[0] = 0) and (UserItem.Dura = UserItem.DuraMax) then begin
...
begin
  Result := False;
  nCount := Str_ToInt(QuestConditionInfo.sParam2, -1);
  if not (nCount in [0..6]) then begin
...
begin
  ...
  if (nCount < 0) or (sItemName = '') then begin
  ...
  for i := 0 to PlayObject.m_ItemList.Count - 1 do begin
    ...
    if (pStdItem <> nil) and
      ...
      (CompareText(sItemName, pStdItem.Name) = 0) then begin
      Inc(n);
    end;
  end;
  cMethod := QuestConditionInfo.sParam3[1];
