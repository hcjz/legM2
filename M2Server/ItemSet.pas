...
type
  TfrmItemSet = class(TForm)
    ...
    Label134: TLabel;
    Label135: TLabel;
    EditRefineRandomAddValue: TSpinEdit;
    Label136: TLabel;
    TabSheet22: TTabSheet;
    seEffectBonuPointLevel: TSpinEdit;
    Label211: TLabel;
    GroupBox56: TGroupBox;
    Label137: TLabel;
    Label138: TLabel;
    Label139: TLabel;
    EditNeckLace19LuckAddValueMaxLimit: TSpinEdit;
    EditNeckLace19LuckAddValueRate: TSpinEdit;
    EditNeckLace19LuckAddRate: TSpinEdit;
    cbAddValEx: TCheckBox;
    Label140: TLabel;
    seMaxPowerLuck: TSpinEdit;
    cbSearchHumanOutSafeZone: TCheckBox;
    ...
    procedure cbAddValExClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
    procedure ModValue();
...
begin
  ...
  ButtonEctypeSave.Enabled := False;
end;
...
begin
  ...
  cbAddValEx.Checked := g_Config.boAddValEx;

  EditItemExpRate.Value := g_Config.nItemExpRate;
  EditItemPowerRate.Value := g_Config.nItemPowerRate;
  EditItemAcRate.Value := g_Config.nItemAcRate;
  EditItemMAcRate.Value := g_Config.nItemMacRate;
  ...
  EditNeckLace19LuckAddRate.Value := g_Config.nNeckLace19LuckAddRate;
  EditNeckLace19LuckAddValueRate.Value := g_Config.nNeckLace19LuckAddValueRate;
  EditNeckLace19LuckAddValueMaxLimit.Value := g_Config.nNeckLace19LuckAddValueMaxLimit;
...
begin
  ...
  g_Config.nNeckLace19DCAddValueMaxLimit :=
    EditNeckLace19DCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nNeckLace19MCAddValueMaxLimit :=
    EditNeckLace19MCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  if Sender = EditNeckLace19SCAddValueMaxLimit then
    g_Config.nNeckLace19SCAddValueMaxLimit := EditNeckLace19SCAddValueMaxLimit.Value;
  if Sender = EditNeckLace19LuckAddValueMaxLimit then
    g_Config.nNeckLace19LuckAddValueMaxLimit := EditNeckLace19LuckAddValueMaxLimit.Value;
...
begin
  if not boOpened then
    Exit;
  if Sender = EditNeckLace19SCAddValueRate then
    g_Config.nNeckLace19SCAddValueRate := EditNeckLace19SCAddValueRate.Value;
  if Sender = EditNeckLace19LuckAddValueRate then
    g_Config.nNeckLace19LuckAddValueRate := EditNeckLace19LuckAddValueRate.Value;
  ModValue();
end;
...
begin
  if not boOpened then
    Exit;
  if Sender = EditNeckLace19SCAddRate then
    g_Config.nNeckLace19SCAddRate := EditNeckLace19SCAddRate.Value;
  if Sender = EditNeckLace19LuckAddRate then
    g_Config.nNeckLace19LuckAddRate := EditNeckLace19LuckAddRate.Value;
  ModValue();
end;

procedure TfrmItemSet.EditNeckLace202124DCAddValueMaxLimitChange(Sender:
  TObject);
begin
  if not boOpened then
    Exit;
  g_Config.nNeckLace202124DCAddValueMaxLimit :=
    EditNeckLace202124DCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nNeckLace202124DCAddValueRate :=
    EditNeckLace202124DCAddValueRate.Value;
  ModValue();
end;

procedure TfrmItemSet.EditNeckLace202124MCAddValueMaxLimitChange(Sender:
  TObject);
begin
  if not boOpened then
    Exit;
  g_Config.nNeckLace202124MCAddValueMaxLimit :=
    EditNeckLace202124MCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nNeckLace202124MCAddValueRate :=
    EditNeckLace202124MCAddValueRate.Value;
  ModValue();
end;

procedure TfrmItemSet.EditNeckLace202124SCAddValueMaxLimitChange(Sender:
  TObject);
begin
  if not boOpened then
    Exit;
  g_Config.nNeckLace202124SCAddValueMaxLimit :=
    EditNeckLace202124SCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nNeckLace202124SCAddValueRate :=
    EditNeckLace202124SCAddValueRate.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nArmRing26DCAddValueMaxLimit :=
    EditArmRing26DCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nArmRing26MCAddValueMaxLimit :=
    EditArmRing26MCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  g_Config.nArmRing26SCAddValueMaxLimit :=
    EditArmRing26SCAddValueMaxLimit.Value;
  ModValue();
end;
...
begin
  ...
  cbSearchHumanOutSafeZone.Checked := g_Config.boSearchHumanOutSafeZone;

  CheckBoxUserMoveCanOnItem.Checked := g_Config.boUserMoveCanOnItem;
  EditUserMoveTime.Value := g_Config.dwUserMoveTime;
  seEffectBonuPointLevel.Value := g_Config.nEffectBonuPointLevel;
  seMaxPowerLuck.Value := g_Config.btMaxPowerLuck; 
end;
...
begin
  if not boOpened then
    Exit;
  if Sender = cbSearchHumanOutSafeZone then
    g_Config.boSearchHumanOutSafeZone := cbSearchHumanOutSafeZone.Checked
  else
...
begin
  if not boOpened then
    Exit;
  if Sender = seEffectBonuPointLevel then
    g_Config.nEffectBonuPointLevel := seEffectBonuPointLevel.Value
  else if Sender = seEffectBonuPointLevel then
    g_Config.dwUserMoveTime := EditUserMoveTime.Value
  else if Sender = seMaxPowerLuck then
    g_Config.btMaxPowerLuck := seMaxPowerLuck.Value;    

  ModValue();
end;

procedure TfrmItemSet.ButtonEctypeSaveClick(Sender: TObject);
begin
  g_Config.nAutoClearEctypeTick := EditAutoClearEctype.Value * 1000;
  g_Config.boAutoClearEctype := CheckBoxAutoClearEctype.Checked;
  ...
  ButtonClearEctype.Enabled := CheckBoxAutoClearEctype.Checked;
  uModValue();
end;

procedure TfrmItemSet.RefItemSetEx;
begin
  EditAutoClearEctype.Value := g_Config.nAutoClearEctypeTick div 1000;
  CheckBoxAutoClearEctype.Checked := g_Config.boAutoClearEctype;
  EditAutoClearEctype.Enabled := g_Config.boAutoClearEctype;
  ButtonClearEctype.Enabled := g_Config.boAutoClearEctype;
  CheckBoxSaveRcdPromptly.Checked := g_Config.boSaveRcdNow;

  CheckBoxAutoClearEctype.Enabled := False;
  ButtonClearEctype.Enabled := False;
  EditAutoClearEctype.Enabled := False;

end;

procedure TfrmItemSet.CheckBoxAutoClearEctypeClick(Sender: TObject);
begin
  if not boOpened then Exit;
  EditAutoClearEctype.Enabled := CheckBoxAutoClearEctype.Checked;
  ButtonClearEctype.Enabled := False;
  ModValue();
end;

procedure TfrmItemSet.EditAutoClearEctypeChange(Sender: TObject);
begin
  if not boOpened then Exit;
  ModValue();
end;

procedure TfrmItemSet.ButtonClearEctypeClick(Sender: TObject);
begin
  ClearEctypeManus;
  Application.MessageBox('清理在线所有玩家非法复制物品完毕', '信息提示', MB_ICONINFORMATION + MB_OK);
end;

procedure TfrmItemSet.CheckBoxSaveRcdPromptlyClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boSaveRcdNow := CheckBoxSaveRcdPromptly.Checked;
  ModValue();
end;

procedure TfrmItemSet.EditItemAcRateChange(Sender: TObject);
begin
  if not boOpened then
    Exit;
  g_Config.nItemAcRate := EditItemAcRate.Value;
  ModValue();
end;

procedure TfrmItemSet.EditItemMAcRateChange(Sender: TObject);
begin
  if not boOpened then
    Exit;
  g_Config.nItemMacRate := EditItemMAcRate.Value;
  ModValue();
end;
