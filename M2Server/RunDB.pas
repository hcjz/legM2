...
function SaveHumRcdToDB(sAccount, sCharName: string; nSessionID: Integer; HumanRcd: THumDataInfo; var nHeroFlag: Integer): Boolean;
function SaveRcd(sAccount, sCharName: string; nSessionID: Integer; HumanRcd: THumDataInfo; var nHeroFlag: Integer): Boolean;
...
function QueryLevelRank(nType, nPage: Integer; sCharName: string; var LR: string; var HLR: string): Integer;
...
begin
  ...
  if GetDBSockMsg(nQueryID, {wSeries,} nIdent, nRecog, sRestStr, 5000 - 3, 1) then begin
    Result := False;
    if nIdent = DBR_LOADHEROS then begin
...
begin
  ...
  FillChar(HumanRcd, SizeOf(THumDataInfo), #0);
  //FillChar(HerosInfo, SizeOf(THerosInfo), #0);
  if LoadRcd(sAccount, sCharName, sStr, nCertCode, HumanRcd {, HerosInfo}) then begin
    if (HumanRcd.Data.sChrName = sCharName) and ((HumanRcd.Data.sAccount = '') or (HumanRcd.Data.sAccount = sAccount)) then begin
      Result := True;
    end
  end;
  Inc(g_Config.nLoadDBCount);
...
begin
  ...
  end else if nHeroFlag = -3 then begin
  ...
  if GetDBSockMsg(nQueryID, {wSeries,} nIdent, nRecog, sRetCode, 5000 - 1, 0) then begin
    if sRetCode <> '' then begin
...
begin
  ...
  if GetDBSockMsg(nQueryID, {wSeries,} nIdent, nRecog, sLevelRankStr, 2000, 2) then begin
    if nIdent = DBR_LOADRANKDATA then begin
      Result := nRecog;
...
begin
  ...
  sDBMsg := EncodeMessage(DefMsg) + EncodeBuffer(@LoadHuman, SizeOf(TLoadHuman));
  SendDBSockMsg(nQueryID, sDBMsg {, True});
  if GetDBSockMsg(nQueryID, nIdent, nRecog, sHumanRcdStr, 5000, 1) then begin
    Result := False;
...
var
  sSENDMSG                  : string;
  s, n, nn, nRet, nCheckCode: Integer;
  sCheckStr                 : string;
  Config                    : pTConfig;
{$IF DBSOCKETMODE <> TIMERENGINE}
