...
var
  TempConnInfo, TempPassword, BeforeEncodePwd, EncodePwd: string;
  TempSaveStr, TempFinalStr : string;
...
begin
  ...
  with FADOQuery do begin
    ...
    if RecordCount = 1 then begin
      if LoadItemFromDB(BuyItem, FADOQuery) then begin
        Result := UMRESULT_SUCCESS;
...
begin
  ...
  with FADOQuery do begin
    ...
    if RecordCount = 1 then begin
      if LoadItemFromDB(Cancelitem, FADOQuery) then begin
...
begin
  ...
  begin
    ...
    if RecordCount = 1 then begin
      if LoadItemFromDB(GetPayitem, FADOQuery) then begin
        Result := UMRESULT_SUCCESS;
