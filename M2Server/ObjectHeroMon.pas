...
begin
  if not m_boRunReady then begin
    if GetTickCount - m_dwStartWalkTick > 980 then begin
      m_boRunReady := True;
  ...
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if (m_Master <> nil) and (m_Master.m_PEnvir <> nil) and
      ...
      if m_Master.m_boHeroSearchTag then m_Master.m_boHeroSearchTag := False;
    ...
    if (m_Master = nil) or not m_Master.m_boSlaveRelax then begin
    ...
    if not m_boWalkWaitLocked and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
      ...
      if (m_Master <> nil) then begin
        ...
        nDist := 20;
        if (m_PEnvir <> m_Master.m_PEnvir) or ((not m_Master.m_boSlaveRelax) and ((abs(m_nCurrX - m_Master.m_nCurrX) > nDist) or (abs(m_nCurrY - m_Master.m_nCurrY) > nDist))) then begin
          nX := m_nTargetX;
          nY := m_nTargetY;
      ...
      if m_nTargetX <> -1 then begin
        if (abs(m_nCurrX - m_nTargetX) > 1) or (abs(m_nCurrY - m_nTargetY) > 1) then begin
          if g_Config.boHeroEvade then begin
            if m_btDirection3 > -1 then begin
              if not RuntoTargetXY(m_btDirection3, False) then
                GotoTargetXY();
              m_btDirection3 := -1;
            end else if m_btDirection2 > -1 then begin
              if not RuntoTargetXY(m_btDirection2, False) then
                GotoTargetXY();
              if m_boNextDir then begin
                m_boNextDir := False;
                m_btDirection3 := (m_btDirection2 + 1) mod 8;
              end;
              m_btDirection2 := -1;
            end else if not RuntoTargetXY(GetNextDirection(m_nCurrX, m_nCurrY, m_nTargetX, m_nTargetY), False) then
              GotoTargetXY();
          end else begin
...
begin
  ...
  if m_TargetCret <> nil then begin
    ...
    if (m_MagicArr[0][56] <> nil) and ((GetTickCount - m_dwLatestPursueHitTick) > (g_Config.nHeroFireSwordTime + 5000)) and (GetLongAttackDirX(m_TargetCret, btDir)) then begin
    ...
    if (m_MagicArr[0][66] <> nil) and (m_MagicArr[0][66].btKey = 0) and (GetTickCount - m_dwLatestHeroLongHitTick > 4 * 1000) and
      ...
      if (m_MagicArr[0][SKILL_ERGUM] <> nil) and (m_MagicArr[0][SKILL_ERGUM].btKey = 0) and (Random(nRange + 20) = 0) or (abs(m_nCurrX - m_TargetCret.m_nCurrX) > 2) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 2) then begin
        ...
        if (m_MagicArr[0][75] <> nil) and (m_nMagShieldHP <= 0) then begin
        ...
        if GetAttackDir(m_TargetCret, btDir) then begin
          if Integer(GetTickCount - m_dwHitTick) > dwAttackTime then begin
            ...
            if (m_MagicArr[0][56] <> nil) and ((GetTickCount - m_dwLatestPursueHitTick) > g_Config.nHeroFireSwordTime) then begin
              m_dwLatestPursueHitTick := GetTickCount();
              nHitMode := 13;
              m_boPursueHitSkill := True;
              goto LabelStartAttack;
            ...
            if (m_MagicArr[0][43] <> nil) and ((GetTickCount - m_dwLatestTwinHitTick) > 15 * 1000) then begin
            ...
            if (m_MagicArr[0][SKILL_FIRESWORD] <> nil) and ((GetTickCount - m_dwLatestFireHitTick) > g_Config.nHeroFireSwordTime) then begin
            ...
            if (m_MagicArr[0][66] <> nil) and ((GetTickCount - m_dwLatestHeroLongHitTick) > 6000) and TargetInSwordLongAttackRangeX() then begin
              m_dwLatestHeroLongHitTick := GetTickCount();
              nHitMode := 12;
            ...
            if (m_MagicArr[0][66] <> nil) and ((GetTickCount - m_dwLatestHeroLongHitTick) > 8000) then begin
              m_dwLatestHeroLongHitTick := GetTickCount();
              nHitMode := 12;
              if (m_WAbil.Level > m_TargetCret.m_WAbil.Level) then
                m_btSquareHit := 2
            ...
            if GetTickCount - m_dwMagStickTick > (8 * 1000 + Random(8 * 1000)) then begin
              m_dwMagStickTick := GetTickCount();
              UserMagic := m_MagicArr[0][41]; //TPlayObject(self).GetMagicInfo(41);
              if (GetTagXYRangeCount(1, 3, 1) > 3) and (UserMagic <> nil) then begin
                m_dwTargetFocusTick := GetTickCount();
            ...
            LabelStartAttack:
            m_dwTargetFocusTick := GetTickCount();
            if nHitMode in [0, 3, 4, 5, 7, 9, 12, 13] then begin
              m_dwHitTick := GetTickCount();
              if m_boLAT then
...
begin
  ...
  if m_Abil.Level >= 7 then begin
    if m_TargetCret <> nil then begin
      ...
      bola := False;
      boWarr := False;
      if (m_TargetCret.m_btRaceServer in [RC_PLAYOBJECT, RC_HERO]) and (m_TargetCret.m_btJob = 0) and ((m_TargetCret.m_MagicArr[0][12] <> nil)) then begin
        boWarr := True;
        if m_TargetCret.GetLongAttackDir(self, btDir) then
          bola := True;
      end;

      if {(m_WAbil.MP > 0) and}((nAbsX > 1) or (nAbsY > 1)) and (not boWarr or not bola) then begin
        if (nAbsX <= g_Config.nMagicAttackRage) and (nAbsY <= g_Config.nMagicAttackRage) then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            ...
            if m_boStrike and (m_dwCloneDispearTick = 0) and (m_MagicArr[0][74] <> nil) then begin
            ...
            if (m_wStatusTimeArr[STATE_BUBBLEDEFENCEUP] <= 0) and (m_MagicArr[0][31] <> nil) then begin
              m_dwTargetFocusTick := GetTickCount();
              Result := HeroDoSpell(m_MagicArr[0][31], self, boTrainOk);
              Exit;
            end;

            if (m_MagicArr[0][22] <> nil) then begin
              if (GetTagXYRangeCount(1, 6) >= 20) then begin
            ...
            if m_btMagPassTh > 0 then begin
              ...
              if m_MagicArr[0][9] <> nil then begin
                btDir := GetNextDirection(m_nCurrX, m_nCurrY, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
                if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, btDir, 1, nNX, nNY) then begin
            ...
            if nTag <= 1 then begin
              //ReGetMagID:
              if (m_TargetCret.m_Abil.Level < 50) and (m_TargetCret.m_Abil.Level + 10 < m_Abil.Level) and (m_TargetCret.m_btLifeAttrib = LA_UNDEAD) and (m_MagicArr[0][32] <> nil) then begin
                m_dwTargetFocusTick := GetTickCount();
                Result := HeroDoSpell(m_MagicArr[0][32], m_TargetCret, boTrainOk);
                Exit;
              end;
              if (m_TargetCret.m_btRaceServer in [RC_PLAYOBJECT, RC_HERO]) and (m_TargetCret.m_btJob = 0) and (Random(nAbsX + nAbsX) <= 2) and (m_TargetCret.m_Abil.Level + 3 < m_Abil.Level) and (m_MagicArr[0][44] <> nil) then begin
                m_dwTargetFocusTick := GetTickCount();
              ...
              if (Random(6) > 3) and (m_MagicArr[0][45] <> nil) then
                m_nCurMagId := 45;
              if (m_nCurMagId > 0) and (m_MagicArr[0][m_nCurMagId] <> nil) then begin
                m_dwTargetFocusTick := GetTickCount();
            ...
            if (m_btDirection3 > -1) and g_Config.boHeroEvade and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
        ...
        if (nAbsX > g_Config.nMagicAttackRage) or (nAbsY > g_Config.nMagicAttackRage) then begin
          if (m_TargetCret.m_PEnvir = m_PEnvir) then
            SetTargetXY(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY)
          else
            DelTargetCreat();
        ...
        if (nAbsX <= 1) and (nAbsY <= 1) and not bola then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            m_dwHitTick := GetTickCount();
            nTag := GetTagXYRangeCount(1, 1, 1);
            if (nTag >= 4) then begin
              if (Random(12) = 0) and (m_MagicArr[0][24] <> nil) then begin
                m_dwTargetFocusTick := GetTickCount();
            ...
            if (m_MagicArr[0][8] <> nil) and (m_Abil.Level > m_TargetCret.m_Abil.Level) then begin
        ...
        if (m_TargetCret.m_PEnvir = m_PEnvir) then begin
          ...
          if g_Config.boHeroEvade then begin
            if not m_boNextDir and (boWarr or ((nAbsX = 0) or (nAbsY = 0) {or (nAbsX = nAbsY)})) then begin
              Inc(m_btDirection, 2);
              m_boNextDir := True;
            end else begin
              Inc(m_btDirection);
            ...
            if (m_btDirection3 > -1) and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
              //SetTargetXY(m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
              //m_dwWalkTick := 0;
              Result := False;
              Exit;
...
label
  ...
  begin
    Result := True;
    case nType of
      1: if not AutoTakeOnItem(U_ARMRINGL, '黄色药粉(大量)', False) and not AutoTakeOnItem(U_ARMRINGL, '黄色药粉(中量)', False) and not AutoTakeOnItem(U_ARMRINGL, '黄色药粉(小量)', False) then begin
          //if GetTickCount - m_dwHintMsgTick > 30 * 1000 then begin
          //  m_dwHintMsgTick := GetTickCount();
          //  SysMsg('包裹或身上没有黄色药粉！', c_Purple, t_Hint);
          //end;
        end else Result := False;
      2: if not AutoTakeOnItem(U_ARMRINGL, '灰色药粉(大量)', False) and not AutoTakeOnItem(U_ARMRINGL, '灰色药粉(中量)', False) and not AutoTakeOnItem(U_ARMRINGL, '灰色药粉(小量)', False) then begin
          //if GetTickCount - m_dwHintMsgTick > 30 * 1000 then begin
          //  m_dwHintMsgTick := GetTickCount();
  ...
  if m_Abil.Level >= 18 then begin
    if (Integer(GetTickCount - m_dwSpellTick)) > (m_nNextHitTime * 3) then begin
      ...
      if m_SlaveList.Count <= 0 then begin
        ...
        if UserMagic = nil then UserMagic := m_MagicArr[0][30];
        if UserMagic = nil then UserMagic := m_MagicArr[0][17];
        if (UserMagic <> nil) then begin
      ...
      if (m_MagicArr[0][14] <> nil) then begin
        if m_Master <> nil then begin
      ...
      if (m_MagicArr[0][15] <> nil) then begin
        if m_Master <> nil then begin
      ...
      if (m_MagicArr[0][2] <> nil) then begin
        if m_Master <> nil then begin
    ...
    if m_TargetCret = nil then begin
      if Integer(GetTickCount - m_dwHitTick) > (m_nNextHitTime) then begin
        m_dwHitTick := GetTickCount();
        if (m_MagicArr[0][34] <> nil) then begin
          if m_Master <> nil then begin
        ...
        if (m_MagicArr[0][29] <> nil) then begin
          if m_Master <> nil then begin
      ...
      if {(m_WAbil.MP > 0) and}((nAbsX > 1) or (nAbsY > 1)) and (not boWarr or not bola) then begin
        if (nAbsX <= g_Config.nMagicAttackRage) and (nAbsY <= g_Config.nMagicAttackRage) then begin
          if Integer(GetTickCount - m_dwHitTick) > (m_nNextHitTime) then begin
            ...
            labWidAttack:
            if (m_MagicArr[0][50] <> nil) then begin
            ...
            if (m_MagicArr[0][6] <> nil) or (m_MagicArr[0][38] <> nil) then begin
            ...
            if (m_MagicArr[0][18] <> nil) and (m_wStatusTimeArr[STATE_TRANSPARENT] <= 0) and (Random(10) = 0) then begin
            ...
            if (m_MagicArr[0][13] <> nil) or (m_MagicArr[0][57] <> nil) then begin
              if (m_MagicArr[0][57] <> nil) and (((Round(m_WAbil.HP / m_WAbil.MaxHP * 100) < 80) and (Random(100 - Round(m_WAbil.HP / m_WAbil.MaxHP * 100)) > 5)) or (Random(10) > 6)) then begin
                m_dwTargetFocusTick := GetTickCount();
                if m_MagicArr[0][57] <> nil then HeroDoSpell(m_MagicArr[0][57], m_TargetCret, boTrainOk);
                Result := True;
              end else if (m_MagicArr[0][13] <> nil) and MagCanHitTarget(m_nCurrX, m_nCurrY, m_TargetCret) then begin
                m_dwTargetFocusTick := GetTickCount();
        ...
        if (nAbsX <= 1) and (nAbsY <= 1) and not bola then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            ...
            if (m_MagicArr[0][48] <> nil) and (m_Abil.Level > m_TargetCret.m_Abil.Level) and (nTag >= 2) then begin //抗拒...
              m_dwTargetFocusTick := GetTickCount();
