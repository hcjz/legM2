  ...
  try
    ...
    if nLen >= SizeOf(TMsgHeader) then begin
      while (True) do begin
        ...
        if (MsgHeader.dwCode = RUNGATECODE) and (nCheckMsgLen < $8000) then begin //0319
  ...
  if boStartReady then begin
    ...
    try
      ...
      for i := Low(g_GateArr) to High(g_GateArr) do begin
        Gate := @g_GateArr[i];
        nC := 2;
        if Gate.boUsed and (Gate.Socket <> nil) and (Gate.BufferList <> nil) then begin
          nC := 3;
          EnterCriticalSection(m_RunSocketSection);
          try
            nC := 4;
    ...
    try
      ...
      if (Length(sData) > 2) and (sData[1] = '*') and (sData[2] = '*') then begin
        ...
        if sIdx = '0' then
          ...
          boFlag := False;
        if (sAccount <> '') and (sChrName <> '') and (nSessionID >= 2) and (sHWID <> '') then begin
  ...
  try
    if GateUser.sAccount = '' then begin
      if TagCount(sMsg, '!') > 0 then begin
        ...
        if GetCertification(sMsg, sAccount, sChrName, nSessionID, nClientVersion, tHWID, boFlag) then begin
...
begin
  ...
  dwRunTick := GetTickCount();
  if Gate.nSendChecked > 0 then begin   {如果网关未回复状态消息，则不再发送数据}
  ...
  try
    if MsgList.Count > 1 then begin     //将小数据合并为一个指定大小的数据, 减少send()缓冲区压力
      i := 0;
      BufferA := MsgList.Items[i];
      while (True) do begin
        nRet := i + 1;
        if nRet >= MsgList.Count then Break;
        ...
        Inc(i);
        BufferA := BufferB;
        if i > 10240 then Break;        //0901 10240     1001
      end;
    end;
  except
    on E: Exception do begin
  ...
  try
    ...
    while MsgList.Count > 0 do begin
      ...
      if (Gate.nSendChecked = 0) and ((Gate.nSendBlockCount + nSendBuffLen) >= g_Config.nCheckBlock) then begin
        if (Gate.nSendBlockCount = 0) and (g_Config.nCheckBlock <= nSendBuffLen) then begin //丢掉大数据包
          nCheckCode := 4;
          MsgList.Delete(0);
          FreeMem(BufferA);
          Break;
          ...
          nCheckCode := 5;
          if Gate.nSendBlockCount + nSendBuffLen >= g_Config.nCheckBlock * 2 then begin
      ...
      if nSendBuffLen > 0 then begin
        if Gate.Socket <> nil then begin
          ...
          if Gate.Socket.Connected then begin
            ...
            if nRet <= 0 then begin     //发送未成功
              Inc(nSendFail);

              nCheckCode := 10;
              if (GetTickCount - dwRunTick) > g_dwSocLimit then begin //超时处理
                Result := False;
                Break;
              end;
              ...
              //0625
              nCheckCode := 11;
              if MsgList.Count > 32 then begin //移动到数据末端，等待下次发送   0802
                MsgList.Move(0, MsgList.Count - 1);
                Continue;
              end;

              if nSendFail <= 512 then begin
                Continue;
              end;
            ...
            if Gate.UserList.Items[i] <> nil then begin
              ...
              if GateUser.nSocket = nSocket then begin
                ...
                try
                  ...
                  if GateUser.PlayObject <> nil then begin
                    n := 1;
                    if TPlayObject(GateUser.PlayObject).m_boSetOffLine and not TPlayObject(GateUser.PlayObject).m_boDeath and TPlayObject(GateUser.PlayObject).InSafeZone() then begin
                      //n := 3;
                      if (TPlayObject(GateUser.PlayObject).m_HeroObject <> nil) and not TPlayObject(GateUser.PlayObject).m_HeroObject.m_boGhost then begin
                    ...
                    n := 9;
                    if not TPlayObject(GateUser.PlayObject).m_boOffLineFlag then begin
                      TPlayObject(GateUser.PlayObject).m_boSoftClose := True
                    end else if not TPlayObject(GateUser.PlayObject).m_boReconnection then begin
...
begin
  try
    case MsgHeader.wIdent of
      ...
      GM_DATA: begin
          GateUser := nil;
          if MsgHeader.wUserListIndex >= 1 then begin //speed up
        ...
        try
          for ii := 0 to Gate.UserList.Count - 1 do begin
            ...
            if (GateUserInfo.sAccount = sAccount) or (GateUserInfo.nSessionID = nSessionID) then begin
              ...
              if (GateUserInfo.PlayObject <> nil) and not GateUserInfo.PlayObject.m_boOffLineFlag then begin
...
begin
  Result := sIPaddr;
  for i := 0 to m_nIPCount - 1 do begin
