...
uses
  ...
  Grobal2, ObjNpc, UsrEngn, Forms, ActiveX, IniFiles, PlugIn;
...
uses ObjBase, Envir, svMain, EDcode,DES;
  ...
  try
    try
      ...
      for i := 0 to Query.RecordCount - 1 do begin
        ...
        if g_Config.tiOpenSystem and fUseItem then begin
          if not g_DisTIList.Exists(StdItem.Name) then
            StdItem.Eva.EvaTimesMax := 3;
        end;

        if fUseItem then begin
        ...
        if StdItem.StdMode = 51 then begin
          if UserEngine.TitleList.Count = StdItem.Shape - 1 then begin
            UserEngine.TitleList.Add(StdItem);
          end else begin
            g_MainMemo.Lines.Add(Format('加载称号(Idx:%d Name:%s)数据失败，物品Shape请保持连续(1~100)', [Idx, StdItem.Name]));
            Result := -100;
            Break;
          end;
        end;
    ...
    if UserEngine.TitleList.Count > 0 then begin
      MakeTitleSendBuffer();
    end;
  ...
  for i := 0 to LoadList.Count - 1 do begin
    ...
    if (sLine <> '') and (sLine[1] <> ';') then begin
      if sLine[1] = '[' then begin
        if List <> nil then g_RefineItemList.AddObject(sRefine, List);
        List := TList.Create;
        ...
        if (sAddValueRate <> '') and (sRandomAddValue <> '') then begin
          New(pRefineItem);
          ...
          for nIxd := Low(pRefineItem.RamAddValue.abAddValueRate) to High(pRefineItem.RamAddValue.abAddValueRate) do begin
...
begin
  ...
  if g_BoxItemList.Count > 0 then begin
  ...
  for i := 0 to LoadList.Count - 1 do begin
    ...
    if (sLine <> '') and (sLine[1] <> ';') then begin
      if sLine[1] = '[' then begin
        if List <> nil then g_BoxItemList.AddObject(sBox, List);
        List := TList.Create;
        ...
        if (sItemName <> '') and (sItemCount <> '') and (sGetItemRate <> '') then begin
          New(pBoxItem);
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (s30 <> '') and (s30[1] = '[') then begin
        ...
        while (True) do begin
          ...
          if CompareLStr(s34, 'COLLECTEXP', Length('COLLECTEXP')) then begin
          ...
          if CompareText(s34, 'CANRIDE') = 0 then begin
            MapFlag.boCANRIDE := True;
            Continue;
          end;
          if CompareText(s34, 'CANBAT') = 0 then begin
          ...
          if CompareText(s34, 'NOTAGMAPINFO') = 0 then begin
          ...
          if CompareLStr(s34, 'NOTALLOWUSEITEMS', Length('NOTALLOWUSEITEMS')) then begin
            ...
            if s38 <> '' then begin
              if MapFlag.sNotAllowUseItems = nil then MapFlag.sNotAllowUseItems := TList.Create;
          ...
          if CompareLStr(s34, 'NOTALLOWUSEMAGIC', Length('NOTALLOWUSEMAGIC')) then begin
            ...
            if s38 <> '' then begin
              if MapFlag.sNotAllowUseMag = nil then MapFlag.sNotAllowUseMag := TList.Create;
              sList.Text := AnsiReplaceText(s38, '|', #13);
              for ii := 0 to sList.Count - 1 do begin
          ...
          if CompareText(s34, 'NORECALLHERO') = 0 then begin
          ...
          if CompareText(s34, 'STALL') = 0 then begin
            MapFlag.boStall := True;
            Continue;
          end;
          if CompareText(s34, 'NOSWITCHATTACKMODE') = 0 then begin
          ...
          if CompareLStr(s34, 'GREATTHUNDER', Length('GREATTHUNDER')) then begin
            ArrestStringEx(s34, '(', ')', s38);
          ...
          if CompareLStr(s34, 'SPURT', Length('SPURT')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            MapFlag.nSpurt := Str_ToInt(s38, -1);
            Continue;
          end;
          if CompareLStr(s34, 'DIGITEM', Length('DIGITEM')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            MapFlag.nDigItem := Str_ToInt(s38, -1);
            s4C := '';
            if Pos('|', sMapName) > 0 then begin
              GetValidStr3(sMapName, s4C, ['|']);
            end else if Pos('>', sMapName) > 0 then begin
          ...
          if CompareLStr(s34, 'SECRET', Length('SECRET')) then begin
            ArrestStringEx(s34, '(', ')', s38);
            if s38 <> '' then begin
              s38 := GetValidStr3(s38, s44, ['|']);
        ...
        if g_MapManager.AddMapInfo(sMapName, sMapDesc, nServerIndex, @MapFlag, QuestNPC) = nil then begin
          Result := -10;
          LoadList.Free;
          sList.Free;
          Exit;
...
begin
  try
    ...
    if not FileExists(sScriptFile) then begin
      ...
      SaveList.Add('#say');
      SaveList.Add('LEGEND传奇登录脚本运行成功，欢迎进入本游戏！\ \');
  ...
  if FileExists(sFileName) then begin
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if (MonGenInfo.sMapName <> '') and (MonGenInfo.sMonName <> '') and (MonGenInfo.dwZenTime <> 0) and (g_MapManager.GetMapInfo(g_nServerIndex, MonGenInfo.sMapName) <> nil) then begin
...
{$IF USEHASHLIST = 1}
      if UserEngine.MonsterList.Exists(sName) then begin
...
{$IFEND}
      ...
      Monster.btExplore := Query.FieldByName('ExploreItem').AsInteger;
      Monster.wInLevel := Query.FieldByName('InLevel').AsInteger;
      Monster.dwIPExp := Query.FieldByName('IPExp').AsInteger;
...
var
  ...
  begin
    ...
    if FileExists(sFileName) then begin
      ...
      DeCodeStringList(LoadStrList); //解密加密脚本 Development 2019-02-03
...
//              TDeCryptString(PlugProcArray[nDeCryptString].nProcAddr)(@sLine[1], @Dest, Length(sLine), nDest);
      ...
      if not bo1D then
        ...
        if not Result then MainOutMessageAPI('#CALL 脚本错误，没有}号，文件名: ' + sFileName);
    end else
  ...
  begin
    ...
    if Pos('.', sCmd) > 0 then begin
      ...
      if sActName <> '' then begin
        QuestConditionInfo.sOpName := sActName;
        if Pos('.', sCmd) > 0 then begin
          sCmd := GetValidStrCap(sCmd, sActName, ['.']);
          if sActName = 'H' then QuestConditionInfo.sOpHName := 'H';
        end;
      end;
    end;
    nCMDCode := 0;
    ...
    if sCmd = sSC_ISONCASTLEWAR then begin
    ...
    if sCmd = sSC_CHECKNIMBUS then begin
      nCMDCode := nSC_CHECKNIMBUS;
      goto L001;
    end;
    if sCmd = sSC_CHECKATTACKMODE then begin
      nCMDCode := nSC_CHECKATTACKMODE;
      goto L001;
    end;
    if sCmd = sSC_CHECKESCORTINNEAR then begin
      nCMDCode := nSC_CHECKESCORTINNEAR;
      goto L001;
    end;
    if sCmd = sSC_IsEscortIng then begin
    ...
    if sCmd = sSC_CHECKITEMLUCK then begin
    ...
    if sCmd = sSC_CHECKRANGEMONCOUNTEX then begin
      nCMDCode := nSC_CHECKRANGEMONCOUNTEX;
      goto L001;
    end;
    if sCmd = sSC_CHECKSTRINGLIST then begin
    ...
    if sCmd = sSC_CHECKTITLE then begin
    ...
    if sCmd = sSC_CHECKGAMEGOLDDEAL then begin
    ...
    if sCmd = sSC_CHECKHEROCREDITPOINT then begin
      nCMDCode := nSC_CHECKHEROCREDITPOINT;
      goto L001;
    end;
    if sCmd = sSC_CHECKDLGITEMTYPE then begin
      nCMDCode := nSC_CHECKDLGITEMTYPE;
      goto L001;
    end;
    if sCmd = sSC_CHECKDLGITEMNAME then begin
      nCMDCode := nSC_CHECKDLGITEMNAME;
      goto L001;
    end;
    if sCmd = sSC_CHECKPOSDLGITEMNAME then begin
      nCMDCode := nSC_CHECKPOSDLGITEMNAME;
      goto L001;
    end;
    if sCmd = sSC_GIVEOK then begin
      nCMDCode := nSC_GIVEOK;
      goto L001;
    end;
    if sCmd = SSC_UNWRAPNIMBUSITEM then begin
      nCMDCode := nSC_UNWRAPNIMBUSITEM;
      goto L001;
    end;
    if sCmd = SSC_CHECKNIMBUSITEMCOUNT then begin
      nCMDCode := nSC_CHECKNIMBUSITEMCOUNT;
      goto L001;
    end;

    if sCmd = SSC_CHECKMAPNIMBUSCOUNT then begin
      nCMDCode := nSC_CHECKMAPNIMBUSCOUNT;
      goto L001;
    end;
    if sCmd = SSC_CHECKVENATIONLEVEL then begin
      nCMDCode := nSC_CHECKVENATIONLEVEL;
      goto L001;
    end;

    if sCmd = sSC_CHECKMAPRANGEHUMANCOUNT then begin
      nCMDCode := nSC_CHECKMAPRANGEHUMANCOUNT;
      goto L001;
    end;
    if sCmd = sSC_CHECKDLGITEMADDVALUE then begin
      nCMDCode := nSC_CHECKDLGITEMADDVALUE;
      goto L001;
    end;

    if sCmd = sSC_CHECKMASTERONLINE then begin
      nCMDCode := nSC_CHECKMASTERONLINE;
      goto L001;
    end;
    if sCmd = sSC_CHECKDEARONLINE then begin
      nCMDCode := nSC_CHECKDEARONLINE;
      goto L001;
    end;
    if sCmd = sSC_CHECKMASTERONMAP then begin
      nCMDCode := nSC_CHECKMASTERONMAP;
      goto L001;
    end;
    if sCmd = sSC_CHECKDEARONMAP then begin
      nCMDCode := nSC_CHECKDEARONMAP;
      goto L001;
    end;
    if sCmd = sSC_CHECKPOSEISPRENTICE then begin
    ...
    if sCmd = sSC_CHECKCURRENTDATE then begin
    ...
    if sCmd = sSC_CHECKIPLEVEL then begin
    ...
    if (sCmd = sSC_CHECKMAGICNAME) or (sCmd = 'CheckSkill') then begin
      nCMDCode := nSC_CHECKMAGICNAME;
      goto L001;
    end;
    if sCmd = sSC_CHECKMAGICLEVEL then begin
      nCMDCode := nSC_CHECKMAGICLEVEL;
      goto L001;
    end;
    if sCmd = sCHECKUSERDATE then begin
      nCMDCode := nCHECKUSERDATE;
      goto L001;
    end;
    if sCmd = sCheckCodeList then begin
    ...
    if sCmd = sADDUSERDATE then begin
    ...
    if sCmd = sYCCALLMOB then begin
      nCMDCode := nYCCALLMOB;
      goto L001;
    end;
    if sCmd = sCheckDiemon then begin
      nCMDCode := nCheckDiemon;
      goto L001;
    end;
    if sCmd = scheckkillplaymon then begin
      nCMDCode := ncheckkillplaymon;
      goto L001;
    end;
    if sCmd = sSC_CHEckISGROUPMASTER then begin
      nCMDCode := nSC_CHEckISGROUPMASTER;
      goto L001;
    end;
    if (sCmd = sSC_CHECKISONMAP) or (sCmd = sSC_CHECKMAPNAME) then begin
      nCMDCode := nSC_CHECKISONMAP;
      goto L001;
    end;
    if sCmd = sSC_IsSameGuildOnMap then begin
  ...
  begin
    ...
    if Pos('.', sCmd) > 0 then begin
      ...
      if sActName <> '' then begin
        ...
        if Pos('.', sCmd) > 0 then begin
          sCmd := GetValidStrCap(sCmd, sActName, ['.']);
          if UpperCase(sActName) = 'H' then QuestActionInfo.sOpHName := 'H';
        end;
      end;
    end;
    ...
    if sCmd = sSC_CONFERTITLE then begin
      nCMDCode := nSC_CONFERTITLE;
      goto L001;
    end;
    if sCmd = sSC_DEPRIVETITLE then begin
    ...
    if sCmd = sSC_READRANDOMLINE then begin
      nCMDCode := nSC_READRANDOMLINE;
      goto L001;
    end;
    if sCmd = sSC_SENDSCROLLMSG then begin
      nCMDCode := nSC_SENDSCROLLMSG;
      goto L001;
    end;
    if sCmd = sSC_SETMERCHANTDLGIMGNAME then begin
    ...
    if sCmd = sSC_OPENGAMEGOLDDEAL then begin
      nCMDCode := nSC_OPENGAMEGOLDDEAL;
      goto L001;
    end;
    if sCmd = sSC_QUERYGAMEGOLDDEAL then begin
      nCMDCode := nSC_QUERYGAMEGOLDDEAL;
      goto L001;
    end;
    if sCmd = sSC_QUERYGAMEGOLDSELL then begin
    ...
    if sCmd = sSC_CHANGEIPLEVEL then begin
    ...
    if sCmd = sSC_TAKEDLGITEM then begin
      nCMDCode := nSC_TAKEDLGITEM;
      goto L001;
    end;
    if sCmd = sSC_ADDLINELIST then begin
      nCMDCode := nSC_ADDLINELIST;
      goto L001;
    end;
    if sCmd = sSC_DELLINELIST then begin
      nCMDCode := nSC_DELLINELIST;
      goto L001;
    end;
    if sCmd = SSC_CREATEMAPNIMBUS then begin
      nCMDCode := NSC_CREATEMAPNIMBUS;
      goto L001;
    end;
    if sCmd = sSC_KILLSLAVENAME then begin
      nCMDCode := nSC_KILLSLAVENAME;
      goto L001;
    end;

    if sCmd = sSC_CLEARMAPITEM then begin
      nCMDCode := NSC_CLEARMAPITEM;
      goto L001;
    end;
    if sCmd = sSC_ADDMAPROUTE then begin
      nCMDCode := nSC_ADDMAPROUTE;
      goto L001;
    end;
    if sCmd = sSC_DELMAPROUTE then begin
    ...
    if sCmd = sSC_HEROSKILLLEVEL then begin
    ...
    if sCmd = sSC_CHANGEIPEXP then begin
    ...
    if sCmd = sSC_NIMBUS then begin
      nCMDCode := nSC_NIMBUS;
      goto L001;
    end;
    if sCmd = sSC_ABILITYADD then begin
    ...
    if sCmd = sSC_GROUPMAPTING then begin
    ...
    if sCmd = sSC_DETOXIFCATION then begin
    ...
    if sCmd = sSC_NAMECOLOR then begin
      nCMDCode := nSC_NAMECOLOR;
      goto L001;
    end;
    if sCmd = sSC_STATUSRATE then begin
    ...
    if (sCmd = sSC_KICK) or (sCmd = 'KICKOFFLINE') then begin
      nCMDCode := nSC_KICK;
      goto L001;
    end;
    ...
    if sCmd = sSC_HEROCREDITPOINT then begin
      nCMDCode := nSC_HEROCREDITPOINT;
      goto L001;
    end;
    if sCmd = sSC_CHANGEATTACKMODE then begin
      nCMDCode := nSC_CHANGEATTACKMODE;
      goto L001;
    end;
    if sCmd = sSC_SETMISSION then begin
      nCMDCode := nSC_SETMISSION;
      goto L001;
    end;
    if sCmd = sSC_CLEARMISSION then begin
    ...
    if sCmd = sSC_MONGENEX2 then begin
      nCMDCode := nSC_MONGENEX2;
      goto L001;
    end;
    if sCmd = sSC_MoveToEscort then begin
      nCMDCode := nSC_MoveToEscort;
      goto L001;
    end;
    if sCmd = sSC_EscortFinish then begin
      nCMDCode := NSC_EscortFinish;
      goto L001;
    end;
    if sCmd = sSC_GiveUpEscort then begin
    ...
    if sCmd = sDELUSERDATE then begin
      nCMDCode := nDELUSERDATE;
      goto L001;
    end;
    if sCmd = sClearCodeList then begin
    ...
    if (sCmd = sgroupmapmove) or (sCmd = sSC_GROUPMOVE) then begin
      nCMDCode := ngroupmapmove;
      goto L001;
    end;
    if sCmd = sthroughhum then begin
      nCMDCode := nthroughhum;
      goto L001;
    end;
    if sCmd = sSC_CLEAREctype then begin
      nCMDCode := nSC_CLEAREctype;
      goto L001;
    end;
    if (sCmd = sSC_OFFLINE) or (sCmd = 'OFFLINEPLAY') then begin
    ...
    if sCmd = sSC_QUERYBINDITEM then begin
      nCMDCode := nSC_QUERYBINDITEM;
      goto L001;
    end;
    if sCmd = sSC_BINDRESUME then begin
      nCMDCode := nSC_BINDRESUME;
      goto L001;
    end;
    if sCmd = sSC_UNBINDRESUME then begin
      nCMDCode := nSC_UNBINDRESUME;
      goto L001;
    end;

    if sCmd = sSC_SETOFFLINE then begin
    ...
    if sCmd = sSC_CHATCOLOR then begin
      nCMDCode := nSC_CHATCOLOR;
      goto L001;
    end;
    if sCmd = sSC_CHATFONT then begin
      nCMDCode := nSC_CHATFONT;
      goto L001;
    end;
    if (sCmd = sSC_GUILDMAPMOVE) or (sCmd = 'GUILDMOVE') then begin
      nCMDCode := nSC_GUILDMAPMOVE;
      goto L001;
    end;
    if sCmd = sSC_RECALLPNEUMA then begin
      nCMDCode := nSC_RECALLPNEUMA;
      goto L001;
    end;
    if sCmd = sSC_RECALLHeroEX then begin
      nCMDCode := nSC_RECALLHeroEX;
      goto L001;
    end;
    if sCmd = sSC_ADDGUILD then begin
    ...
    if sCmd = sSC_BONUSABIL then begin
    ...
    if sCmd = sSC_CREATEHEROEX then begin
    ...
    if sCmd = sSC_AFFILIATEGUILD then begin
      nCMDCode := nSC_AFFILIATEGUILD;
      goto L001;
    end;
    if sCmd = sSC_SETTARGETXY then begin
      nCMDCode := nSC_SETTARGETXY;
      goto L001;
    end;
    if sCmd = sSC_MAPMOVEHUMAN then begin
      nCMDCode := nSC_MAPMOVEHUMAN;
      goto L001;
    end;

    if sCmd = sSC_QUERYVALUE then begin
      nCMDCode := nSC_QUERYVALUE;
      goto L001;
    end;
    if sCmd = sSC_RELEASECOLLECTEXP then begin
      nCMDCode := nSC_RELEASECOLLECTEXP;
      goto L001;
    end;
    if sCmd = sSC_RESETCOLLECTEXPSTATE then begin
      nCMDCode := nSC_RESETCOLLECTEXPSTATE;
      goto L001;
    end;
    if sCmd = sSC_GETSTRLENGTH then begin
      nCMDCode := nSC_GETSTRLENGTH;
      goto L001;
    end;

    if sCmd = sSC_CHANGEVENATIONLEVEL then begin
      nCMDCode := nSC_CHANGEVENATIONLEVEL;
      goto L001;
    end;
    if sCmd = sSC_CHANGEVENATIONPOINT then begin
      nCMDCode := nSC_CHANGEVENATIONPOINT;
      goto L001;
    end;
    if sCmd = sSC_CLEARVENATIONDATA then begin
      nCMDCode := nSC_CLEARVENATIONDATA;
      goto L001;
    end;
    if sCmd = sSC_CONVERTSKILL then begin
      nCMDCode := nSC_CONVERTSKILL;
      goto L001;
    end;

    if sCmd = sSC_QUERYITEMDLG then begin
      nCMDCode := nSC_QUERYITEMDLG;
      goto L001;
    end;
    if sCmd = sSC_GETDLGITEMVALUE then begin
      nCMDCode := nSC_GETDLGITEMVALUE;
      goto L001;
    end;
    if sCmd = sSC_UPGRADEDLGITEM then begin
    ...
    if sCmd = sSC_GETPOSENAME then begin
    ...
    if nCMDCode > 0 then begin
      ...
      if (sParam9 <> '') and (sParam9[1] = '"') then ArrestStringEx(sParam9, '"', '"', sParam9);
      if (sParam10 <> '') and (sParam10[1] = '"') then ArrestStringEx(sParam10, '"', '"', sParam10);
      ...
      if IsStringNumber(sParam9) then QuestActionInfo.nParam9 := Str_ToInt(sParam9, 0);
      if IsStringNumber(sParam10) then QuestActionInfo.nParam10 := Str_ToInt(sParam10, 0);
      Result := True;
    end;
  end;
begin
  ...
  if FileExists(sScritpFileName) then begin
    ...
    DeCodeStringList(LoadList); //解密加密脚本  Development 2019-02-03 添加
    if LoadList.Count > 0 then begin
    ...
    i := 0;
    while (True) do begin
      if not LoadScriptCall(LoadList) then Break;
      Inc(i);
      if i >= 99 then
        Break;
    end;
    ...
    for i := 0 to LoadList.Count - 1 do begin
        ...
        if s34[1] = '(' then begin
          ...
          if s34 <> '' then begin
            while (s34 <> '') do begin
              ...
              if CompareText(s30, sMAKEPEUMA) = 0 then begin
                TMerchant(NPC).m_boMakeHero := True;
                Continue;
              end;
              if CompareText(s30, sCERATEHERO) = 0 then begin
              ...
              if CompareText(s30, sYBDEAL) = 0 then begin
                TMerchant(NPC).m_boYBDeal := True;
                Continue;
              end;
              if CompareText(s30, sITEMMARKET) = 0 then begin
                TMerchant(NPC).m_boItemMarket := True;
                Continue;
              end;
              if CompareLStr(s30, sMDlgImgName, Length(sMDlgImgName)) then begin
      ...
      if s34[1] = '[' then begin
        ...
        SayingProcedure.boLabel := False;  //支持#OR
      ...
      if (Script <> nil) and (SayingRecord <> nil) then begin
        if (n6C >= 10) and (n6C < 20) and (s34[1] = '#') then begin
          if CompareText(s34, '#IF') = 0 then begin
           ...
           if CompareText(s34, '#OR') = 0 then begin
            if (SayingProcedure.ConditionList.Count > 0) or (SayingProcedure.sSayMsg <> '') then begin //0048BE53
              New(SayingProcedure);
              SayingRecord.ProcedureList.Add(SayingProcedure);
              SayingProcedure.ConditionList := TList.Create;
              ...
              SayingProcedure.ElseActionList := TList.Create;
              SayingProcedure.sElseSayMsg := '';
              SayingProcedure.boLabel := True;  //支持#OR
            end;                        //0048BECE
            n6C := 11;
          end;                         //0048BED5
...
begin
  ...
  try
    for i := 0 to UserEngine.m_MerchantList.Count - 1 do begin
      ...
      if Merchant.m_boIsHide then Continue; //By Blue
      Merchant.m_nFlag := -1;
    end;
    LoadList := TStringList.Create;
    LoadList.LoadFromFile(sFileName);
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if boNewNpc and not Merchant.m_boIsHide then begin //By Blue
...
Except 
  MainOutMessageAPI('[Exception] UnLocalDB.ScriptCodeString');
End; 
end;
...
Except 
  MainOutMessageAPI('[Exception] UnLocalDB.DeCodeString');
End; 
end;
...
Try
  ...
  for I := 0 to StringList.Count - 1 do begin
    ...
    if I=0 then begin
      ...
      if CompareStr(sLine,'ENCYPTSCRIPTFLAG')=0 then begin
        KeyClass:=1;
      end else break;
    end else
    if I=1 then begin
...
Except
  MainOutMessageAPI('[Exception] TFrmDB.DeCodeStringList');
End;
end;
...
begin
  Result := 0;
  if not NPC.m_boYBDeal then Exit;
...
begin
  ...
  if FileHandle > 0 then begin
    ...
    for i := 0 to NPC.m_PostSellList.Count - 1 do begin
...
begin
  ...
  if FileHandle > 0 then begin
    ...
    for i := 0 to NPC.m_PostGoldList.Count - 1 do begin
...
begin
  ...
  if FileExists(sFileName) then begin
    FileHandle := FileOpen(sFileName, fmOpenRead or fmShareDenyNone);
    if FileHandle > 0 then begin
      if FileRead(FileHandle, Count, SizeOf(Integer)) = SizeOf(Integer) then begin
        List := nil;
        for i := 0 to Count - 1 do begin
          New(SellOff);
          if FileRead(FileHandle, SellOff^, SizeOf(TSellOff)) = SizeOf(TSellOff) then begin
            if List = nil then begin
              List := TList.Create;
              List.Add(SellOff)
            end
            else begin
              if pSellOff(List.Items[0])^.item.wIndex = SellOff.item.wIndex then begin
                List.Add(SellOff);
              end
              else begin
...
begin
  ...
  if NPC.m_SellOffList = nil then Exit;
  if FileExists(sFileName) then
    FileHandle := FileOpen(sFileName, fmOpenWrite or fmShareDenyNone)
  else
    FileHandle := FileCreate(sFileName);
  if FileHandle > 0 then begin
    ...
    for i := 0 to NPC.m_SellOffList.Count - 1 do begin
...
begin
  ...
  if FileExists(sFileName) then begin
    FileHandle := FileOpen(sFileName, fmOpenRead or fmShareDenyNone);
    if FileHandle > 0 then begin
      if FileRead(FileHandle, Count, 4) = 4 then begin
        for i := 0 to Count - 1 do begin
          New(SellOff);
          if FileRead(FileHandle, SellOff^, SizeOf(TSellOff)) = SizeOf(TSellOff) then begin
