...
function TPlayMonster.AttackTarget(): Boolean;
  ...
  begin
    ...
    if g_Config.boHeroNeedAmulet then begin
      if CheckAmulet(TPlayObject(self), 1, 1, nAmuletIdx) then begin
        if UserEngine.GetStdItem(m_UseItems[nAmuletIdx].wIndex) <> nil then begin
          if m_SetUseMagic = t_Norm then begin
  ...
  var
    i, nAmuletIdx           : Integer;
    UserMagic               : pTUserMagic;
    boReadMagic, boNoPoison : Boolean;
    StdItem                 : pTStdItem;
    sNItemName              : string;
  label
    sLabel;
  begin
    ...
    if (m_TargetCret = nil) then begin
      if m_Master <> nil then begin
        if (m_Master.m_wStatusTimeArr[POISON_DECHEALTH] <> 0) or
          ...
          (m_Master.m_wStatusTimeArr[POISON_STONE] <> 0) then begin
    ...
    if (m_SetUseMagic = t_Norm) and (m_Master <> nil) then begin
    ...
    if (m_SetUseMagic = t_Norm) then begin
      if g_Config.boHeroNeedAmulet then begin
        if CheckAmulet(TPlayObject(self), 1, 2, nAmuletIdx) then begin
          ...
          if (m_TargetCret <> nil) and (StdItem <> nil) then begin
            if (m_TargetCret.m_wStatusTimeArr[POISON_DECHEALTH] <= 0) and (StdItem.Shape = 1) then begin
              m_SetUseMagic := t_Poison_0;
            end;
            if (m_SetUseMagic = t_Norm) and (m_TargetCret.m_wStatusTimeArr[POISON_DAMAGEARMOR] <= 0) and (StdItem.Shape = 2) then begin
        ...
        if (m_TargetCret <> nil) then begin
          ...
          if (m_SetUseMagic = t_Norm) and (m_TargetCret.m_wStatusTimeArr[POISON_DAMAGEARMOR] <= 0) then begin
            m_btUseAmulet := 2;
            m_SetUseMagic := t_Poison_1;
          end;
        end;
  ...
  var
    ...
    btDir                   : byte;
    nHitMode                : Integer;
    i, nX, nY, nRange, nMapRangeCount: Integer;
    Envir                   : TEnvirnoment;
  ...
  begin
    ...
    m_nCurMagId := 25;
    if GetLongAttackDir(m_TargetCret, btDir) and (m_nLongAttackCount <= _MAX(1, Random(5))) then begin
      ...
      if (Random(18) <= 1) then begin
        if (m_TargetCret.m_PEnvir = m_PEnvir) then
          SetTargetXY(m_TargetCret.m_nCurrX + ((Random(3) - 1) * 2), m_TargetCret.m_nCurrY + ((Random(3) - 1) * 2))
        else
          DelTargetCreat();
      end else if GetAttackDir(m_TargetCret, btDir) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          m_dwHitTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();
          nHitMode := 0;
          if g_Config.boHeroDoMotaebo then begin
            if (GetTickCount - m_dwDoMotaeboTick) > 6 * 1000 then begin
              if (m_WAbil.Level > m_TargetCret.m_WAbil.Level) and not m_TargetCret.m_boStickMode then begin
          ...
          if Integer(GetTickCount - m_dwMag41Tick) > 10 * 1000 then begin
            m_dwMag41Tick := GetTickCount();
            if GetTagXYRangeCount(1, 3) > 2 then begin
              m_nCurMagId := 41;
          ...
          if (GetTickCount - m_dwLatestFireHitTick) > g_Config.nHeroFireSwordTime then begin
        ...
        if ((abs(m_nCurrX - m_TargetCret.m_nCurrX) > 3) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 3)) then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            ...
            if nMapRangeCount > 3 then begin
              if not (m_TargetCret.m_btRaceServer in [RC_PLAYOBJECT, RC_HERO]) and (m_TargetCret.m_Abil.Level + 8 < m_Abil.Level) then begin
                m_nCurMagId := 71;
                goto lNext;
              end;
            end;
  ...
  var
    ...
    nMapRangeCount, nNX, nNY: Integer;
    ...
    if m_TargetCret <> nil then begin
      ...
      if (nAbsX > 2) or (nAbsY > 2) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          ...
          if m_MagicArr[0][10] <> nil then begin
            ...
            if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, btDir, 1, nNX, nNY) then begin
              ...
              if CheckMagPassThrough(nNX, nNY, nTX, nTY, btDir) >= 3 then begin
                m_nCurMagId := 10;
                goto LabelDoSpell;
              end;
            end;
          ...
          if GetTagXYRangeCount(0, 1) > 1 then begin
            if Random(10) > 4 then
              m_nCurMagId := 33
            else
              m_nCurMagId := 47;
        ...
        if (nAbsX <= 1) and (nAbsY <= 1) and (Random(3) = 0) then begin
          if (m_TargetCret.m_Abil.Level < m_Abil.Level) and (Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime) then begin
            m_dwHitTick := GetTickCount();
            m_dwTargetFocusTick := GetTickCount();
            if Random(3) = 0 then
              m_nCurMagId := 24
        ...
        end else if (Random(8) > 5) and (Round(m_Abil.HP / m_Abil.MaxHP * 100) > 30) then begin
          if (Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime) then begin
            m_dwHitTick := GetTickCount();
            m_dwTargetFocusTick := GetTickCount();
            m_nCurMagId := 24;
  ...
  begin
    borunaway := False;
    case m_SetUseMagic of
      t_AddHP_M, t_Defence_m, t_Defence_m_m, t_UNAMYOUNSUL: begin
          if m_Master <> nil then begin
      ...
      t_UNAMYOUNSULSELF, t_AddHP_S, t_Defence_s, t_Defence_s_m, t_MobSlave, t_SCDouble: begin
          nCurrX := m_nCurrX;
          nCurrY := m_nCurrY;
          TargetCret := self;
          borunaway := (m_TargetCret <> nil);
        end;
      t_Attack, t_Poison_0, t_Poison_1: begin
          if m_TargetCret <> nil then begin
      ...
      if (abs(m_nCurrX - m_TargetCret.m_nCurrX) > 2) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 2) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          ...
          if m_SetUseMagic = t_Poison_0 then
            ...
            else ;                      //m_btUseAmulet := 2;
          if m_SetUseMagic = t_Poison_1 then
            if g_Config.boHeroNeedAmulet then
              TakeOnItem(2, '灰色药粉(大量)')
            else ;                      //m_btUseAmulet := 1;
          BreakHolySeizeMode();
        ...
        if (m_TargetCret.m_Abil.Level < m_Abil.Level) and ((abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 1) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 1)) then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            m_dwHitTick := GetTickCount();
            m_dwTargetFocusTick := GetTickCount();
            m_nCurMagId := 48;
  ...
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if not m_boWalkWaitLocked and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
      ...
      if m_Master <> nil then begin
        ...
        if (not m_Master.m_boSlaveRelax) and ((m_PEnvir <> m_Master.m_PEnvir) or (abs(m_nCurrX - m_Master.m_nCurrX) > 20) or (abs(m_nCurrY - m_Master.m_nCurrY) > 20)) then begin
          if m_Master.m_PEnvir <> nil then SpaceMove(m_Master.m_PEnvir.m_sMapFileName, m_nTargetX, m_nTargetY, 1);
        end;
      end;
      ...
      if m_nTargetX <> -1 then begin
        if (abs(m_nCurrX - m_nTargetX) > 1) or (abs(m_nCurrY - m_nTargetY) > 1) then begin
          ...
          GotoTargetXY();
      end else if m_TargetCret = nil then
        Wondering();
    end;
  end;
  //SearchTarget_Ex();
...
function THeroMonster.AttackTarget(): Boolean;
  ...
  begin
    ...
    m_SetUseMagic := t_Norm;
    //if g_Config.nRegCheckCode < 0 then Exit;
    if g_Config.boAllowBodyMakeSlave and (m_Master <> nil) {and (TPlayObject(m_Master).m_btHeroMakeSlave <> 0)} then begin
      if g_Config.boHeroNeedAmulet then begin
        if CheckAmulet(TPlayObject(self), 1, 1, nAmuletIdx) then begin
          if UserEngine.GetStdItem(m_UseItems[nAmuletIdx].wIndex) <> nil then begin
            if m_SetUseMagic = t_Norm then begin
  ...
  var
    boTrainOk               : Boolean;
    btDir                   : byte;
    i, nX, nY, nRange, nHitMode, nMapRangeCount: Integer;
  label
  ...
  begin
    ...
    if (m_MagicArr[0][12] <> nil) and GetLongAttackDir(m_TargetCret, btDir) and (m_nLongAttackCount <= _MAX(1, Random(5))) then begin
      if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
        Inc(m_nLongAttackCount);
        m_dwHitTick := GetTickCount();
        m_dwTargetFocusTick := GetTickCount();
    ...
    m_nLongAttackCount := 0;
    if m_boLongAttack or (Random(12) <= 1) then begin
      if (m_TargetCret.m_PEnvir = m_PEnvir) then
        SetTargetXY(m_TargetCret.m_nCurrX + ((Random(3) - 1) * 2), m_TargetCret.m_nCurrY + ((Random(3) - 1) * 2))
      else
        DelTargetCreat();
      ...
      if GetAttackDir(m_TargetCret, btDir) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          ...
          UserMagic := MonGetMagicInfo();
          if (UserMagic <> nil) and (m_WAbil.Level > m_TargetCret.m_WAbil.Level) and not m_TargetCret.m_boStickMode then begin
          ...
          if Integer(GetTickCount - m_dwMag41Tick) > (8 * 1000 + Random(8 * 1000)) then begin
            m_dwMag41Tick := GetTickCount();
            m_nCurMagId := 41;
            UserMagic := MonGetMagicInfo();
            if (UserMagic <> nil) and (GetTagXYRangeCount(1, 3) > 2) then begin
              Result := TPlayObject(self).DoSpell(UserMagic, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY, m_TargetCret, boTrainOk);
              goto LabelExit;
            end;
          end;

          if (m_MagicArr[0][SKILL_YEDO] {m_MagicPowerHitSkill} <> nil) and (m_UseItems[U_WEAPON].Dura > 0) and (Random(9) = 0) then begin
            nHitMode := 3;
            m_boPowerHit := True;
            goto LabelStartAttack;
          end;

          if (m_MagicArr[0][SKILL_BANWOL] {m_MagicBanwolSkill} <> nil) and (m_Abil.MP >= 3) and TargetInSwordWideAttackRange then begin
            nHitMode := 5;
            goto LabelStartAttack;
          end;

          if (m_MagicArr[0][SKILL_ERGUM] {m_MagicErgumSkill} <> nil) and TargetInSwordLongAttackRange then begin
            nHitMode := 4;
            m_boUseThrusting := True;
            goto LabelStartAttack;
          end;

          if (m_MagicArr[0][SKILL_FIRESWORD] {m_MagicFireSwordSkill} <> nil) and ((GetTickCount - m_dwLatestFireHitTick) > g_Config.nHeroFireSwordTime) then begin
            m_dwLatestFireHitTick := GetTickCount();
            nHitMode := 7;
            m_boFireHitSkill := True;
            goto LabelStartAttack;
  ...
  begin
    if m_TargetCret <> nil then begin
      ...
      if (nAbsX > 2) or (nAbsY > 2) then begin
        ...
        end else if (Random(3) = 0) and (Round(m_Abil.HP / m_Abil.MaxHP * 100) > 30) then begin
          if (Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime) then begin
            m_dwHitTick := GetTickCount();
            m_dwTargetFocusTick := GetTickCount();
            m_nCurMagId := 24;
  ...
  begin
    ...
    if not borunaway then begin
      ...
      if (abs(m_nCurrX - m_TargetCret.m_nCurrX) > 2) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 2) then begin
        ...
        if ((abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 1) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 1)) and (m_TargetCret.m_Abil.Level < m_Abil.Level) then begin
          if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
            m_dwHitTick := GetTickCount();
            m_dwTargetFocusTick := GetTickCount();
            m_nCurMagId := 48;
          ...
          if Random(10) = 0 then
            ...
            GetBackPositionEx(nX, nY);
          if (m_TargetCret.m_PEnvir = m_PEnvir) and ((abs(nX - m_Master.m_nCurrX) < 10) and (abs(nY - m_Master.m_nCurrY) < 10)) then begin
            SetTargetXY(nX, nY);
          end else
            DelTargetCreat();
        end;
...
begin
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if not m_boWalkWaitLocked and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
      ...
      if (m_Master <> nil) and not m_Master.m_boSlaveRelax then begin
        if AttackTarget then begin
          inherited;
          Exit;
        end;
...
begin
  ...
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if (m_btRaceServer <> RC_MISSION) then begin
      if Think then begin
        inherited Run;
        Exit;
      end;
    ...
    if not m_boWalkWaitLocked and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
      ...
      if (m_Master <> nil) and (m_Master.m_btRaceServer = RC_HERO) then begin
        if (m_Master.m_Master <> nil) and (m_Master.m_Master.m_boSlaveRelax) then begin
          inherited;
          Exit;
        end;
      end;
      if (m_nTargetX <> -1) and ((m_boApproach) or (m_Master <> nil)) then begin //blue
...
var
  i                         : Integer;
begin
  if not m_boDeath and not m_boGhost and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
...
begin
  if not m_boDeath and not m_boGhost and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    if Integer(GetTickCount - m_dwWalkTick) >= m_nWalkSpeed then begin
      m_nTargetX := -1;
      if m_boMission then begin
        if (m_nCurrX = m_nMissionX) and (m_nCurrY = m_nMissionY) then begin
          MakeGhost;
          inherited;
          Exit;
        end;
        if m_WayPoint <> nil then begin
          if m_nMoveStep <= High(m_WayPoint) then begin
...
var
  i, nC, nOX, nOY, nNX, nNY, n20, n24: Integer;
  Envir                     : TEnvirnoment;
  boTagOK                   : Boolean;
resourcestring
...
begin
  ...
  try
    ...
    nC := 1;
    if (nNX >= 0) and ((m_PEnvir.m_MapHeader.wWidth - 1) >= nNX) and (nNY >= 0) and ((m_PEnvir.m_MapHeader.wHeight - 1) >= nNY) then begin
      boTagOK := True;
      nC := 2;
      ...
      nC := 4;
      if boTagOK then begin
        if m_PEnvir.MoveToMovingObject(m_nCurrX, m_nCurrY, self, nNX, nNY, True) > 0 then begin
          m_nCurrX := nNX;
          m_nCurrY := nNY;
        end;
      end;
    ...
    if (m_nCurrX <> nOX) or (m_nCurrY <> nOY) then begin
      ...
      if Walk(RM_WALK) then begin
        ...
        nC := 8;
        if m_PEnvir.DeleteFromMap(m_nCurrX, m_nCurrY, OS_MOVINGOBJECT, self) = 1 then begin //0628
          m_nCurrX := nOX;
          m_nCurrY := nOY;
...
var
  btDir                     : byte;
  WAbil                     : pTAbility;
  n, m, l, nLevel, nDamage  : Integer;
  nPower, nSecPwr           : Integer;
begin
  Result := False;
  if (m_TargetCret = nil) or (m_TargetCret.m_boDeath) or (m_TargetCret.m_boGhost) then
    Exit;

  if TargetInSpitRange(m_TargetCret, btDir) or GetLongAttackDir_Dis(m_TargetCret, btDir, 3 + 3) then begin
    if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
      m_dwHitTick := GetTickCount();
      m_dwTargetFocusTick := GetTickCount();
      SpitAttack2(btDir);
...
begin
  Result := False;
  if m_TargetCret <> nil then begin
    if m_wAppr = 3711 then begin
      if GetAttackDir(m_TargetCret, bt06) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          m_dwHitTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();

          if (Random(4) = 0) or ((Random(2) = 0) and ((m_WAbil.HP / m_WAbil.MaxHP * 100) < 40)) then begin
    ...
    end else if m_wAppr = 3712 then begin
      if ((m_WAbil.HP / m_WAbil.MaxHP * 100) < 65) and (Random(3) = 0) and GetSUQAttack() then begin
        WAbil := @m_WAbil;
        ...
        Result := True;
      end else if GetSelfRangeCount(3) > 0 then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          m_dwHitTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();

          if (Random(5) = 0) or ((Random(3) = 0) and ((m_WAbil.HP / m_WAbil.MaxHP * 100) < 75)) then begin
            WAbil := @m_WAbil;
            nPower := (Random(Abs(HiWord(WAbil.DC) - LoWord(WAbil.DC)) + 1) + LoWord(WAbil.DC));
    ...
    end else if m_wAppr = 3713 then begin
      if (GetTickCount - m_dwSpellTick > (10 + Random(10)) * 1000) and (GetSelfRangeCount(6) > 0) then begin
        m_dwSpellTick := GetTickCount;
        WAbil := @m_WAbil;
        ...
        BreakHolySeizeMode();
        Result := True;
      end else if (Random(2) = 0) and (TargetInSpitRange(m_TargetCret, bt06) or GetLongAttackDir_Dis(m_TargetCret, bt06, 4)) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          m_dwHitTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();
          SpitAttack2(bt06);
        ...
        Result := True;
      end else if GetAttackDir(m_TargetCret, bt06) then begin
        if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
          m_dwHitTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();
...
begin
  ...
  if (not targ.m_boDeath) and IsProperTarget(targ) then begin
...
begin
  Result := False;
  if GetCurrentTime - m_dwHitTick > m_nNextHitTime then begin
  ...
  if IsMoveAble then begin
    if GetTickCount - m_dwSearchEnemyTick > 5000 then begin
      ...
      for i := 0 to m_VisibleActors.Count - 1 do begin
        ...
        if (not cret.m_boDeath) and (IsProperTarget(cret)) and (not cret.m_boHideMode or m_boFixedHideMode) then begin
    ...
    if (m_WAbil.HP / m_WAbil.MaxHP * 100) < 80 then begin
      if GetTickCount - m_dwSpellTick > 2200 then begin
        m_dwSpellTick := GetTickCount;
        WAbil := @m_WAbil;
    ...
    if GetCurrentTime - m_dwWalkTick > m_nWalkSpeed then begin
      if m_TargetCret <> nil then
        ...
        end else if (abs(m_nCurrX - m_TargetCret.m_nCurrX) > 5) or (abs(m_nCurrY - m_TargetCret.m_nCurrY) > 5) then begin
...
var
  n8, nC, n10               : Integer;
begin
  if (not m_boDeath) and (not m_boGhost) and ((GetTickCount - m_dwRunTickMon) > 30 * 1000) then begin
    m_dwRunTickMon := GetTickCount();
    if (m_TargetCret <> nil) then begin
    ...
    if (m_nDangerLevel >= 2) and (m_nDangerLevel <> n10) then begin
      m_boDanger := True;
      m_dwDangerTick := GetTickCount();
    end;
    if m_boDanger then begin
    ...
    if m_boCrazy then begin
      if (GetTickCount - m_dwCrazyTick) < 8000 then begin
        m_nNextHitTime := 500;
        m_nWalkSpeed := 400;
      end else begin
        m_boCrazy := False;
...
begin
  ...
  nPower := GetAttackPower(LoWord(WAbil.DC), SmallInt(HiWord(WAbil.DC) - LoWord(WAbil.DC)));
  if m_boCrazy then begin
    if m_nDangerLevel >= 60 then begin
    ...
    end else if m_nDangerLevel >= 30 then begin
...
var
  n8, nC, n10               : Integer;
begin
  if (not m_boDeath) and (not m_boGhost) and ((GetTickCount - m_dwRunTickMon) > 5 * 1000) then begin
    m_dwRunTickMon := GetTickCount();
    n10 := m_nDangerLevel;
    m_nDangerLevel := 100 - m_WAbil.HP div _MAX(1, (m_WAbil.MaxHP div 100));
    if (m_nDangerLevel <> n10) then begin
      if (m_nDangerLevel >= 60) then begin
        m_boDanger := True;
        m_dwDangerTick := GetTickCount();
      end else if (m_nDangerLevel >= 30) then begin
    ...
    if m_boDanger then begin
      if (GetTickCount - m_dwDangerTick) > 5 * 1000 then begin
        m_boDanger := False;
        m_boCrazy := True;
        m_dwCrazyTick := GetTickCount();
      end;
    ...
    if m_boCrazy then begin
      if (GetTickCount - m_dwCrazyTick) < 10 * 1000 then begin
        if m_nDangerLevel >= 60 then begin
          m_nNextHitTime := 500;
          m_nWalkSpeed := 400;
        end else if m_nDangerLevel >= 30 then begin
...
begin
  if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) and ((GetTickCount - m_dwSearchEnemyTick) > 8000) then begin
...
var
  n08, nAttackDir           : Integer;
begin
  if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) and ((GetTickCount - m_dwSearchEnemyTick) > 2000) then begin
    if ((GetTickCount - m_dwSearchEnemyTick) > 1000) and (m_TargetCret = nil) then begin
      m_dwSearchEnemyTick := GetTickCount();
      SearchTarget();
    end;
    if (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) and (m_TargetCret <> nil) and
      ...
      if (abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 2) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 2) {and (Random(3) <> 0)} then begin
        inherited;
        Exit;
      end;
      //GetBackPosition(m_nTargetX, m_nTargetY);
...
begin
  if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) and ((GetTickCount - m_dwSearchEnemyTick) > 2000) then begin
    ...
    if (m_TargetCret <> nil) and
      ...
      m_dwHitTick := GetTickCount();
      nAttackDir := GetNextDirection(m_nCurrX, m_nCurrY, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
      if ((m_WAbil.HP / m_WAbil.MaxHP * 100) < 45) and (Random(3) = 0) or (Random(15) = 0) then begin
        LightingAttack(nAttackDir);
      end else
...
begin
  ...
  for i := 0 to BaseObjectList.Count - 1 do begin
    ...
    if IsProperTarget(BaseObject) then begin
      ...
      if (not BaseObject.m_boUnParalysis) and (BaseObject.m_wStatusTimeArr[POISON_STONE] = 0) then begin
...
begin
  if not m_boGhost and not m_boDeath and not m_boFixedHideMode and not m_boStoneMode and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    if (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) and
      ...
      (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 2) then begin
      n08 := Round(m_WAbil.HP * 7 / m_WAbil.MaxHP) + 1;
      if Random(n08) <= 3 then CharPushAttack();
      inherited;
      Exit;
    end;
    ...
    if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
      ...
      (abs(m_nCurrX - m_TargetCret.m_nCurrX) < 12) and
        (abs(m_nCurrY - m_TargetCret.m_nCurrY) < 12) then begin
        n08 := Round(m_WAbil.HP * 40 / m_WAbil.MaxHP) + 1;
        n08 := Random(n08) + 15;
        if (GetTickCount - m_dwGroupParalysisTick > (n08 * 1000)) or (Random(15) = 0) then begin
          m_dwGroupParalysisTick := GetTickCount();
          nAttackDir := GetNextDirection(m_nCurrX, m_nCurrY, m_TargetCret.m_nCurrX, m_TargetCret.m_nCurrY);
...
var
  ...
  BaseObject                : TBaseObject;
begin
  if (not m_boGhost) and (not m_boDeath) and (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) and (Integer(GetTickCount - m_dwWalkTick) > m_nWalkSpeed) then begin
  ...
  if m_TargetCret <> nil then begin
    if GetAttackDir(m_TargetCret, btDir) then begin
      if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
        ...
        if m_btRaceImg in [91..93] then begin
          if Random(99 - m_btRaceImg) = 0 then begin
            m_boPowerHit := True;
...
var
  ldc, hdc                  : Integer;
begin
  if (g_Config.nBoneFammDcEx > 0) and (m_Master <> nil) then begin
...
begin
  ...
  if m_TargetCret = nil then
    Exit;
  if ((m_wAppr <> 704) and (m_wAppr <> 706) and (m_wAppr <> 708)) then begin
    Result := inherited AttackTarget;
    Exit;
  end;
  if TargetInSpitRange(m_TargetCret, btDir) or GetLongAttackDir_Dis(m_TargetCret, btDir, 3) then begin
    if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
      m_dwHitTick := GetTickCount();
      m_dwTargetFocusTick := GetTickCount();
      SpitAttack2(btDir);
...
var
  ...
  nX, nY                    : Integer;
begin
  if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
...
begin
  for i := 0 to m_VisibleActors.Count - 1 do begin
    ...
    if (BaseObject = nil) or BaseObject.m_boDeath then
      Continue;
    if IsProperTarget(BaseObject) then begin
      if (abs(m_nCurrX - BaseObject.m_nCurrX) <= m_nViewRangeX) and (abs(m_nCurrY - BaseObject.m_nCurrY) <= m_nViewRangeY) then begin
        nDamage := 0;
        ...
        if nDamage > 0 then begin
          ...
          if Random(5) - 1 < 0 then begin
            rX := Random(14);
            if rX in [7..10] then rX := 11;
            rY := Random(14);
            if rY in [4..7] then rY := 3;
            nX := (m_nCurrX - 7) + rX;
            nY := (m_nCurrY - 7) + rY;
            if m_PEnvir.CanWalk(nX, nY, False) then
              BaseObject.SpaceMove(m_sMapName, nX, nY, 0);
          end else if Random(6) - 1 < 0 then begin
...
begin
  ...
  while (True) do begin
    ...
    if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, n10, 1, nX, nY) then begin
      BaseObject := m_PEnvir.GetMovingObject(nX, nY, True);
      if (nHitPower > 0) and (BaseObject <> nil) and IsProperTarget(BaseObject) then begin
...
var
  ...
  BaseObject                : TBaseObject;
begin
  if (not m_boGhost) and (not m_boDeath) and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) and (Integer(GetTickCount - m_dwWalkTick) >= m_nWalkSpeed) then begin
    if ((GetTickCount - m_dwSearchEnemyTick) > 5000) or (((GetTickCount - m_dwSearchEnemyTick) > 1000) and (m_TargetCret = nil)) then begin
      m_dwSearchEnemyTick := GetTickCount();
      SearchTarget();
      if (m_nDangerLevel > m_WAbil.HP / m_WAbil.MaxHP * 5) and (m_nDangerLevel > 0) then begin
        Dec(m_nDangerLevel);
...
begin
  case BaseObject.m_btRaceServer of
    207: begin
        for i := 0 to 5 - 1 do begin    //三个火
          BaseObject.m_PEnvir.GetNextPosition(nX, nY, m_btDirection, i, nRecallX, nRecallY);
          if BaseObject.m_PEnvir.GetEvent(nRecallX, nRecallY) = nil then begin
            FireBurnEvent := TFireBurnEvent.Create(BaseObject, nRecallX, nRecallY, ETTPYE, 5 * 1000, nDamage, 0);
            g_EventManager.AddEvent(FireBurnEvent);
          end;
        end;
      end;
    121, 209: for i := nX - 3 to nX + 3 do begin
        for ii := nY - 3 to nY + 3 do begin
          if (i = nX) and (ii = nY) then Continue;
          if m_PEnvir.GetEvent(i, ii) = nil then begin
...
begin
  ...
  if n10 > 0 then begin
    SendRefMsg(RM_HIT, m_btDirection, m_nCurrX, m_nCurrY, 0, '');
    BaseObject := GetPoseCreate();
    if (BaseObject <> nil) and IsProperTarget(BaseObject) and (Random(BaseObject.m_btSpeedPoint) < m_btHitPoint) then begin
      n10 := BaseObject.GetMagStruckDamage(self, n10);
      if n10 > 0 then begin
        BaseObject.StruckDamage(self, n10, 0);
        BaseObject.SendDelayMsg(TBaseObject(RM_STRUCK),
...
begin
  inherited;
  if (g_Config.nAngelDcEx > 0) and (m_Master <> nil) then begin
...
begin
  ...
  if (nLevel > 0) and ((Random(5) = 0) or (Random(nLevel) >= (Random(120)))) then begin
    btEffect := 101;
    nDamage := nDamage * 2;
  end;
  if nDamage > 0 then begin
...
begin
  ...
  nRage := 9999;
  if not m_boDeath and not m_boGhost and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    if (GetTickCount - m_dwSearchEnemyTick) >= 2500 then begin
      m_dwSearchEnemyTick := GetTickCount();
...
begin
  ...
  if m_wAppr = 250 then begin
    if GetAttackDir(m_TargetCret, btDir) then begin
      ...
      Result := True;
    end else if GetLongAttackDirX(m_TargetCret, btDir) then begin
      if Integer(GetTickCount - m_dwSpellTick2) > Round(m_nNextHitTime * 1.5) then begin
  ...
  if m_wAppr = 255 then begin
    if GetAttackDir(m_TargetCret, btDir) then begin
      if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
        ...
        if (not m_TargetCret.m_boUnParalysis) and (m_TargetCret.m_wStatusTimeArr[POISON_STONE] = 0) then begin
          if (m_WAbil.HP / m_WAbil.MaxHP * 100) < 50 then begin
            if GetTickCount - m_dwSpellTick2 > ((10 + Random(6)) * 1000) then begin
              m_dwSpellTick2 := GetTickCount;
  ...
  if (m_wAppr = 256) or (m_wAppr = 268) or (m_wAppr = 267) then begin
    case m_wAppr of
      267: begin
          m := 4;
    ...
    if (n > m) and (Random(l) = 0) {GetLongAttackDirX(m_TargetCret, btDir)} then begin
      if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
        m_dwHitTick := GetTickCount();
        m_btDirection := btDir;
        m_dwTargetFocusTick := GetTickCount();
    ...
    if IsProperTarget(m_TargetCret) then begin
      if m_TargetCret.m_PEnvir = m_PEnvir then begin
        ...
        if (abs(m_nCurrX - m_TargetCret.m_nCurrX) <= 8) and (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 8) then begin
          ...
          case m_wAppr of
            254: begin
                if (m_WAbil.HP / m_WAbil.MaxHP * 100) < 70 then begin
                  if GetTickCount - m_dwSpellTick > 3200 then begin
                    m_dwSpellTick := GetTickCount;
                    WAbil := @m_WAbil;
                    nDamage := (Random(Abs(HiWord(WAbil.DC) - LoWord(WAbil.DC)) + 1) + LoWord(WAbil.DC)) * 2;
                    SendDelayMsg(self, RM_MAGHEALING, 0, nDamage, 0, 0, '', 800);
...
begin
  ...
  case m_wAppr of
    252: begin
        ...
        nDamage := (Random(Abs(HiWord(WAbil.DC) - LoWord(WAbil.DC)) + 1) + LoWord(WAbil.DC));
        btEffect := 4;
        if (Target.m_wStatusTimeArr[POISON_DECHEALTH] = 0) and (Random(Target.m_btAntiPoison + 2) = 0) then begin
    ...
    253: begin
        ...
        nDamage := (Random(Abs(HiWord(WAbil.DC) - LoWord(WAbil.DC)) + 1) + LoWord(WAbil.DC));
        btEffect := 4;
        if (Target.m_wStatusTimeArr[POISON_DAMAGEARMOR] = 0) and (Random(Target.m_btAntiPoison + 2) = 0) then begin
...
begin
  if (not m_boDeath) and
    (not m_boGhost) and
    (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
...
begin
  if (not m_boDeath) and (not m_boGhost) and (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    if m_TargetCret <> nil then begin
      if (Integer(GetTickCount - m_dwSpellTick) > 2800) then begin
        m_dwSpellTick := GetTickCount();
        m_dwHitTick := GetTickCount();
        MagicAttack();
      end;
...
begin
  if (not m_boGhost) and (not m_boDeath) then begin
    if GetCurrentTime - m_dwWalkTick > 5000 then begin
      ...
      if Random(2) = 0 then begin
        SendRefMsg(RM_TURN, 0, m_nCurrX, m_nCurrY, GetFeatureToLong, '');
      end;
    end;
  end;
  inherited;
...
begin
  if not m_boDeath and not m_boGhost and (m_wStatusTimeArr[POISON_STONE {5 0x6A}] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    if (m_TargetCret <> nil) then begin
      if TargetInSpitRange(m_TargetCret, btDir) or GetLongAttackDir_Dis(m_TargetCret, btDir, 3) then begin
        if ((GetTickCount - m_dwSpellTick) > 3000 + Random(8 * 1000)) then begin
          m_dwHitTick := GetTickCount();
          m_dwSpellTick := GetTickCount();
          m_dwTargetFocusTick := GetTickCount();
          m_btDirection := btDir;
...
var
  WAbil                     : pTAbility;
  nC, n10, n14, n18, n1C, n20: Integer;
  nX, nY                    : Integer;
  BaseObject                : TBaseObject;
begin
  if m_TargetCret = nil then
...
var
  ...
  WAbil                     : pTAbility;
begin
  if IsMoveAble and (m_TargetCret <> nil) then begin
...
begin
  ...
  if m_TargetCret <> nil then begin
    ...
    if GetAttackDir(m_TargetCret, btDir) then begin
      if Integer(GetTickCount - m_dwHitTick) > m_nNextHitTime then begin
        ...
        if (not m_TargetCret.m_boUnParalysis) and (Random(5) = 0) then begin
  ...
  if (m_TargetCret <> nil) and (GetTickCount - m_dwSpellTick > (18 * 1000 + Random(5) * 1000)) and IsProperTarget(m_TargetCret) and
    ...
    (abs(m_nCurrY - m_TargetCret.m_nCurrY) <= 8) then begin
    m_dwSpellTick := GetTickCount();
