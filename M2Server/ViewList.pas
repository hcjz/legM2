...
uses
  ...
  Spin, Grobal2, Buttons, Menus, TLHelp32, IniFiles;
...
type
  TfrmViewList = class(TForm)
    ...
    TabSheet1: TTabSheet;
    ListViewSaleItem: TListView;
    GroupBox22: TGroupBox;
    ListBoxSaleItemList: TListBox;
    EditSaleItemName: TEdit;
    Label28: TLabel;
    Label30: TLabel;
    EditSaleItemPrice: TSpinEdit;
    MemoItemDes: TEdit;
    Label31: TLabel;
    btnSaleItemAdd: TButton;
    btnSaleItemDel: TButton;
    btnSaleItemSave: TButton;
    btnReloadSaleItemList: TButton;
    Label23: TLabel;
    ...
    Label38: TLabel;
    SpinEditShopItemType: TSpinEdit;
    ...
    procedure ListViewSaleItemClick(Sender: TObject);
    procedure btnSaleItemAddClick(Sender: TObject);
    procedure btnSaleItemDelClick(Sender: TObject);
    procedure btnSaleItemSaveClick(Sender: TObject);
    procedure btnReloadSaleItemListClick(Sender: TObject);
    procedure ListBoxSaleItemListClick(Sender: TObject);
...
uses M2Share, UsrEngn, Envir, HUtil32, LocalDB, svMain;
...
begin
  ...
  try
    ...
    ListBoxSaleItemList.Items.Text := ListBoxServerItemList.Items.Text;
...
var
  i                         : Integer;
  ListItem                  : TListItem;
  sText, s1, s2, s3         : string;
  pSaleItem                 : pTSaleItem;
begin
  EditSaleItemName.Text := '';
  EditSaleItemPrice.Value := 100;
  MemoItemDes.Text := '';
  btnSaleItemDel.Enabled := False;
  g_SaleItemList.Lock;
  try
    for i := 0 to g_SaleItemList.Count - 1 do begin
      pSaleItem := g_SaleItemList.Items[i];
      ListItem := ListViewSaleItem.Items.Add;
      ListItem.Caption := pSaleItem^.sStdItem.Name;
      ListItem.SubItems.Add(IntToStr(pSaleItem^.sStdItem.Price div 100));
      ListItem.SubItems.Add(StrPas(pSaleItem^.sItemDesc));
    end;
  finally
  ...
  if ListBoxMapList.ItemIndex >= 0 then begin
    for i := 0 to ListBoxDisableMoveMap.Items.Count - 1 do begin
      if ListBoxDisableMoveMap.Items.Strings[i] =
        ListBoxMapList.Items.Strings[ListBoxMapList.ItemIndex] then begin
...
var
  i                         : Integer;
begin
  if ListBoxLogItemList.ItemIndex >= 0 then begin
    for i := 0 to ListBoxGameLogList.Items.Count - 1 do begin
      if ListBoxGameLogList.Items.Strings[i] =
        ListBoxLogItemList.Items.Strings[ListBoxLogItemList.ItemIndex] then begin
...
begin
  ...
  for i := 0 to ListBoxLogItemList.Items.Count - 1 do begin
...
{$IFEND}
  if
    Application.MessageBox('此设置必须重新加载物品数据库才能生效，是否重新加载？', '确认信息', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    FrmDB.LoadItemsDB();
  end;
end;
...
begin
  if EditClearTime.Text = '' then begin
...
begin
  if ListBoxServerItemList.ItemIndex >= 0 then begin
    sItemName := ListBoxServerItemList.Items.Strings[ListBoxServerItemList.ItemIndex];
    for i := 0 to ListBoxLimitItemList.Items.Count - 1 do begin
      nLimitItem := pTItemLimit(ListBoxLimitItemList.Items.Objects[i]);
      if nLimitItem.sItemName = sItemName then begin
...
var
  i                         : Integer;
begin
  if SelLimitItem = nil then Exit;
  g_ItemLimitList.Lock;
  try
    for i := 0 to g_ItemLimitList.Count - 1 do begin
      if SelLimitItem = g_ItemLimitList.Items[i] then begin
...
begin
  if not boOpened then Exit;
  if Sender = ListBoxServerItemList then begin
...
begin
  ...
  try
    ...
    for i := 0 to ListBoxServerItemList.Items.Count - 1 do begin
      New(nLimitItem);
...
begin
  try
    if not BLUE_InputQuery('查找物品', '请输入您要查找的物品名字:', sInputText) then Exit;
    if sInputText = '' then begin
      Application.MessageBox('请输入物品名字', '错误信息', MB_OK + MB_ICONWARNING);
      Exit;
    end;
    for i := 0 to ListBoxServerItemList.Count - 1 do begin
      SC := ListBoxServerItemList.Items.Strings[i];
      if CompareText(SC, sInputText) = 0 then begin
...
begin
  ...
  if Sender = ButtonQVFilterAdd then
    ...
    if Sender = ButtonGuildRankNameFilterAdd then
...
begin
  ...
  if (nSelectIndex >= 0) and (nSelectIndex < ListBoxQueryValueFilter.Items.Count) then begin
  ...
  if ListBoxQueryValueFilter.ItemIndex < 0 then begin
...
begin
  if (ListBoxQueryValueFilter.ItemIndex >= 0) and
    (ListBoxQueryValueFilter.ItemIndex < ListBoxQueryValueFilter.Items.Count) then begin
...
var
  sInputText                : string;
begin
  if (ListBoxQueryValueFilter.ItemIndex >= 0) and (ListBoxQueryValueFilter.ItemIndex < ListBoxQueryValueFilter.Items.Count) then begin
...
begin
  ...
  if ListBoxGuildRankNameFilter.ItemIndex < 0 then begin
...
var
  sInputText                : string;
begin
  if (ListBoxGuildRankNameFilter.ItemIndex >= 0) and (ListBoxGuildRankNameFilter.ItemIndex < ListBoxGuildRankNameFilter.Items.Count) then begin
...
begin
  ...
  try
    ...
    for i := 0 to ListBoxGuildRankNameFilter.Items.Count - 1 do begin
...
begin
  if (ListBoxGuildRankNameFilter.ItemIndex >= 0) and
    (ListBoxGuildRankNameFilter.ItemIndex < ListBoxGuildRankNameFilter.Items.Count) then begin
...
begin
  ...
  if (nIndex < 0) and (nIndex >= g_UserCmdList.Count) then Exit;
...
begin
  ...
  try
    for i := 0 to g_UserCmdList.Count - 1 do begin
      if CompareText(g_UserCmdList.Strings[i], sUserCmd) = 0 then begin
        Application.MessageBox('输入的命令名已经在列表中', '提示信息', MB_OK + MB_ICONERROR);
        Exit;
      end;
      if Integer(g_UserCmdList.Objects[i]) = nCmdNo then begin
...
begin
  if ListBoxItemList.ItemIndex >= 0 then begin
    for i := 0 to ListBoxPetPickUpItemList.Items.Count - 1 do begin
      if ListBoxPetPickUpItemList.Items.Strings[i] =
        ListBoxItemList.Items.Strings[ListBoxItemList.ItemIndex] then begin
        Application.MessageBox('此物品已在列表中', '错误信息', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
...
begin
  if ListBoxPetPickUpItemList.ItemIndex >= 0 then begin
...
var
  i                         : Integer;
begin
  if ListBoxItemListAll.ItemIndex >= 0 then begin
    for i := 0 to ListBoxUpgradeItemList.Items.Count - 1 do begin
      if ListBoxUpgradeItemList.Items.Strings[i] =
        ListBoxItemListAll.Items.Strings[ListBoxItemListAll.ItemIndex] then begin
        Application.MessageBox('此物品已在列表中', '错误信息', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
...
var
  i, ii                     : Integer;
begin
  if ListBoxUpgradeItemList.SelCount > 0 then begin
    for ii := 0 to ListBoxUpgradeItemList.SelCount - 1 do begin
      i := 0;
      while True do begin
        if i >= ListBoxUpgradeItemList.Items.Count then Break;
...
begin
  ...
  for i := 0 to ListBoxItemListAll.Items.Count - 1 do begin
...
procedure TfrmViewList.ListViewSaleItemClick(Sender: TObject);
var
  ListItem                  : TListItem;
  nIndex                    : Integer;
begin
  nIndex := ListViewSaleItem.ItemIndex;
  if (nIndex < 0) or (nIndex >= g_SaleItemList.Count) then
    Exit;
  ListItem := ListViewSaleItem.Items.item[nIndex];
  EditSaleItemName.Text := ListItem.Caption;
  EditSaleItemPrice.Value := StrToInt(ListItem.SubItems.Strings[0]);
  MemoItemDes.Text := ListItem.SubItems.Strings[1];
  btnSaleItemDel.Enabled := True;
end;

procedure TfrmViewList.btnSaleItemAddClick(Sender: TObject);
var
  sItemName, sItemDse       : string;
  i, nItemPrice             : Integer;
  ListItem                  : TListItem;
begin
  sItemName := Trim(EditSaleItemName.Text);
  nItemPrice := EditSaleItemPrice.Value;
  sItemDse := MemoItemDes.Text;
  if sItemName = '' then begin
    Application.MessageBox('物品名称输入不正确！', '提示信息', MB_OK + MB_ICONERROR);
    EditSaleItemName.SetFocus;
    Exit;
  end;
  if nItemPrice <= 0 then begin
    Application.MessageBox('物品价格输入不正确！', '提示信息', MB_OK + MB_ICONERROR);
    EditSaleItemPrice.SetFocus;
    Exit;
  end;
  if sItemDse = '' then begin
    Application.MessageBox('请输入物品出售描述！', '提示信息', MB_OK + MB_ICONERROR);
    MemoItemDes.SetFocus;
    Exit;
  end;

  for i := 0 to ListViewSaleItem.Items.Count - 1 do begin
    ListItem := ListViewSaleItem.Items.item[i];
    if CompareText(ListItem.Caption, sItemName) = 0 then begin
  ...
  btnSaleItemSave.Enabled := True;
end;

procedure TfrmViewList.btnSaleItemDelClick(Sender: TObject);
var
  nIndex                    : Integer;
begin
  nIndex := ListViewSaleItem.ItemIndex;
  if nIndex < 0 then
    Exit;
  ListViewSaleItem.Items.Delete(nIndex);
  btnSaleItemSave.Enabled := True;
end;

procedure TfrmViewList.btnSaleItemSaveClick(Sender: TObject);
var
  i                         : Integer;
  sFileName                 : string;
  ListItem                  : TListItem;
...
begin
  ...
  for i := 0 to ListViewSaleItem.Items.Count - 1 do begin
    ListItem := ListViewSaleItem.Items.item[i];
    LoadList.Add(ListItem.Caption + #9 + ListItem.SubItems.Strings[0] + #9 + ListItem.SubItems.Strings[1]);
  end;
  LoadList.SaveToFile(sFileName);
  LoadList.Free;
  btnSaleItemSave.Enabled := False;
  btnReloadSaleItemList.Enabled := True;
end;

procedure TfrmViewList.btnReloadSaleItemListClick(Sender: TObject);
begin
  LoadUserBuyItemList();
  btnReloadSaleItemList.Enabled := False;
  Application.MessageBox('重新加载商铺物品列表完成！', '提示信息', MB_OK + MB_ICONINFORMATION);
end;

procedure TfrmViewList.ListBoxSaleItemListClick(Sender: TObject);
var
  nIndex                    : Integer;
begin
  nIndex := ListBoxSaleItemList.ItemIndex;
  if (nIndex < 0) or (nIndex >= ListBoxSaleItemList.Count) then
    Exit;
  EditSaleItemName.Text := ListBoxSaleItemList.Items.Strings[nIndex];
  btnSaleItemAdd.Enabled := True;
end;
...
begin
  ...
  SpinEditShopItemType.Text := ListItem.Caption;
  ...
  EditShopItemDetail.Text := ListItem.SubItems.Strings[5];
...
begin
  ...
  for i := 0 to ListViewShopItemList.Items.Count - 1 do begin
    ListItem := ListViewShopItemList.Items.item[i];
    if CompareText(ListItem.SubItems.Strings[0], sItemName) = 0 then begin
      Application.MessageBox('输入的物品已经在列表中！', '提示信息', MB_OK + MB_ICONERROR);
      Exit;
    end;
  end;
  ListItem := ListViewShopItemList.Items.Add;
  ListItem.Caption := IntToStr(SpinEditShopItemType.Value);
...
begin
  ...
  LoadList := TStringList.Create;
  LoadList.Add(';物品类别'#9'物品名称'#9'Item.wil序号'#9'出售价格'#9'Deco.wil序号1'#9'Deco.wil序号2'#9'描述');
  for i := 0 to ListViewShopItemList.Items.Count - 1 do begin
    ListItem := ListViewShopItemList.Items.item[i];
...
var
  nC                        : Integer;
  sName, sAttrib, sItemName, sItemDesc, sLineText: string;
  ListItem                  : TListItem;
  SuiteItems                : pTSuiteItems;
begin
  ...
  if CheckBoxParalysis.Checked then Inc(SuiteItems.aSuitSubRate[14]);
  if CheckBoxMagicShield.Checked then Inc(SuiteItems.aSuitSubRate[15]);
  if CheckBoxTeleport.Checked then Inc(SuiteItems.aSuitSubRate[16]);
  if CheckBoxRevival.Checked then Inc(SuiteItems.aSuitSubRate[17]);
  if CheckBoxMuscleRing.Checked then Inc(SuiteItems.aSuitSubRate[18]);
  if CheckBoxFastTrain.Checked then Inc(SuiteItems.aSuitSubRate[19]);
  if CheckBoxProbeNecklace.Checked then Inc(SuiteItems.aSuitSubRate[20]);
  if CheckBoxHongMoSuite.Checked then Inc(SuiteItems.aSuitSubRate[21]);
  if CheckBoxHideMode.Checked then Inc(SuiteItems.aSuitSubRate[22]);
  if CheckBoxUnParalysis.Checked then Inc(SuiteItems.aSuitSubRate[23]);
  if CheckBoxUnAllParalysis.Checked then Inc(SuiteItems.aSuitSubRate[24]);
  if CheckBoxUnRevival.Checked then Inc(SuiteItems.aSuitSubRate[25]);
  if CheckBoxUnMagicShield.Checked then Inc(SuiteItems.aSuitSubRate[26]);
  if CheckBoxRecallSuite.Checked then Inc(SuiteItems.aSuitSubRate[27]);
  if CheckBoxNoDropItem.Checked then Inc(SuiteItems.aSuitSubRate[28]);
  if CheckBoxNoDropUseItem.Checked then Inc(SuiteItems.aSuitSubRate[29]);
  ...
  if CheckBox1.Checked then SuiteItems.SendToClient := True;
...
begin
  if g_SuiteItems = nil then
    Exit;
  for i := g_SuiteItemsList.Count - 1 downto 0 do begin
    if g_SuiteItems = g_SuiteItemsList.Items[i] then begin
...
var
  ...
  //pstd                      : pTStdItem;
begin
  if g_SuiteItemsList.Count <= 0 then Exit;
...
var
  ...
  sName, sAttrib, sItemName, sItemDesc, sLineText: string;
begin
  if (g_SuiteItems = nil) {or (g_ListItemSuite = nil)} then Exit;
  sItemName := Trim(Edit1.Text) + Trim(Edit2.Text) + Trim(Edit3.Text) + Trim(Edit4.Text) + Trim(Edit5.Text) + Trim(Edit6.Text) + Trim(Edit7.Text) + Trim(Edit8.Text) + Trim(Edit9.Text) + Trim(Edit10.Text) + Trim(Edit11.Text) + Trim(Edit12.Text) + Trim(Edit13.Text);
  sItemDesc := Trim(EditSuiteEffectMsg.Text);
  if sItemName = '' then begin
    Application.MessageBox('套装物品名称输入不正确！', '提示信息', MB_OK + MB_ICONERROR);
  ...
  if CheckBoxParalysis.Checked then g_SuiteItems.aSuitSubRate[14] := 1 else g_SuiteItems.aSuitSubRate[14] := 0;
  if CheckBoxMagicShield.Checked then g_SuiteItems.aSuitSubRate[15] := 1 else g_SuiteItems.aSuitSubRate[15] := 0;
  if CheckBoxTeleport.Checked then g_SuiteItems.aSuitSubRate[16] := 1 else g_SuiteItems.aSuitSubRate[16] := 0;
  if CheckBoxRevival.Checked then g_SuiteItems.aSuitSubRate[17] := 1 else g_SuiteItems.aSuitSubRate[17] := 0;
  if CheckBoxMuscleRing.Checked then g_SuiteItems.aSuitSubRate[18] := 1 else g_SuiteItems.aSuitSubRate[18] := 0;
  if CheckBoxFastTrain.Checked then g_SuiteItems.aSuitSubRate[19] := 1 else g_SuiteItems.aSuitSubRate[19] := 0;
  if CheckBoxProbeNecklace.Checked then g_SuiteItems.aSuitSubRate[20] := 1 else g_SuiteItems.aSuitSubRate[20] := 0;
  if CheckBoxHongMoSuite.Checked then g_SuiteItems.aSuitSubRate[21] := 1 else g_SuiteItems.aSuitSubRate[21] := 0;
  if CheckBoxHideMode.Checked then g_SuiteItems.aSuitSubRate[22] := 1 else g_SuiteItems.aSuitSubRate[22] := 0;
  if CheckBoxUnParalysis.Checked then g_SuiteItems.aSuitSubRate[23] := 1 else g_SuiteItems.aSuitSubRate[23] := 0;
  if CheckBoxUnAllParalysis.Checked then g_SuiteItems.aSuitSubRate[24] := 1 else g_SuiteItems.aSuitSubRate[24] := 0;
  if CheckBoxUnRevival.Checked then g_SuiteItems.aSuitSubRate[25] := 1 else g_SuiteItems.aSuitSubRate[25] := 0;
  if CheckBoxUnMagicShield.Checked then g_SuiteItems.aSuitSubRate[26] := 1 else g_SuiteItems.aSuitSubRate[26] := 0;
  if CheckBoxRecallSuite.Checked then g_SuiteItems.aSuitSubRate[27] := 1 else g_SuiteItems.aSuitSubRate[27] := 0;
  if CheckBoxNoDropItem.Checked then g_SuiteItems.aSuitSubRate[28] := 1 else g_SuiteItems.aSuitSubRate[28] := 0;
  if CheckBoxNoDropUseItem.Checked then g_SuiteItems.aSuitSubRate[29] := 1 else g_SuiteItems.aSuitSubRate[29] := 0;
