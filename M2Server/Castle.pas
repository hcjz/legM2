...
uses UsrEngn, M2Share, HUtil32, CastleAttackEdit;
...
begin
  LoadConfig();
  LoadAttackSabukWall();
  if m_nMapOfServerIndex = g_nServerIndex then begin
    ...
    try
      ...
      for i := 0 to LoadList.Count - 1 do begin
        ...
        if Guild <> nil then begin
          if not m_AttackWarList.Exists(sGuildName) then begin
            New(AttackerInfo);
            try
...
{$IF SoftVersion <> VERDEMO}
    ...
    if m_boUnderWar then begin
      if m_btForceStartWar = 2 then begin //force stop
        m_btForceStartWar := 0;
        m_btForceStopWar := 1;
        m_boStartWar := True;
      ...
      if m_btForceStartWar = 1 then begin //force start
        m_btForceStartWar := 0;
        m_boStartWar := False;
    ...
    if not m_boStartWar and not m_boUnderWar then begin
      ...
      if (Hour = nStartCastlewarTime) then begin
        cc := 5;
        m_boStartWar := True;
        cc := 6;
    ...
    if m_boUnderWar then begin
      ...
      if not m_boShowOverMsg then begin
        if (m_btForceStopWar = 1) or ((GetTickCount - m_dwStartCastleWarTick) > (g_Config.dwCastleWarTime - g_Config.dwShowCastleWarEndMsgTime)) then begin
...
begin
  Result := False;
  if (Envir = m_MapCastle) and (abs(m_nHomeX - nX) < m_nWarRangeX {100}) and (abs(m_nHomeY - nY) < m_nWarRangeY {100}) then begin
    Result := True;
    Exit;
  end;
  ...
  //增加取得城堡所有地图列表
  if (LatestCastleEnvirIdx >= 0) and (LatestCastleEnvirIdx < m_EnvirList.Count) then begin
    if m_EnvirList.Objects[LatestCastleEnvirIdx] = Envir then begin
...
begin
  Result := False;
  if GetTickCount - m_dwStartCastleWarTick > g_Config.dwGetCastleTime then begin
    List14 := TList.Create;
    ...
    for i := 0 to List14.Count - 1 do begin
      ...
      if not PlayObject.m_boDeath and not PlayObject.m_boObMode and not PlayObject.m_boAdminMode and (PlayObject.m_MyGuild <> Guild) then begin
        Result := False;
        Break;
      end;
    end;
...
begin
  ...
  if (CastleDoor.BaseObject <> nil) then begin
...
begin
  ...
  for i := 0 to m_CastleList.Count - 1 do begin
    ...
    if Castle.InCastleWarArea(BaseObject.m_PEnvir, BaseObject.m_nCurrX, BaseObject.m_nCurrY, idx) then begin
      if BaseObject.m_nCastleEnvirListIdx <> idx then BaseObject.m_nCastleEnvirListIdx := idx;
      Result := Castle;
      Break;
    end;
  end;
...
begin
  ...
  for i := 0 to m_CastleList.Count - 1 do begin
    Castle := TUserCastle(m_CastleList.Items[i]);
    if Castle.InCastleWarArea(Envir, nX, nY, LatestCastleEnvirIdx) then begin
      Result := Castle;
      Break;
    end;
  end;
...
begin
  ...
  try
    if m_CastleList.Count <= 0 then begin
      ...
      if Castle.m_EnvirList.IndexOf('0151') < 0 then Castle.m_EnvirList.Add('0151');
      if Castle.m_EnvirList.IndexOf('0152') < 0 then Castle.m_EnvirList.Add('0152');
      if Castle.m_EnvirList.IndexOf('0153') < 0 then Castle.m_EnvirList.Add('0153');
      if Castle.m_EnvirList.IndexOf('0154') < 0 then Castle.m_EnvirList.Add('0154');
      if Castle.m_EnvirList.IndexOf('0155') < 0 then Castle.m_EnvirList.Add('0155');
      if Castle.m_EnvirList.IndexOf('0156') < 0 then Castle.m_EnvirList.Add('0156');
