      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if CompareText(sActionType, sROAUTORUN) = 0 then begin
          if CompareText(sRunCmd, sRONPCLABLEJMP) = 0 then begin
            ...
            if CompareText(sMoethod, sRUNONHOUR) = 0 then AutoRunInfo.nMoethod := nRUNONHOUR;
            if CompareText(sMoethod, sRUNONMIN) = 0 then AutoRunInfo.nMoethod := nRUNONMIN;
            if CompareText(sMoethod, sRUNONSEC) = 0 then AutoRunInfo.nMoethod := nRUNONSEC;
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if (sRobotName <> '') and (sScriptFileName <> '') then begin
          ...
          RobotHuman.m_sFCharName := FilterCharName(RobotHuman.m_sCharName);
