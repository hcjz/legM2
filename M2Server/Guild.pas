...
begin
  ...
  if i >= 0 then begin
    ...
    if Guild.m_RankList.Count <= 1 then begin
...
begin
  ...
  for i := 0 to GuildList.Count - 1 do begin
    Guild := TGuild(GuildList.Values[GuildList.Keys[i]]);
    if Guild.IsMember(sName) then begin
...
begin
  ...
  for i := 0 to m_RankList.Count - 1 do begin
    GuildRank := m_RankList.Items[i];
    if GuildRank.MemberList.Exists(sName) then begin
      Result := True;
      Exit;
    end;
    ...
    case n28 of                         //
      ...
      2: begin
          while (s18 <> '') do begin
            ...
            if s1C = '' then
              Break;
            if not m_GuildWarList.Exists(s1C) then begin
              New(GuildWar);
      ...
      3: begin
          while (s18 <> '') do begin
            ...
            if s1C = '' then
              Break;
            if not m_GuildAllList.Exists(s1C) then begin
...
begin
  if g_nServerIndex = 0 then begin
...
begin
  Result := False;
  if IsFull {or (g_Config.nRegCheckCode <= 0)} then Exit;
  nRankNo := 99;
  GuildRank18 := nil;
  for i := 0 to m_RankList.Count - 1 do begin
    GuildRank := m_RankList.Items[i];
    if GuildRank.nRankNo = nRankNo then begin
...
begin
  ...
  if GuildRank.MemberList.Count <> 1 then
    Exit;
  if GuildRank.MemberList.Keys[0] = sHumName then begin
    BackupGuildFile();
    Result := True;
  end;
...
begin
  ...
  while (True) do begin
    ...
    if sRankInfo[1] = '#' then begin
      ...
      if IsInGuildRankNameFilterList(sRankName) then begin
        Result := -15;
        Exit;
      end;
  ...
  if m_RankList.Count = GuildRankList.Count then begin
    ...
    for i := 0 to m_RankList.Count - 1 do begin
      ...
      if (GuildRank.nRankNo = NewGuildRank.nRankNo) and
        ...
        for ii := 0 to GuildRank.MemberList.Count - 1 do begin
          if GuildRank.MemberList.Keys[ii] <> NewGuildRank.MemberList.Keys[ii]  then begin   
            boCheckChange := False;      
            Break;
          end;
        end;
  ...
  if Result = 0 then begin
    ...
    if GuildRank.MemberList.Count <= 2 then begin
      ...
      for i := 0 to GuildRank.MemberList.Count - 1 do begin
        if UserEngine.GetPlayObject(GuildRank.MemberList.Keys[i]) = nil then begin
          Dec(n28);
          Break;
        end;
      end;
    ...
    for i := 0 to m_RankList.Count - 1 do begin
      ...
      for ii := 0 to GuildRank.MemberList.Count - 1 do begin
        ...
        for III := 0 to GuildRankList.Count - 1 do begin
          ...
          if NewGuildRank.MemberList.Exists(sMemberName) then begin
            boCheckChange := True;
            Break;
          end;
...
begin
  Result := False;
  if (Guild <> nil) and not m_GuildAllList.Exists(Guild.sGuildName) then begin
...
begin
  ...
  if Guild <> nil then begin
    if not IsAllyGuild(Guild) then begin
      ...
      if WarGuild = nil then begin
        if not m_GuildWarList.Exists(Guild.sGuildName) then begin
          New(WarGuild);
