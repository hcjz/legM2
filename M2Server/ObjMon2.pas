...
var
  bo05                      : Boolean;
begin
  if not m_boGhost and not m_boDeath and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
...
begin
  if n550 > 0 then begin
    if m_PEnvir.GetNextPosition(m_nCurrX, m_nCurrY, m_btDirection, 1, n08, n0C) then begin
      if m_PEnvir.CanWalk(n08, n0C, False) then begin
...
begin
  if not m_boDeath and not m_boGhost then begin
    if (GetTickCount - MakeTime) > 60 * 1000 then begin
      MakeTime := GetTickCount();
      DoSelfExplosion();
    end;
  end;
...
begin
  ...
  if m_Castle <> nil then begin
    ...
    if (m_btRaceServer = 109) and (BaseObject.m_btRaceServer = 108) then
      Result := True;
    if BaseObject.m_boStruck then begin
    ...
    if TUserCastle(m_Castle).m_MasterGuild <> nil then begin
      if BaseObject.m_Master = nil then begin
        if (TUserCastle(m_Castle).m_MasterGuild = BaseObject.m_MyGuild) or (TUserCastle(m_Castle).m_MasterGuild.IsAllyGuild(TGuild(BaseObject.m_MyGuild))) then begin
          if m_LastHiter <> BaseObject then
            Result := False;
        end;
      end else begin
        if (TUserCastle(m_Castle).m_MasterGuild = BaseObject.m_Master.m_MyGuild) or (TUserCastle(m_Castle).m_MasterGuild.IsAllyGuild(TGuild(BaseObject.m_Master.m_MyGuild))) then begin
    ...
    if BaseObject.m_boAdminMode or BaseObject.m_boStoneMode or BaseObject.m_boSuperMode or
      ...
      (BaseObject = self) or (BaseObject.m_Castle = m_Castle) then begin
      Result := False;
    end;
...
begin
  ...
  if not m_boDeath and not m_boGhost and (m_wStatusTimeArr[POISON_STONE] = 0) and (m_wStatusTimeArr[POISON_PURPLE] = 0) then begin
    ...
    end else if (m_nDirection >= 0) and (m_btDirection <> m_nDirection) then
      TurnTo(m_nDirection);
  end;
  inherited;
end;
