...
begin
  ...
  if Self.Exists(sMapName) then begin
  ...
  for i := 0 to MiniMapList.Count - 1 do begin
    if (CompareText(MiniMapList.Strings[i], Envir.m_sMapFileName) = 0) or
      (CompareText(MiniMapList.Strings[i], Envir.m_sClientMapName) = 0) then begin
      Envir.m_nMinMap := Integer(MiniMapList.Objects[i]);
      Break;
    end;
  end;

  if Envir.LoadMapData(g_Config.sMapDir + sSendMapName + '.map') then begin
...
{$ELSE}
    Add(Envir);
{$IFEND}
    if (Envir.m_MapFlag.nThunder <> 0) or (Envir.m_MapFlag.nGreatThunder <> 0) or
      (Envir.m_MapFlag.nLava <> 0) or (Envir.m_MapFlag.nSpurt <> 0) then
    ...
    if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) then begin
      if MapCellInfo.ObjList = nil then begin
        ...
        if btType = OS_ITEMOBJECT then begin
          if CompareText(pTMapItem(pRemoveObject).Name, sSTRING_GOLDNAME) = 0 then begin
          ...
          if not bMapCellFill then begin
            nGoldCount := 0;
            for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
              OSObject := MapCellInfo.ObjList.Items[i];
              if (OSObject <> nil) and (OSObject.btType = OS_ITEMOBJECT) then begin
      ...
      if not bMapCellFill then begin
        ...
        if (btType = OS_EVENTOBJECT) and (TEvent(pRemoveObject).m_nEventType in [ET_NIMBUS_1, ET_NIMBUS_2, ET_NIMBUS_3]) then begin
...
begin
  ...
  try
    if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) then begin
      ...
      if not TBaseObject(OSObject.CellObj).m_boAddToMaped then begin
...
begin
  Result := False;
  if (nX >= 0) and (nX < m_MapHeader.wWidth) and (nY >= 0) and (nY < m_MapHeader.wHeight) then begin
...
var
  i                 : Integer;
  MapCellInfo       : pTMapCellinfo;
  MapCellInfoCurrent: pTMapCellinfo;
  MapCellInfoTarget : pTMapCellinfo;
  BaseObject        : TBaseObject;
  OSObject          : pTOSObject;
  boRemoveObj, bThrough: Boolean;
  bMoveOK, boGetMapCell      : Boolean;
resourcestring
...
begin
  ...
  if bThrough then begin
    if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag <> 0) then begin
      ...
      if bMoveOK and GetMapCellInfo(nX, nY, MapCellInfo) then begin
        if MapCellInfo.ObjList = nil then
          MapCellInfo.ObjList := TList.Create;
        New(OSObject);
    ...
    if not boFlag and (MapCellInfo.ObjList <> nil) then begin
      for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
        ...
        if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
          BaseObject := TBaseObject(OSObject.CellObj);
          if BaseObject <> nil then begin
            if (PlayObject <> nil) and (TPlayObject(PlayObject).m_btRaceServer = RC_PLAYOBJECT) then begin
              Castle := g_CastleManager.InCastleWarArea(BaseObject);
              if g_Config.boWarDisHumRun and (Castle <> nil) and (Castle.m_boUnderWar) then
                //
              else if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
                if g_Config.boRUNHUMAN or m_MapFlag.boRUNHUMAN or (nCanRun = 0) or (nCanRun = 2) then
                  Continue;
              end else if BaseObject.m_btRaceServer in [RC_NPC, RC_TRAINER] then begin
                if g_Config.boRunNpc or (nCanRun = 0) then
                  Continue;
              end else if BaseObject.m_btRaceServer in [RC_GUARD, RC_ARCHERGUARD] then begin
                if g_Config.boRunGuard or (nCanRun = 0) then
                  Continue;
              end else if g_Config.boRUNMON or m_MapFlag.boRUNMON or (nCanRun = 0) or (nCanRun = 1) then
                Continue;
            end;
            if not BaseObject.m_boGhost and BaseObject.m_boHoldPlace and not BaseObject.m_boDeath and not BaseObject.m_boFixedHideMode and not BaseObject.m_boObMode then begin
              Result := False;
              Break;
            end;
          end;
...
begin
  ...
  for i := 0 to g_StartPointManager.m_InfoList.Count - 1 do begin
    ...
    if (StartPointInfo.Envir = Self) and
      ...
      (abs(nY - nSafeY) <= StartPointInfo.nSize) then begin
      Result := True;
  ...
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) then begin
    ...
    if (MapCellInfo.ObjList <> nil) then begin
      for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
        ...
        if not boItem and (OSObject <> nil) and (OSObject.btType = OS_ITEMOBJECT) then begin
          Result := False;
          Break;
        end;
      end;
...
{$ELSE}
    for i := 0 to Count - 1 do begin
      ...
      if CompareText(Map.m_sMapFileName, sMapName) = 0 then begin
        Result := Map;
        Break;
      end;
    end;
...
{$ELSE}
    for i := 0 to Count - 1 do begin
      Map := TEnvirnoment(Items[i]);
      if (CompareText(Map.m_sMapFileName, sMapName) = 0) and (Map.m_MapFlag.nGuildTerritory = GTNumber) then begin
        Result := Map;
        Break;
      end;
    end;
...
{$ELSE}
    for i := 0 to Count - 1 do begin
      ...
      if (Envir.m_nServerIndex = nServerIdx) and (CompareText(Envir.m_sMapFileName, sMapName) = 0) then begin
        Result := Envir;
        Break;
      end;
    end;
          ...
          while (True) do begin
            ...
            if OSObject <> nil then begin
              if (OSObject.btType = btType) and (OSObject.CellObj = pRemoveObject) then begin
                ...
                if (NimbusCount > 0) and (btType = OS_EVENTOBJECT) and (TEvent(pRemoveObject).m_nEventType in [ET_NIMBUS_1, ET_NIMBUS_2, ET_NIMBUS_3]) then begin
                  NimbusCount := NimbusCount - 1;
                end;
                if MapCellInfo.ObjList.Count > 0 then
                  Continue;
...
begin
  Result := nil;
  m_boChFlag := False;
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo <> nil) and (MapCellInfo.chFlag = 0) then begin
    m_boChFlag := True;
    if MapCellInfo.ObjList <> nil then begin
      for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
        OSObject := MapCellInfo.ObjList.Items[i];
  ...
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo <> nil) and (MapCellInfo.chFlag = 0) then begin
    if MapCellInfo.ObjList <> nil then begin
      for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
        ...
        if OSObject = nil then Continue;
        if OSObject.btType = OS_ITEMOBJECT then begin
          if p = pTMapItem(OSObject.CellObj) then begin
            Result := pTMapItem(OSObject.CellObj);
            Exit;
          end;
        end;
...
{$ELSE}
    for i := 0 to Count - 1 do begin
      Envir := Items[i];
      if (CompareText(Envir.m_sMapFileName, sMapName) = 0) then begin
        Result := Envir.m_nServerIndex;
        Break;
      end;
    end;
...
{$ELSE}
  for i := 0 to Count - 1 do begin
    ...
    for ii := 0 to MiniMapList.Count - 1 do begin
      if CompareText(MiniMapList.Strings[ii], Envirnoment.m_sMapFileName) = 0 then begin
        Envirnoment.m_nMinMap := Integer(MiniMapList.Objects[ii]);
        Break;
      end;
    end;
...
{$IF USEHASHLIST = 1}
    ...
    for i := 0 to Count - 1 do begin
      ...
      for ii := 0 to slist.Count - 1 do begin
        if CompareText(slist.Strings[ii], Envirnoment.m_sMapFileName) = 0 then begin
...
{$ELSE}
    ...
    for i := 0 to Count - 1 do begin
      Envirnoment := TEnvirnoment(Items[i]);
      for ii := 0 to slist.Count - 1 do begin
        if slist.Strings[ii] = Envirnoment.m_sMapFileName then begin
          Envirnoment.m_MapFlag.pDigItemList := PTDigItemLists(slist.Objects[ii]);
          Break;
        end;
      end;
...
var
  Space             : Boolean;
  X, Y              : Integer;
  MapCellInfo, Mc, Mc2: pTMapCellinfo;
  OSObject          : pTOSObject;
resourcestring
...
begin
  ...
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) then begin
    if MapCellInfo.ObjList = nil then
      MapCellInfo.ObjList := TList.Create;
    if nType = OS_EVENTOBJECT then begin
      New(OSObject);
...
begin
  ...
  try
    if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag <> 0) then begin
      ...
      //人物可以走到的地方才放上矿藏
      Space := False;
      for X := nX - 1 to nX + 1 do begin
        for Y := nY - 1 to nY + 1 do begin
          if GetMapCellInfo(X, Y, Mc) then begin
            if (Mc.chFlag = 0) then
              Space := True;
          end;
          if Space then
...
begin
  Result := False;
  if (nX = nX2) and (nY = nY2) then begin
    Exit;
  end;
...
begin
  ...
  for nX := 0 to m_MapHeader.wWidth - 1 do begin
    for nY := 0 to m_MapHeader.wHeight - 1 do begin
...
{$IFEND}
      ...
      for nW := 0 to m_MapHeader.wWidth - 1 do begin
        n24 := nW * m_MapHeader.wHeight;
        for nH := 0 to m_MapHeader.wHeight - 1 do begin
{$IF VER_PATHMAP = 1}
...
begin
  Result := 0;
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.ObjList <> nil){$IF NEWGETMAPCELL = 1} and (MapCellInfo.chFlag <> 1){$IFEND} then begin
    for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
      OSObject := MapCellInfo.ObjList.Items[i];
      if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
        BaseObject := TBaseObject(OSObject.CellObj);
      ...
      if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
        ...
        if BaseObject <> nil then begin
          if not BaseObject.m_boGhost and
            ...
            ((BaseObject.m_btRaceServer in [RC_NPC, RC_PEACENPC]) or (BaseObject = PlayObject)) then begin
            Inc(Result);
          end;
        end;
      end;
...
begin
  ...
  case nDir of
    ...
    DR_DOWN: if sny < (m_MapHeader.wHeight - nFlag) then Inc(sny, nFlag);
    DR_LEFT: if snx > nFlag - 1 then Dec(snx, nFlag);
    DR_RIGHT: if snx < (m_MapHeader.wWidth - nFlag) then Inc(snx, nFlag);
    DR_UPLEFT: begin
    ...
    DR_UPRIGHT: begin
        if (snx > nFlag - 1) and (sny < (m_MapHeader.wHeight - nFlag)) then begin
          Inc(snx, nFlag);
          Dec(sny, nFlag);
        end;
      end;
    DR_DOWNLEFT: begin
        if (snx < (m_MapHeader.wWidth - nFlag)) and (sny > nFlag - 1) then begin
          Dec(snx, nFlag);
          Inc(sny, nFlag);
        end;
      end;
    DR_DOWNRIGHT: begin
        if (snx < (m_MapHeader.wWidth - nFlag)) and (sny < (m_MapHeader.wHeight - nFlag)) then begin
          Inc(snx, nFlag);
          Inc(sny, nFlag);
        end;
      end;
  ...
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.chFlag = 0) then begin
    ...
    for i := MapCellInfo.ObjList.Count - 1 downto 0 do begin
      ...
      if (OSObject <> nil) and (OSObject.btType = OS_EVENTOBJECT) then begin
        if TEvent(OSObject.CellObj).m_nDamage > 0 then begin
          Result := False;
          Break; //0615
        end;
      end;
    ...
    if nFlagValue = MapQuestFlag.nValue then begin
      if (boFlag = MapQuestFlag.boGrouped) or (not boFlag) then begin
        ...
        if (MapQuestFlag.sMonName <> '') and (MapQuestFlag.sItemName <> '') then begin
          if (MapQuestFlag.sMonName = sCharName) and (MapQuestFlag.sItemName = sItem) then begin
            bo1D := True;
        ...
        if (MapQuestFlag.sMonName <> '') and (MapQuestFlag.sItemName = '') then begin
          if (MapQuestFlag.sMonName = sCharName) and (sItem = '') then begin
            bo1D := True;
            Result := MapQuestFlag.NPC;
            Break;
          end;
        ...
        if (MapQuestFlag.sMonName = '') and (MapQuestFlag.sItemName <> '') then begin
          if (MapQuestFlag.sItemName = sItem) then begin
            bo1D := True;
            Result := MapQuestFlag.NPC;
            Break;
          end;
  ...
  if GetMapCellInfo(nX, nY, MapCellInfo) and (MapCellInfo.ObjList <> nil){$IF NEWGETMAPCELL = 1} and (MapCellInfo.chFlag <> 1){$IFEND} then begin
    for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
      ...
      if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
        BaseObject := TBaseObject(OSObject.CellObj);
        if BaseObject <> nil then begin
          if not BaseObject.m_boGhost and BaseObject.m_boHoldPlace then begin
...
begin
  Result := nil;
  m_boChFlag := False;
  if not (range in [1..10]) then Exit;

  for X := nX - range to nX + range do begin
    for Y := nY - range to nY + range do begin
      if GetMapCellInfo(X, Y, MapCellInfo) and (MapCellInfo.ObjList <> nil) then begin
        for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
          OSObject := MapCellInfo.ObjList.Items[i];
          if (OSObject <> nil) and (OSObject.btType = OS_EVENTOBJECT) and (id = Integer(OSObject.CellObj)) then begin
            Result := OSObject.CellObj;
            Break;
          end;
        end;
...
begin
  Result := False;
  if GetMapCellInfo(nMapX, nMapY, MapCellInfo) and (MapCellInfo.ObjList <> nil){$IF NEWGETMAPCELL = 1} and (MapCellInfo.chFlag <> 1){$IFEND} then begin
    for i := 0 to MapCellInfo.ObjList.Count - 1 do begin
      OSObject := MapCellInfo.ObjList.Items[i];
      if (OSObject <> nil) and (OSObject.btType = OS_MOVINGOBJECT) then begin
        BaseObject := TBaseObject(OSObject.CellObj);
...
begin
  ...
  if (X < 0) or (X >= m_MapHeader.wWidth) or (Y < 0) or (Y >= m_MapHeader.wHeight) then
    ...
    if (X < m_MapHeader.wWidth - m_nPathWidth) and (X > m_nPathWidth) and (Y < m_MapHeader.wHeight - m_nPathWidth) and (Y > m_nPathWidth) then begin
...
begin
  ...
  for i := 0 to LoadList.Count - 1 do begin
    ...
    if (sLineText <> '') and (sLineText[1] <> ';') then begin
      ...
      New(MapEvent);
      MapEvent.nCurrX := StrToInt(sCurrX);
      MapEvent.nCurrY := StrToInt(sCurrY);
      sFlag := GetValidStr3(sFlag, sFlag_a, [':']);
      sFlag := GetValidStr3(sFlag, sFlag_b, [':']);
      MapEvent.cFlag.nFlag := StrToInt(sFlag_a);
      MapEvent.cFlag.boFlag := StrToInt(sFlag_b) > 0;
      ...
      MapEvent.cCondition.nAction := StrToInt(sCondition_a);
      MapEvent.cCondition.sItemName := sCondition_b;
      MapEvent.cCondition.boGroup := StrToInt(sCondition_c) > 0;

      MapEvent.nRate := StrToInt(sRate);

      sEnent := GetValidStr3(sEnent, sEnent_a, [':']);
      sEnent := GetValidStr3(sEnent, sEnent_b, [':']);
      MapEvent.cEnent.boCall := StrToInt(sEnent_a) > 0;
