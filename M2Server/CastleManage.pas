...
begin
  ...
  if CurCastle.m_boUnderWar then begin
    EditStatus.Text := '攻城中...';
  ...
  ListViewWar.Clear;
  if CurCastle.m_AttackWarList.Count > 0 then begin
...
begin
  if ListViewWar.ItemIndex >= 0 then begin
...
begin
  b := False;
  if (CurCastle = nil) or CurCastle.m_boUnderWar or (CurCastle.m_btForceStartWar <> 0) then Exit;
...
begin
  if (CurCastle = nil) or not CurCastle.m_boUnderWar or (CurCastle.m_btForceStartWar <> 0) then Exit;
  EnterCriticalSection(ProcessHumanCriticalSection);
  try
    g_CastleManager.Lock;
    try
...
procedure TfrmCastleManage.ButtonAddClick(Sender: TObject);
  ...
  begin
    ...
    for i := 0 to CurCastle.m_AttackWarList.Count - 1 do begin
      AttackerInfo := pTAttackerInfo(CurCastle.m_AttackWarList.Values[CurCastle.m_AttackWarList.keys[i]]);
      if (CompareText(AddingInfo^.sGuildName, AttackerInfo^.sGuildName) = 0) or (AddingInfo^.Guild = AttackerInfo^.Guild) then begin
        DecodeDate(AddingInfo^.AttackDate, wYear, wMonth, wDay);
        DecodeDate(AttackerInfo^.AttackDate, sYear, sMonth, sDay);
        if (wYear = sYear) and (wMonth = wMonth) and (wDay = sDay) then begin
          Result := True;
          Break;
        end;
      end;
...
begin
  ...
  if boAddAll then begin
    ...
    for i := 0 to g_GuildManager.GuildList.Count - 1 do begin
      ...
      if not CurCastle.m_AttackWarList.Exists(Guild.sGuildName) then begin
        New(AttackerInfo);
    ...
    Dispose(INFO);
    RefCastleInfo;
  end else if INFO.Guild <> nil then begin
    if CurCastle <> nil then begin
      if IsInAttackList(INFO) then begin
