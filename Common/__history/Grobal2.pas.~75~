...
uses
  Windows, Classes, Graphics, SysUtils, D7ScktComp, MD5;
...
{$IFEND VER_176}
  ...
  MAXMISSION = 20;
  DEFBLOCKSIZE = 16; //SizeOf(TCmdPack) div 3 * 4;
  BUFFERSIZE = 16 * 1024; //10000
  DATA_BUFSIZE = 8192;
  MAXLEVEL = 500;
  GROUPMAX = 11;
  BAGGOLD = 5000000;
  ...
  POISON_SHOCKED = 2; //$20000000  //BLUE
  POISON_DONTMOVE = 3; //$10000000  //YELLOW
  POISON_FREEZE = 4; //$08000000  //PURPLE
  POISON_STONE = 5; //$04000000
  POISON_PURPLE = 6; //$02000000
  POISON_7 = 7; //$01000000
  ...
  STATE_8 = 8; //$00800000
  STATE_9 = 9; //$00400000
  STATE_10 = 10; //$00200000
  STATE_11 = 11; //$00100000
  STATE_12 = 12; //$00080000    //Ä§·¨¶Ü

  STATE_13 = 13; //$00040000
  STATE_14 = 14; //$00020000
  STATE_15 = 15; //$00010000
  STATE_16 = 16; //$00008000
  STATE_17 = 17; //$00004000
  STATE_18 = 18; //$00002000
  STATE_19 = 19; //$00001000
  STATE_20 = 20; //$00000800
  STATE_21 = 21; //$00000400
  STATE_22 = 22; //$00000200
  STATE_23 = 23; //$00000100
...
//----------------------------------------------------
  SM_OPENBIGDIALOGBOX = 1309;      //Ê¹ÓÃNPC´ó¶Ô»°¿ò  Development 2019-01-14
//----------------------------------------------------
...
const
  ...
  g_SuitSubRateName: array[0..29 + 10] of string = (
    'HPÉÏÏÞ +%d',
    'MPÉÏÏÞ +%d',
    'Îï·À +%d',
    'Ä§·À +%d',
...
type
  ...
  //packed record½á¹¹£¬²»ÐèÒª×Ö¶ÔÆë£¬record½á¹¹ÐèÒª×Ö¶ÔÆë£¬
  //Òò´Ë£¬packed recordÕ¼ÓÃÄÚÈÝ¿Õ¼ä±ÈrecordÒªÐ¡£¬µ«·ÃÎÊÊ±packed record±ÈrecordÉÔÂý
  ...
  TCItemRule = packed record
    Name: string[20];  //ÎïÆ·Ãû³Æ
    ntype: Byte;       //ÎïÆ·Àà±ð
    rare: Boolean;     //¼«Æ·ÏÔÊ¾
    pick: Boolean;     //×Ô¶¯Ê°È¡
    Show: Boolean;     //ÏÔÊ¾Ãû×Ö
  end;
  ...
  TSuiteIndex = array[0..High(Word)] of Word;  //À©Õ¹Ì××°ÊôÐÔÖ§³Ö65535 Development 2019-01-15
  TSuitSubRate = array[0..29 + 10] of Word;
  TSuiteNames = array[0..12 {13?}] of string[ItemNameLen];
  ...
  TAddValueEx_176 = array[0..6] of Byte;
...
{$IFEND VER_176}
  ...
  TMonSpAbil = packed record
    btIgnoreTagDefence: Byte;
    btDamageAddOn: Byte;
    btDamageRebound: Byte;
    btACDamageReduction: Byte;
    btMCDamageReduction: Byte;

    boMonParalysis: Boolean;
    boMonUnParalysis: Boolean;
    boMonUnAllParalysis: Boolean;
    boMonUnRevival: Boolean; //ÆÆ¸´»î
    boMonUnMagicShield: Boolean; //ÆÆ»¤Éí
  end;
...
{$IFEND}
  ...
  TUserItem = packed record //56
    ...
    Dura: Word;
    DuraMax: Word;
    btValue: array[0..25] of Byte; //22..25   ¿Õ£º19~21
{$IF V_DIGITEM}
    btValueEx: array[0..19] of Byte; //22..25   ¿Õ£º19
{$IFEND V_DIGITEM}
    //btValue:
    //14        ·À±¬
    //22~25     °ó¶¨
...
{$ENDIF}
  ...
  TMsgType = (t_Mon, t_Hint, t_GM, t_System, t_Notice, t_Cust, t_Castle, t_Say, t_SysRight);
  TMonStatus = (s_KillHuman, s_UnderFire, s_Die, s_MonGen);
  TItemType = (t_aMake, t_dMake, t_dTakeOff, t_aSellOff, t_dSell, t_dDeal, t_dDrop, t_dStorage, t_pOnLogon, t_dUpgrade, t_dRepair, t_sDrop, t_ADrop, t_dCustomName, t_dHeroUse, t_dps, t_nsc, t_dTakeOn);
  ...
  TVenationInfo = packed record //2
    Level: Byte; //-1Î´Í¨ 0ÒÑÍ¬ 1ÖØ...
    Point: Byte; //Êæ¾­»îÂçÍè ´òÍ¨
  end;
  ...
  THumTitles = array[0..5] of THumTitle;

//ÈËÎïÊý¾ÝÀàÐÍ½á¹¹ 2019-02-07 Development ·ÖÎö 
  THumData = packed record
    ...
    SeriesSkillArr: TSeriesSkillArr; //6
    btSPLuck: Byte;
    btSPEnergy: Byte;
    btActiveTitle: Byte; //.............
    btUnUsed1: Byte;
    ...
    wStatusTimeArrEx: TStatusTimeEx; //0x54
    HumTitles: THumTitles;
    tempBytes: array[0..157] of Byte;
  end;
  pTHumData = ^THumData;
...
{$IFEND}
  ...
  TLoginToolConfig = packed record
    FileName: string[50];//µÇÂ¼Æ÷¿ÉÖ´ÐÐ³ÌÐòÃû³Æ£¬²»º¬À©Õ¹Ãûexe
    Config1: string[50];
    Config2: string[50];
    //Config3: string[200];
    Config3: array[0..1023] of AnsiChar;
    Config7: string[49];
    Config8: Byte;   //Í¨¹ýÎ»ÔËËã£¬±£´æÊÇ·ñ¶ÁÈ¡ Wis, Wil, Wzl µÄ±êÖ¾
  end;

  TCliCmdLines = array[0..21] of string[28];//ÓÉ³¤¶È28µÄstringµÄÔªËØ¹¹³ÉµÄÊý×é
  ...
  TRcHeader = packed record
    sCompany: string[050];  //¹«Ë¾Ãû³Æ// = 'ÈÈÑª´«Ææ';
    sFileName: string[050]; //µÇÂ¼Æ÷Ãû³Æ£¬ÏÔÊ¾ÔÚµÇÂ¼Æ÷ÉÏµÄ±êÌâ// = 'ÈÈÑª´«ÆæµÇÂ½Æ÷';
    sWebLink: string[255];  //ÍøÕ¾¹«¸æµÄÍøÖ·// = '55B19C2CB50DA4D16F8E345A00496A41359108DC0D5C25228D808D71F2E82078';
    sWebSite: string[255];  //¹Ù·½ÍøÕ¾ = '55B19C2CB50DA4D16F8E345A00496A4167BEFB55C9F9AD2F';
    sBbsSite: string[255];  //¹Ù·½ÂÛÌ³// = '55B19C2CB50DA4D16F8E345A00496A4167BEFB55C9F9AD2F';

    sListSite: string[255];
    GameList: string[255]; //·þÎñÆ÷ÁÐ±íÎÄ¼þÍøÖ·// = '55B19C2CB50DA4D16F8E345A00496A41555B8ECAE49841B72C28D608615BB0E306F0C168FEC29FAECAFD4FDDAAE82DD8';
    GameList2: string[255];//ºó±¸·þÎñÆ÷ÁÐ±íÎÄ¼þÍøÖ·
    sSiteUrl: string[255]; //UpGradeµÄÍøÖ· = '55B19C2CB50DA4D16F8E345A00496A41555B8ECAE49841B74F398006846553C8AA3E4F7A4F6BDEAD';
...
var
{$IF SERIESSKILL}
  g_VLevelStr: array[1..5] of string = ('Ò»', '¶þ', 'Èý', 'ËÄ', 'Îå');
  g_VaStrs: array[0..3] of string = ('³åÂö', 'ÒõõÎ', 'ÒõÎ¬', 'ÈÎÂö');
  g_VPStrs: array[0..3, 1..5] of string = (
    ('ÓÄÃÅ', 'Í¨¹È', 'ÉÌÇú', 'ËÄÂú', 'ºá¹Ç'),
    ('¾¦Ã÷', 'ÅÌÈ±', '½»ÐÅ', 'ÕÕº£', 'È»¹È'),
    ('Á®Èª', 'ÆÚÃÅ', '¸®Éá', '³åÃÅ', 'Öþ±ö'),
    ('³Ð½¬', 'ÌìÍ»', 'ð¯Î²', 'Æøº£', 'Çú¹Ç')
    );
  g_JobofSeriesSkill: array[0..2] of TSeriesSkills = (
    ('Èý¾øÉ±', '×·ÐÄ´Ì', '¶ÏÔÀÕ¶', 'ºáÉ¨Ç§¾ü'),
    ('Ë«ÁúÆÆ', '·ïÎè¼À', '¾ªÀ×±¬', '±ùÌìÑ©µØ'),
    ('»¢Ð¥¾÷', '°ËØÔÕÆ', 'ÈýÑæÖä', 'Íò½£¹é×Ú')
    );
