...
{$EXTERNALSYM LPFN_GETACCEPTEXSOCKADDRS}
  LPFN_GETACCEPTEXSOCKADDRS = procedure(lpOutputBuffer: Pointer;
    ...
    var LocalSockaddr: PSockAddr {TSockAddr}; var LocalSockaddrLength: Integer;
    var RemoteSockaddr: PSockAddr {TSockAddr}; var RemoteSockaddrLength: Integer); stdcall;
...
function CMSGDATA_ALIGN(const Alength: NativeUInt): NativeUInt;
...
function CMSG_SPACE(const Alength: NativeUInt): NativeUInt;
// #define CMSG_LEN WSA_CMSG_LEN
{$EXTERNALSYM CMSG_LEN}
function CMSG_LEN(const Alength: SIZE_T): SIZE_T;
//
{$EXTERNALSYM WSA_CMSGHDR_ALIGN}
function WSA_CMSGHDR_ALIGN(const Alength: NativeUInt): NativeUInt;
{$EXTERNALSYM WSA_CMSGDATA_ALIGN}
function WSA_CMSGDATA_ALIGN(const Alength: NativeUInt): NativeUInt;
...
function IP_MSFILTER_SIZE(const numsrc: DWORD): NativeUInt;
...
function GROUP_FILTER_SIZE(const numsrc: DWord): NativeUInt;
...
begin
  if hWinSockDll = 0 then begin
    ...
    raise Exception.Create('初始化Socket错误,错误编号：' + inttostr(LError));
  end;
end;
...
begin
  if hMSWSockDll = 0 then begin
    ...
    if hMSWSockDll = 0 then begin
      raise Exception.Create('加载Dll文件错误!');
    end;
  end;
end;
{$ENDIF}
...
begin
  ...
  if LStatus <> 0 then begin
    raise Exception.Create('加载函数' + AName + '出错！');
  end;
end;
...
begin
  ...
  begin
    if TSocket(FDSet.fd_array[i]) = ASocket then exit;
...
begin
  if cmsg = nil then begin
    ...
    if (NativeUInt(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len) + SIZE_WSACMSGHDR) > (NativeUInt(msg^.Control.buf) + msg^.Control.len) then begin
      Result := nil;
    end else begin
      Result := LPWSACMSGHDR(NativeUInt(cmsg) + WSA_CMSGHDR_ALIGN(cmsg^.cmsg_len));
    end;
  end;
end;
...
{$IFDEF USE_INLINE}inline; {$ENDIF}
begin
  Result := PByte(NativeUInt(cmsg) + WSA_CMSGDATA_ALIGN(SIZE_WSACMSGHDR));
end;

function WSA_CMSG_SPACE(const Alength: NativeUInt): NativeUInt;
{$IFDEF USE_INLINE}inline; {$ENDIF}
begin
  Result := WSA_CMSGDATA_ALIGN(NativeUInt(SIZE_WSACMSGHDR + WSA_CMSGHDR_ALIGN(Alength)));
end;
