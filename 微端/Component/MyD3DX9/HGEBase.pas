...
interface
uses
  Windows, SysUtils, Classes, DirectXD3D9;
...
function ColorXto32(Source: Pointer; SrcFmt: TColorFormat; mode:integer=0): Longword;
procedure Pixel32toX(Source: Longword; Dest: Pointer; DestFmt: TColorFormat);
procedure LineConvBiTto32(Source, Dest: Pointer; Count: Integer);
...
function ColorXto32(Source: Pointer; SrcFmt: TColorFormat; mode:integer): Longword;
const
 bAnd: array[0..8] of Longword = (0, 1, 3, 7, 15, 31, 63, 127, 255);
var
 Px                               :Longword;
 Bits                             :Word;
 RedBits, GreenBits, BlueBits     :Integer;
 Red, Green, Blue, Alpha          :Longword;
begin
 Px:= 0;
 ...
 Move(Source^, Px, Format2Bytes[SrcFmt]);
   ...
   case mode of
    ...
    2: Alpha := $000032;
    3: begin
      if Integer(Px) <> $FFFFFFFF then Alpha := $ffffff
      else  Alpha := $000000;
    end;
    4: begin
      if Integer(Px) > 0 then Alpha := $ffffff
      else  Alpha := $000000;
    end;
   end;
...
begin
 ...
 for i:= 0 to Count - 1 do
  begin
   Write^:= ColorXto32(Read, COLOR_A8R8G8B8);
   Inc(Write);
   Inc(Integer(Read), 4);
  end;
end;
