...
type
  ...
  //2018.04.14 增加地图更新回调函数
  TOnUpdateMap = procedure(sMap: string) of object;
  TOnUpdateMapComplete = procedure(sMap: string) of object;

  TMapStatus = (msNoLoad, msNormal, msLoading, msLoadOver, msNotFound);
  ...
  private
    FPath: TPath;
    FMapStatus:TMapStatus;
    FOnUpdateMap: TOnUpdateMap;
    FMapPath:string;
    FMapName:string;
    FOnUpdateMapComplete:TOnUpdateMapComplete;
  public
    MapHeader: TMapHeader;
    MapData: array[0..1000 * 1000 - 1] of TSdoMapInfo;//新动态地图
    ...
    procedure UpdateMapData(sMap:string; pData:PByte; dwMapSize:LongWord);
  protected
    function GetCost(X, Y, Direction: Integer): Integer; override;
  public
    property OnUpdateMap: TOnUpdateMap read FOnUpdateMap write FOnUpdateMap;
    property OnUpdateMapComplete: TOnUpdateMapComplete read FOnUpdateMapComplete write FOnUpdateMapComplete;
  end;
...
procedure TLegendMap.UpdateError;
begin
  FMapStatus:= msNotFound;
end;

procedure TLegendMap.UpdateMapData(sMap:string; pData:Pbyte; dwMapSize:LongWord);
var
  sLoadMap:string;
  MapStream:TFileStream;
begin
  sLoadMap:= ExtractFileName(FMapPath);
  if sMap = sLoadMap then begin
    MapStream := TFileStream.Create(FMapPath, fmCreate or fmOpenReadWrite or fmShareDenyNone);
    if MapStream <> nil then begin
      MapStream.WriteBuffer(pData^, dwMapSize);
      MapStream.Free;
      //重新加载地图数据
      LoadFileData(FMapPath);
      if Assigned(FOnUpdateMapComplete) then begin
        sMap:= ExtractFileNameNoExt(sMap);
        FOnUpdateMapComplete(sMap);
      end;
    end;
  end;
end;
...
procedure TLegendMap.LoadFileData(sFileName: string);    //新动态地图
var
  aMapFile: TFileStream;
...
begin
  if FileExists(sFileName) then begin
    ...
    try
      ...
      if (MapWidth <= 1000) and (MapHeight <= 1000) and (MapWidth > 0) and (MapHeight > 0) then begin
        ...
        if boNewMap  then begin
                 //新动态地图
           if MapHeader.Reserved[0] = #$2 then begin
           //新土城地图格式数据
              SetLength(GSMapData, MapWidth * MapHeight);
              aMapFile.Read(GSMapData[0], MapWidth * SizeOf(TMapInfo) * MapHeight);
              for I := Low(GSMapData) to High(GSMapData) do begin
                MapData[i].wBkImg := GSMapData[i].wBkImg;
                MapData[i].wMidImg := GSMapData[i].wMidImg;
                MapData[i].wFrImg := GSMapData[i].wFrImg;
                MapData[i].btDoorIndex := GSMapData[i].btDoorIndex;
                MapData[i].btDoorOffset := GSMapData[i].btDoorOffset;
                MapData[i].btAniFrame := GSMapData[i].btAniFrame;
                MapData[i].btAniTick := GSMapData[i].btAniTick;
                MapData[i].btArea := GSMapData[i].btArea;
                MapData[i].btlight := GSMapData[i].btlight;
                MapData[I].btBkIndex := GSMapData[i].btBkIndex;
                MapData[I].btSmIndex := GSMapData[i].btSmIndex;
                MapData[i].wXXXXXX1 := 0;
                MapData[i].wXXXXXX2:= 0;;
                MapData[i].cbXXXXXX3:= 0;
	              MapData[i].cbXXXXXX4:= 0;
	              MapData[i].cbXXXXXX5:= 0;
	              MapData[i].cbXXXXXX6:= 0;
	              MapData[i].cbXXXXXX7:= 0;
	              MapData[i].cbXXXXXX8:= 0;
	              MapData[i].cbXXXXXX9:= 0;
	              MapData[i].cbXXXXXX10:= 0;
	              MapData[i].cbXXXXXX11:= 0;
	              MapData[i].cbXXXXXX12:= 0;
	              MapData[i].cbXXXXXX13:= 0;
	              MapData[i].cbXXXXXX14:= 0;
                MapData[i].cbXXXXXX15:= 0;
	              MapData[i].cbXXXXXX16:= 0;
                MapData[i].cbXXXXXX17:= 0;
	              MapData[i].cbXXXXXX18:= 0;
	              MapData[i].cbXXXXXX19:= 0;
	              MapData[i].cbXXXXXX20:= 0;
              end;
              GSMapData := nil;
           end
          else begin
            //新盛大地图格式数据
           aMapFile.Read(MapData[0], MapWidth * SizeOf(TSdoMapInfo) * MapHeight);
          end;
        end else begin
        //旧盛大地图格式数据
    ...
    FMapPath:= sFileName;
    sMap:= ExtractFileName(sFileName);
    if Assigned(FOnUpdateMap) and (sMap <> '') and (FMapStatus = msNoLoad) then begin
      FMapStatus:= msLoading;
      FOnUpdateMap(sMap);
    end;
  end;
end;
