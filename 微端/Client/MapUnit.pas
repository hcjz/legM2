...
uses
  Windows, Classes, SysUtils, Math, FindMapPath, CShare;
...
//    m_sMapBase: string;
    m_boNewMap: Boolean;
    m_sFileName: string;
    m_MArr: array[0..MAXX * 3, 0..MAXY * 3] of TSdoMapInfo;      //–¬∂ØÃ¨µÿÕº
    m_boChange: Boolean;
    m_ClientRect: TRect;
...
begin
  ...
  for i := nLx to nRx - 1 do begin
    if (i >= 0) and (i < Header.wWidth) then begin
      Move(g_LegendMap.MapData[Header.wHeight * i + nTy], m_MArr[i - nLx, 0], Sizeof(TSdoMapInfo) * (nBy - nTy));    //–¬∂ØÃ¨µÿÕº
    end;
  end;
...
begin
  ...
  m_nBlockLeft := max(0, (cx - 1) * LOGICALMAPUNIT);
  m_nBlockTop := max(0, (cy - 1) * LOGICALMAPUNIT);
