...
uses
  ...
  StdCtrls, ExtCtrls, ComCtrls, VCLUnZip, IdHashMessageDigest, IdGlobal, IdHash;
...
begin
  g_UpgradeStep := uGetUpgrade;
  sStatusText := '';
  MemoryStream := nil;
  FrmMain.ProgressBarCurDownload.Percent := 0;
  FrmMain.ProgressBarAll.Percent := 0;
  sSelfFileName := ExtractFileName(g_sLoginTool);
  sSelfFileName_Bin := ChangeFileExt(sSelfFileName, '.bin');
          ...
          //  OutputDebugString(pchar(sRealDownLoadFile));
            ...
            //下载失败 得到文件md5
              try
                ...
                IdHTTGetMD5.Get(('http://bluedlq.oss-cn-hangzhou.aliyuncs.com/MD5.txt'), temp);
                ...
                //    OutputDebugString('down load yun');
                    ...
                    IdHTTPDownLoad.Get(('http://bluedlq.oss-cn-hangzhou.aliyuncs.com/gamemodule.dll'), MemoryStream);
                  except
                  end;
                end;
