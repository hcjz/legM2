...
uses GShare, HUtil32, Grobal2, GLoginServer,  EDcode, Share;
...
begin
  if (FindFirst(CurrentDir + SearchFileType, faAnyFile, SearchRec) = 0) then begin
    repeat
      if CurrentIsValidDir(SearchRec) = 0 then begin
        Inc(Number);
  ...
  if (FindFirst(CurrentDir + '*.*', faDirectory, SearchRec) = 0) then begin
    repeat
      if CurrentIsValidDir(SearchRec) = 1 then begin
  ...
  FindClose(SearchRec);
  for i := 0 to Subdir.Count - 1 do begin
...
begin
  ...
  if Copy(g_sWinrarFile, Length(g_sWinrarFile), 1) <> '\' then g_sWinrarFile := g_sWinrarFile + '\';
  ...
  MainOutMessage('LEGEND游戏数据引擎控制台启动成功...');
...
begin
  ...
  if MirServer.boGetStart then begin
...
begin
  ...
  if MirServer.boGetStart and (MirServer.btStartStatus in [2, 3]) then begin
    GetExitCodeProcess(MirServer.ProcessHandle, dwExitCode);
    if dwExitCode = STILL_ACTIVE then begin
      if MirServer.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
  ...
  try
    if FileExists(s) then begin
      ...
      for i := 0 to tlist.Count - 1 do begin
        l := tlist[i];
        if (l = '') or (l[1] = ';') or (l[1] = '/') or (l[1] = '[') or (l[1] = '#') then continue;
        //g_Config.sEnvirDir + 'UnMarry.txt';

        if (Pos(uppercase('LoadVar'), uppercase(l)) > 0) or
          (Pos(uppercase('SaveVar'), uppercase(l)) > 0) then begin
...
//(Pos(uppercase('ReadRandomLine'), uppercase(l)) > 0) or
        ...
        (Pos(uppercase('AddNameDateList'), uppercase(l)) > 0) or
          ...
          (Pos(uppercase('ClearNameList'), uppercase(l)) > 0) then begin
          l := GetValidStr3(l, c, [' ', #9]);
...
begin
  ...
  for i := 0 to CheckListBoxClear.Items.Count - 1 do begin
    if CheckListBoxClear.Selected[i] and FileExists(CheckListBoxClear.Items[i]) then begin
...
begin
  ...
  for i := 0 to CheckListBoxDel.Items.Count - 1 do begin
    if CheckListBoxDel.Selected[i] and FileExists(CheckListBoxDel.Items[i]) then begin
...
begin
  m := -1;
  for n := 0 to CheckListBoxClear.Items.Count - 1 do begin
    if CompareText(CheckListBoxClear.Items.Strings[n], s) = 0 then begin
      m := n;
      Break;
    end;
  end;
...
begin
  m := -1;
  for n := 0 to CheckListBoxDel.Items.Count - 1 do begin
    if CompareText(CheckListBoxDel.Items.Strings[n], s) = 0 then begin
      m := n;
      Break;
    end;
  end;
...
begin
  ...
  if Application.MessageBox('是否确认关闭游戏控制台？', '确认信息', MB_YESNO + MB_ICONQUESTION) = mrYes then begin
    CanClose := True;
  end else begin
    CanClose := False;
  end;
...
begin
  Result := False;
  MainOutMessage('正在启动LEGEND游戏引擎控制台...');
  ...
  MainOutMessage('LEGEND游戏引擎控制台启动完成...');
  Result := True;
end;
  ...
  try
    ...
    if CheckBox13.Checked then begin
      ...
      if DirectoryExists(s) then begin
        ...
        if FileExists(sCastleFile) then begin
          ...
          for i := 0 to LoadList.Count - 1 do begin
            ...
            if sCastleDir <> '' then begin
              ...
              if DirectoryExists(sCastleDir) then begin
    ...
    if CheckBox14.Checked then begin
      ...
      if DirectoryExists(GuildDir) then begin
    ...
    ProgressBarCur.Position := 0;
    for i := 0 to CheckListBoxClear.Items.Count - 1 do begin
      if FileExists(CheckListBoxClear.Items.Strings[i]) then begin
        LoadList2.Clear;
...
var
  s, s1, s2                 : string;
begin
  if CheckBoxBackupTimer.Checked then begin
    frmMain.Caption := 'LEGEND游戏控制台 ' + FormatDateTime('[yyyy-MM-dd dddd]', Now);
    s := IntToStr(SpinEditHour.Value) + ':' + IntToStr(SpinEditMin.Value);
    s1 := s + ':00';
    s2 := s + ':59';
    s := TimeToStr(Now);
    if RadioButtonWeek.Checked then begin
      if (s >= s1) and (s < s2) then
        BackUpFiles(Sender);
    end;
    if RadioButtonMin.Checked then begin
      if (GetTickCount() - InterTime) > (SpinEditMinInter.Value * 60000) then begin
        InterTime := GetTickCount();
        BackUpFiles(Sender);
      end;
    end;
...
begin
  if (Sender = CheckBox9) and CheckBox9.Checked then
    if not FileExists(EditWinrarFile.Text + 'Rar.exe') then begin
...
begin
  Dir := (EditIDDB.Text);
  if SelectDirectory('选择IDDB目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
    EditIDDB.Text := Dir + '\';
end;
...
begin
  Dir := (EditFDB.Text);
  if SelectDirectory('选择FDB目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
    EditFDB.Text := Dir + '\';
end;
...
begin
  Dir := (EditGuild.Text);
  if SelectDirectory('选择GuildBase目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
    EditGuild.Text := Dir + '\';
end;
...
begin
  Dir := (EditData.Text);
  if SelectDirectory('选择数据文件目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
    EditData.Text := Dir + '\';
end;
...
begin
  ...
  if SelectDirectory('选择大包裹目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
...
begin
  Dir := (EditSabuk.Text);
  if SelectDirectory('选择Castle目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
    EditSabuk.Text := Dir + '\';
end;
...
begin
  Dir := (EditBakDir.Text);
  if SelectDirectory('数据将备份到目录:', Folder, Dir, [sdNewUI, sdNewFolder], TWinControl(Self)) then
    EditBakDir.Text := Dir + '\';
end;
...
begin
  ...
  if SelectDirectory('选择自定义目录1:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
...
begin
  ...
  if SelectDirectory('选择自定义目录2:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
...
begin
  ...
  if SelectDirectory('选择传奇的基本目录:', Folder, Dir, [sdNewUI], TWinControl(Self)) then begin
    ...
    if DirectoryExists(Dir + 'Mir200\Envir') and
      ...
      FileExists(Dir + 'Mir200\!Setup.txt') then begin
...
begin
  ...
  if SelectDirectory('选择WinRar路径:', Folder, Dir, [sdNewUI], TWinControl(Self)) then
...
var
  i, c                      : Integer;
  s, fdir, TempDir, sBFile, sRarpath: string;
begin
  sRarpath := EditWinrarFile.Text;
  if Copy(sRarpath, Length(sRarpath), 1) <> '\' then sRarpath := sRarpath + '\';
  ...
  if CheckDir() then begin
    ...
    if bHint then MessageBox(0, PChar(LabelNotice.Caption), '提示信息', MB_ICONINFORMATION + MB_OK);
...
begin
  if Application.MessageBox('是否恢复默认设置？', '确认信息', MB_YESNO + MB_ICONQUESTION) = IDyes then begin
...
begin
  ShellAbout(Handle, PChar('LEGEND游戏引擎控制台 v1.05 By LEGEND'),
    PChar('版权所有 (C) 2009 LEGENDSOFT'), HICON(nil));
end;
...
begin
  ...
  if Module32First(ModuleList, FModuleEntry32) then begin
  ...
  for i := ModuleNameList.Count - 1 downto 0 do begin
    ModuleName := ModuleNameList.Strings[i];
    for ii := i - 1 downto 0 do begin
      if ModuleName = ModuleNameList.Strings[ii] then begin
