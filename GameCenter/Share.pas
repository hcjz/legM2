...
resourcestring
  g_sProductName            = '程序名称: 传奇数据备份工具';
  g_sVersion                = '程序版本: 1.00 Build 2019/01/25';
  g_sUpDateTime             = '更新日期: 2019/01/25';
  g_sProgram                = '程序制作: LEGEND';
  g_sWebSite                = '程序网站: http://www.Legendm2.com';
  g_sBbsSite                = '程序论坛: http://bbs.Legendm2.com';
...
begin
  ...
  if frmMain.CheckBox1.Checked then
    if not DirectoryExists(sIDDBDir) or (sIDDBDir[Length(sIDDBDir)] <> '\') then begin
  ...
  if frmMain.CheckBox2.Checked then
    if not DirectoryExists(sFDBDir) or (sFDBDir[Length(sFDBDir)] <> '\') then begin
  ...
  if frmMain.CheckBox3.Checked then
    if not DirectoryExists(sGuildDir) or (sGuildDir[Length(sGuildDir)] <> '\') then begin
  ...
  if frmMain.CheckBox4.Checked then
    if not DirectoryExists(sDataDir) or (sDataDir[Length(sDataDir)] <> '\') then begin
  ...
  if frmMain.CheckBox5.Checked then
    if not DirectoryExists(sBigBagSize) or (sBigBagSize[Length(sBigBagSize)] <> '\') then begin
  ...
  if frmMain.CheckBox6.Checked then
    if not DirectoryExists(sSabukDir) or (sSabukDir[Length(sSabukDir)] <> '\') then begin
  ...
  if frmMain.CheckBox7.Checked then
    if not DirectoryExists(sOtherSourceDir1) or (sOtherSourceDir1[Length(sOtherSourceDir1)] <> '\') then begin
  ...
  if frmMain.CheckBox8.Checked then
    if not DirectoryExists(sOtherSourceDir2) or (sOtherSourceDir2[Length(sOtherSourceDir2)] <> '\') then begin
      frmMain.LabelNotice.Caption := (s + '其他源备份目录(2)设置错误...');
      frmMain.EditOtherSourceDir2.SetFocus;
      Result := false;
      Exit;
    end;
  if not DirectoryExists(sBackupDir) or (sBackupDir[Length(sBackupDir)] <> '\') then begin
