        ...
        if FileExists(Config.sFeedIDList) then begin
          ...
          if AccountLoadList.Count > 0 then begin
            ...
            for I := 0 to AccountLoadList.Count - 1 do begin
              ...
              s18 := GetValidStr3(s18, s1C, [' ', #9]); //sid
              s18 := GetValidStr3(s18, s24, [' ', #9]); //sday
              s18 := GetValidStr3(s18, s28, [' ', #9]); //shour
              n10 := Str_ToInt(s24, 0);
              n14 := Str_ToInt(s28, 0);
              nC := MakeLong(_MAX(n10, 0), _MAX(n14, 0));
              AccountCostList.AddRecord(s1C, nC);
              if not boFastLoading then begin
