...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, StdCtrls, D7ScktComp;
...
type
  ...
  TFrmMasSoc = class(TForm)
    ...
    procedure MSocketClientError(Sender: TObject; Socket: TCustomWinSocket; ErrorEvent: TErrorEvent; var ErrorCode: Integer);
    ...
    if MsgServer.Socket = Socket then begin
      ...
      while (Pos(')', sReviceMsg) > 0) do begin
        ...
        case nCode of
          ...
          SS_CLOSESESSIONA: begin
              sMsg := GetValidStr3(sMsg, sAccount, ['/']);
              CloseUserA(Config, sAccount, Str_ToInt(sMsg, 0));
            end;
          SS_SERVERINFO: begin
...
begin
  try
    sLName := LimitName(sServerName);
    sSendMsg := Format(sFormatMsg, [wIdent, sMsg]);
    for I := 0 to m_ServerList.Count - 1 do begin
      MsgServer := pTMsgServerInfo(m_ServerList.Items[I]);
      if not MsgServer.Socket.Connected then Continue;
      if (sLName = '') or (MsgServer.sServerName = '') or
        (CompareText(MsgServer.sServerName, sLName) = 0) or
        (MsgServer.nServerIndex = 99) then
  ...
  if FileExists(sFileName) then begin
    ...
    for I := 0 to LoadList.Count - 1 do begin
      ...
      if sServerName <> '' then begin
        ...
        UserLimit[nC].nLimitCountMax := Str_ToInt(s14, 10000);
