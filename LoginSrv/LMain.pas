  ...
  try
    for I := 0 to Config.GateList.Count - 1 do begin
      ...
      if (GateInfo.Socket <> nil) then begin
        if (GetTickCount - GateInfo.dwConnectTick > g_nCloseLoginGateTime * 3600 * 1000) then begin
...
begin
  ...
  while (true) do begin
    ...
    if GetTickCount - LongWord(StringList_0.Objects[I]) > 60 * 1000 then begin
...
{$IFDEF SQLDB}
  ...
  if (AccountDB <> nil) and AccountDB.Open then begin
...
begin
  ...
  boOK := False;
  if FilterIPList.Count > 0 then begin
    for I := 0 to FilterIPList.Count - 1 do begin
      if CompareText(sRemoteIPaddr, FilterIPList[I]) = 0 then begin
        boOK := true;
        Break;
      end;
    end;
...
begin
  ...
  if (nSocketIndex >= 0) and (nSocketIndex < GATEMAXSESSION) and (sReviceMsg <> '') and ExecTimer.Enabled then begin
    if nReviceLen > 1000 then begin
...
begin
  ...
  try
    for I := Config.SessionList.Count - 1 downto 0 do begin
      ConnInfo := Config.SessionList.Items[I];
      if ConnInfo.boKicked and ((GetTickCount - ConnInfo.dwKickTick) > 3000) then begin
        Dispose(ConnInfo);
        Config.SessionList.Delete(I);
      end;
    end;
...
begin
  try
    ...
    case DefMsg.Ident of
      CM_SELECTSERVER: if not UserInfo.boSelServer then AccountSelectServer(Config, UserInfo, sData);
      ...
      CM_ADDNEWUSER: begin
          if Config.boEnableMakingID then begin
            if (GetTickCount - UserInfo.dwClientTick) > Config.dwAddNewUserTick then begin
              UserInfo.dwClientTick := GetTickCount();
      ...
      CM_CHANGEPASSWORD: begin
          if Config.boAllowChangePassword then begin
            if UserInfo.sAccount = '' then begin
              if (GetTickCount - UserInfo.dwClientTick) > Config.dwChgPassWordTick then begin
                UserInfo.dwClientTick := GetTickCount();
      ...
      CM_UPDATEUSER: begin
          if (GetTickCount - UserInfo.dwClientTick) > Config.dwUpdateUserInfoTick then begin
            UserInfo.dwClientTick := GetTickCount();
      ...
      CM_GETBACKPASSWORD: begin
          if Config.boGetbackPassword then begin
            if (sData = '') or (Length(sData) < 20) then begin
              //MainOutMessage('[攻击操作] IDDB攻击 /' + UserInfo.sUserIPaddr);
              Exit;
            end;
            if (GetTickCount - UserInfo.dwClientTick) > Config.GetBackPasswordTick then begin
              UserInfo.dwClientTick := GetTickCount();
...
begin
  try
    ...
    if (sLoginID = '') or (sNewPassword = '') or (Length(sLoginID) > 10) then begin
      DefMsg := MakeDefaultMsg(SM_CHGPASSWD_FAIL, 0, 0, 0, 0);
      SendGateMsg(UserInfo.Socket, UserInfo.sSockIndex, EncodeMessage(DefMsg));
      Exit;
    end;
    nCode := 0;
    try
      if (Length(sNewPassword) >= 3) and AccountDB.Open then begin
        n10 := AccountDB.Index(sLoginID);
        if (n10 >= 0) and (AccountDB.Get(n10, DBRecord) >= 0) then begin
          if (DBRecord.nErrorCount < 5) or ((GetTickCount - DBRecord.dwActionTick) > 3 * 60 * 1000) then begin
  ...
  try
    ...
    try
      if AccountDB.Open then begin
        ...
        if (n10 >= 0) and (AccountDB.Get(n10, DBRecord) >= 0) then begin
          if (DBRecord.nErrorCount < 5) or ((GetTickCount - DBRecord.dwActionTick) > 30 * 1000) then begin
...
begin
  ...
  try
    for I := Config.SessionList.Count - 1 downto 0 do begin
      ConnInfo := Config.SessionList.Items[I];
      if not ConnInfo.boKicked and not Config.boTestServer and not ConnInfo.boCheckOK then begin
...
begin
  ...
  //Memo1.Lines.Add(sUpdate);
  Memo1.Lines.Add(sWebSite);
  //Memo1.Lines.Add(SBBsSite);
...
begin
  ...
  while (true) do begin
    ...
    Dispose(UserData);
    if (GetTickCount - dwProcessReviceMsgLimiTick) > 30 then Break;
  end;
end;
...
begin
  ...
  if (UserData.nSocketIdx >= 0) and (UserData.nSocketIdx < GATEMAXSESSION) then begin
    if (UserData.nSocketHandle = SessionArray[UserData.nSocketIdx].nSckHandle) and (SessionArray[UserData.nSocketIdx].nPacketErrCount < 15) and (SessionArray[UserData.nSocketIdx].Socket <> nil) then begin
      ...
      while (true) do begin
        ...
        if nDataLen > 1 then begin
          if (nDataLen >= DEFBLOCKSIZE) then begin
            ...
            DefMsg := DecodeMessage(sDefMsg);
            if DefMsg.Recog <> g_Config.CLPwd then begin
