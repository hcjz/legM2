...
begin
  ...
  if not DirectoryExists(sIdDir) or (sIdDir[Length(sIdDir)] <> '\') then begin
    Application.MessageBox('IdDir目录设置错误！', '错误信息', MB_OK + MB_ICONERROR);
    EditIdDir.SetFocus;
    Exit;
  end;
  if not DirectoryExists(sWebLogDir) or (sWebLogDir[Length(sWebLogDir)] <> '\') then begin
    Application.MessageBox('WebLogDir目录设置错误！', '错误信息', MB_OK + MB_ICONERROR);
    EditWebLogDir.SetFocus;
    Exit;
  end;
  if not DirectoryExists(sCountLogDir) or (sCountLogDir[Length(sCountLogDir)] <> '\') then begin
    Application.MessageBox('CountLogDir目录设置错误！', '错误信息', MB_OK + MB_ICONERROR);
    EditCountLogDir.SetFocus;
    Exit;
  end;
  if not DirectoryExists(sChrLogDir) or (sChrLogDir[Length(sChrLogDir)] <> '\') then begin
  ...
  if Config.CLOpen <> frmMain.ServerSocket.Active then begin
...
begin
  if Application.MessageBox('是否确认恢复默认设置?', '确认信息', MB_ICONINFORMATION + MB_YESNO) = IDNO then Exit;
