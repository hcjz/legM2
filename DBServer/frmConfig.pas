...
begin
  ...
  if g_nAllowDelChrLvl > High(Word) then begin
    g_nAllowDelChrLvl := High(Word);
    SpinEditAllowDelChrLvl.Value := g_nAllowDelChrLvl;
  end;
  if g_nAllowDelChrLvl < 1 then begin
    g_nAllowDelChrLvl := 1;
    SpinEditAllowDelChrLvl.Value := g_nAllowDelChrLvl;
  end;
  MobValue();
...
begin
  if CheckBoxMultiChr = Sender then begin
...
begin
  if (ListBoxFilterText.ItemIndex >= 0) and (ListBoxFilterText.ItemIndex <
    ListBoxFilterText.Items.Count) then begin
