...
uses
  Windows, Messages, Classes, SysUtils, IniFiles, Grobal2, MudUtil, D7ScktComp, SDK, tlhelp32, PsAPI, CnHashTable;
...
var
  g_sSQLDatabase            : string = 'LEGENDOFMIR';
  g_sSQLHost                : string = '127.0.0.1';
  g_sSQLUserName            : string = 'legendm2';
  ...
  sWellCome                 : string = '欢迎使用LEGEND系统软件...';
  sProgram                  : string = '程序制作：QQ:8302775';
  sOwen                     : string = 'LEGEND';
  sUpdate                   : string = '更新日期：2019/01/25';
  sVersion                  : string = '程序版本：Ver 1.00 Build 2019/01/25';
  sWebSite                  : string = '程序网站：http://www.legendm2.com';
  sBbsSite                  : string = '程序论坛：http://www.legendm2.com';
...
begin
  Result := False;
  L := Length(AHzStr);
  if not g_boUseSpecChar then begin
    i := 1;
    while L >= i do begin
      if (AHzStr[i] >= #$A0) and (L > i) and (AHzStr[i + 1] >= #$A0) then begin
  ...
  while L >= i do begin
    if (AHzStr[i] >= #$A0) and (L > i) and (AHzStr[i + 1] >= #$A0) then begin
      ...
      if not IsChs then begin
        ...
        if Pos(ChsChar, g_SpaceCharList) > 0 then begin
          Result := True;
          Exit;
        end;
      end;
...
begin
  for i := Low(g_aLevelRankList) to High(g_aLevelRankList) do begin
    for ii := g_aLevelRankList[i].Count - 1 downto 0 do begin
...
var
  i, ii                     : Integer;
begin
  if List.Count > 2000 then begin
    for ii := List.Count - 1 downto 2000 do begin
...
begin
  ...
  for i := 1 to Length(sChrName) do begin
    case ByteType(sChrName, i) of
      ...
      mbLeadByte: begin
          Inc(n, 2);                    //汉字的第一个字节
          if i = Length(sChrName) then begin
            Result := False;
            Exit;
          end;
        end;
  ...
  if n <> Length(sChrName) then begin
    Result := False;
    Exit;
  end;
...
{$I Lic.inc}
      if (UpperCase(ExtractFileName(FProcessEntry32.szExeFile)) = UpperCase(spName)) or (UpperCase(FProcessEntry32.szExeFile) = UpperCase(spName)) then begin
{$I Lic.inc}
...
{$I Lic.inc}
  Result := False;
{$I Lic.inc}
  if sProcessName = sM2server then
    szExePath := 'D:\MirServer\Mir200\'
  else
    szExePath := ExtractFilePath(ParamStr(0));
...
{$I Lic.inc}
        if LowerCase(Trim(ExtractFilePath(FModuleEntry32.szExePath))) = LowerCase(szExePath) then begin
{$I Lic.inc}
