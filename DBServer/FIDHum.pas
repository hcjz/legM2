...
begin
  ...
  if sChrName = '' then
    Exit;
  if MessageBox(0, PChar('是否确认删除人物 ' + sChrName + ' ?'), '确认信息', MB_ICONQUESTION + mb_YesNo) = ID_Yes then begin
    try
...
begin
  ...
  if sChrName = '' then
    Exit;
  if MessageBox(0, PChar('是否确认禁用人物 ' + sChrName + ' ?'), '确认信息', MB_ICONQUESTION + mb_YesNo) = ID_Yes then begin
    try
      if HumChrDB.Open then begin
...
begin
  ...
  if sChrName = '' then
    Exit;
  if MessageBox(0, PChar('是否确认启用人物 ' + sChrName + ' ?'), '确认信息', MB_ICONQUESTION + mb_YesNo) = ID_Yes then begin
    try
      if HumChrDB.Open then begin
        nIndex := HumChrDB.Index(sChrName);
        HumChrDB.Get(nIndex, HumRecord);
...
begin
  ...
  if sChrName = '' then
    Exit;
  if MessageBox(0, PChar('是否确认删除人物 ' + sChrName + ' 及人物数据？'), '确认信息', MB_ICONQUESTION + mb_YesNo) = ID_Yes then begin
    try
...
begin
  ...
  if MessageBox(0, PChar('是否确认禁用记录 ' + IntToStr(nIndex) + ' ？'), '确认信息', MB_ICONQUESTION + mb_YesNo) = ID_Yes then begin
    try
      if HumChrDB.Open then begin
