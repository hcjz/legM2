...
interface
uses
  Windows, Classes, SysUtils, ActiveX, DBTables, Grobal2;
  ...
  try
    try
      ...
      for i := 0 to Query.RecordCount - 1 do begin
        ...
        Magic.btclass := 0;
        if CompareText(Magic.sDescr, 'Ó¢ÐÛ') = 0 then
          Magic.btclass := 0
        else if CompareText(Magic.sDescr, 'Å­Ö®') = 0 then
          Magic.btclass := 1
        else if CompareText(Magic.sDescr, '¾²Ö®') = 0 then
          Magic.btclass := 2;
