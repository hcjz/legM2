...
uses
  ...
  Buttons, Menus, Grobal2, D7ScktComp, ComCtrls, Grids, ipinfo_dll;
type
  ...
  try
    ...
    if CreateProcess(nil, CmdLine, nil, nil, True, NORMAL_PRIORITY_CLASS, nil, nil, SI, Pi) then begin
      ...
      while ExitCode = 0 do begin
        ...
        if PeekNamedPipe(hReadPipe, @Dest[0], SizeOf(Dest), @Avail, nil, nil) then begin
...
begin
{$I License.inc}
  sIPaddr := sIPlocalDLL + ' ' + sPlugOfScript + ' ' + sPlugOfEngine + ' ' + sSystemModule + ' ' + sPlugOfAccess;
{$I License.inc}
  if IsHookDll(sM2server, sIPaddr) then begin
{$I License.inc}
    Exit;
{$I License.inc}
  end;
...
{$I License.inc}
  sIPaddr := ' ';
{$I License.inc}
  if IsHookDll(ExtractFileName(Application.ExeName), sIPaddr) then begin
{$I License.inc}
    Exit;
{$I License.inc}
  end;
...
begin
{$I License.inc}
  sStr := sIPlocalDLL + ' ' + sPlugOfScript + ' ' + sPlugOfEngine + ' ' + sSystemModule + ' ' + sPlugOfAccess;
{$I License.inc}
  if IsHookDll(sM2server, sStr) then begin
{$I License.inc}
    Exit;
{$I License.inc}
  end;
...
{$I License.inc}
  sStr := ' ';
{$I License.inc}
  if IsHookDll(ExtractFileName(Application.ExeName), sStr) then begin
{$I License.inc}
    Exit;
{$I License.inc}
  end;
    ...
    if ServerInfo.nSckHandle = Socket.SocketHandle then begin
      ...
      if sMsg <> '' then begin
        ...
        if Pos('!', sMsg) > 0 then begin
          ...
          Inc(m_nServerPacket);
          Break;
        end else if Length(ServerInfo.sStr) > 81920 then begin
          ServerInfo.sStr := '';
          Inc(n4ADC2C);
        end;
      end;
...
{$I License.inc}
  sStr := sIPlocalDLL + ' ' + sPlugOfScript + ' ' + sPlugOfEngine + ' ' + sSystemModule + ' ' + sPlugOfAccess;
{$I License.inc}
  if IsHookDll(ExtractFileName(Application.ExeName), ' ') or IsHookDll(sM2server, sStr) then begin
{$I License.inc}
    sRemovedIP := sIP;
{$I License.inc}
  end else if g_sRegUserName <> sRemovedIP then begin
{$I License.inc}
    sRemovedIP := sIP;
{$I License.inc}
  end;
...
{$I License.inc}
  ...
  //MainOutMessage('sRemovedIP:' + sRemovedIP + '  g_sRegUserName:' + g_sRegUserName);
{$I License.inc}
end;
...
{$ELSE}
  try
    if HumDataDB.Open then begin
      ...
      if FileRead(HumDataDB.m_nFileHandle, DBHeader, SizeOf(TDBHeader)) = SizeOf(TDBHeader) then begin
        for nIndex := 0 to DBHeader.nHumCount - 1 do begin
          ...
          if FileRead(HumDataDB.m_nFileHandle, HumData, SizeOf(THumDataInfo)) <> SizeOf(THumDataInfo) then
            Break;
          if not HumData.Header.boDeleted and (HumData.Data.Abil.Level >= g_dwRankLevelMin) then begin
            if HumData.Data.sHeroMasterName <> '' then begin
              if AdminChrNameList.Exists(HumData.Data.sHeroMasterName) then Continue;
              ...
              if AdminChrNameList.Exists(HumData.Data.sChrName) then Continue;
              New(pLR);
  ...
  for i := Low(g_aLevelRankList) to High(g_aLevelRankList) do begin
    List := g_aLevelRankList[i];
    if i in [4..7] then begin
    ...
    end else if i in [0..3] then begin
      if List.Count > 0 then begin
        List.Lock;
        try
    ...
    try
      if HumDataDB.OpenEx then begin
        ...
        if nIndex >= 0 then begin
          if FrmUserSoc.QueryHeroChr(sAccount, sHumName, sIPaddr, nSessionID, HerosInfo) then begin
            nCheckCode := 1;
          end;
        end else
          nCheckCode := -3;
...
begin
  ...
  if (sAccount <> '') and (sHumName <> '') then begin
    ...
    if (sIPaddr = '@HERO2@') then begin
      nCheckCode := 3;
    end else if (sIPaddr = g_sAutoLogin) then begin
      nCheckCode := 1;
    end else begin
  ...
  if nCheckCode in [1, 2, 3] then begin
    try
      if HumDataDB.OpenEx then begin
        nIndex := HumDataDB.Index(sHumName);
        if nIndex >= 0 then begin
...
begin
  ...
  if (sHeroFlag <> '') and ((sHeroFlag = sCreateHeroFlag) or (sHeroFlag = sCreateHeroFlag2)) then begin
    nc := -1;
    ...
    if (nc = -1) then begin
      for i := 1 to Length(sChrName) do begin
        if IsFilterChar(sChrName[i]) then begin
          nc := 0;
          Break;
        end;
      end;
...
{$IFEND}
  ...
  if FileExists(sStr) then begin
    ...
    if nHandle > 0 then begin
      ...
      sStr := Header.sDesc;
      //'LEGENDM2 120615'
      if (StrLIComp(PChar(sStr), PChar(sDBHeaderDesc), Length(sDBHeaderDesc)) <> 0) then begin
...
begin
  sStr := sIPlocalDLL + ' ' + sPlugOfScript + ' ' + sPlugOfEngine + ' ' + sSystemModule + ' ' + sPlugOfAccess;
  if not IsHookDll(sM2server, sStr) and not IsHookDll(ExtractFileName(Application.ExeName), ' ') then begin
...
{$ELSE}
      ...
      HumDataDB := TFileDB.Create(sDataDBFilePath + 'Mir.DB');
{$ENDIF}
      if g_boOpenHRSystem then begin
...
begin
  if boHumDBReady and boDataDBReady then begin
    FrmIDHum.Top := Top + 20;
    FrmIDHum.Left := Left;
    FrmIDHum.Open;
  end;
...
begin
  ...
  MainOutMessage(sVersion);
  MainOutMessage(sWebSite);
  MainOutMessage(sBbsSite);
  IsHookDll(ExtractFileName(Application.Title), ' ');
end;
...
begin
  if Application.MessageBox('是否清理提示日志信息？', '提示信息', MB_IconInformation + MB_YesNo) = IDNO then Exit;
  MemoLog.Lines.Clear;
end;
...
begin
  try
    if g_boOpenHRSystem then begin
      if GetTickCount - g_dwRenewLevelRankList > g_dwRenewLevelRankTime * 60 * 1000 then begin
        g_dwRenewLevelRankList := GetTickCount;
        Timer1.Enabled := True;
      end;
    end;

    if g_boCloseSelGate then begin
      FrmUserSoc.CS_GateSession.Enter;
      try
        for ii := Low(g_GateUserSocArr) to High(g_GateUserSocArr) do begin
          GateInfo := @g_GateUserSocArr[ii];
          if GateInfo.boUsed and (GateInfo.Socket <> nil) then begin
            if (GetTickCount - GateInfo.dwConnectTick > g_nCloseSelGateTime * 3600 * 1000) then begin
    ...
    for i := 0 to GridGate.RowCount - 1 do begin
      ...
      try
        for ii := Low(g_GateUserSocArr) to High(g_GateUserSocArr) do begin
          ...
          if GateInfo.boUsed and (GateInfo.Socket <> nil) then begin
            ...
            if dwCheckUserSocTimeMax > 1 then Dec(dwCheckUserSocTimeMax);
      ...
      if FrmIDSoc.IDSocket.Socket.Connected then begin
        ...
        if dwCheckIDSocTimeMax > 1 then Dec(dwCheckIDSocTimeMax);
      ...
      if ServerList.Count > 0 then begin
        ...
        for ii := 0 to ServerList.Count - 1 do begin
          ServerInfo := ServerList.Items[ii];
          if ServerInfo.Socket.SocketHandle > 0 then begin
    ...
    if boOpenDBBusy then begin
      sCaption := Caption;
      if (n4ADB18 > 0) and not bo4ADB1C then {Label4.} Caption := '[1/4] ' + IntToStr(Round((n4ADB10 / n4ADB18) * 1.0E2)) + '% ' + IntToStr(n4ADB14) + '/' + IntToStr(n4ADB18);
      if (n4ADB04 > 0) and not boHumDBReady then {Label4.} Caption := '[3/4] ' + IntToStr(Round((n4ADAFC / n4ADB04) * 1.0E2)) + '% ' + IntToStr(n4ADB00) + '/' + IntToStr(n4ADB04);
      if (n4ADAF0 > 0) and not boDataDBReady then {Label4.} Caption := '[4/4] ' + IntToStr(Round((n4ADAE4 / n4ADAF0) * 1.0E2)) + '% ' + IntToStr(n4ADAE8) + '/' + IntToStr(n4ADAEC) + '/' + IntToStr(n4ADAF0);
      Caption := sCaption;
    end;
    ...
    on E: Exception do MainOutMessage(E.Message);
  end;
end;
    ...
    if nType in [4..7] then begin
      ...
      try
        for i := 0 to List.Count - 1 do begin
          pHLR := List.Items[i];
          if CompareText(pHLR.sMasterName, s) = 0 then begin
            j := i;
            boFind := True;
            Break;
          end;
      ...
      if boFind then begin
        ...
        List.Lock;
        try
          for i := (j div 10 * 10) to List.Count - 1 do begin
            pHLR := List.Items[i];
      ...
      try
        for i := 0 to List.Count - 1 do begin
          pLR := List.Items[i];
          if CompareText(pLR.sCharName, s) = 0 then begin
            j := i;
            boFind := True;
            Break;
          end;
    ...
    if nType in [4..7] then begin
      ...
      try
        if List.Count >= n then begin
          ...
          SendSocket(Socket, EncodeMessage(m_DefMsg) + EncodeBuffer(@HLR, SizeOf(THeroLevelRanks)));
        end else if List.Count > j then begin
          for i := j to List.Count - 1 do begin
            pHLR := List.Items[i];
            HLR[i mod 10].sHeroName := pHLR.sHeroName;
            HLR[i mod 10].sMasterName := pHLR.sMasterName;
            HLR[i mod 10].nLevel := pHLR.nLevel;
          ...
          m_DefMsg := MakeDefaultMsg(DBR_LOADRANKDATA, nPage, nType, 0, 0);
          SendSocket(Socket, EncodeMessage(m_DefMsg) + EncodeBuffer(@HLR, SizeOf(THeroLevelRanks)));
        end else if List.Count > 0 then begin
          n := 0;
