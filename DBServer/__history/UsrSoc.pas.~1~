...
begin
  try
    ...
    if not boOpenDBBusy then begin
      ...
      try
        for i := Low(g_GateUserSocArr) to High(g_GateUserSocArr) do begin
...
begin
  CS_GateSession.Enter;
  try
    for GateIdx := Low(g_GateUserSocArr) to High(g_GateUserSocArr) do begin
      GateInfo := @g_GateUserSocArr[GateIdx];
      if (GateInfo <> nil) and (GateInfo.Socket = Socket) then begin
...
begin
  ...
  try
    for GateIdx := Low(g_GateUserSocArr) to High(g_GateUserSocArr) do begin
      GateInfo := @g_GateUserSocArr[GateIdx];
      if GateInfo.boUsed and (GateInfo.Socket = Socket) then begin
        CurGate := GateInfo;
...
begin
  ...
  try
    if (nSex in [0, 1]) and (nJob in [0..2]) and HumDataDB.Open and (HumDataDB.Index(sChrName) = -1) then begin
...
begin
  ...
  for i := 0 to LoadList.Count - 1 do begin
    ...
    if (sLineText <> '') and (sLineText[1] <> ';') then begin
      ...
      if (sIdx = '') or (sLineText = '') then Continue;
...
begin
  ...
  while (True) do begin
    ...
    if UserInfo.sProcessMsg = '' then Break;
  end;
end;
...
begin
  ...
  case Msg.Ident of
    CM_QUERYCHR: begin
        if not UserInfo.boChrQueryed or ((GetTickCount - UserInfo.dwChrTick) > 50) then begin
          UserInfo.dwChrTick := GetTickCount();
    ...
    CM_SELCHR: begin
        if not UserInfo.boChrSelected then begin
          if (UserInfo.sAccount <> '') and FrmIDSoc.CheckSession(UserInfo.sAccount, UserInfo.sUserIPaddr, UserInfo.nSessionID) then begin
            if SelectChr(sMsg, UserInfo) then begin
    ...
    CM_QUERYDELCHR: begin
        if g_boAllowGetBackDelChr then begin
          if (GetTickCount - UserInfo.dwChrTick) > 280 then begin
            UserInfo.dwChrTick := GetTickCount();
...
{$I License.inc}
      ...
      *)
    CM_GETBACKDELCHR: begin
        if g_boAllowGetBackDelChr then begin
          if (GetTickCount - UserInfo.dwChrTick) > 120 then begin
            UserInfo.dwChrTick := GetTickCount();
        ...
        try
          if HumDataDB.OpenEx then begin
            for i := 0 to ChrList.Count - 1 do begin
              ...
              if QuickID.nSelectID <> UserInfo.nSelGateID then Continue; //强行登陆
              if HumChrDB.GetBy(QuickID.nIndex, HumRecord) and (not HumRecord.boDeleted) then begin
                ...
                if (nIndex < 0) or (nChrCount >= (2 + Integer(g_boAllowMultiChr))) then Continue;
                ...
                if HumDataDB.Get(nIndex, ChrRecord) >= 0 then begin
                  ...
                  btSex := 0;
                  if not (ChrRecord.Data.btSex in [0, 1]) then begin
                    ChrRecord.Data.btSex := ChrRecord.Data.btSex mod 2;
                    btSex := 1;
                  end;
                  if not (ChrRecord.Data.btJob in [0..2]) then begin
...
begin
  boCheck := False;
  boDelLvl := False;
  if not g_boAllowDelChr then goto FailExit;
          ...
          try
            if HumDataDB.OpenEx then begin
              ...
              if nIndex >= 0 then begin
                ...
                if ChrRecord.Data.Abil.Level >= g_nAllowDelChrLvl then
                  boDelLvl := True;
              end;
            end;
          finally
...
begin
  ...
  if nCode = -1 then begin
    ...
    try
      if HumChrDB.Open then begin       //if HumChrDB.ChrCountOfAccount(sAccount) < 2 then
...
begin
  ...
  if UserInfo.sAccount = sAccount then begin
        ...
        if HumChrDB.FindByAccount(sAccount, ChrList) >= 0 then begin
          for i := 0 to ChrList.Count - 1 do begin
            ...
            if HumChrDB.GetBy(nIndex, HumRecord) then begin
              ...
              end else if HumRecord.boSelected then begin
    ...
    try
      if {boCharOK and} HumDataDB.OpenEx then begin
        nIndex := HumDataDB.Index(sChrName);
        if nIndex >= 0 then begin
          HumDataDB.Get(nIndex, ChrRecord);
...
begin
  ...
  sSendMsg := '';
  nDelHumCount := 0;
  if FrmIDSoc.CheckSession(UserInfo.sAccount, UserInfo.sUserIPaddr, UserInfo.nSessionID) then begin
    ChrList := TStringList.Create;
    try
      if HumChrDB.Open then begin
        if HumChrDB.FindByAccount(UserInfo.sAccount, ChrList) >= 0 then begin
          try
            if HumDataDB.OpenEx then begin
              for i := 0 to ChrList.Count - 1 do begin
                QuickID := pTQuickID(ChrList.Objects[i]);
                if QuickID.nSelectID = UserInfo.nSelGateID then begin
                  if HumChrDB.GetBy(QuickID.nIndex, HumRecord) then begin
                    if HumRecord.boDeleted then begin
                      ...
                      if nIndex >= 0 then begin
                        if HumDataDB.Get(nIndex, ChrRecord) >= 0 then begin
                          if ChrRecord.Data.sHeroMasterName = '' then begin
...
begin
  ...
  if sChrName <> '' then begin
    try
      if HumDataDB.OpenEx then begin
        ...
        if nIndex2 >= 0 then begin
          if HumDataDB.Get(nIndex2, ChrRecord) >= 0 then begin
            if ChrRecord.Data.sHeroMasterName <> '' then begin
              tIsHero := True;
            end;
          end else
    ...
    if tIsHero then begin
      nRetCode := 4;
    end else begin
      try
        if HumChrDB.Open then begin
          nIndex := HumChrDB.Index(sChrName);
          if (nIndex >= 0) and (HumChrDB.Get(nIndex, HumRecord) <> -1) then begin
            if (HumRecord.sAccount = UserInfo.sAccount) then begin
              if HumChrDB.ChrCountOfAccount(HumRecord.sAccount) < (2 + Integer(g_boAllowMultiChr)) then begin
                if HumRecord.boDeleted then begin
  ...
  if nRetCode = 1 then Result := True;
end;
...
begin
  ...
  for i := Low(g_RouteInfo) to High(g_RouteInfo) do begin
    ...
    if (RouteInfo.nServerIdx = nSerIdx) and (RouteInfo.sSelGateIP = sGateIP) then begin
...
begin
  Result := True;
  if not g_boAllowCreateCharOpt1 then begin
    List := TStringList.Create;
    for i := $30 to $39 do List.Add(Char(i));
    for i := $61 to $7A do List.Add(Char(i));
    for i := 0 to List.Count - 1 do begin
      if Pos(List.Strings[i], sChrName) > 0 then begin
        Result := False;
        List.Free;
        Exit;
      end;
  ...
  for i := 0 to DenyChrOfNameList.Count - 1 do begin
    if Pos(DenyChrOfNameList.Strings[i], sChrName) > 0 then begin
      Result := False;
      Break;
    end;
  end;
...
begin
  ...
  if not g_fMutiHero then begin
    Exit;
  end;

  if FrmIDSoc.CheckSession(sAccount, sIPaddr, nSessionID) then begin
    //FrmIDSoc.SetGlobaSessionNoPlay(nSessionID);
    ChrList := TStringList.Create;
    try
      if HumChrDB.Open and (HumChrDB.FindByAccount(sAccount, ChrList) >= 0) then begin
              ...
              if HumChrDB.GetBy(QuickID.nIndex, HumRecord) then begin
                ...
                if HumDataDB.Get(nIndex, ChrRecord) >= 0 then begin
                  if ChrRecord.Data.sHeroMasterName = '' then begin
                    ...
                    //英雄
                    if not g_cbGetAllHeros then begin
