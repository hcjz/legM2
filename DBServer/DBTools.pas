...
{$ELSE}
      ...
      if FileRead(HumDataDB.m_nFileHandle, DBHeader, SizeOf(TDBHeader)) = SizeOf(TDBHeader) then begin
        ...
        for nIndex := 0 to DBHeader.nHumCount - 1 do begin
          ...
          if FileRead(HumDataDB.m_nFileHandle, HumData, SizeOf(THumDataInfo)) <> SizeOf(THumDataInfo) then
            Break;
          if not HumData.Header.boDeleted and (HumData.Data.Abil.Level >= lmin) and (HumData.Data.Abil.Level <= lmax) then begin
            if (HumData.Data.sHeroMasterName = '') and (HumData.Data.sAccount <> '') then begin
              if hList.IndexOf(HumData.Data.sAccount) < 0 then begin
  ...
  if (List.Count > 0) and DirectoryExists('..\Mir200\Envir') then begin
...
begin
  ...
  try
    if HumDataDB.Open and HumChrDB.Open then begin
      ...
      for i := 0 to HumDataDB.m_MirQuickList.Count - 1 do begin
        nProcID := i;
