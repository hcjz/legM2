...
uses
  Classes, Windows, DirectXGraphics,D3DX81mo;
...
type
  ...
  private
    ...
    //增加麻痹效果支持
    FPixelShaderHandle: DWord;
    FPixelShaderBuf : ID3DXBuffer;
    mmmmmmmz:string;
...
begin
  inherited;
   D3DXAssembleShader(ShaderCode, Length(ShaderCode), 0, nil, @FPixelShaderBuf, nil);
  FActive := False;
...
// Set up Full Screen presentation parameters
  ...
  FD3DPPFS.SwapEffect       := D3DSWAPEFFECT_COPY;
...
begin
  Result := False;
   //增加麻痹效果
  if (FPixelShaderHandle <> 0) then
      FD3DDevice.DeletePixelShader(FPixelShaderHandle);
  FPixelShaderHandle:= 0;

  if FD3DDevice <> nil then
    FD3DDevice.CreatePixelShader(FPixelShaderBuf.GetBufferPointer, FPixelShaderHandle);
...
procedure THGEImpl.GetViewport(out x, y, Width, Height: Integer);
var
  vp: TD3DViewport8;
begin
  if (FD3DDevice = nil) then
...
procedure THGEImpl.SetViewport(x, y, Width, Height: Integer);
var
  vp: TD3DViewport8;
begin
...
begin
  ...
  if Blend_GrayScale = Blend then begin
    with FD3DDevice do
      begin
    ...
    FD3DDevice.SetPixelShader(FPixelShaderHandle);
  end else begin
    FD3DDevice.SetPixelShader(0);
    case Blend of
       Blend_Default:
       begin
         FD3DDevice.SetRenderState(D3DRS_SRCBLEND, D3DBLEND_SRCALPHA);
