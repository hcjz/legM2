...
uses
  Windows, Graphics, SysUtils, DirectXGraphics, HGEBase, HGE, HGETextures, HGEFonts, HGEFont;
...
type
  ...
  public
    ...
    procedure DrawEx(Texture: TDXTexture; X, Y, CenterX, CenterY, ScaleX, ScaleY: Single; MirrorX, MirrorY: Boolean; Color: Cardinal; BlendMode: Integer);
    ...
    procedure DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height,
      ScaleX, ScaleY, CenterX, CenterY: Single; MirrorX, MirrorY: Boolean; Color: Cardinal; BlendMode: Integer); overload;
    procedure DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height: Single;
  ...
  public
    ...
    function TextWidth(Str: WideString; NewFont: Boolean = True): Integer;
    function TextHeight(Str: WideString; NewFont: Boolean = True): Integer;
    ...
    procedure TextRectR(Rect: TRect; Text: WideString; FColor: Cardinal; TextFormat: TTextFormat = []);
    procedure TextRectX(Rect: TRect; x, Y: Integer;Text: WideString; FColor: Cardinal; TextFormat: TTextFormat = []; NewFont: Boolean = True);
    ...
    procedure BoldTextOutZ( X, Y, Z, fcolor: Integer; Str: string);overload; //鉴宝文字显示换行 2019-01-27 Development
    procedure SkidADTextOut(x, Y: Integer; Str: WideString; FColor: Cardinal; BColor: Cardinal);
    procedure TextOutR(x, Y: Integer; Str: WideString; FColor: Cardinal; BColor: Cardinal);
    procedure FillRectAlpha(const Rect: TRect; DevColor: Cardinal; Alpha: Byte);
    procedure TextOutX(x, Y: Integer; Str: WideString; FColor, BColor: Cardinal);
  end;

var
  g_DXCanvas: TDXDrawCanvas = nil;
  //FreeTextInfoLock:TRTLCriticalSection; //添加空闲列表的缓冲区
...
procedure TextOutBoldR(Canvas: TCanvas; {x, y, }fcolor, bcolor: integer; str: string); //飘血描边
var
  nLen: Integer;
begin
  nLen := Length(str);
  ...
  {
  Canvas.Brush.Color := Tcolor($00050505);
  Windows.SetBkMode(Canvas.Handle, TRANSPARENT);
  Windows.SetBkColor(Canvas.Handle, Canvas.Brush.Color);
  Windows.SetTextColor(Canvas.Handle, bcolor);
  Windows.TextOut(Canvas.Handle, x, y - 1, PChar(str), nlen);
  ...
  Windows.SetBkMode(Canvas.Handle, TRANSPARENT);
  Windows.SetTextColor(Canvas.Handle, bcolor);
  Windows.TextOut(Canvas.Handle, 0, 1, PChar(str), Length(str));
  Windows.TextOut(Canvas.Handle, 2, 1, PChar(str), Length(str));
  Windows.TextOut(Canvas.Handle, 1, 0, PChar(str), Length(str));
  Windows.TextOut(Canvas.Handle, 1, 2, PChar(str), Length(str));
  Windows.SetTextColor(Canvas.Handle, fcolor);
  Windows.TextOut(Canvas.Handle, 1, 1, PChar(str), Length(str));
  //Windows.SetTextColor(Canvas.Handle, fcolor);
  //Windows.TextOut(Canvas.Handle, x, y, PChar(str), nLen)
end;

procedure TextOutBoldX(Canvas: TCanvas; x, y, fcolor, bcolor: integer; str: string);
var
  nLen: Integer;
begin
  nLen := Length(str);
...
procedure TDXCanvas.DrawEx(Texture: TDXTexture; X, Y,
 CenterX, CenterY, ScaleX,
 ScaleY: Single; MirrorX, MirrorY: Boolean;
...
begin
  ...
  FHGE.Gfx_RenderLine(x - 1, y, x + w, y, Color);
...
procedure TDXCanvas.DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height,
...
begin
 ...
 // FTY := SrcY;
  ...
  if Assigned(Texture) then begin
    FTexWidth := FHGE.Texture_GetWidth(Texture.Image);
    FTexHeight := FHGE.Texture_GetHeight(Texture.Image);
  end else begin
    FTexWidth := 1;
    FTexHeight := 1;
  end;

  FQuad.Tex := Texture.Image;
...
procedure TDXCanvas.DrawPart(Texture: TDXTexture; X, Y, SrcX, SrcY, Width, Height: Single;
   Color: Cardinal; BlendMode: Integer);
begin
...
function TDXDrawCanvas.TextHeight(Str: WideString; NewFont: Boolean): Integer;
begin
  if NewFont then
...
procedure TDXDrawCanvas.TextRectR(Rect: TRect;  Text: WideString; FColor: Cardinal; TextFormat: TTextFormat);
var
  Color4: LongWord;
  nY, nX, I: Integer;
  Texture: TDXTexture;
  ...
  if tfRight in TextFormat then begin
   nX := Rect.Right;
    ...
    if Texture = nil then begin
      ...
      Texture.Size := Point(TextWidth(Text, True) + 2, TextHeight('0', True) + 2);
      ...
      try
        ...
        TextOutBoldR(BitMap.Canvas, FColor, $00050505, Text);
...
procedure TDXDrawCanvas.TextRectX(Rect: TRect;  x, Y: Integer;Text: WideString; FColor: Cardinal; TextFormat: TTextFormat; NewFont: Boolean);
var
  Color4: LongWord;
  nY, nX, I: Integer;
  Texture: TDXTexture;
...
begin
 if NewFont then begin
  ...
  if (nWidth <= 0) or (nHeight <= 0) then
    exit;
  nY := (nHeight - TextHeight('中', True)) div 2;
  if nY < 0 then
    nY := 0;
  ...
  if tfRight in TextFormat then begin
    ...
    if Texture = nil then begin
      ...
      try
        ...
        BitMap.Width := Width;
        BitMap.Height := Heigth;
        TextOutBold(BitMap.Canvas, 2, 1, FColor, $00050505, Text);
        Texture.LoadFromBitmap(BitMap);
      finally
        FreeAndNil(BitMap);
      end;
...
function TDXDrawCanvas.TextWidth(Str: WideString; NewFont: Boolean): Integer;
var
  sWord: Word;
  I: Integer;
begin
...
var
  ...
  Width, Heigth: Integer;
begin
  //EnterCriticalSection(FreeTextInfoLock);   //添加空闲列表的缓冲区
  try
    Dec(X);
    Dec(Y);
    if Str = '' then Exit;
...
//    DrawFont(Texture, X, Y, Texture.ClientRect.Left, Texture.ClientRect.Top,
//    Texture.ClientRect.Right, Texture.ClientRect.Bottom,
//     fxBlend, (Alpha shl 24) or FColor);
     DrawRect(Texture, X, Y, Texture.ClientRect, fxBlend);

  finally
      //LeaveCriticalSection(FreeTextInfoLock);
  end;
end;


//鉴宝文字显示换行 2019-01-27 Development
procedure TDXDrawCanvas.BoldTextOutZ( X, Y, Z, fcolor: Integer; Str: string);
var
  tStr, temp        : string;
  ...
  if tStr <> '' then begin
    ...
    while True do begin
      ...
      if (temp <> '') and (tStr[i] = #$0D) and ((i + 1 <= Len) and (tStr[i + 1] = #$0A)) then begin
        BoldTextOut( X, Y + n * 14, fcolor,  temp);
...
procedure TDXDrawCanvas.SkidADTextOut(X, Y: Integer; Str: WideString; FColor: Cardinal; BColor: Cardinal);
var
  Texture: TDXTexture;
  FontText: pTFontText;
  BitMap : TBitmap;
...
begin
    ...
    if Texture = nil then begin
      ...
      try
        ...
        BitMap.Height := Heigth;
        Bitmap.Canvas.FillRect(HGEBase.Rect(0, 0, Width, Heigth));
        TextOutBold(BitMap.Canvas, 1, 1, FColor, BColor, Str);
        Texture.LoadFromBitmap(BitMap);
      finally
        FreeAndNil(BitMap);
      end;
...
procedure TDXDrawCanvas.TextOutR(x, Y: Integer; Str: WideString; FColor: Cardinal; BColor: Cardinal);
var
  Texture: TDXTexture;
  FontText: pTFontText;
  BitMap : TBitmap;
...
begin
    ...
    if Texture = nil then begin
      ...
      try
        ...
        BitMap.Height := Heigth;
        Bitmap.Canvas.FillRect(HGEBase.Rect(0, 0, Width, Heigth));
        TextOutBoldR(BitMap.Canvas, {1, 1,} FColor, BColor, Str);
        Texture.LoadFromBitmap(BitMap);
      finally
        FreeAndNil(BitMap);
      end;
...
procedure TDXDrawCanvas.FillRectAlpha(const Rect: TRect; DevColor: Cardinal; Alpha: Byte);
begin //Alpha 调整色深   255为纯色
  DevColor := DisplaceRB(DevColor or $FF000000);
  Rectangle(rect.Left, rect.Top, rect.Right - 1 - rect.Left, rect.Bottom - 1 - rect.Top, SetA(DevColor, Alpha), True);
end;

procedure TDXDrawCanvas.TextOutX(x, Y: Integer; Str: WideString; FColor, BColor: Cardinal);
var
  Texture: TDXTexture;
  FontText: pTFontText;
  BitMap : TBitmap;
...
begin
  ...
  if FColor = clBlack then
     FColor := $00050505;
  {if BColor = $00050505 then
    BColor := clBlack; }

  Texture := ImageFont.GetTextDIB(Str, FColor, BColor);
  if Texture = nil then begin
    Texture := TDXTexture.Create;
    Texture.Size := Point(TextWidth(Str, True) + 2, TextHeight(Str, True) + 2);
    Texture.Format := D3DFMT_A4R4G4B4;
    New(FontText);
    BitMap := TBitmap.Create;
    try
      ...
      BitMap.Canvas.Handle := GetDC(GetDesktopWindow());
      BitMap.Canvas.Font.Color := FColor;
      Width := BitMap.Canvas.TextWidth(Str);
      Heigth := BitMap.Canvas.TextHeight(Str);
      BitMap.Width := Width;
      BitMap.Height := Heigth;
      Bitmap.Canvas.FillRect(HGEBase.Rect(0, 0, Width, Heigth));
      TextOutBoldX(BitMap.Canvas, 0, 0, FColor, $00050505, Str);
      Texture.LoadFromBitmap(BitMap);
     // BitMap.SaveToFile(str + '.bmp');
    finally
      FreeAndNil(BitMap);
    end;
  ...
  DrawRect(Texture, X + 2, Y, Texture.ClientRect.Left, Texture.ClientRect.Top,
  Texture.ClientRect.Right, Texture.ClientRect.Bottom, fxBlend{, (255 shl 24) or FColor});
end;
initialization
  FHGE := nil;
