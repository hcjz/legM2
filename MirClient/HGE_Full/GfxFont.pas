...
type
  ...
  public
    // 渲染文本
    procedure Print(X: Single; Y: Single; Text: PWideChar;underline: Boolean);
    // 设置与获取颜色
...
procedure TGfxFont.Print(X, Y: Single; Text: PWideChar;underline: Boolean);//下划线
var
...
begin
  ...
  if underline then
  begin
   H := GetTextSize(Text).cy;
   w := GetTextSize(Text).cx;
   m_pHGE.Gfx_RenderLine(X,Y+H+2,X+W,Y+H+2,m_psprite.GetColor());
  end;
