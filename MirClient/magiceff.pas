...
uses
  ...
  Grobal2, HGE,HGECanvas,HGETextures, cliUtil, ClFunc,
   HUtil32, WIL;
  ...
  if m_boActive and ((abs(FlyX - fireX) > 30) or (abs(FlyY - fireY) > 30) or FixedEffect) then begin
    ...
    if not FixedEffect then begin
      case m_nMagEffectNo of
        10: begin
            ...
            if EffectBase2 >= 0 then begin
              ...
              d := ImgLib.GetCachedImage(img + curframe, px, py);
              if d <> nil then DrawBlend(Surface, FlyX + px - UNITX div 2 - shx, FlyY + py - UNITY div 2 - shy, d, 1);
            end;
          end;
        0075: begin
      ...
      if m_nMagEffectNo = MAGIC_FOX_FIRE2 then begin
        ...
        d := ImgLib.GetCachedImage(img + curframe, px, py);
        if d <> nil then DrawBlend(Surface, FlyX + px - UNITX div 2, FlyY + py - UNITY div 2, d, 1)
...
procedure THeroCharEffect.DrawEff(Surface: TDirectDrawSurface);
var
  d                         : TDirectDrawSurface;
begin
  if TargetActor <> nil then begin
...
procedure TRedGroundThunderEffect.DrawEff(Surface: TDirectDrawSurface);
var
  img, px, py               : Integer;
  d                         : TDirectDrawSurface;
begin
...
procedure TSpurtEffect.DrawEff(Surface: TDirectDrawSurface);
var
  img, px, py               : Integer;
  d                         : TDirectDrawSurface;
begin
