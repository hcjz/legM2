...
uses
  Windows, Classes, SysUtils, Graphics, Grobal2, HUtil32,
   HGE,HGECanvas,HGETextures, cliUtil, MShare;
...
begin
  if m_nCurrentMap <> 0 then begin
    ...
    if byte(m_MapHeader.Reserved[0]) <> 2 then begin
      //FillChar(m_TempMArr^, SizeOf(TMArr_Old), #0);
      nAline := SizeOf(TMapInfo_Old) * m_MapHeader.wHeight;
      for i := nLx to nRx - 1 do begin
        if (i >= 0) and (i < m_MapHeader.wWidth) then begin
          FileSeek(m_nCurrentMap, SizeOf(TMapHeader) + (nAline * i) + (SizeOf(TMapInfo_Old) * nTy), 0);
          //FileRead(m_nCurrentMap, m_MArr[i - nLx, 0], SizeOf(TMapInfo_Old) * (nBy - nTy));
          for j := 0 to nBy - nTy - 1 do
            FileRead(m_nCurrentMap, m_MArr[i - nLx, j], SizeOf(TMapInfo_Old));
        end;
      end;
      {for i := 0 to MAXX * 3 do begin
        for j := 0 to MAXY * 3 do begin
          Move(m_TempMArr[i, j], m_MArr[i, j], SizeOf(TMapInfo_Old));
        end;
      end;}
    end else begin
      nAline := SizeOf(TMapInfo) * m_MapHeader.wHeight;
      ...
      {for i := 0 to MAXX * 3 do begin
        for j := 0 to MAXY * 3 do begin
          pos := FileSeek(m_nCurrentMap, SizeOf(TMapHeader) + ((i + nLx) * m_MapHeader.wHeight + j + nTy) * SizeOf(TMapInfo), 0);
          if pos <> -1 then begin
            FileRead(m_nCurrentMap, m_MArr[i, j], SizeOf(TMapInfo));
          end;
        end;
      end;}
    end;
