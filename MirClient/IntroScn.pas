...
uses
  ...
  ExtCtrls, HGE,HGECanvas,HGETextures,HGEFont,
   FState, Grobal2, cliUtil, ClFunc, SoundUtil,
  HUtil32, DWinCtl;
...
begin
//通过绘图文字创建LOGO 需要填充底色 Development 2019-01-22 分析
//MSurface.Fill(clWhite);
//  n := MainForm.Canvas.Font.Size;
//  MainForm.Canvas.Font.Size := 145;
//  s := MainForm.Canvas.Font.Name;
//  MainForm.Canvas.Font.Name := '黑体';
//  MainForm.Canvas.Font.Style := [fsBold];
//  try
//    g_DXCanvas.BoldTextOut(
//      (SCREENWIDTH - g_DXCanvas.TextWidth(COPYRIGHTNAME)) div 2,
//      (SCREENHEIGHT - g_DXCanvas.TextHeight(COPYRIGHTNAME)) div 2,
//      GetRGB(249),
//      COPYRIGHTNAME);
//  finally
//    MainForm.Canvas.Font.Size := n;
//    MainForm.Canvas.Font.Name := s;
//    MainForm.Canvas.Font.Style := [];
//  end;
//  Sleep(10);


  //开始游戏时的LOGO  Development 2019-01-22 添加
...
//  if g_WMainPAK.ImageCount <> 55 then begin //必备补丁 NewProUI.pak
//    FrmDlg.DMessageDlg('缺少客户端必备补丁，请联系游戏管理员。', [mbIgnore]);
//    FrmMain.Close;
//  end;
end;
...
{$IFEND}
  ...
  if m_boNowOpening then begin
    if GetTickCount - m_dwStartTime > 150{230} then begin //开门间隔
      m_dwStartTime := GetTickCount;
      Inc(m_nCurFrame);
    end;
...
//      if g_nFadeIndex >= 255 then begin
//        Sleep(500);
...
//        if g_Logined then FrmDlg.DscStart.tag := FrmDlg.DscStart.ULib.Images[Format(g_sDscStart, [byte(FrmDlg.DscStart.Downed)])].Height;
//        if not g_boDoFadeOut and not g_boDoFadeIn then begin
//          //g_boDoFadeOut := True;
//          g_boDoFadeIn := True;
//          g_nFadeIndex := 0;
...
//    end;
    ...
    if g_boDoFadeOut then begin
      if g_nFadeIndex <= 1 then begin
        ...
        g_WChrSelImages.ClearCache;
        if g_Logined then FrmDlg.DscStart.tag := FrmDlg.DscStart.ULib.Images[Format(g_sDscStart, [byte(FrmDlg.DscStart.Downed)])].Height;
        if not g_boDoFadeOut and not g_boDoFadeIn then begin //由暗变亮，进入角色界面。
          g_boDoFadeIn := True;
          g_nFadeIndex := 0;
        end;
        DScreen.ChangeScene(stSelectChr);
...
begin
  Key := #13;
  FrmDlg.DxEditPasswordKeyPress(FrmDlg.DxEditPassword, key);
end;
...
procedure TSelectChrScene.SelChrStartClick; //开始
var
  chrname: string;
begin
{$IFNDEF TEST}
...
//              dd.free;
      ...
      if n = 0 then begin
        if ChrArr[n].UserChr.Name <> '' then begin
          with g_DXCanvas do begin
            ...
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 117 {117}, (SCREENHEIGHT - 600) div 2 + 492 + 2 {492+2}, clWhite,  ChrArr[n].UserChr.Name);
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 117 {117}, (SCREENHEIGHT - 600) div 2 + 523 {523}, clWhite,  IntToStr(ChrArr[n].UserChr.Level));
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 117 {117}, (SCREENHEIGHT - 600) div 2 + 553 {553}, clWhite,  GetJobName(ChrArr[n].UserChr.job));
            //Canvas.Release;
          end;
        end;
      end else begin
        if ChrArr[n].UserChr.Name <> '' then begin
          with g_DXCanvas do begin
            //SetBkMode(Canvas.Handle, Transparent);
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 671 {671}, (SCREENHEIGHT - 600) div 2 + 492 + 4 {492+4}, clWhite, ChrArr[n].UserChr.Name);
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 671 {671}, (SCREENHEIGHT - 600) div 2 + 525 {525}, clWhite,  IntToStr(ChrArr[n].UserChr.Level));
            BoldTextOut( (SCREENWIDTH - 800) div 2 + 671 {671}, (SCREENHEIGHT - 600) div 2 + 555 {555}, clWhite,  GetJobName(ChrArr[n].UserChr.job));
            //Canvas.Release;
          end;
        end;
      end;
      with g_DXCanvas do begin
        ...
        BoldTextOut( SCREENWIDTH div 2 {405} - TextWidth(svname) div 2, (SCREENHEIGHT - 600) div 2 + 8 {8}, clWhite, svname);
        //Canvas.Release;
      end;
    end;
  end;
