...
uses
  ...
  Grobal2, HGE,HGECanvas,HGETextures, cliUtil, magiceff, WIL,
  ClFunc, GList, Actor, StallSystem;
...
type
  ...
  private
    ...
    m_HumWinSurface: TDirectDrawSurface;
  ...
  public
    ...
    m_WeaponEffect: TDirectDrawSurface;
    m_nFrame: Integer;
    ...
    procedure DrawChr_Transparent(dsurface: TDirectDrawSurface; dx, dy: Integer; blend: Boolean; boFlag: Boolean);
  end;
...
begin
  ...
  if m_boUseCboLib and (m_nCurrentAction = SM_SMITELONGHIT) and (m_boSmiteLongHit = 1) then begin
    if m_nHairOffsetEx >= 0 then
      m_HairSurface := g_WHair2ImgImages.GetCachedImage(m_nHairOffsetEx + GetDefaultFrame(True), m_nHpx, m_nHpy)
...
procedure THumActor.DrawChr(dsurface: TDirectDrawSurface; dx, dy: Integer; blend: Boolean; boFlag: Boolean; DrawOnSale: Boolean);
var
...
begin
      ...
      1: begin
          case m_StallMgr.StallType of
            0: begin
                ax := -20;
                ay := -22;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [10])];
              end;
            1: begin
                ax := -6;
                ay := -40;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [14])];
              end;
            2: begin
                ax := -18;
                ay := -54;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [18])];
              end;
          end;
        end;
      3: begin
          case m_StallMgr.StallType of
            0: begin
                ax := -25;
                ay := -10;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [07])];
              end;
          end;
        end;
      5: begin
          case m_StallMgr.StallType of
            0: begin
                ax := -47;
                ay := -10;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [09])];
              end;
          end;
        end;
      7: begin
          case m_StallMgr.StallType of
            0: begin
                ax := -52;
                ay := -30;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [08])];
              end;
            1: begin
                ax := -46;
                ay := -44;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [12])];
              end;
            2: begin
                ax := -56;
                ay := -48;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [16])];
              end;
          end;
        end;
    end;
  ...
  if bWeapon and (m_nWpord = 0) and (not blend) then begin
  ...
  if not m_boDeath and not m_boUseCboLib then begin
    if m_nState and $00100000 <> 0 then begin //显示魔法盾时效果
      ...
      if d <> nil then
        DrawBlend(dsurface, ShiftX + ax, ShiftY + ay, d, 1);
    end;
      ...
      3: begin
          case m_StallMgr.StallType of
            1: begin
                ax := -8;
                ay := -24;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [11])];
              end;
            2: begin
                ax := -16;
                ay := -20;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [15])];
              end;
          end;
        end;
      5: begin
          case m_StallMgr.StallType of
            1: begin
                ax := -48;
                ay := -18;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [13])];
              end;
            2: begin
                ax := -50;
                ay := -18;
                dd := g_WMainUibImages.Images[format(g_StallLooks, [17])];
              end;
          end;
        end;
    end;
  ...
  if m_boUseMagic and (m_CurMagic.EffectNumber > 0) then begin //sm_spell
    if m_nCurEffFrame in [0..m_nSpellFrame - 1] then begin
      if m_boUseCboLib then begin
        ...
        if d <> nil then DrawBlend(dsurface, ShiftX + ax, ShiftY + ay, d, 1);
      end else begin
...
procedure THumActor.DrawChr_Transparent(dsurface: TDirectDrawSurface; dx, dy: Integer; blend: Boolean; boFlag: Boolean);
var
  bWarMode                  : Boolean;
  nOff, Idx, ax, ay         : Integer;
  Effect                    : TMagicEff;
