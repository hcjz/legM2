...
uses
  ...
  PsAPI, Clipbrd, HGE, HGEFont, HGEFonts, HGECanvas, HGEBase, HGETextures,
  Wilpion, DirectXGraphics, D3DX81mo, DirectX, DrawScrn, IntroScn, PlayScn,
  MapUnit, WIL, Grobal2, Actor, HumanActor, Registry, StdCtrls, cliUtil,
  ExtCtrls, HUtil32, EDcode, DWinCtl, GList, MMSystem, MaketSystem, ClFunc,
  magiceff, SoundUtil, clEvent, IniFiles, HashList, MShare, MD5, uSMBIOS,
  StallSystem, VCLUnZip, zlib, jpeg, D7ScktComp;
...
type
  ...
  private
    ...
    procedure SendItemSumCoundt(OrgItemIndex, ExItemIndex, hero: Integer;StrOrgItem, StrExItem: string);
...
//    FOldTime: DWORD;
    ...
    //CSocket: TClientSocket; //解决小退黑屏把组件放到窗体上 2018-12-26  Development注释
    DXRETime: LongWord;
...
//    procedure MyDeviceLogoSetup;
    procedure DisplayChange(boReset: Boolean);
  end;
  //解决小退黑屏把组件放到窗体上 2018-12-26  Development注释
  (*TSockMgr = class
  private
  protected
  public
...
var
  HGE              :IHGE = nil;
  //SockMgr: TSockMgr; //解决小退黑屏把组件放到窗体上 2018-12-26  Development注释
  frmMain: TfrmMain;
...
uses
  FState, CnHashTable, frmWebBroser, HeroActor, DxHint, DlgConfig, NewFont;
...
begin
  ...
  if FileExists(g_Debugflname) then begin
    Append(fhandle);
  end else begin
    Rewrite(fhandle);
  end;
...
var
  n: Integer;
  ini: TIniFile;
  szParamStr, szLine, szIP, szPort: string;
  nPort: Integer;
begin
  try
    MainForm := Self;
    ...
    g_EiImageDraw := TEiImageDraw.Create; //创建HGE字体 类似DX字体
    g_EiImageDraw.iniClass(); //HGE字体初始化

    FVCLUnZip := TVCLUnZip.Create(nil);
    InitScreenConfig(); //加载屏幕相关数值
    ScreenChanged(); //调整绘制地图范围大小
    ...
    if g_boFullScreen then begin
      ...
      AddHeight := frmMain.Height - ClientRect.Bottom;
      AddWidth := frmMain.Width - ClientRect.Right;
      ClientWidth := SCREENWIDTH;
      ClientHeight := SCREENHEIGHT;
      Left := (Screen.width - ClientWidth) div 2;
      Top := (Screen.Height - ClientHeight) div 2;
      Height := SCREENHEIGHT + AddHeight;
    ...
    Map := TMap.Create;
    //SockMgr := TSockMgr.Create(Self); //用不到了2018-12-26  Development注释
...
{$IFEND SERIESSKILL}
    ...
    ini := TIniFile.Create('.\lscfg.ini');
    try
      szParamStr := ini.ReadString('Setup', 'LoginHost', '');
    ...
    LoadWMImagesLib; //加载客户端资源
    InitWMImagesLib(''); //初始化客户端资源
    //---------HGE初始化---------------
    HGE := HGECreate(HGE_VERSION);
    HGE.System_SetState(HGE_SCREENBPP, 32);
    HGE.System_SetState(HGE_WINDOWED, true);
    HGE.System_SetState(HGE_FScreenWidth, SCREENWIDTH);
    HGE.System_SetState(HGE_FScreenHeight, SCREENHEIGHT);
    HGE.System_SetState(HGE_HIDEMOUSE, False);
    HGE.System_SetState(HGE_HWNDPARENT, frmMain.Handle);
...
procedure TfrmMain.MyDeviceInitialize(Sender: TObject; var Success: Boolean; var ErrorMsg: string); //内部初始化
var
  nCount: Integer;
begin
  ...
  g_nInitializePer := 30;
  AppOnIdle();
  Success := g_PlayScene.Initialize;
  ...
  try
    ...
    LoadColorLevels(); //加载颜色
    ErrorMsg := 'Error Code = 2';
    DScreen.ChangeScene(stIntro);//调整位置延长LOGO图像时间
    //Sleep(100);  //这里是设置LOGO时间的 2018-12-27 2:35 Development 注释
    ...
    (*if Win32Platform = VER_PLATFORM_WIN32_NT then begin //整理内存
      begin
        SetProcessWorkingSetSize(GetCurrentProcess, $FFFFFFFF, $FFFFFFFF);
        Application.ProcessMessages;
      end;
    end;*)  //内存别再自己骗自己了  Development 2018-12-30 注释
    ErrorMsg := 'Error Code = 5';
  except
    Success := False;
    DebugOutStr('[Exception]: TfrmMain.MyDeviceInitialize ' + ErrorMsg);
  end;
end;
...
begin
  try
    ...
    DebugOutStr('TfrmMain.MyDeviceFinalize ' + IntToStr(ECode));
  end;
end;
  ...
  try
    case Msg of
      msgDeviceLost: begin
          ...
          FrmDlg.UnInitGrobalSurface();
          g_PlayScene.Lost;
          MShare.g_DXFont.Lost;  //主表面字体 无边无底色字体  ClearHint;
          DScreen.ClearHint;
        end;
      msgDeviceRecovered: begin
           ECode := 1;
           FrmDlg.InitGrobalSurface();
           g_PlayScene.Recovered;
           MShare.g_DXFont.Recovered;  //主表面字体
           Map.m_OldClientRect := Rect(0, 0, 0, 0); //重绘地图  2019.1.4
           DScreen.ClearHint;
        end;
      msgDeviceRestoreSize: begin
          ...
          if g_boFullScreen then begin
            if FIDDraw <> nil then begin
              FIDDraw.SetDisplayMode(SCREENWIDTH, SCREENHEIGHT, 32);
            end;
          end else begin
            Left := (Screen.width - ClientWidth) div 2;
            Top := (Screen.Height - ClientHeight) div 2;
    ...
    DebugOutStr('TfrmMain.MyDeviceNotifyEvent ' + IntToStr(ECode));
  end;
end;
  ...
  if boReset then begin
    if FboDisplayChange then begin
      ...
      if FDDrawHandle > 0 then
        FreeLibrary(FDDrawHandle); //指定要释放的动态链接库的名称
      FDDrawHandle := 0;
      FboDisplayChange := False;
      UnRegisterHotKey(Handle, FHotKeyId);//该函数释放调用线程先前登记的热键
    end;
  end else begin
    if not FboDisplayChange then begin
      ...
      if FDDrawHandle > 0 then
        FreeLibrary(FDDrawHandle);  //指定要释放的动态链接库的名称
      FDDrawHandle := LoadLibrary('DDraw.dll'); //指定要载入的动态链接库的名称
...
begin
  if frmMain.WindowState <> wsNormal then frmMain.WindowState := wsNormal;
end;
...
begin
  try
    Ecode := 0;
    FreeAndNil(g_DXCanvas);
    FreeAndNil(g_DXFont);
    FreeAndNil(ImageFont);
    FIDDraw := nil;
    FreeAndNil(g_EiImageDraw);
...
{$IFEND SERIESSKILL}
    ...
    DebugOutStr('[Exception]: TfrmMain.FormDestroy ' + IntToStr(Ecode));
  end;
end;
...
var
  ...
  I: Integer;
begin
 if GetTickCount - DXRETime > 30 then  begin
    ...
    if not boInitialize then begin
      if DScreen.CurrentScene = g_PlayScene then begin
        ...
        if g_boCanDraw then begin
          if g_PlayScene.CanDrawTileMap then begin
            ...
            HGE.Gfx_Clear(0);
            HGE.RenderBatch;
            g_PlayScene.DrawTileMap(nil); //绘制地图
            HGE.Gfx_EndScene;
          end;
          if g_PlayScene.m_boPlayChange then begin
            HGE.Gfx_BeginScene(g_PlayScene.m_ObjSurface.Target);
            HGE.Gfx_Clear(0);
            HGE.RenderBatch;
            g_PlayScene.PlaySurface(nil);
            HGE.Gfx_EndScene;
          end;
          HGE.Gfx_BeginScene(g_PlayScene.m_MagSurface.Target);
          HGE.Gfx_Clear(0);
          HGE.RenderBatch;
          g_PlayScene.MagicSurface(nil);
          HGE.Gfx_EndScene;
        end;
...
procedure TfrmMain.MyDeviceRender(Sender: TObject); //渲染
var
  CanDraw, bb: Boolean;
  n: Integer;
  s: string;
begin
  try
    if not g_boInitialize then begin
      ...
      ProcessMagic;
      if DScreen.CurrentScene = g_PlayScene then begin
        g_DXCanvas.Draw(SOFFX, SOFFY, g_PlayScene.m_MagSurface.ClientRect, g_PlayScene.m_MagSurface, True);
      end;

      DScreen.DrawScreen(MShare.g_DXCanvas.DrawTexture);
      if g_boCanDraw then begin
        if g_gcGeneral[4] then g_PlayScene.DropItemsShow; //物品显示开关 Development 2019-01-11
        DScreen.DrawScreenTop(MShare.g_DXCanvas.DrawTexture);
        DScreen.DrawScreenBottom(MShare.g_DXCanvas.DrawTexture);
        g_DWinMan.DirectPaint(MShare.g_DXCanvas.DrawTexture);
        DScreen.DrawHint(MShare.g_DXCanvas.DrawTexture);
        DrawMovingObject(); //绘制鼠标移动中的物品
        if DScreen.CurrentScene = LoginScene then DrawGameNotice(); //登陆画面健康公告
        DoFade(); //画黑暗变亮效果
      end;

    end;
  except
    DebugOutStr('[Exception]: TfrmMain.MyDeviceRender');
  end;
end;
...
begin
  if g_boItemMoving then begin
    ...
    if d <> nil then begin
      ...
      g_DXCanvas.DrawTexture.Draw(P.X - (d.ClientRect.Right div 2), P.Y - (d.ClientRect.Bottom div 2), d.ClientRect, d, True);
//
      if (g_MovingItem.Item.S.Overlap > 0) and bitem then begin
        g_DXCanvas.BoldTextOut( P.X + 14, P.Y + 2, clLime,  Format('%d', [g_MovingItem.Item.Dura]));
      end;

      if (g_MovingItem.Item.S.StdMode in [15..24, 26..30]) then begin
        if (g_MovingItem.Item.S.Shape = 190) or (g_MovingItem.Item.S.reserve[3] = 4) then begin
          if GetTickCount - g_MovingItemShine.tick > 60 then begin
          ...
          if d <> nil then
            DrawBlend(g_DXCanvas.DrawTexture,
              P.X - (d.ClientRect.Right div 2),
              P.Y - (d.ClientRect.Bottom div 2),
              d, 1);
        end else if g_MovingItem.Item.S.reserve[3] = 1 then begin
          n := 0;
          if g_MovingItem.Item.S.StdMode in [15, 16] then
            n := 2;
          if GetTickCount - g_MovingItemShine.tick > 100 then begin
            g_MovingItemShine.tick := GetTickCount;
            Inc(g_MovingItemShine.idx);
          ...
          if d <> nil then
            DrawBlend(g_DXCanvas.DrawTexture,
              ...
              P.Y - (d.ClientRect.Bottom div 2),
              d, 1);
        end else if g_MovingItem.Item.S.reserve[3] = 2 then begin
          if GetTickCount - g_MovingItemShine.tick > 100 then begin
            g_MovingItemShine.tick := GetTickCount;
            Inc(g_MovingItemShine.idx);
          ...
          if d <> nil then
            DrawBlend(g_DXCanvas.DrawTexture,
              ...
              P.Y - (d.ClientRect.Bottom div 2),
              d, 1);
        end else if g_MovingItem.Item.S.reserve[3] = 3 then begin
          if GetTickCount - g_MovingItemShine.tick > 100 then begin
            g_MovingItemShine.tick := GetTickCount;
            Inc(g_MovingItemShine.idx);
          ...
          if d <> nil then
            DrawBlend(g_DXCanvas.DrawTexture,
              ...
              P.Y - (d.ClientRect.Bottom div 2),
              d, 1);
        end else if g_MovingItem.Item.S.reserve[3] = 5 then begin
          if GetTickCount - g_MovingItemShine.tick > 100 then begin
            g_MovingItemShine.tick := GetTickCount;
            Inc(g_MovingItemShine.idx);
            if g_MovingItemShine.idx > 9 then
          ...
          if d <> nil then
            DrawBlend(g_DXCanvas.DrawTexture,
              ...
              P.Y - (d.Height div 2),
              d, 1);
        end else if g_MovingItem.Item.S.reserve[3] in [6..10] then begin
...
FontColor :tColor ;
begin
  if not g_boDoFadeOut and (DScreen.CurrentScene = LoginScene) then begin
    with g_DXCanvas do begin
      FontColor := $0093F4F2;
      TextOut((SCREENWIDTH - TextWidth(g_affiche1)) div 2, SCREENHEIGHT - 70, FontColor, g_affiche1);
      TextOut((SCREENWIDTH - TextWidth(g_affiche2)) div 2, SCREENHEIGHT - 50, FontColor, g_affiche2);
      TextOut((SCREENWIDTH - TextWidth(g_affiche3)) div 2, SCREENHEIGHT - 35, FontColor, g_affiche3);
      FontColor := clLime;
      if not g_gcGeneral[11] then
        TextOut((SCREENWIDTH - TextWidth(g_affiche0)) - 10, SCREENHEIGHT - 50, FontColor, g_affiche0);
      TextOut(SCREENWIDTH - TextWidth(g_sChgWindowMsg) - 10, SCREENHEIGHT - 20, FontColor, g_sChgWindowMsg);
...
begin
  if mrOk = FrmDlg.DMessageDlg('确认退出到选择角色界面吗？', [mbOk, mbCancel]) then begin
    AppLogoutEx;
  end;
end;

//小退返回角色处理过程 2018-12-26  Development
...
begin
  ...
  try
    ...
    FrmDlg.DrawMiniMap.Visible := False; //退出时关闭小地图 2019-01-12  Development
...
begin
 ...
 if not DirectoryExists('Images') then  CreateDir('Images');
  ...
  if g_MySelf <> nil then DScreen.AddChatBoardString(Format('[屏幕截图: %s]', [ExtractFileName(flname)]), clGreen, clWhite);
end;
...
begin
  ...
  if GetTickCount - g_PlayScene.ProcMagic.dwTick > 5000 then begin
    g_PlayScene.ProcMagic.dwTick := GetTickCount;
    g_PlayScene.ProcMagic.nTargetX := -1;
    Exit;
  end;

  if GetTickCount - m_dwProcUseMagicTick > 28 then begin
    m_dwProcUseMagicTick := GetTickCount;

    if g_PlayScene.ProcMagic.fUnLockMagic then begin
      targid := 0;
      targx := g_PlayScene.ProcMagic.nTargetX;
      targy := g_PlayScene.ProcMagic.nTargetY;
    end else if (g_PlayScene.ProcMagic.xTarget <> nil) and not g_PlayScene.ProcMagic.xTarget.m_boDeath then begin
    ...
    if (nSX <= g_nMagicRange) and (nSY <= g_nMagicRange) then begin
      if g_PlayScene.ProcMagic.fContinue or (CanNextAction() and ServerAcceptNextAction) then begin
        g_dwLatestSpellTick := GetTickCount;
...
var
  dwExit: LongWord;
begin
  if ActionKey = 0 then Exit;
  if (g_MySelf <> nil) and g_MySelf.m_StallMgr.OnSale then begin
    Exit;
  end;
...
begin
  ...
  if (g_nTargetX >= 0) and CanNextAction and ServerAcceptNextAction then begin
    ...
    /////////////////////////////////////////////////
    if g_boOpenAutoPlay and (g_APMapPath <> nil) and (g_APStep >= 0) and (0 < High(g_APMapPath)) then begin
      if (abs(g_APMapPath[g_APStep].X - g_MySelf.m_nCurrX) <= 3) and (abs(g_APMapPath[g_APStep].Y - g_MySelf.m_nCurrY) <= 3) then begin
        if High(g_APMapPath) >= 2 then begin //3点以上

          if g_APStep >= High(g_APMapPath) then begin //当前点在终点...
            //终点 <-> 起点 距离过远...
            if (abs(g_APMapPath[High(g_APMapPath)].X - g_APMapPath[0].X) >= 36) or (abs(g_APMapPath[High(g_APMapPath)].Y - g_APMapPath[0].Y) >= 36) then begin
              g_APGoBack := True; //原路返回
              g_APLastPoint := g_APMapPath[g_APStep];
              Dec(g_APStep);
            end else begin //循环到起点...
              g_APGoBack := False;
              g_APLastPoint := g_APMapPath[g_APStep];
              g_APStep := 0;
            end;
          end else begin
            if g_APGoBack then begin //原路返回
              g_APLastPoint := g_APMapPath[g_APStep];
              Dec(g_APStep);
              if g_APStep <= 0 then begin //已回到起点
                g_APStep := 0;
                g_APGoBack := False;
              end;
            end else begin //循环...
    ...
    if (g_nTargetX <> g_MySelf.m_nCurrX) or (g_nTargetY <> g_MySelf.m_nCurrY) then begin
      ...
      if (g_MySelf.m_nTagX > 0) and (g_MySelf.m_nTagY > 0) then begin
        if g_MoveBusy then begin
          if GetTickCount - g_MoveErrTick > 60 then begin
      ...
      case g_ChrAction of
        caWalk: begin
            ...
            if IsUnLockAction() and (crun > 0) then begin
              GetNextPosXY(ndir, mx, my);
              bowalk := True;
              bostop := False;
              if not g_PlayScene.CanWalk(mx, my) then begin
                if g_boOpenAutoPlay and g_boAPAutoMove and (g_APPathList.count > 0) then begin
                  Init_Queue2();
                  g_nTargetX := -1;
                end;
                bowalk := False;
                ...
                if not bostop and {not}(g_PlayScene.CrashMan(mx, my) or not Map.CanMove(mx, my)) then begin
                  mx := g_MySelf.m_nCurrX;
                  my := g_MySelf.m_nCurrY;
                  adir := PrivDir(ndir);
        ...
        caRun: begin
            ...
            if g_boCanStartRun or (g_nRunReadyCount >= 1) then begin
              ...
              //骑马开始
              if (g_MySelf.m_btHorse <> 0) and (GetDistance(mx, my, dx, dy) >= 3) and (crun > 0) and IsUnLockAction() then begin
                GetNextHorseRunXY(ndir, mx, my);
                if g_PlayScene.CanRun(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, mx, my) then begin
    ...
    end else if g_boOpenAutoPlay and g_boAPAutoMove and (g_AutoPicupItem <> nil) then begin
      frmMain.SendPickup;
      g_sAPStr := '拾取物品';
      if g_boAPAutoMove and (g_APPathList.count > 0) then begin
        Init_Queue2();
        g_nTargetX := -1;
      end;
    end;
  ...
  if g_MySelf.RealActionMsg.ident > 0 then begin
    ...
    if g_nStallX <> -1 then begin
      if (abs(g_nStallX - g_MySelf.m_nCurrX) >= 8) or (abs(g_nStallY - g_MySelf.m_nCurrY) >= 8) then begin
        g_nStallX := -1;
...
begin
  ...
  if g_MySelf.m_btJob = 0 then begin
    if g_MagicArr[0][g_SeriesSkillArr[0]].Def.wMagicid = 100 then begin
  ...
  end else if g_SeriesSkillStep <= _MIN(High(g_SeriesSkillArr) + 1, g_MagicList2.count) then begin
...
begin
  if (g_APMapPath <> nil) and (High(g_APMapPath) > 0) then begin
    n14 := 0;
    g_APLastPoint.X := -1;
    n10 := 999;
    for i := Low(g_APMapPath) to High(g_APMapPath) do begin
...
begin
  case Key of
    ...
    VK_RETURN: if (ssAlt in Shift) then begin
        {if g_DlgInitialize and (g_hImagesThread <> INVALID_HANDLE_VALUE) then begin
          WaitForSingleObject(g_hImagesThread, 5 * 1000);
        end;
        if not g_DlgInitialize then begin}
          Key := 0;
...
//  end;
  ...
  case Key of   //重复了 2018-11-05
    VK_UP:
      if (ssCtrl in Shift) {and g_PlayScene.EdChat.Visible} then begin //Ctrl + ↑ 翻看历史记录

        g_PlayScene.EdChat.Visible := True;
        if g_SendSayListIdx > 0 then
          Dec(g_SendSayListIdx);

          //ShowMsg('11111');
        if g_SendSayListIdx < g_SendSayList.count then
          g_PlayScene.EdChat.Text := g_SendSayList[g_SendSayListIdx];
        Key := 0;
        g_PlayScene.EdChat.SelStart := Length(g_PlayScene.EdChat.Text); //设置光标在结尾
        g_PlayScene.EdChat.SelLength := 0; //选中字符串长度
      end;
    VK_DOWN:
      if (ssCtrl in Shift) and g_PlayScene.EdChat.Visible then begin //Ctrl + ↓ 翻看历史记录
        if g_SendSayListIdx < g_SendSayList.count - 1 then
  ...
  if g_gcHotkey[0] then begin
    MD := 0;
    if ssCtrl in Shift then MD := MD or MOD_CONTROL;
    if ssAlt in Shift then MD := MD or MOD_ALT;
    if ssShift in Shift then MD := MD or MOD_SHIFT;
    if (MD <> 0) or ((MD = 0) and not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible}) then begin

      if FrmDlg.DEHeroCallHero.HotKey <> 0 then begin
        SeparateHotKey(FrmDlg.DEHeroCallHero.HotKey, M, k);
        if (MD = M) and (Key = k) then begin
          g_pbRecallHero^ := True;
      ...
      if FrmDlg.DEHeroSetTarget.HotKey <> 0 then begin
        SeparateHotKey(FrmDlg.DEHeroSetTarget.HotKey, M, k);
        if (MD = M) and (Key = k) then begin
          if (g_MySelf <> nil) and (g_MySelf.m_HeroObject <> nil) and (g_FocusCret <> nil) then begin
            SendHeroSetTarget();
            Exit;
          end;
        end;
      end;
      if FrmDlg.DEHeroUnionHit.HotKey <> 0 then begin
      ...
      if FrmDlg.DEHeroSetAttackState.HotKey <> 0 then begin
      ...
      if FrmDlg.DEHeroSetGuard.HotKey <> 0 then begin
      ...
      if FrmDlg.DESwitchAttackMode.HotKey <> 0 then begin
      ...
      if FrmDlg.DESwitchMiniMap.HotKey <> 0 then begin
      ...
      if FrmDlg.DxEditSSkill.HotKey <> 0 then begin
  ...
  case Key of
    ...
    Word('B'): if ssCtrl in Shift then
    ...
    Word('H'): if ssCtrl in Shift then SendSay('@AttackMode');
    Word('A'): if ssCtrl in Shift then SendSay('@rest');
    Word('D'): if ssCtrl in Shift then begin
        SendFireSerieSkill();
        //SeriesSkillFire();
    ...
    Word('S'): if ssCtrl in Shift then begin
        if (g_MySelf.m_HeroObject <> nil) then
          SendHeroJoinAttack();
      end else if ssAlt in Shift then begin
        if (g_FocusCret <> nil) and (g_FocusCret <> g_MySelf) and (g_FocusCret.m_btRace = 0) and (g_FocusCret.m_btIsHero = 0) then begin
          wc := FrmDlg.m_BlockList.IndexOf(g_FocusCret.m_sUserName);
          if wc >= 0 then begin
            FrmDlg.m_BlockList.Delete(wc);
            FrmDlg.DFrdSave();
    ...
    Word('W'): if ssCtrl in Shift then begin
        SendHeroSetTarget;
      end else if ssAlt in Shift then begin
        if (g_FocusCret <> nil) and (g_FocusCret <> g_MySelf) and (g_FocusCret.m_btRace = 0) and (g_FocusCret.m_btIsHero = 0) then begin
          ...
          g_PlayScene.EdChat.Text := g_FocusCret.m_sUserName; //FrmDlg.DEdChat.Text := g_FocusCret.m_sUserName;
        end;
      end;
    Word('F'): begin
    ...
    Word('Z'): begin
        ...
        if not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then begin
    ...
    Word('X'): begin
        if g_MySelf = nil then Exit;
        if g_boOpenAutoPlay and (Shift = [ssCtrl, ssAlt]) then begin
          g_gcAss[0] := not g_gcAss[0];
          frmMain.TimerAutoPlay.Enabled := g_gcAss[0];
          if frmMain.TimerAutoPlay.Enabled then begin
            g_APTagget := nil;
        ...
        if ssAlt in Shift then
          AppLogout
        else if not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then begin
    ...
    Word('R'): if not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then begin
        FrmDlg.DMyStateClick(FrmDlg.DBotBelt, 0, 0);
      end;
    Word('O'): if not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then begin
    ...
    Word('P'): begin
        if not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then
  ...
  case Key of
    VK_UP:
      if (ssCtrl in Shift) and g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} then begin
        if g_SendSayListIdx > 0 then
          Dec(g_SendSayListIdx);

          //ShowMsg('33333');
        if g_SendSayListIdx < g_SendSayList.count then
          g_PlayScene.EdChat.Text := g_SendSayList[g_SendSayListIdx]; //FrmDlg.DEdChat.Text := g_SendSayList[g_SendSayListIdx];
        Key := 0;
//        FrmDlg.DEdChat.ChangeCurPos(Length(FrmDlg.DEdChat.Text), True);
//        FrmDlg.DEdChat.SelLength := 0;
        g_PlayScene.EdChat.SelStart := Length(g_PlayScene.EdChat.Text); //设置光标在结尾
        ...
        if g_PlayScene.EdChat.Visible then begin
...
begin
  if not IntroScene.m_boOnClick then begin
  ...
  if DScreen.CurrentScene = g_PlayScene then begin
    if g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible} and (FrmDlg.DEdChat <> MouseCaptureControl) then begin
//      frmMain.SendSay(FrmDlg.DEdChat.Text);
//      FrmDlg.DEdChat.Text := '';
//      FrmDlg.DEdChat.Visible := False;
      frmMain.SendSay(g_PlayScene.EdChat.Text);
      g_PlayScene.EdChat.Text := '';
      g_PlayScene.EdChat.Visible := False;
    ...
    case BYTE(Key) of
      ...
      //G键呼出调试窗口 Development 2019-01-22 注释
       byte('G'),byte('g'):
         begin
...
//          end;
          g_PlayScene.EdChat.Visible := True;
          g_PlayScene.EdChat.SetFocus;
          //SetImeMode(FrmDlg.DEdChat.Handle, g_LocalLanguage);
          if FrmDlg.BoGuildChat then begin
            g_PlayScene.EdChat.Text := '!~';
            g_PlayScene.EdChat.SelStart := Length(g_PlayScene.EdChat.Text);
            g_PlayScene.EdChat.SelLength := 0;
          end else begin
            g_PlayScene.EdChat.Text := '';
          end;
        end;
...
//          end;
          ...
          if Key = '/' then begin
            if WhisperName = '' then
              g_PlayScene.EdChat.Text := Key
            else if Length(WhisperName) > 2 then
              g_PlayScene.EdChat.Text := '/' + WhisperName + ' '
            else
              g_PlayScene.EdChat.Text := Key;
            g_PlayScene.EdChat.SelStart := Length(g_PlayScene.EdChat.Text);
            g_PlayScene.EdChat.SelLength := 0;
...
begin
  ...
  if pcm.Def.wMagicid = 114 then begin
    if g_boSkill_114_MP then
      boSeriesSkill := False
    else
      boSeriesSkill := True;
  end else if pcm.Def.wMagicid in [68, 78] then begin
  ...
  if (SpellSpend <= defSpellSpend) then begin
    if pcm.Def.btEffectType = 0 then begin
      ...
      if (pcm.Def.wMagicid in [27]) then begin //野蛮时间间隔
        if (GetTickCount - g_dwLatestRushRushTick <= 3 * 1000) then
          Exit;
      end;

      ///////////////////////////////////////////////
      if pcm.Def.wMagicid = 100 then begin
        if boContinue or (CanNextAction and ServerAcceptNextAction and CanNextHit) then begin

        end else
          Exit;
      end;

      if pcm.Def.wMagicid = 101 then begin
        if g_boNextTimeSmiteHit or (GetTickCount - g_dwLatestSmiteHitTick <= 1 * 100) then
          Exit;
      end;
      if pcm.Def.wMagicid = 102 then begin
        if g_boNextTimeSmiteLongHit or (GetTickCount - g_dwLatestSmiteLongHitTick <= 1 * 100) then
          Exit;
      end;
      if pcm.Def.wMagicid = 103 then begin
      ...
      if g_boSpeedRate then begin
        if boContinue or (GetTickCount - g_dwLatestSpellTick > g_dwSpellTime - g_MagSpeedRate * 20) then begin
          g_dwLatestSpellTick := GetTickCount;
        ...
        if boContinue or (GetTickCount - g_dwLatestSpellTick > g_dwSpellTime) then begin
          g_dwLatestSpellTick := GetTickCount;
          g_dwMagicDelayTime := 0;
          SendSpellMsg(CM_SPELL, g_MySelf.m_btDir {x}, 0, pcm.Def.wMagicid, 0, False);
        end;
      ...
      if g_MagicTarget <> nil then begin
        if not g_boMagicLock or
          ...
          not g_PlayScene.IsValidActor(g_MagicTarget) then begin
          g_MagicTarget := nil;
          g_MagicLockActor := nil;

        end;
      end;
      if (g_MagicTarget <> nil) and (g_MagicTarget is THumActor) then begin
        if THumActor(g_MagicTarget).m_StallMgr.OnSale then begin
          g_MagicTarget := nil;
          g_MagicLockActor := nil;
        end;
      end;
      ...
      if (abs(g_MySelf.m_nCurrX - targx) > g_nMagicRange) or (abs(g_MySelf.m_nCurrY - targy) > g_nMagicRange) then begin
        if g_gcTec[14] and (fUnLockMagic or (targid <> 0)) then begin
      ...
      g_PlayScene.ProcMagic.nTargetX := -1;
      if boContinue or (CanNextAction and ServerAcceptNextAction) then begin
    ...
    if boSeriesSkill then begin
      if GetTickCount - g_IPointLessHintTick > 5000 then begin
        g_IPointLessHintTick := GetTickCount;
        DScreen.AddSysMsg(Format('需要 %d 内力值才能释放 %s', [SpellSpend, pcm.Def.sMagicName]));
      end;
    end else if GetTickCount - g_MPLessHintTick > 1000 then begin
...
begin
  ...
  if Actor <> nil then begin
    ...
    Actor.SendMsg(SM_SPELL, effnum div 255, magic_id div 300, adir, Integer(UseMagic), 0, '', 0);
    Inc(g_nSpellCount);
  end else
...
begin
  ...
  Actor := g_PlayScene.FindActor(who);
  if Actor <> nil then begin
    Actor.SendMsg(SM_MAGICFIRE, target, efftype, effnum, targetx, targety, IntToStr(maglv), Sound);
...
begin
  if (Actor.m_HeroObject <> nil) and not Actor.m_HeroObject.m_boDeath then begin
...
begin
  ...
  for i := MAXBAGITEM - (1 + 0) downto 0 do begin
    if (g_ItemArr[i].S.Name <> '') and (g_ItemArr[i].S.NeedIdentify < 4) then begin
      case g_ItemArr[i].S.StdMode of
        00: case g_ItemArr[i].S.Shape of
            0: begin //普通药
                if g_gcProtect[0] and (g_ItemArr[i].S.AC > 0) and (g_ItemArr[i].S.AC < MaxHP) then begin
                  MaxHP := g_ItemArr[i].S.AC;
                  hidx := i;
                end;
                if g_gcProtect[1] and (g_ItemArr[i].S.MAC > 0) and (g_ItemArr[i].S.MAC < MaxMP) then begin
                  MaxMP := g_ItemArr[i].S.MAC;
                  midx := i;
                end;
              end;
            1: begin //速效药
                if g_gcProtect[3] and (g_ItemArr[i].S.AC > 0) and (g_ItemArr[i].S.AC < MaxSP) then begin
                  MaxSP := g_ItemArr[i].S.AC;
                  sidx := i;
                end;
              end;
          end;
        2, 3: if g_gcProtect[5] then begin
            if CompareText(g_ItemArr[i].S.Name, g_sRenewBooks[g_gnProtectPercent[6]]) = 0 then
              bidx := i;
          end;
        31: case g_ItemArr[i].S.AniCount of
            1: if g_gcProtect[0] then uhidx := i;
            2: if g_gcProtect[1] then umidx := i;
            3: if g_gcProtect[3] then usidx := i;
  ...
  bEatSp := False;
  bEatOK := False;
  if GetTickCount - g_MySelf.m_dwMsgHint > 15 * 1000 then begin
    g_MySelf.m_dwMsgHint := GetTickCount;
    bHint := True;
  end;
  if not bNeedSP then begin
    if g_gcProtect[0] and IsPersentHP(g_MySelf.m_Abil.HP, g_MySelf.m_Abil.MaxHP) then begin
      if GetTickCount - g_MySelf.m_dwHealthHP > g_gnProtectTime[0] then begin
        g_MySelf.m_dwHealthHP := GetTickCount;
        if hidx > -1 then begin
          EatItem(hidx);
          bEatOK := True;
        end else if (nCount > 4) and (uhidx > -1) then begin
          EatItem(uhidx);
          bEatOK := True;
        end else begin
          bEatSp := True;
  ...
  if not bNeedSP then begin
    if g_gcProtect[1] and IsPersentMP(g_MySelf.m_Abil.MP, g_MySelf.m_Abil.MaxMP) then begin
      if GetTickCount - g_MySelf.m_dwHealthMP > g_gnProtectTime[1] then begin
        g_MySelf.m_dwHealthMP := GetTickCount;
        if midx > -1 then begin
          EatItem(midx);
          bEatOK := True;
        end else if (nCount > 4) and (umidx > -1) then begin
          EatItem(umidx);
          bEatOK := True;
        end else begin
  ...
  if not bEatOK then begin
    if g_gcProtect[3] and (bNeedSP or bEatSp or (g_gcProtect[11] and IsPersentSpc(g_MySelf.m_Abil.MP, g_MySelf.m_Abil.MaxMP))) then begin
      if GetTickCount - g_MySelf.m_dwHealthSP > g_gnProtectTime[3] then begin
  ...
  if g_gcProtect[5] and IsPersentBook(g_MySelf.m_Abil.HP, g_MySelf.m_Abil.MaxHP) then begin
    if GetTickCount - g_MySelf.m_dwHealthBK > g_gnProtectTime[5] then begin
...
begin
  ...
  MaxMP := High(Integer) div 2 - 1;
  MaxSP := High(Integer) div 2 - 1;
  for i := g_nHeroBagSize - 1 {MAXBAGITEM - (1 + 6)} downto 0 do begin
    if g_HeroItemArr[i].S.Name <> '' then begin
      case g_HeroItemArr[i].S.StdMode of
        00: case g_HeroItemArr[i].S.Shape of
            0: begin //普通药
                if (g_HeroItemArr[i].S.AC > 0) and (g_HeroItemArr[i].S.AC < MaxHP) then begin
                  MaxHP := g_HeroItemArr[i].S.AC;
                  hidx := i;
                end;
                if (g_HeroItemArr[i].S.MAC > 0) and (g_HeroItemArr[i].S.MAC < MaxMP) then begin
                  MaxMP := g_HeroItemArr[i].S.MAC;
                  midx := i;
                end;
              end;
            1: begin //速效药
                if (g_HeroItemArr[i].S.AC > 0) and (g_HeroItemArr[i].S.AC < MaxSP) then begin
  ...
  bEatSp := False;
  bEatOK := False;
  if GetTickCount - g_MySelf.m_HeroObject.m_dwMsgHint > 15 * 1000 then begin
    g_MySelf.m_HeroObject.m_dwMsgHint := GetTickCount;
    bHint := True;
  end;
  if not bNeedSP then begin
    if g_gcProtect[7] and IsPersentHPHero(g_MySelf.m_HeroObject.m_Abil.HP, g_MySelf.m_HeroObject.m_Abil.MaxHP) then begin
      if GetTickCount - g_MySelf.m_HeroObject.m_dwHealthHP > g_gnProtectTime[7] then begin
  ...
  if not bNeedSP then begin
    if g_gcProtect[8] and IsPersentMPHero(g_MySelf.m_HeroObject.m_Abil.MP, g_MySelf.m_HeroObject.m_Abil.MaxMP) then begin
      if GetTickCount - g_MySelf.m_HeroObject.m_dwHealthMP > g_gnProtectTime[8] then begin
  ...
  if not bEatOK then begin
    if g_gcProtect[9] and (bNeedSP or bEatSp or (g_gcProtect[11] and IsPersentSpcHero(g_MySelf.m_HeroObject.m_Abil.MP, g_MySelf.m_HeroObject.m_Abil.MaxMP))) then begin
      if GetTickCount - g_MySelf.m_HeroObject.m_dwHealthSP >= g_gnProtectTime[9] then begin
...
var
  i: Integer;
begin
  if (idx in [0..5]) and (sItem <> '') then begin
    if g_ItemArr[idx].S.Name = '' then begin
      for i := MAXBAGITEMCL - 1 downto 6 do begin
        if g_ItemArr[i].S.Name = sItem then begin
...
begin
  if (sItem <> '') and (nType <> 0) then begin
    boIsUnBindItem := False;
    for i := Low(g_UnBindItems) to High(g_UnBindItems) do begin
      if sItem = g_UnBindItems[i] then begin
        boIsUnBindItem := True;
        Break;
      end;
    end;
    if not boIsUnBindItem then Exit;
    n := 0;
    boUnBindAble := False;
    ...
    if not boUnBindAble then Exit;
    idx := -1;
    for i := MAXBAGITEMCL - 1 downto 6 do begin
      if g_ItemArr[i].S.StdMode = 31 then begin
        if g_ItemArr[i].S.Name <> '' then begin
          if g_ItemArr[i].S.Shape = nType then begin
            idx := i;
            Break;
          end;
        end;
    ...
    if idx > -1 then begin
      ...
      if (g_ItemArr[idx].S.Overlap >= 1) and (g_ItemArr[idx].Dura > 1) then begin
...
begin
  Result := False;
  if (Str = '小退') and (g_MySelf.m_nHiterCode > 0) then begin
    AppLogoutEx();
    Exit;
  end;
  if (Str = '大退') and (g_MySelf.m_nHiterCode > 0) then begin
    DScreen.ClearHint;
    Application.Terminate;
    Exit;
  end;
  for i := 0 to MAXBAGITEMCL - 1 do begin
    if (g_ItemArr[i].S.Name = Str) and (g_ItemArr[i].S.NeedIdentify < 4) then begin
      EatItem(i);
      Result := True;
      Exit;
    end;
...
begin
  ...
  if idx in [0..MAXBAGITEMCL - 1] then begin
    ...
    if (g_EatingItem.S.Name = '') and (g_ItemArr[idx].S.Name <> '') and (g_ItemArr[idx].S.NeedIdentify < 4) then begin
      if (g_ItemArr[idx].S.StdMode <= 3) or (g_ItemArr[idx].S.StdMode = 31) then begin
        if (g_ItemArr[idx].S.Overlap >= 1) and (g_ItemArr[idx].Dura > 1) then begin
          g_ItemArr[idx].Dura := g_ItemArr[idx].Dura - 1;
          g_EatingItem := g_ItemArr[idx];
          g_ItemArr[idx].S.Name := '';
        ...
        if (g_ItemArr[idx].S.Overlap >= 1) then begin
          if (g_ItemArr[idx].Dura > 1) then begin
          ...
          lab1:
          if (g_ItemArr[idx].S.StdMode = 46) and (g_ItemArr[idx].S.Shape in [2..6]) then begin
            if not g_RareBoxWindow.m_boKeyAvail and (g_OpenBoxItem.Item.S.Name = '') and not FrmDlg.DWBoxBKGnd.Visible then begin
          ...
          if (g_ItemArr[idx].S.StdMode = 41) and
            ...
            (FrmDlg.DWBuildAcus.tag in [1, 2]) then begin
            n := -1;
            for i := 0 to 7 do begin
              if g_BuildAcuses[i].Item.S.Name = '' then begin
                if ((g_ItemArr[idx].S.Shape in [30..34]) and (i in [5..7])) or
                  ((g_ItemArr[idx].S.Shape in [10..14]) and (i in [0..4])) then begin
                  n := i;
                  Break;
                end;
              end;
        ...
        if where in [0..U_FASHION] then begin //takeon...
          takeon := True;
          g_EatingItem := g_ItemArr[idx];
          g_ItemArr[idx].S.Name := '';
        end;
  ...
  end else if (idx = -1) and g_boItemMoving then begin
    //if g_WaitingUseItem.Item.S.Name = '' then begin
    //end;

    if (g_MovingItem.Item.S.StdMode <= 4) or (g_MovingItem.Item.S.StdMode = 31) and (g_MovingItem.Item.S.NeedIdentify < 4) then begin
      if ((g_MovingItem.Item.S.StdMode <= 3) or (g_MovingItem.Item.S.StdMode = 31)) and (g_MovingItem.Item.S.Overlap >= 1) and (g_MovingItem.Item.Dura > 1) then begin
      ...
      if (g_EatingItem.S.StdMode = 4) and (g_EatingItem.S.Shape < 50) then begin
      ...
      if (g_MovingItem.Item.S.Overlap >= 1) then begin
        if (g_MovingItem.Item.Dura > 1) then begin
        ...
        lab2:
        if (g_MovingItem.Item.S.StdMode = 46) and (g_MovingItem.Item.S.Shape in [2..6]) then begin
          if not g_RareBoxWindow.m_boKeyAvail and (g_OpenBoxItem.Item.S.Name = '') and not FrmDlg.DWBoxBKGnd.Visible then begin
          ...
          for i := 0 to 7 do begin
            if g_BuildAcuses[i].Item.S.Name = '' then begin
              if ((g_MovingItem.Item.S.Shape in [30..34]) and (i in [5..7])) or
                ((g_MovingItem.Item.S.Shape in [10..14]) and (i in [0..4])) then begin
                n := i;
                Break;
              end;
            end;
      ...
      if where in [0..U_FASHION] then begin
        takeon := True;
        g_boItemMoving := False;
        g_EatingItem := g_MovingItem.Item;
        g_MovingItem.Item.S.Name := '';
        idx := frmMain.m_nEatRetIdx;
      end else {if g_WaitingUseItem.Item.S.Name = '' then}  begin
        //
      end;
    end;
  end;
  if eatable then begin
    m_nEatRetIdx := idx;
    m_boSupplyItem := True;
    g_dwEatTime := GetTickCount;
    SendEat(g_EatingItem.MakeIndex, g_EatingItem.S.Name, g_EatingItem.S.StdMode);
    g_SndMgr.ItemUseSound(g_EatingItem.S.StdMode);
  end else if takeon then begin
    m_nEatRetIdx := idx;
    g_dwEatTime := GetTickCount;
...
begin
  takeon := False;
  eatable := False;
  if idx in [0..MAXBAGITEMCL - 1 - 6] then begin
    if (g_EatingItem.S.Name <> '') and (GetTickCount - g_dwHeroEatTime > 5000) then
      g_EatingItem.S.Name := '';
    if (g_EatingItem.S.Name = '') and (g_HeroItemArr[idx].S.Name <> '') then begin
      if (g_HeroItemArr[idx].S.StdMode <= 3) or (g_HeroItemArr[idx].S.StdMode = 31) then begin
        if (g_HeroItemArr[idx].S.Overlap >= 1) and (g_HeroItemArr[idx].Dura > 1) then begin
    ...
    if (g_MovingItem.Item.S.StdMode <= 4) or (g_MovingItem.Item.S.StdMode in [31, 42]) then begin
      if ((g_MovingItem.Item.S.StdMode <= 3) or (g_MovingItem.Item.S.StdMode = 31)) and (g_MovingItem.Item.S.Overlap >= 1) and (g_MovingItem.Item.Dura > 1) then begin
        g_MovingItem.Item.Dura := g_MovingItem.Item.Dura - 1;
        g_boItemMoving := False;
        g_EatingItem := g_MovingItem.Item;
      ...
      if (g_EatingItem.S.StdMode = 4) and (g_EatingItem.S.Shape < 50) then begin
        if mrYes <> FrmDlg.DMessageDlg(Format('英雄[%s]是否确认开始练习 "' + g_EatingItem.S.Name + '"？', [g_MySelf.m_HeroObject.m_sUserName]), [mbYes, mbNo]) then begin
...
begin
  ...
  case ndir of
    ...
    DR_DOWN: if sny < (Map.m_MapHeader.wHeight - nFlag) then Inc(sny, nFlag);
    DR_LEFT: if snx > nFlag - 1 then Dec(snx, nFlag);
    DR_RIGHT: if snx < (Map.m_MapHeader.wWidth - nFlag) then Inc(snx, nFlag);
    DR_UPLEFT: begin
    ...
    DR_UPRIGHT: begin
        if (snx > nFlag - 1) and (sny < (Map.m_MapHeader.wHeight - nFlag)) then begin
          Inc(snx, nFlag);
          Dec(sny, nFlag);
        end;
      end;
    DR_DOWNLEFT: begin
        if (snx < (Map.m_MapHeader.wWidth - nFlag)) and (sny > nFlag - 1) then begin
          Dec(snx, nFlag);
          Inc(sny, nFlag);
        end;
      end;
    DR_DOWNRIGHT: begin
        if (snx < (Map.m_MapHeader.wWidth - nFlag)) and (sny < (Map.m_MapHeader.wHeight - nFlag)) then begin
          Inc(snx, nFlag);
          Inc(sny, nFlag);
        end;
      end;
...
begin
  ...
  nC := 1;
  while True do begin
    if GetNextPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, ndir, nC, nX, nY) then begin
      Actor := g_PlayScene.FindActorXY(nX, nY);
...
begin
  ...
  if ((ssLeft in Shift) or (ssRight in Shift)) and (GetTickCount - m_dwMouseDownTime >= 300) then begin
    m_boMouseUpEnAble := True;
...
begin
  ...
  if GetTickCount - m_dwMouseDownTime > 120 then begin
...
var
  i: Integer;
begin
  if m_boMouseUpEnAble then begin
  ...
  with g_MySelf do
    case btDir of
      DR_UP: begin
          ...
          if not g_PlayScene.CanWalk(nX, nY) then begin
            nY := m_nCurrY + 2;
          end;
        end;
      DR_DOWN: begin
...
begin
  ...
  if (abs(g_MySelf.m_nCurrX - target.m_nCurrX) <= 1) and (abs(g_MySelf.m_nCurrY - target.m_nCurrY) <= 1) and (not target.m_boDeath) then begin
    if TimerAutoPlay.Enabled then begin
      g_boAPAutoMove := False;
      if g_APTagget <> nil then g_sAPStr := Format('[挂机] 怪物目标：%s (%d,%d) 正在使用普通攻击', [g_APTagget.m_sUserName, g_APTagget.m_nCurrX, g_APTagget.m_nCurrY]);
    end;

    if CanNextAction and ServerAcceptNextAction {and (CanNextHit or (g_NextSeriesSkill))} then begin

      if CanNextHit(False) or g_NextSeriesSkill then begin
        g_NextSeriesSkill := False;
        if g_boNextTimeSmiteHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimeSmiteHit := False;
          nHitMsg := CM_SMITEHIT;
          g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
          goto lab;
        end else if g_boNextTimeSmiteLongHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimeSmiteLongHit := False;
          nHitMsg := CM_SMITELONGHIT;
          g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
          goto lab;
        end else if g_boNextTimeSmiteWideHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimeSmiteWideHit := False;
          nHitMsg := CM_SMITEWIDEHIT;
          g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
          goto lab;
        end else if g_boNextTimeSmiteLongHit2 and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimeSmiteLongHit2 := False;
      ...
      if CanNextHit then begin
        if g_boNextTimeSmiteWideHit2 and (g_MySelf.m_Abil.MP >= 1) then begin
          g_boNextTimeSmiteWideHit2 := False;
          nHitMsg := CM_SMITEWIDEHIT2;
        end else if g_boNextTimeSmiteLongHit3 and (g_MySelf.m_Abil.MP >= 1) then begin
          g_boNextTimeSmiteLongHit3 := False;
          nHitMsg := CM_SMITELONGHIT3;
        end else if g_boNextTimeTwinHit and (g_MySelf.m_Abil.MP >= 10) then begin
          g_boNextTimeTwinHit := False;
          nHitMsg := CM_TWNHIT;
        end else if g_boNextTimePursueHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimePursueHit := False;
          nHitMsg := CM_PURSUEHIT;
        end else if g_boNextTimeFireHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boNextTimeFireHit := False;
          nHitMsg := CM_FIREHIT;
        end else if g_boCanSLonHit and (g_MySelf.m_Abil.MP >= 7) then begin
          g_boCanSLonHit := False;
          nHitMsg := CM_HERO_LONGHIT2;
        end else if g_boNextTimePowerHit then begin
          g_boNextTimePowerHit := False;
          nHitMsg := CM_POWERHIT;
        end else if g_boCanSquHit and (g_MySelf.m_Abil.MP >= 3) and (g_nSquHitPoint > 0) then begin
          nHitMsg := CM_SQUHIT;
        end else if (g_MySelf.m_Abil.MP >= 3) and (g_boCanWideHit or (g_gcTec[1] and (GetMagicByID(25) <> nil) and TargetInSwordWideAttackRange(tdir))) then begin
          nHitMsg := CM_WIDEHIT;
        end else if g_boCanCrsHit and (g_MySelf.m_Abil.MP >= 6) then begin
          nHitMsg := CM_CRSHIT;
        end else if g_boCanLongHit and (TargetInSwordLongAttackRange(tdir)) then begin
          nHitMsg := CM_LONGHIT;
        end;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
      end;
    ...
    if g_boNextTimeSmiteWideHit2 and (g_MySelf.m_Abil.MP >= 1) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        if (abs(g_MySelf.m_nCurrX - target.m_nCurrX) <= 5) and
          (abs(g_MySelf.m_nCurrY - target.m_nCurrY) <= 5) then begin
          g_boNextTimeSmiteWideHit2 := False;
          nHitMsg := CM_SMITEWIDEHIT2;
          g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boNextTimeSmiteLongHit3 and (g_MySelf.m_Abil.MP >= 1) and TargetInSwordLongAttackRangeA(tdir) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        g_boNextTimeSmiteLongHit3 := False;
        nHitMsg := CM_SMITELONGHIT3;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boNextTimeSmiteLongHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeA(tdir) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        g_boNextTimeSmiteLongHit := False;
        nHitMsg := CM_SMITELONGHIT;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boNextTimeSmiteLongHit2 and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        g_boNextTimeSmiteLongHit2 := False;
        nHitMsg := CM_SMITELONGHIT2;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boNextTimePursueHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        g_boNextTimePursueHit := False;
        nHitMsg := CM_PURSUEHIT;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boCanSLonHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        g_boCanSLonHit := False;
        nHitMsg := CM_HERO_LONGHIT2;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
    ...
    if g_boCanLongHit and (g_MySelf.m_btJob = 0) and (not target.m_boDeath) and g_boAutoLongAttack and g_gcTec[10] and (g_MagicArr[0][12] <> nil) and
      TargetInSwordLongAttackRange2(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, target.m_nCurrX, target.m_nCurrY) then begin
      if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
        nHitMsg := CM_LONGHIT;
        g_MySelf.SendMsg(nHitMsg, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
        g_dwLastAttackTick := GetTickCount;
      end else if g_boAutoLongAttack and g_gcTec[10] and TimerAutoPlay.Enabled then begin //走刺杀位
        Result := True;
        Exit;
      end;
    end else begin
      ...
      if (g_MySelf.m_btJob = 0) and g_boAutoLongAttack and g_gcTec[10] and (g_MagicArr[0][12] <> nil) then begin
    ...
    if TimerAutoPlay.Enabled then begin
      g_boAPAutoMove := True;
      if target <> nil then g_sAPStr := Format('[挂机] 怪物目标：%s (%d,%d) 正在跑向', [target.m_sUserName, target.m_nCurrX, target.m_nCurrY]);
    end;
  end;
end;

var
  g_dwThreadExit: LongWord;
...
begin
  ...
  if boClick then begin
    if (Button = mbRight) and (g_OpenBoxItem.Item.S.Name <> '') and not g_RareBoxWindow.m_boRareBoxShow and (FrmDlg.DWBoxBKGnd.Visible) then begin
    ...
    if (Button = mbRight) and g_boItemMoving then begin //当前是否在移动物品
  ...
  if (ssRight in Shift) then begin
    if boClick then begin
      ...
      if TimerAutoMove.Enabled then begin
...
//              FrmDlg.DEdChat.SelLength := 0;
              g_PlayScene.EdChat.Visible := True;
              g_PlayScene.EdChat.SetFocus;
              g_PlayScene.EdChat.Text := '/' + target.m_sUserName + ' ';
              g_PlayScene.EdChat.SelStart := Length(g_PlayScene.EdChat.Text);
              g_PlayScene.EdChat.SelLength := 0;
              Exit;
            end;
      ...
      if g_FocusItem2 <> nil then begin
    ...
    if (abs(g_MySelf.m_nCurrX - g_nMouseCurrX) <= 1) and (abs(g_MySelf.m_nCurrY - g_nMouseCurrY) <= 1) then begin //目标座标
      if boClick then begin
      ...
      if g_UseItems[U_WEAPON].S.Shape = 19 then begin
        ...
        if not Map.CanMove(nX, nY) or (ssShift in Shift) then begin
          if CanNextAction and ServerAcceptNextAction and CanNextHit then
            g_MySelf.SendMsg(CM_HEAVYHIT, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, 0, 0, '', 0);
          g_boAutoDig := True;
          Exit;
        end;
      end;
    ...
    if (ssAlt in Shift) and (g_MySelf.m_btHorse = 0) then begin
      ...
      if (target <> nil) or (ssShift in Shift) then begin
        ...
        if target <> nil then begin
          if GetTickCount - g_dwLastMoveTick > 500 then begin
            //
            if boClick and (target is THumActor) and THumActor(target).m_StallMgr.OnSale then begin
            ...
            if boClick and (target.m_btRace = RCC_MERCHANT) then begin
              //SendClientMessage(CM_CLICKNPC, target.m_nRecogId, 0, 0, 0);
              //NPC大窗口相关 Development 2019-01-14
              SendClientMessage (CM_CLICKNPC, target.m_nRecogId, 0, 0,  BoolToInt(FrmDlg.DMerchantBigDlg.Visible));
              g_dwLastMoveTick := GetTickCount;
          ...
          if boClick and
            ...
            (not (target is THumActor) or not THumActor(target).m_StallMgr.OnSale) then begin
            g_TargetCret := target;
            if ((target.m_btRace <> 0)
              ...
              or (g_gcGeneral[2])
              or (ssShift in Shift)
              {or (target.m_btNameColor = ENEMYCOLOR)}then begin
              AttackTarget(target);
            end;
          end;
        end else begin //骑马不允许操作
          if (g_MySelf.m_btHorse = 0) then begin
            ...
            if CanNextAction and ServerAcceptNextAction and CanNextHit then begin
              if g_boNextTimeSmiteWideHit2 and (g_MySelf.m_Abil.MP >= 1) then begin
                g_boNextTimeSmiteWideHit2 := False;
                nHitMsg := CM_SMITEWIDEHIT2;
              end else if g_boNextTimeSmiteLongHit3 and (g_MySelf.m_Abil.MP >= 1) and TargetInSwordLongAttackRangeA(tdir) then begin
                g_boNextTimeSmiteLongHit3 := False;
                nHitMsg := CM_SMITELONGHIT3;
              end else if g_boNextTimeSmiteLongHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeA(tdir) then begin
                g_boNextTimeSmiteLongHit := False;
                nHitMsg := CM_SMITELONGHIT;
              end else if g_boNextTimeSmiteLongHit2 and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
                g_boNextTimeSmiteLongHit2 := False;
                nHitMsg := CM_SMITELONGHIT2;
              end else if g_boNextTimePursueHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
                g_boNextTimePursueHit := False;
                nHitMsg := CM_PURSUEHIT;
              end else if g_boCanSLonHit and (g_MySelf.m_Abil.MP >= 7) and TargetInSwordLongAttackRangeX(tdir) then begin
                g_boCanSLonHit := False;
                nHitMsg := CM_HERO_LONGHIT2;
              end else begin
                ...
                if g_boCanSquHit and (g_MySelf.m_Abil.MP >= 3) and (g_nSquHitPoint > 0) then begin
                  if g_boCanRunAllInWarZone then begin
                    GetFrontPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, tdir, nX, nY);
                    Actor := g_PlayScene.FindActorXY(nX, nY);
        ...
        end else if GetTickCount - g_dwLastAttackTick > 900 then begin //最后攻击操作停留指定时间才能移动
          if ssCtrl in Shift then begin
            //
            g_ChrAction := caRun;
          end else
...
begin
  ...
  if (g_gcGeneral[1] or g_gcGeneral[9]) and (GetTickCount - m_dwDuraWarningTick > 60 * 1000) then begin
    m_dwDuraWarningTick := GetTickCount;
    if (g_MySelf <> nil) and not g_MySelf.m_boDeath then begin
      for i := High(g_UseItems) downto Low(g_UseItems) do begin
        if (g_UseItems[i].S.Name <> '') then begin
          if g_UseItems[i].S.StdMode in [7, 25] then
            Continue;
          if g_UseItems[i].Dura < 1500 then begin
            if g_gcGeneral[1] then
              DScreen.AddSysMsgCenter(Format('你的[%s]持久已到底限，请及时修理！', [g_UseItems[i].S.Name]), clLime, clblack, 10);
            if g_gcGeneral[9] then begin
              fixidx := -1;
              for ii := MAXBAGITEM - (1 + 0) downto 0 do begin
                if (g_ItemArr[ii].S.NeedIdentify < 4) and (g_ItemArr[ii].S.Name <> '') and (g_ItemArr[ii].S.StdMode = 2) and (g_ItemArr[ii].S.Shape = 9) and (g_ItemArr[ii].Dura > 0) then begin
                  fixidx := ii;
                  Break;
                end;
              end;
              if fixidx > -1 then begin
                EatItem(fixidx);
              end else begin
    ...
    if (g_MySelf <> nil) and (g_MySelf.m_HeroObject <> nil) and not g_MySelf.m_HeroObject.m_boDeath then begin
      for i := High(g_HeroUseItems) downto Low(g_HeroUseItems) do begin
        if (g_HeroUseItems[i].S.Name <> '') then begin
          if g_HeroUseItems[i].S.StdMode in [7, 25] then
            Continue;
          if g_HeroUseItems[i].Dura < 1500 then begin
            if g_gcGeneral[1] then
              DScreen.AddSysMsgCenter(Format('(英雄) 你的[%s]持久已到底限，请及时修理！', [g_HeroUseItems[i].S.Name]), clLime, clblack, 10);
            if g_gcGeneral[9] then begin
              fixidx := -1;
              for ii := MAXBAGITEM - (1 + 0) downto 0 do begin
                if (g_HeroItemArr[ii].S.Name <> '') and (g_HeroItemArr[ii].S.StdMode = 2) and (g_HeroItemArr[ii].S.Shape = 9) and (g_HeroItemArr[ii].Dura > 0) then begin
                  fixidx := ii;
                  Break;
                end;
              end;
  ...
  if (g_MySelf <> nil) and not g_MySelf.m_boDeath and (g_MySelf.m_nIPowerLvl > 5) and (g_MySelf.m_nIPower < 30) and (GetTickCount - dwIPTick > 30 * 1000) then begin
    dwIPTick := GetTickCount;
    fixidx := -1;
    for ii := MAXBAGITEM - (1 + 0) downto 0 do begin
      if (g_ItemArr[ii].S.NeedIdentify < 4) and (g_ItemArr[ii].S.Name <> '') and (g_ItemArr[ii].S.StdMode = 2) and (g_ItemArr[ii].S.Shape = 13) and (g_ItemArr[ii].DuraMax > 0) then begin
        fixidx := ii;
        Break;
      end;
    end;
  ...
  if (g_MySelf <> nil) and (g_MySelf.m_HeroObject <> nil) and not g_MySelf.m_HeroObject.m_boDeath and
    (g_MySelf.m_HeroObject.m_nIPowerLvl > 5) and (g_MySelf.m_HeroObject.m_nIPower < 30) and (GetTickCount - dwhIPTick > 30 * 1000) then begin
    dwhIPTick := GetTickCount;
    fixidx := -1;
    for ii := MAXBAGITEM - (1 + 0) downto 0 do begin
      if (g_HeroItemArr[ii].S.Name <> '') and (g_HeroItemArr[ii].S.StdMode = 2) and (g_HeroItemArr[ii].S.Shape = 13) and (g_HeroItemArr[ii].DuraMax > 0) then begin
        fixidx := ii;
        Break;
      end;
    end;
  ...
  if g_TargetCret <> nil then begin
    if ActionKey > 0 then begin
      ...
      if not g_TargetCret.m_boDeath and g_PlayScene.IsValidActor(g_TargetCret)
        and (not (g_TargetCret is THumActor) or not THumActor(g_TargetCret).m_StallMgr.OnSale) then begin
        ...
        if GetKeyboardState(keyvalue) then begin
          ...
          or ((ssShift in Shift) and (not g_PlayScene.EdChat.Visible{FrmDlg.DEdChat.Visible})) then begin
  ...
  if (g_MySelf <> nil) and (g_boAutoDig or g_boAutoSit) then begin
    if CanNextAction and ServerAcceptNextAction and (g_boAutoSit or CanNextHit) then begin
      if g_boAutoDig then
        g_MySelf.SendMsg(CM_HIT + 1, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_MySelf.m_btDir, 0, 0, '', 0);

      if g_boAutoSit then begin
        target := g_PlayScene.ButchAnimal(g_nMouseCurrX, g_nMouseCurrY);
        if target <> nil then begin
          ...
          g_MySelf.SendMsg(CM_SITDOWN, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, ii, 0, 0, '', 0);
        end else begin
          ii := GetNextDirection(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nMouseCurrX, g_nMouseCurrY);
  ...
  //动自捡取
  if g_boAutoPickUp and (g_MySelf <> nil) and (((GetTickCount() - g_dwAutoPickupTick) > g_dwAutoPickupTime) {or g_boQuickPickup}) then begin
...
begin
  ...
  with FrmDlg do begin
    if DBeltWindow.Visible then begin
    ...
    DMerchantBigDlg.Visible := False; //NPC大窗口相关  Development 2019-01-14
...
begin
  CloseAllWindows;
  ClearDropItems;
  if g_RareBoxWindow <> nil then g_RareBoxWindow.Initialize();

  for i := Low(FrmDlg.m_MissionList) to High(FrmDlg.m_MissionList) do begin
...
begin
  ...
  if g_ConnectionStep = cnsLogin {cnsIntro} then begin
...
//{$IFEND}
  ...
  if g_ConnectionStep = cnsPlay then begin
    if not g_boServerChanging then begin
      ...
      if not g_boDoFadeOut and not g_boDoFadeIn then begin //由暗变亮，进入登录公告界面。
        g_boDoFadeIn := True;
        g_nFadeIndex := 0;
      end;
    end else
...
begin
  g_boServerConnected := False;
  if (DScreen.CurrentScene = LoginScene) {(g_ConnectionStep = cnsIntro)} and not g_boSendLogin then begin
...
begin
  if (nPage = FrmDlg.m_nLastLevelRankPage) and (nType = FrmDlg.m_nLastLevelRankType) then Exit;
  if GetTickCount - FrmDlg.m_dwSendMessageTick > 300 then begin
...
begin
   ...
   SendSocket (EncodeMessage (msg) + EncodeString(uid + '/' + passwd));//20090309 增加服务名
   g_boSendLogin:=True;
end;
...
//      FrmDlg.DEdChat.Visible := False;
      g_PlayScene.EdChat.PasswordChar := #0;
      g_PlayScene.EdChat.Visible := False;
      if not g_ChatStatusLarge then FrmDlg.DBChat.Visible := False;
      SendPassword(Str, 1);
      Exit;
    ...
    if CompareLstr(Str, sam, Length(sam)) then begin
...
//      else FrmDlg.DEdChat.PasswordChar := #0;
      if g_PlayScene.EdChat.PasswordChar = #0 then
        g_PlayScene.EdChat.PasswordChar := '*'
      else g_PlayScene.EdChat.PasswordChar := #0;
      Exit;
    end;
//    if FrmDlg.DEdChat.PasswordChar = '*' then
//      FrmDlg.DEdChat.PasswordChar := #0;
    if g_PlayScene.EdChat.PasswordChar = '*' then
      g_PlayScene.EdChat.PasswordChar := #0;
...
var
  msg: TDefaultMessage;
begin
  if (g_MySelf.m_HeroObject <> nil) and (g_FocusCret <> nil) then begin
  ...
  if Length(rstr) >= 2 then begin
    ...
    if (rstr[1] = '@') and (rstr[2] = '@') then begin
      if CompareText(rstr, '@@buildguildnow') = 0 then
        ...
        FrmDlg.DMessageDlg('请输入英雄的名字：', [mbOk, mbAbort])
      else if CompareText(rstr, '@@TreasureIdentify') = 0 then begin
        FrmDlg.OpenDBTI();
      end else if CompareText(rstr, '@@ExchangeBook') = 0 then begin
      ...
      end else if CompareText(rstr, '@@SecretProperty') = 0 then begin
        FrmDlg.OpenDBSP();
      end
      else
    ...
    if (rstr[1] = '@') then begin
      if CompareText(rstr, '@closewin') = 0 then begin
        FrmDlg.CloseMDlg;
      end else if CompareText(rstr, '@buildacus') = 0 then begin
...
begin
  ...
  if g_MySelf.m_nIPower < 5 then begin
    if GetTickCount - g_IPointLessHintTick > 10000 then begin
      g_IPointLessHintTick := GetTickCount;
      DScreen.AddSysMsg('内力值不足...');
    end;
    Exit;
  end;
  if (g_MySelf.m_nState and $04000000 = 0) and (g_MySelf.m_nState and $02000000 = 0) then begin
    if GetTickCount - g_SendFireSerieSkillTick > 1000 then begin
...
begin
  ...
  if ActionLock then begin
    if GetTickCount - ActionLockTime > 5 * 1000 then begin
      ActionLock := False;
      //Dec (WarningLevel);
    end;
    Result := False;
...
begin
  ...
  if GetTickCount - LastHitTick > LongWord(NextHitTime) then begin
    if settime then LastHitTick := GetTickCount;
    Result := True;
  end else
    Result := False;
end;
...
begin
  ...
  if (btPacket = 0) and (datablock[1] = '+') then begin
  ...
  if g_MySelf = nil then begin
    case msg.ident of
      ...
      //123456
      SM_OVERCLIENTCOUNT: begin
          g_boDoFastFadeOut := False;
          FrmDlg.DMessageDlg('客户端开启数量过多，连接被断开！！！', [mbOk]);
          Close();
          Exit;
        end;
      SM_CDVERSION_FAIL: begin
          g_boDoFastFadeOut := False;
  ...
  case msg.ident of
    ...
    SM_MOVEDETECTITEM_FALI: begin
        case msg.Recog of
          00: begin
              if g_WaitingDetectItem.Item.S.Name <> '' then begin
    ...
    SM_SENDHEROS: begin
        ...
        for i := Low(g_heros) to High(g_heros) do begin
          if g_heros[i].chrname <> '' then begin
            Str := '               ';
    ...
    SM_UPDATESTALLITEM: begin
        ...
        else if g_WaitingStallItem.Item.S.Name <> '' then begin
...
{$IF SERIESSKILL}
    ...
    SM_TRAINVENATION: begin
        if msg.series = 0 then begin
          case msg.Recog of
            00: begin
                DecodeBuffer(body, @g_VenationInfos, SizeOf(g_VenationInfos));
                FrmDlg.PageChanged;
                if g_VLastSender <> nil then FrmDlg.DBV1Click(g_VLastSender, 0, 0);
              end;
          ...
          case msg.Recog of
            00: begin
                ...
                if g_hVLastSender <> nil then FrmDlg.DBhV1Click(g_hVLastSender, 0, 0);
              end;
    ...
    SM_SETSERIESSKILL: begin
        if (msg.param in [0..High(TSeriesSkillArr)]) then begin
          if msg.series = 0 then begin
            if (msg.Recog < 0) then begin
...
{$IFEND SERIESSKILL}
    SM_SETMISSION: begin
        if msg.param in [1..4] then begin //class
            ...
            1: begin //add update
                ...
                try
                  for i := 0 to List.count - 1 do begin
                    pMission := List[i];
                    if IntToStr(msg.Recog) = pMission.sindex then begin
    ...
    SM_QUERYBINDITEM: if msg.param = 0 then ClientGetSendBindItem(msg.Recog) else ClientGetSendUnBindItem(msg.Recog);

    SM_ITEMDLGSELECT: begin
        if msg.param = 255 then
          g_SellDlgItem.S.Name := '';
        if msg.Recog = 0 then
          FrmDlg.CloseDSellDlg;
      end;
    SM_SETTARGETXY: begin
        //
      end;
    SM_AFFIRMYBDEA_FAIL: begin
        case msg.Recog of
    ...
    SM_QUERYYBSELL_SELL: begin
        if (msg.Recog = -1) or (msg.Recog = -2) then begin
          FrmDlg.ShowMDlg(0, '', '[失败]: 没有查询到指定的记录！');
        end else begin
          FrmDlg.ServerSendYBSell(msg.Recog, msg.series, body);
        end;
      end;
    SM_QUERYYBSELL_DEAL: begin
        if (msg.Recog = -1) or (msg.Recog = -2) then begin
          FrmDlg.ShowMDlg(0, '', '[失败]: 没有查询到指定的记录！');
        end else begin
    ...
    SM_OPENBOX: if g_RareBoxWindow.m_boKeyAvail or (msg.param = 1) then begin
        ...
        if g_OpenBoxItem.Item.S.Name <> '' then g_OpenBoxItem.Item.S.Name := '';
        if g_WaitingUseItem.Item.S.Name <> '' then g_WaitingUseItem.Item.S.Name := '';
    ...
    SM_LEVELRANK: begin
        ...
        if msg.Recog >= 0 then begin //reture page
          if FrmDlg.m_nLevelRankPage <> msg.Recog then FrmDlg.m_nLevelRankPage := msg.Recog;
          case msg.param of
            0..3: begin
    ...
    SM_LOGON: begin
        ...
        if not g_gcGeneral[11] then begin
    ...
    SM_SERVERCONFIG2: begin
        ...
        if msg.series > 200 then g_nEatItemInvTime := msg.series;
    ...
    SM_SERVERCONFIG3: begin
{$IF CONFIGTEST}
        g_boSpeedRate := True;
        g_boSpeedRateShow := g_boSpeedRate;
{$ELSE}
        if (LoByte(msg.series) > 0) then begin
          g_boSpeedRate := True;
...
{$IFEND CONFIGTEST}
        ...
        if data <> '' then begin
          ...
          if Actor <> nil then begin
            ...
            if Pos('(', Actor.m_sUserName) <> 0 then begin
              ...
              if data = g_MySelf.m_sUserName then begin
                j := 0;
                for i := 0 to g_MySelf.m_SlaveObject.count - 1 do begin
                  if TActor(g_MySelf.m_SlaveObject[i]) = Actor then begin
                    j := 1;
                    Break;
                  end;
                end;
    ...
    SM_SPACEMOVE_HIDE, SM_SPACEMOVE_HIDE2: if msg.Recog <> g_MySelf.m_nRecogId then g_PlayScene.SendMsg(msg.ident, msg.Recog, msg.param {x}, msg.tag {y}, 0, 0, 0, '');
    SM_HEROLOGIN: begin
        Actor := g_PlayScene.FindActor(msg.Recog);
        if Actor <> nil then begin
    ...
    SM_SPACEMOVE_SHOW, SM_SPACEMOVE_SHOW2: begin
        n := GetCodeMsgSize(SizeOf(TCharDesc) * 4 / 3);
        if Length(body) > n then begin
          body2 := Copy(body, n + 1, Length(body));
          data := DecodeString(body2);
    ...
    SM_RUSH, SM_RUSHEX, SM_RUSHKUNG: begin
    ...
    SM_WALK, SM_RUN, SM_HORSERUN: begin
        DecodeBuffer(body, @desc, SizeOf(TCharDesc));
        if msg.Recog <> g_MySelf.m_nRecogId then
          g_PlayScene.SendMsg(msg.ident, msg.Recog,
            msg.param {x},
    ...
    SM_LAMPCHANGEDURA: if g_UseItems[U_RIGHTHAND].S.Name <> '' then g_UseItems[U_RIGHTHAND].Dura := msg.Recog;
    SM_HEROLAMPCHANGEDURA: if g_HeroUseItems[U_RIGHTHAND].S.Name <> '' then g_HeroUseItems[U_RIGHTHAND].Dura := msg.Recog;
    ...
    SM_BUTCH: begin  //挖肉动作封包 Development 2019-01-31 分析
        DecodeBuffer(body, @desc, SizeOf(TCharDesc));
        if msg.Recog <> g_MySelf.m_nRecogId then begin
          Actor := g_PlayScene.FindActor(msg.Recog);
          if Actor <> nil then begin
...
//                Actor.m_boDigFragment := True;
    ...
    SM_SITDOWN: begin  //蹲下动作封包 Development 2019-01-31 分析
        DecodeBuffer(body, @desc, SizeOf(TCharDesc));
        if msg.Recog <> g_MySelf.m_nRecogId then begin
          Actor := g_PlayScene.FindActor(msg.Recog);
          if Actor <> nil then begin
...
//                Actor.m_boDigFragment := True;
    ...
    SM_FLYAXE, SM_81, SM_82, SM_83: begin
    ...
    SM_LIGHTING, SM_LIGHTING_1..SM_LIGHTING_3: begin
        DecodeBuffer(body, @wl, SizeOf(TMessageBodyWL));
        Actor := g_PlayScene.FindActor(msg.Recog);
        if Actor <> nil then begin
          Actor.SendMsg(msg.ident,
    ...
    SM_DEATH, SM_NOWDEATH: begin
        ...
        if Actor <> nil then begin
          ...
          g_PlayScene.SendMsg(SM_DEATH, msg.Recog, msg.param {x}, msg.tag {y}, msg.series {dir}, desc.Feature, desc.Status, '', desc.StatusEx);
        end;
      end;
    SM_SKELETON: begin
        DecodeBuffer(body, @desc, SizeOf(TCharDesc));
        g_PlayScene.SendMsg(SM_SKELETON, msg.Recog, msg.param {HP}, msg.tag {maxHP}, msg.series {damage}, desc.Feature, desc.Status, '', desc.StatusEx);
      end;
    SM_ALIVE: begin
        DecodeBuffer(body, @desc, SizeOf(TCharDesc));
        g_PlayScene.SendMsg(SM_ALIVE, msg.Recog, msg.param {HP}, msg.tag {maxHP}, msg.series {damage}, desc.Feature, desc.Status, '', desc.StatusEx);
      end;
    SM_ABILITY: begin
...
//          g_boViewFog := True;
    ...
    SM_WINNIMBUSEXP: begin
        if msg.Recog > 0 then DScreen.AddSysMsgBottom(Format('当前灵气值 %d', [msg.Recog]));
      end;
    SM_HEROWINNIMBUSEXP: begin
        if msg.Recog > 0 then DScreen.AddSysMsgBottom(Format('(英雄)当前灵气值 %d', [msg.Recog]));
      end;
    SM_WINIPEXP: begin
        g_MySelf.m_nIPowerExp := msg.Recog;
        ipExp := LongWord(Makelong(msg.param, msg.tag));
        if ipExp > 0 then DScreen.AddSysMsgBottom(Format('%d点内功经验增加', [ipExp]));
        if msg.series in [3..28] then
          g_nMagicRange := msg.series;      
      end;
    SM_HEROWINIPEXP: begin
        if g_MySelf.m_HeroObject <> nil then begin
          g_MySelf.m_HeroObject.m_nIPowerExp := msg.Recog;
          ipExp := LongWord(Makelong(msg.param, msg.tag));
          if ipExp > 0 then DScreen.AddSysMsgBottom(Format('(英雄)%d点内功经验增加', [ipExp]));
        end;
      end;
    SM_LEVELUP: begin
...
{$IFEND}
    ...
    SM_STRUCK: begin
        ...
        if Actor <> nil then begin
          ...
          if (Actor <> g_MySelf) and (Actor <> g_MySelf.m_HeroObject) then begin //blue
            if (Actor.m_btRace <> 0) or not g_gcGeneral[15] then
              Actor.UpdateMsg(SM_STRUCK, wl.lTag2, 0, msg.series {damage}, wl.lParam1, wl.lParam2, '', wl.lTag1);
          end;
{$IF HIGHHP}
...
{$IFEND}
          if g_boOpenAutoPlay and TimerAutoPlay.Enabled then begin //0613 自己受人攻击,小退
            Actor2 := g_PlayScene.FindActor(wl.lTag1);
            if (Actor2 = nil) or ((Actor2.m_btRace <> 0) and (Actor2.m_btIsHero <> 1)) then Exit;

            if (g_MySelf <> nil) then begin
              if (Actor = g_MySelf.m_HeroObject) then begin //英雄受人攻击
                //
                FrmDlg.ClientCallHero();

              end else if (Actor = g_MySelf) then begin //自己受人攻击,小退
                g_nAPReLogon := 1; //保存状态
...
{$IFEND}
    ...
    SM_SYSMESSAGE2: begin
        Str := DecodeString(body);
        DScreen.m_adList.InsertObject(0, Str, TObject(msg.param)); //屏幕顶部滚动公告
        nad := 0;
        DScreen.m_adList2.InsertObject(0, '', TObject(nad));
      end;
    SM_SYSMESSAGE4: begin
        Str := DecodeString(body);
        DScreen.AddSysMsgBottom2(Str);
      end;
    SM_HEAR, SM_CRY, SM_GROUPMESSAGE, SM_GUILDMESSAGE, SM_WHISPER, SM_SYSMESSAGE: begin
        if (msg.ident = SM_HEAR) {or (Msg.ident = SM_GROUPMESSAGE)} and (FrmDlg.DBRefuseSay.tag <> 0) then Exit;
        if (msg.ident = SM_CRY) and (FrmDlg.DBRefuseCry.tag <> 0) then Exit;
        if (msg.ident = SM_WHISPER) and (FrmDlg.DBRefuseWhisper.tag <> 0) then Exit;
        if (msg.ident = SM_GUILDMESSAGE) and (FrmDlg.DBRefuseGuild.tag <> 0) then Exit;
        Str := DecodeString(body);
        if msg.tag > 0 then begin
          DScreen.AddSysMsgCenter(Str, GetRGB(LoByte(msg.param)), GetRGB(HiByte(msg.param)), msg.tag);
          Exit;
        end;
        if FrmDlg.m_BlockList.count > 0 then begin
          Str2 := ExtractUserName(Str);
          nFuncPos := FrmDlg.m_BlockList.IndexOf(Str2);
          if nFuncPos >= 0 then begin
            Exit;
          end;
        end;
        ...
        if msg.ident = SM_WHISPER then begin
          ...
          if FrmDlg.m_xChatRecordList.count > 5000 then begin
        ...
        else if msg.ident = SM_HEAR then begin
          Actor := g_PlayScene.FindActor(msg.Recog);
          if Actor <> nil then
            Actor.Say(Str);
        end;
      end;
    SM_ATTACKMODE: begin //攻击模式
        case msg.param of
          HAM_ALL: begin
    ...
    SM_USERNAME: begin
        ...
        if Actor <> nil then begin
          ...
          if msg.tag in [1..5] then Actor.m_btAttribute := msg.tag;
        end;
      end;
    ...
    SM_HIDE, SM_GHOST, SM_DISAPPEAR: begin
    ...
    SM_HEROSTATE: if g_MySelf <> nil then begin
        DecodeBuffer(body, @wl, SizeOf(TMessageBodyWL));
    ...
    SM_HEROABILITY: if (g_MySelf <> nil) and (g_MySelf.m_HeroObject <> nil) then begin
    ...
    SM_HEROSTATEDISPEAR: if g_MySelf <> nil then begin
        if msg.Recog = 0 then begin
          SaveBagsData();
          HeroClearBag();
          g_MySelf.m_HeroObject := nil;
          ...
          for i := 0 to g_MySelf.m_SlaveObject.count - 1 do begin
            if TActor(g_MySelf.m_SlaveObject[i]).m_nRecogId = msg.Recog then begin
    ...
    SM_HERONAME: begin
        Str := DecodeString(body);
        if (Str <> '') and (g_MySelf.m_HeroObject <> nil) then begin
          g_MySelf.m_HeroObject.m_sDescUserName := GetValidStr3(Str, g_MySelf.m_HeroObject.m_sUserName, ['\']);
          g_MySelf.m_HeroObject.m_sUserNameOffSet := g_DXCanvas.TextWidth(g_MySelf.m_HeroObject.m_sUserName) div 2;
    ...
    SM_COUNTERITEMCHANGE: begin
        if not g_boDealEnd then g_dwDealActionTick := GetTickCount;
        ChangeItemCount(msg.Recog, msg.param, msg.tag, DecodeString(body));
      end;
    SM_HEROCOUNTERITEMCHANGE: begin
        if not g_boDealEnd then g_dwDealActionTick := GetTickCount;
    ...
    SM_ADDITEMTOHEROBAG: if g_WaitingUseItem.Item.S.Name <> '' then begin
        DelStallItem(g_WaitingUseItem.Item);
        HeroAddItemBag(g_WaitingUseItem.Item);
        g_WaitingUseItem.Item.S.Name := '';
      end;
    SM_HEROEXCHGBAGITEM_FAIL: if g_WaitingUseItem.Item.S.Name <> '' then begin
        if msg.Recog = 0 then begin
        ...
        end else if msg.Recog = 2 then begin
          AddItemBag(g_WaitingUseItem.Item);
          DScreen.AddChatBoardString(g_WaitingUseItem.Item.S.Name + '不能放到英雄包裹中', clWhite, clRed);
          g_WaitingUseItem.Item.S.Name := '';
        end;
      end;
    SM_GETITEMFROMHEROBAG: if g_WaitingUseItem.Item.S.Name <> '' then begin
        AddItemBag(g_WaitingUseItem.Item);
        g_WaitingUseItem.Item.S.Name := '';
      end;
    SM_TAKEON_OK: begin
    ...
    SM_TAKEON_FAIL: if g_WaitingUseItem.Item.S.Name <> '' then begin
        AddItemBag(g_WaitingUseItem.Item);
        g_WaitingUseItem.Item.S.Name := '';
      end;
    SM_HEROTAKEON_OK: if g_WaitingUseItem.Item.S.Name <> '' then begin
        if g_WaitingUseItem.Index in [0..U_FASHION] then
          g_HeroUseItems[g_WaitingUseItem.Index] := g_WaitingUseItem.Item;
        g_WaitingUseItem.Item.S.Name := '';
      end;
    SM_HEROTAKEON_FAIL: if g_WaitingUseItem.Item.S.Name <> '' then begin
        HeroAddItemBag(g_WaitingUseItem.Item);
        g_WaitingUseItem.Item.S.Name := '';
      end;
    SM_TAKEOFF_OK: begin
    ...
    SM_HEROTAKEOFF_OK: if g_WaitingUseItem.Item.S.Name <> '' then begin
        HeroAddItemBag(g_WaitingUseItem.Item);
        g_WaitingUseItem.Item.S.Name := '';
      end;

    SM_TAKEOFF_FAIL: if g_WaitingUseItem.Item.S.Name <> '' then begin
    ...
    SM_HEROTAKEOFF_FAIL: if g_WaitingUseItem.Item.S.Name <> '' then begin
        if (g_WaitingUseItem.Index < (0 - HERO_MIIDX_OFFSET)) and (g_WaitingUseItem.Index >= -((U_FASHION + 1) + HERO_MIIDX_OFFSET)) then begin
    ...
    SM_APPRCHANGED, SM_CHARSTATUSCHANGED: begin
        if body <> '' then
    ...
    SM_EAT_OK: if msg.Recog <> 0 then begin
        Str := '';
        if msg.Recog <> g_EatingItem.MakeIndex then begin

          for i := MAXBAGITEMCL - 1 downto 0 do begin
            if g_ItemArr[i].S.Name <> '' then begin
              if g_ItemArr[i].MakeIndex = g_EatingItem.MakeIndex then begin
    ...
    SM_EAT_FAIL: begin
        if msg.Recog = g_EatingItem.MakeIndex then begin
          ...
          if msg.tag > 0 then g_EatingItem.Dura := msg.tag;
    ...
    SM_ADDMAGIC: if body <> '' then ClientGetAddMagic(body);
    SM_HEROADDMAGIC: if body <> '' then ClientHeroGetAddMagic(body);
    SM_SENDMYMAGIC: if body <> '' then ClientGetMyMagics(body);
    SM_HEROMYMAGICS: if body <> '' then ClientGetHeroMagics(body);
    ...
    SM_HEROPOWERUP: if g_MySelf.m_HeroObject <> nil then begin
    ...
    SM_SENDGOODSLIST: ClientGetSendGoodsList(msg.Recog, msg.param, body);
    ...
    SM_ITEMSUMCOUNT_FAIL: begin
        if g_WaitingUseItem.Item.S.Name <> '' then begin
          if msg.series <> 0 then begin
            if msg.Recog = 0 then begin
              HeroAddItemBag(g_WaitingUseItem.Item);
              g_WaitingUseItem.Item.S.Name := '';
              DScreen.AddChatBoardString('(英雄)重叠失败,物品最高数量是 ' + IntToStr(MAX_OVERLAPITEM), clWhite, clRed);
            end else begin
    ...
    SM_STORAGE_OK, SM_STORAGE_FULL, SM_STORAGE_FAIL: begin
        FrmDlg.LastestClickTime := GetTickCount;
        if msg.ident <> SM_STORAGE_OK then begin
          if msg.ident = SM_STORAGE_FULL then
            ...
            else if msg.Recog = 3 then begin
    ...
    SM_DEALADDITEM_FAIL: begin
        ...
        if msg.Recog <> 0 then DScreen.AddChatBoardString('重叠失败,物品最高数量是 ' + IntToStr(MAX_OVERLAPITEM), clWhite, clRed);
      end;
    ...
    SM_MARKET_RESULT: begin
      ...
      SM_OPENBIGDIALOGBOX: begin      //打开NPC大对话框  Development 2019-01-14
        g_nCurMerchantFaceIdx := Msg.Recog;
        FrmDlg.ResetMenuDlg;
        FrmDlg.CloseMDlg;
...
//  DebugOutStr('g_wAvailIDDay: ' + IntToStr(g_wAvailIDDay));
//  DebugOutStr('g_wAvailIDHour: ' + IntToStr(g_wAvailIDHour));
//  DebugOutStr('g_wAvailIPDay: ' + IntToStr(g_wAvailIPDay));
//  DebugOutStr('g_wAvailIPHour: ' + IntToStr(g_wAvailIPHour));

  if (g_wAvailIDHour mod 60) > 0 then //付费时长大于60
    FrmDlg.DMessageDlg ('个人帐户的期限: 剩余 ' + IntToStr(g_wAvailIDHour div 60) + ' 小时 '
      + IntToStr(g_wAvailIDHour mod 60) + ' 分钟.', [mbOk])
  else if g_wAvailIDHour > 0 then
    FrmDlg.DMessageDlg ('个人帐户的期限: 剩余 ' + IntToStr(g_wAvailIDHour) + ' 分钟.', [mbOk]);
...
begin
  if g_boOpenAutoPlay and (g_nAPReLogon = 1) then begin //0613
    g_nAPReLogon := 2;
  ...
  if not g_Logined or not b then begin
    g_ReSelChr := False;
    FrmDlg.DscStart.tag := 0;
  end;
  //end;
...
begin
  ...
  if not g_gcGeneral[11] then begin
    ...
    if g_nLastMapMusic <> msg.Recog then begin
      g_nLastMapMusic := msg.Recog;
...
begin
  if body <> '' then begin
    ...
    AddItemBag(cu);
    if Hint <> 0 then DScreen.AddSysMsg(cu.S.Name + ' 被发现');
  end;
end;
...
begin
  if body <> '' then begin
    ...
    for i := 0 to 1 do begin
      if (g_TIItems[i].Item.MakeIndex = cu.MakeIndex) and (g_TIItems[i].Item.S.Name <> '') then begin
        g_TIItems[i].Item := cu;
        if i = 0 then GetTIHintString1(1, @g_TIItems[0].Item);
      end;
    end;
    AutoPutOntiBooks();

    for i := 0 to 1 do begin
      if (g_spItems[i].Item.MakeIndex = cu.MakeIndex) and (g_spItems[i].Item.S.Name <> '') then begin
        g_spItems[i].Item := cu;
      end;
    end;
...
begin
  if body <> '' then begin
    ...
    for i := 0 to 1 do begin
      if (g_TIItems[i].Item.MakeIndex = cu.MakeIndex) then begin
        g_TIItems[i].Item.S.Name := '';
        if i = 0 then GetTIHintString1(0);
      end;
    end;
    for i := 0 to 1 do begin
      if (g_spItems[i].Item.MakeIndex = cu.MakeIndex) then begin
    ...
    for i := 0 to MAXBAGITEMCL - 1 do begin
      if ItemSaveArr[i].S.Name <> '' then begin
        ...
        for j := 0 to MAXBAGITEMCL - 1 do begin
          if (g_ItemArr[j].S.Name = ItemSaveArr[i].S.Name) and (g_ItemArr[j].MakeIndex = ItemSaveArr[i].MakeIndex) then begin
...
begin
  ...
  if (g_MovingItem.Item.S.Name <> '') and (IsBagItem(g_MovingItem.Index)) then begin
    g_MovingItem.Item.S.Name := '';
    g_boItemMoving := False;
  end;
  ...
  if g_boOpenAutoPlay and (g_nAPReLogon = 4) then begin //0613
    g_nAPReLogon := 0;
    g_nOverAPZone := g_nOverAPZone2;
    g_APGoBack := g_APGoBack2;
    if g_APMapPath2 <> nil then begin
    ...
    DScreen.AddChatBoardString('[挂机] 开始自动挂机...', clWhite, clRed);
    SaveWayPoint;
    if g_nAPrlRecallHero or (g_MySelf.m_HeroObject = nil) then begin //:= (g_MySelf.m_HeroObject <> nil);
      FrmDlg.m_dwUnRecallHeroTick := GetTickCount - 58000;
    end;
...
begin
  ...
  while True do begin
    ...
    if Index in [0..U_FASHION] then begin
...
{$IF SERIESSKILL}
  if pcm.Def.wMagicid in [100..111] then begin
    if pcm.Def.btclass = 0 then begin
      if pcm.Def.btJob = g_MySelf.m_btJob then
        if g_MagicList2.count <= 3 then g_MagicList2.Add(pcm);
      g_MagicList2.Sort(@ListSortCompareLevel);
    end else
      g_IPMagicList.Add(pcm);
  end else {$IFEND SERIESSKILL}begin
  ...
  for i := 0 to g_MagicList.count - 1 do
    if PTClientMagic(g_MagicList[i]).Def.wMagicid = 67 then begin
  ...
  if pcm.Def.wMagicid in [100..111] then begin
    if pcm.Def.btclass = 0 then begin
      if pcm.Def.btJob = g_MySelf.m_HeroObject.m_btJob then
        if g_hMagicList2.count <= 3 then g_hMagicList2.Add(pcm);
...
var
  i: Integer;
begin
  if btclass = 0 then begin
{$IF SERIESSKILL}
    for i := g_MagicList2.count - 1 downto 0 do begin
      if PTClientMagic(g_MagicList2[i]).Def.wMagicid = magid then begin
        Dispose(PTClientMagic(g_MagicList2[i]));
...
{$IFEND SERIESSKILL}
    ...
    for i := g_IPMagicList.count - 1 downto 0 do begin
      if PTClientMagic(g_IPMagicList[i]).Def.wMagicid = magid then begin
        Dispose(PTClientMagic(g_IPMagicList[i]));
...
begin
  ...
  if t1 = 0 then begin
    for i := g_MagicList2.count - 1 downto 0 do begin
      ...
      if pcm.Def.wMagicid = id1 then begin
        //pcm.Def.btclass := t2;
        //pcm.Def.wMagicid := id2;
        //pcm.Def.sMagicName := S;
        pcm^ := cm;
        if t1 = t2 then begin //update
          g_MagicArr[t1][id1] := nil;
          g_MagicArr[t1][id2] := pcm;
        end else begin //convert
          g_MagicList2.Delete(i);
          g_MagicList2.Sort(@ListSortCompareLevel);
          g_IPMagicList.Add(pcm);
          g_MagicArr[t1][id1] := nil;
...
begin
  ...
  if t1 = 0 then begin
    for i := g_hMagicList2.count - 1 downto 0 do begin
    ...
    for i := g_HeroIPMagicList.count - 1 downto 0 do begin
...
{$IF SERIESSKILL}
    for i := g_hMagicList2.count - 1 downto 0 do begin
      if PTClientMagic(g_hMagicList2[i]).Def.wMagicid = magid then begin
        Dispose(PTClientMagic(g_hMagicList2[i]));
        g_hMagicList2.Delete(i);
        Break;
      end;
...
{$IFEND SERIESSKILL}
    ...
    for i := g_HeroIPMagicList.count - 1 downto 0 do begin
      if PTClientMagic(g_HeroIPMagicList[i]).Def.wMagicid = magid then begin
        Dispose(PTClientMagic(g_HeroIPMagicList[i]));
        g_HeroIPMagicList.Delete(i);
        Break;
      end;
...
begin
  nType := HiWord(magid);
  magid := LoWord(magid);
  if hero = 0 then begin
    pcm := g_MagicArr[0][magid];
    if (magid <= 0) or (magid >= 255) then begin
      Exit;
    end;
    if pcm <> nil then
...
begin
  ...
  if nType = 0 then begin
    ...
    for i := g_HeroIPMagicList.count - 1 downto 0 do begin
      if (PTClientMagic(g_HeroIPMagicList[i]).Def.wMagicid = magid) and (nType = PTClientMagic(g_HeroIPMagicList[i]).Def.btclass) then begin
...
begin
  ...
  if Actor.m_btIsHero = 1 then begin
    for i := g_HeroMagicList.count - 1 downto 0 do begin
      if PTClientMagic(g_HeroMagicList[i]).Def.wMagicid = magid then begin
...
begin
  if uidx in [0..U_FASHION] then begin
...
begin
  ...
  with FrmDlg do begin
    ...
    while body <> '' do begin
      ...
      body := GetValidStr3(body, gsex, ['/']);
      if (gname <> '') and (glevel <> '') and (gsex <> '') then begin
        New(pcg);
        pcg.sCharName := gname;
...
begin
  ...
  if g_boOpenAutoPlay and (g_nAPReLogon = 3) then begin
    g_nAPReLogon := 4;
...
var
  ...
  pstep: Integer;
begin
  if g_boShowMemoLog then g_PlayScene.MemoLog.Lines.Add('ClientGetOpenGuildDlg');
...
begin
  ...
  //FreeOldWMImagesLib(); //释放图片 2018-12-26  Development添加
  ImageFont.FreeOldMemorys();
end;
...
{$IFNDEF UI_0508}
  ...
  for i := Low(Titles) to High(Titles) do begin
    if UserState.Titles[i].Index > 0 then begin
...
begin
  ...
  case nType of
    0: ;
    1: Effect := TNormalDrawEffect.Create(nX, nY, g_WMon14Img, 410, 6, 120, False); //赤月恶魔 地钉
...
begin
  ...
  {FileName :=ExtractFilePath(Paramstr(0)) + NpcImageDir + IntToStr(wAppr) + '.wil';
 if FileExists(FileName)  then begin
  WMImage := GetGameImages(FileName);
  WMImage.Appr := wAppr;
  WMImage.Initialize;
...
begin
  ...
  FileName := ExtractFilePath(Paramstr(0)) +ItemImageDir + 'St' + IntToStr(FileIdx) + '.wil';
...
begin
  Result := nil;
  if Weapon <= 0 then Exit;
  if boUseCboLib then begin
    with Actor do begin
...
//      if (FileIdx >= 50) and (FileIdx < 75) then begin
//        Weapon := Weapon - 24;
//        Result := g_cboweapon.GetCachedImage(2000 * Weapon + nFrame, ax, ay);
//      end else if FileIdx >= 75 then begin
...
//  end;
//
//  if (FileIdx < 400) then begin
//    if (FileIdx >= 50) and (FileIdx < 75) then begin
//      if g_WWeapon2Images <> nil then
//        Result := g_WWeapon2Images.GetCachedImage(HUMANFRAME * (Weapon - 100) + nFrame, ax, ay);
//    end else if (FileIdx >= 75) and (FileIdx < 100) then begin
//      if g_WWeapon3Images <> nil then
//        Result := g_WWeapon3Images.GetCachedImage(HUMANFRAME * (Weapon - 150) + nFrame, ax, ay);
//    end else if (FileIdx >= 100) and (FileIdx < 150) then begin
//      if g_WWeapon4Images <> nil then
//        Result := g_WWeapon4Images.GetCachedImage(HUMANFRAME * (Weapon - 250) + nFrame, ax, ay);
//    end else if (FileIdx >= 150) and (FileIdx < 200) then begin
//      if g_WWeapon5Images <> nil then
//        Result := g_WWeapon5Images.GetCachedImage(HUMANFRAME * (Weapon - 350) + nFrame, ax, ay);
//    end else if (FileIdx >= 200) and (FileIdx < 250) then begin
//      if g_WWeapon6Images <> nil then
//        Result := g_WWeapon6Images.GetCachedImage(HUMANFRAME * (Weapon - 450) + nFrame, ax, ay);
//    end else if (FileIdx >= 250) and (FileIdx < 300) then begin
//      if g_WWeapon7Images <> nil then
//        Result := g_WWeapon7Images.GetCachedImage(HUMANFRAME * (Weapon - 550) + nFrame, ax, ay);
//    end else if (FileIdx >= 300) and (FileIdx < 350) then begin
//      if g_WWeapon8Images <> nil then
//        Result := g_WWeapon8Images.GetCachedImage(HUMANFRAME * (Weapon - 650) + nFrame, ax, ay);
//    end else if (FileIdx >= 350) and (FileIdx < 400) then begin
//      if g_WWeapon9Images <> nil then
//        Result := g_WWeapon9Images.GetCachedImage(HUMANFRAME * (Weapon - 750) + nFrame, ax, ay);
//    end else
//      Result := g_WWeaponImages.GetCachedImage(HUMANFRAME * Weapon + nFrame, ax, ay);
//    Exit;
//  end;
////扩展武器自定义素材 Development 2019-01-12
...
//  WMImage := CreateWMImages(t_wmM2Def);
...
//
//  if boUseCboLib then begin
//    if (FileIdx < 75) then begin
//      if (FileIdx >= 25) and (FileIdx < 50) then begin
//        Dress := Dress - 26;
//        Result := g_cbohum.GetCachedImage(2000 * Dress + nFrame, ax, ay);
//      end else if FileIdx >= 50 then begin
//        Dress := Dress - 100;
...
//  end;
//
//  if (FileIdx < 375) then begin
//    if (FileIdx >= 25) and (FileIdx < 50) then begin
//      if g_WHum2ImgImages <> nil then
//        Result := g_WHum2ImgImages.GetCachedImage(HUMANFRAME * (Dress - 50) + nFrame, ax, ay);
//    end else if (FileIdx >= 50) and (FileIdx < 75) then begin
//      if g_WHum3ImgImages <> nil then
//        Result := g_WHum3ImgImages.GetCachedImage(HUMANFRAME * (Dress - 100) + nFrame, ax, ay);
//    end else if (FileIdx >= 75) and (FileIdx < 125) then begin
//      if g_WHum4ImgImages <> nil then
//        Result := g_WHum4ImgImages.GetCachedImage(HUMANFRAME * (Dress - 200) + nFrame, ax, ay);
//    end else if (FileIdx >= 125) and (FileIdx < 175) then begin
//      if g_WHum5ImgImages <> nil then
//        Result := g_WHum5ImgImages.GetCachedImage(HUMANFRAME * (Dress - 300) + nFrame, ax, ay);
//    end else if (FileIdx >= 175) and (FileIdx < 225) then begin
//      if g_WHum6ImgImages <> nil then
//        Result := g_WHum6ImgImages.GetCachedImage(HUMANFRAME * (Dress - 400) + nFrame, ax, ay);
//    end else if (FileIdx >= 225) and (FileIdx < 275) then begin
//      if g_WHum7ImgImages <> nil then
//        Result := g_WHum7ImgImages.GetCachedImage(HUMANFRAME * (Dress - 500) + nFrame, ax, ay);
//    end else if (FileIdx >= 275) and (FileIdx < 325) then begin
//      if g_WHum8ImgImages <> nil then
//        Result := g_WHum8ImgImages.GetCachedImage(HUMANFRAME * (Dress - 600) + nFrame, ax, ay);
//    end else if (FileIdx >= 325) and (FileIdx < 375) then begin
//      if g_WHum9ImgImages <> nil then
//        Result := g_WHum9ImgImages.GetCachedImage(HUMANFRAME * (Dress - 700) + nFrame, ax, ay);
//
//    end else
//      Result := g_WHumImgImages.GetCachedImage(HUMANFRAME * Dress + nFrame, ax, ay);
//    Exit;
//  end;
////扩展衣服自定义素材 Development 2019-01-12
////衣服shape在100~255 对应 Hum+Shape.wil 文件，例：shape=105 对应Hum105.wil
////衣服shape＝100时候，对应与相同数值的wil文件Hum100.wil，一把衣服对应一个wil文件。
...
{$IFEND CONFIGTEST}
  ...
  g_DeathColorEffect := TColorEffect(_MIN(8, ClientConf.btDieColor)); //死亡颜色  Development 2018-12-29
...
begin
  ...
  if not g_DlgInitialize and ((g_MySelf = nil) or (mrOk = FrmDlg.DMessageDlg('确认退出游戏吗？', [mbOk, mbCancel]))) then begin
    if g_MySelf <> nil then begin
      SaveBagsData();
      SaveItemFilter();
    end;
...
begin
  if (g_MySelf <> nil) then begin
    if not g_boMapMoving and not g_boServerChanging then begin
      if GetTickCount - g_MySelf.m_dwAutoTecTick > 100 then begin
        g_MySelf.m_dwAutoTecTick := GetTickCount;
        ActorAutoEat(g_MySelf);
      end;
    end;
    if g_MySelf.m_HeroObject <> nil then begin
      if GetTickCount - g_MySelf.m_dwAutoTecHeroTick > 100 then begin
...
begin
  ...
  if g_SeriesSkillFire_100 and (g_MySelf <> nil) and g_MySelf.ActionFinished {and (CanNextAction and ServerAcceptNextAction and CanNextHit())} then begin
    g_SeriesSkillFire_100 := False;
    g_nCurrentMagic2 := 1;
    g_nCurrentMagic := 888;
    UseMagic(g_nMouseX, g_nMouseY, g_MagicArr[0][g_SeriesSkillArr[0]], False, True);
...
begin
  ...
  if pcm.Def.wMagicid in [13..19, 30, 43, 55, 57] then begin
    Str := '符';
    cStr := '符';
  end else if pcm.Def.wMagicid in [6, 38] then begin
    if (g_MagicTarget <> nil) then begin
      Str := '药';
      g_boExchgPoison := not g_boExchgPoison;
      if g_boExchgPoison then begin
        //dec health
  ...
  if (g_UseItems[U_BUJUK].S.StdMode = 25) and (g_UseItems[U_BUJUK].S.Shape <> 6) and (Pos(cStr, g_UseItems[U_BUJUK].S.Name) > 0) then begin
  ...
  for i := 6 to MAXBAGITEMCL - 1 do begin
    if (g_ItemArr[i].S.NeedIdentify < 4) and (g_ItemArr[i].S.StdMode = 25) and (g_ItemArr[i].S.Shape <> 6) and (Pos(Str, g_ItemArr[i].S.Name) > 0) and (Pos(cStr, g_ItemArr[i].S.Name) > 0) then begin
...
begin
  Result := False;
  if not FrmDlg.DWGameConfig.Visible then Exit;
  if FrmDlg.DxEditSSkill.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DEHeroCallHero.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DEHeroSetTarget.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DEHeroUnionHit.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DEHeroSetAttackState.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DEHeroSetGuard.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DESwitchAttackMode.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
  if FrmDlg.DESwitchMiniMap.HotKey = HotKey then begin
    Result := True;
    Exit;
  end;
end;
      ...
      if ssCtrl in Shift then begin
        ...
        if not FrmDlg.DBChat.Visible then begin
          if ChatBoardTop < ChatStrs.count - 1 then
            Inc(ChatBoardTop);
        end else if ChatBoardTop < ChatStrs.count - 1 then
...
begin
  ...
  if (g_MySelf <> nil) and g_boAutoSay and (FrmDlg.DBAotoSay.tag = 0) and (g_MySelf.m_sAutoSayMsg <> '') then begin
    if GetTickCount - FrmDlg.m_sAutoSayMsgTick > 30 * 1000 then begin
      FrmDlg.m_sAutoSayMsgTick := GetTickCount;
      SendSay(g_MySelf.m_sAutoSayMsg);
    end;
  end;
  if (g_MySelf <> nil) and IsUnLockAction() then begin
    if CanNextAction and ServerAcceptNextAction then begin
      nspeed := 0;
      if g_boSpeedRate then
        nspeed := g_MagSpeedRate * 20;
      if (GetTickCount - g_dwLatestSpellTick > (g_dwSpellTime + g_dwMagicDelayTime - nspeed)) then begin
        if g_gcTec[4] and (g_MySelf.m_nState and $00100000 = 0) {and (g_MySelf.m_nState and $00000100 = 0)} then begin
        ...
        case g_MySelf.m_btJob of
          0: begin
              if g_gcTec[3] and not g_boNextTimePursueHit then begin
                pcm := GetMagicByID(56);
                if pcm <> nil then begin
              ...
              if g_gcTec[11] and not g_boNextTimeSmiteLongHit2 then begin
              ...
              if g_gcTec[2] and not g_boNextTimeFireHit then begin
                pcm := GetMagicByID(26);
                if pcm <> nil then begin
                  UseMagic(SCREENWIDTH div 2, SCREENHEIGHT div 2, pcm);
                  Exit;
              ...
              if g_gcTec[13] and not g_boCanSLonHit then begin
                pcm := GetMagicByID(66);
                if pcm <> nil then begin
                  UseMagic(SCREENWIDTH div 2, SCREENHEIGHT div 2, pcm);
                  Exit;
              ...
              if g_gcTec[9] and not g_boNextTimeTwinHit then begin
                pcm := GetMagicByID(43);
                if pcm <> nil then begin
                  UseMagic(SCREENWIDTH div 2, SCREENHEIGHT div 2, pcm);
                  Exit;
          ...
          2: begin
              if g_gcTec[6] and (g_MySelf.m_nState and $00800000 = 0) then begin
                pcm := GetMagicByID(18);
                if pcm <> nil then
                  UseMagic(SCREENWIDTH div 2, SCREENHEIGHT div 2, pcm);
              end;
        ...
        if g_gcTec[7] and (GetTickCount - g_MySelf.m_dwPracticeTick > _MAX(500, g_gnTecTime[8])) then begin
...
var
  ...
  boCanRun: Boolean;
begin
  if (g_MySelf = nil) or (Map.m_MapBuf = nil) or (not CSocket.Active) then Exit;
  if g_MapPath <> nil then begin

    if ((g_MySelf.m_nCurrX = g_MySelf.m_nTagX) and (g_MySelf.m_nCurrY = g_MySelf.m_nTagY)) then begin
      TimerAutoMove.Enabled := False;
    ...
    if CanNextAction and ServerAcceptNextAction and IsUnLockAction() then begin
      if g_MoveStep <= High(g_MapPath) then begin
        g_nTargetX := g_MapPath[g_MoveStep].X;
        g_nTargetY := g_MapPath[g_MoveStep].Y;
        while (abs(g_MySelf.m_nCurrX - g_nTargetX) <= 1) and (abs(g_MySelf.m_nCurrY - g_nTargetY) <= 1) do begin
          boCanRun := False;
          if g_MoveStep + 1 <= High(g_MapPath) then begin
          ...
          if boCanRun and Map.CanMove(X1, Y1) and not g_PlayScene.CrashMan(X1, Y1) then begin
            Inc(g_MoveStep);
            g_nTargetX := g_MapPath[g_MoveStep].X;
            g_nTargetY := g_MapPath[g_MoveStep].Y;
            if g_MoveStep >= High(g_MapPath) then Break;
          end else begin
            g_nTargetX := g_MapPath[g_MoveStep].X;
            g_nTargetY := g_MapPath[g_MoveStep].Y;
            Break;
        ...
        if (abs(g_MySelf.m_nCurrX - g_MySelf.m_nTagX) <= 1) and
          (abs(g_MySelf.m_nCurrY - g_MySelf.m_nTagY) <= 1) then begin
          g_nTargetX := g_MySelf.m_nTagX;
          g_nTargetY := g_MySelf.m_nTagY;
        end;

        if (abs(g_MySelf.m_nCurrX - g_nTargetX) <= 1) and (abs(g_MySelf.m_nCurrY - g_nTargetY) <= 1) then begin //目标座标
          g_ChrAction := caWalk;
          g_MoveBusy := True;
        end else begin
          if g_MySelf.CanRun > 0 then begin
            g_ChrAction := caRun;
            g_MoveBusy := True;
          end else begin
            g_ChrAction := caWalk;
...
begin
  try
    ...
    DebugOutStr('[Exception]: TfrmMain.FormActivate');
  end;
end;
...
label
  ...
  begin
    ...
    while i < 16 do begin
      if not GetNextPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, ndir, 2, g_nTargetX, g_nTargetY) then begin
        GetFrontPosition(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, ndir, g_nTargetX, g_nTargetY);
        if not g_PlayScene.CanWalk(g_nTargetX, g_nTargetY) then begin
          g_MySelf.SendMsg(CM_TURN, g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, Random(8), 0, 0, '', 0);
          Inc(i);
        end else begin
          b := True;
          Break;
...
begin
  ...
  if (g_MySelf.m_HeroObject <> nil) then begin
    if GetTickCount - FrmDlg.m_dwUnRecallHeroTick > 3000 then begin

      if (g_MySelf.m_HeroObject.m_Abil.HP <> 0) and (Round((g_MySelf.m_HeroObject.m_Abil.HP / g_MySelf.m_HeroObject.m_Abil.MaxHP) * 100) < 20) then begin
    ...
    if GetTickCount - FrmDlg.m_dwUnRecallHeroTick > 62000 then begin

      if TargetHumCount(g_MySelf) <= 3 then begin
        FrmDlg.m_dwUnRecallHeroTick := GetTickCount;
        g_pbRecallHero^ := True;
        FrmDlg.ClientCallHero();
      end;
  ...
  case GetAutoPalyStation() of
    ...
    1: begin //此时为该怪物首次被发现，自动寻找路径
        if HeroAttackTagget(g_APTagget) then begin
          Exit;
        end;
    ...
    2: begin //此时该物品为首次发现，自动寻找路径
        if (g_AutoPicupItem <> nil) and ((g_nAPStatus <> 2) or (g_APPathList.count = 0)) then begin
        ...
        end else if (g_AutoPicupItem <> nil) then begin
          g_nTargetX := g_AutoPicupItem.X;
          g_nTargetY := g_AutoPicupItem.Y;
          AP_findpath(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nTargetX, g_nTargetY);
          g_nTargetX := -1;
    ...
    3: begin
        if (g_APMapPath <> nil) and (g_APStep >= 0) and (g_APStep <= High(g_APMapPath)) then begin
          if (High(g_APMapPath) > 0) then begin
            ...
            if (g_nTargetX = -1) or (g_APPathList.count = 0) then begin
              if b then AP_findpath(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nTargetX, g_nTargetY); //memory leak !!!
              g_sAPStr := Format('[挂机] 定点随机搜寻目标(%d,%d)', [g_APMapPath[g_APStep].X, g_APMapPath[g_APStep].Y]);
              g_nTargetX := -1;
            end;
          end;
        end else if (g_nTargetX = -1) or (g_APPathList.count = 0) then begin
          randomtag();
          if b then AP_findpath(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nTargetX, g_nTargetY); //memory leak !!!
          g_sAPStr := '[挂机] 随机搜寻目标...';
          g_nTargetX := -1;
    ...
    4: begin
        if (g_APMapPath <> nil) and (g_APStep >= 0) and (g_APStep <= High(g_APMapPath)) then begin
          ...
          randomtag();
          if b then AP_findpath(g_MySelf.m_nCurrX, g_MySelf.m_nCurrY, g_nTargetX, g_nTargetY);
  ...
  if (g_APPathList.count > 0) and ((g_nTargetX = -1) or ((g_nTargetX = g_MySelf.m_nCurrX) and (g_nTargetY = g_MySelf.m_nCurrY))) then begin

    T := PFindNOde(g_APPathList[0]);
    g_nTargetX := T.X;
    g_nTargetY := T.Y;

    if g_nAPStatus in [1..4] then begin
      if (abs(g_MySelf.m_nCurrX - g_nTargetX) <= 1) and (abs(g_MySelf.m_nCurrY - g_nTargetY) <= 1) then begin
        if g_PlayScene.CanWalk(g_nTargetX, g_nTargetY) then begin
          if (g_nAPStatus = 2) and (g_AutoPicupItem <> nil) then begin
            if (abs(g_MySelf.m_nCurrX - g_AutoPicupItem.X) > 1) or (abs(g_MySelf.m_nCurrY - g_AutoPicupItem.Y) > 1) then begin
              goto AAAA;
            end else begin
              Dispose(T);
            ...
            AAAA:
            if (g_APPathList.count > 2) then begin
            ...
            if Map.CanMove(X1, Y1) then begin
              if g_PlayScene.CrashMan(X1, Y1) then begin
                g_nTargetX := T.X;
                g_nTargetY := T.Y;
                g_ChrAction := caWalk;
              end else begin
...
begin
  ...
  for i := Low(aaa) to High(aaa) do begin
    New(pClientItem);
    pClientItem^ := aaa[i];
...
begin
  ...
  if pActor <> nil then begin
    ...
    if msg.Recog = g_MySelf.m_nRecogId then begin
      pActor.SendMsg(SM_TURN, msg.param, msg.tag, LoByte(msg.series), psMessage.lFeature, psMessage.nStatus, psMessage.szBuff, 0, 300);
    end else begin
      pActor.SendMsg(SM_TURN, msg.param, msg.tag, LoByte(msg.series), psMessage.lFeature, psMessage.nStatus, psMessage.szBuff, 0, 300);
    end;
  end;
...
begin
  ...
  if (datablock[2] = 'G') and (datablock[3] = 'D') and (datablock[4] = '/') then begin
    ...
    if data <> '' then begin
      ...
      //ActionFailLock := False;
      g_MoveErr := 0;
      if TimerAutoMove.Enabled then Inc(g_MoveStep);
...
{$IFEND CHECKPACKED}
  ...
  if tagstr = 'DIG' then g_MySelf.m_boDigFragment := True
  else if tagstr = 'PWR' then g_boNextTimePowerHit := True
  else if tagstr = 'LNG' then g_boCanLongHit := True
  else if tagstr = 'ULNG' then g_boCanLongHit := False
  else if tagstr = 'WID' then g_boCanWideHit := True
  else if tagstr = 'UWID' then g_boCanWideHit := False
  else if tagstr = 'STN' then g_boCanStnHit := True
  else if tagstr = 'USTN' then g_boCanStnHit := False
  else if tagstr = 'CRS' then begin
    g_boCanCrsHit := True;
    DScreen.AddChatBoardString('双龙斩开启', GetRGB(219), clWhite);
  end else if tagstr = 'UCRS' then begin
    g_boCanCrsHit := False;
    DScreen.AddChatBoardString('双龙斩关闭', GetRGB(219), clWhite);
  end else if tagstr = 'TWN' then begin
  ...
  end else if tagstr = 'SMIL2' then begin
  ...
  end else if tagstr = 'UFIR' then g_boNextTimeFireHit := False
  else if tagstr = 'UPUR' then g_boNextTimePursueHit := False
  else if tagstr = 'USMI' then g_boNextTimeSmiteHit := False
  else if tagstr = 'URSH' then g_boNextTimeRushHit := False
  else if tagstr = 'USMIL' then g_boNextTimeSmiteLongHit := False
  else if tagstr = 'USML3' then g_boNextTimeSmiteLongHit3 := False
  else if tagstr = 'USML2' then begin
    g_boNextTimeSmiteLongHit2 := False;
    //DScreen.AddChatBoardString('[断空斩] 力量消失...', clWhite, clRed);
  end else if tagstr = 'USMIW' then
    g_boNextTimeSmiteWideHit := False
  else if tagstr = 'USMIW2' then
    g_boNextTimeSmiteWideHit2 := False
  else if tagstr = 'USQU' then begin
    g_boCanSquHit := False;
    DScreen.AddChatBoardString('[龙影剑法] 关闭', GetRGB(219), clWhite);
  end else if tagstr = 'SLON' then begin
    g_boCanSLonHit := True;
    g_dwLatestSLonHitTick := GetTickCount;
    DScreen.AddChatBoardString('[开天斩] 力量凝聚...', GetRGB(219), clWhite);
  end else if tagstr = 'USLON' then begin
    g_boCanSLonHit := False;
