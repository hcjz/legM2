...
uses
  ...
  HGE,HGECanvas,HGETextures, HGEFont, IntroScn, DirectXGraphics,
   HumanActor, Actor, cliUtil, ClFunc, HUtil32, GList, DWinCtl, CnHashTable;
...
{$ELSE}
  ...
  for i := 0 to m_HintList.Count - 1 do begin
    pStrins := m_HintList[i];
    if (dwCrc = pStrins.Crc) and (pStrins.ShowType = ShowType) then begin
      pStrins.Show := True;
      pStrins.Quad := ShowItem;
      pStrins.Brush := Brush;
      pStrins.X := X;
...
{$IFEND HintWinList_Hash}
    ...
    case pStrins.ShowType of
      0: begin
          //g_HintSurface_B:= TDXRenderTargetTexture.Create(g_DXCanvas);
          //g_HintSurface_B.Size := Point(SCREENWIDTH - 100, SCREENHEIGHT - 5);
          //g_HintSurface_B.Format := D3DFMT_A4R4G4B4;
          //g_HintSurface_B.Active := True;
          ...
          DrawBlendR(MSurface, hx, hy, d, 0, 0, pStrins.Width, pStrins.Height, 0);

          if pStrins.Quad then begin
             with g_DXCanvas do begin //画矩形线框 中空
              ...
              if pSegment.Image > 0 then begin
                if pSegment.Text <> '' then begin
                  case pSegment.Text[1] of
                    'u': begin
                        dimg := g_wui.Images[pSegment.Image]; //鉴定符号
          ...
          //g_HintSurface_B.Free;
        end;
      1: begin
         //g_HintSurface_Y:= TDXRenderTargetTexture.Create(g_DXCanvas);
         //g_HintSurface_Y.Size := Point(550, 550);
         //g_HintSurface_Y.Format :=D3DFMT_A4R4G4B4;
         //g_HintSurface_Y.Active := True;
